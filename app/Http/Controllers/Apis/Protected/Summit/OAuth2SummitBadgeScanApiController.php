 ...
 */
final class OAuth2SummitBadgeScanApiController extends OAuth2ProtectedController {
  /**
    ...
    return $this->processRequest(function () use ($summit_id, $sponsor_id) {
      ...
      return $this->created(
        SerializerRegistry::getInstance()
          ->getSerializer($grant, $this->addSerializerType())
          ->serialize(
      ...
      function ($filter) use ($summit, $current_member) {
        if ($filter instanceof Filter) {
          ...
          $filter->addFilterCondition(
            FilterElement::makeEqual("user_id", $current_member->getId()),
          );
        }
        return $filter;
      },
        ...
        if ($filter instanceof Filter) {
          ...
          if (!is_null($current_member)) {
            ...
            if ($current_member->isSponsorUser()) {
              ...
              $filter->addFilterCondition(
                FilterElement::makeEqual("sponsor_id", $sponsor_ids, "OR"),
              );
            }
          }
        }
