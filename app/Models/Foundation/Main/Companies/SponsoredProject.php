...
class SponsoredProject extends SilverstripeBaseModel {
   ...
   */
  public function recalculateProjectSponsorshipTypeOrder(
    ProjectSponsorshipType $value,
    $new_order,
  ) {
  ...
  public function addSponsorshipType(ProjectSponsorshipType $sponsorshipType): void {
    if ($this->sponsorship_types->contains($sponsorshipType)) {
      return;
    }
  ...
  public function addSubProject(SponsoredProject $subProject): void {
    if ($this->sub_projects->contains($subProject)) {
      return;
    }
