 ...
 */
final class SummitSponsorshipTypeService extends AbstractService implements
  ISummitSponsorshipTypeService {
  /**
   * @var IFileUploader
   */
  ...
  public function update(
    ...
    return $this->tx_service->transaction(function () use ($summit, $sponsorship_id, $payload) {
      ...
      if (isset($payload["type_id"])) {
        ...
        $former_sponsorship_type = $summit->getSummitSponsorshipTypeByType($type);
        if (
          !is_null($former_sponsorship_type) &&
          $former_sponsorship_type->getId() !== $sponsorship_type->getId()
        ) {
          throw new ValidationException(
            sprintf(
              "There is already a Sponsorship of type %s for summit %s.",
  ...
  public function addBadgeImage(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      $photo = $this->file_uploader->build(
        $file,
        sprintf(
          "summits/%s/summit_sponsorship_types/%s",
          $summit->getId(),
          $summit_sponsorship->getId(),
        ),
        true,
      );
