...
class SummitEventType extends SilverstripeBaseModel {
  ...
  public function isBlackoutAppliedTo(IPublishableEvent $publishable_event) {
    Log::debug(
      sprintf(
        "SummitEventType::isBlackoutAppliedTo blackout_times %s event %s event source %s",
        $this->blackout_times,
        $publishable_event->getId(),
        $publishable_event->getSource(),
      ),
    );
    if ($this->blackout_times === SummitEventTypeConstants::BLACKOUT_TIME_ALL) {
      return true;
    }
    if ($this->blackout_times === SummitEventTypeConstants::BLACKOUT_TIME_NONE) {
      return false;
    }
   ...
   * @return bool
   */
  public static function IsSummitEventType(Summit $summit, $type) {
   ...
   */
  public function setAllowsLocationTimeframeCollision(
    bool $allows_location_timeframe_collision,
  ): void {
