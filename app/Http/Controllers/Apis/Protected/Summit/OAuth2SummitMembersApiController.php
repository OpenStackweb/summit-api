...
final class OAuth2SummitMembersApiController extends OAuth2ProtectedController {
  ...
  public function getMemberFavoritesSummitEvents($summit_id, $member_id) {
    return $this->processRequest(function () use ($summit_id, $member_id) {
      ...
      foreach ($current_member->getFavoritesSummitEventsBySummit($summit) as $favorite_event) {
        if (!$summit->isEventOnSchedule($favorite_event->getEvent()->getId())) {
          continue;
        }
        $favorites[] = SerializerRegistry::getInstance()
          ->getSerializer($favorite_event)
          ->serialize(
            SerializerUtils::getExpand(),
            SerializerUtils::getFields(),
            SerializerUtils::getRelations(),
      ...
      $response = new PagingResponse(count($favorites), count($favorites), 1, 1, $favorites);
  ...
  public function removeEventFromMemberFavorites($summit_id, $member_id, $event_id) {
    return $this->processRequest(function () use ($summit_id, $member_id, $event_id) {
      ...
      $this->summit_service->removeEventFromMemberFavorites(
        $summit,
        $current_member,
        intval($event_id),
      );
  ...
  public function getMemberScheduleSummitEvents($summit_id, $member_id) {
    return $this->processRequest(function () use ($summit_id, $member_id) {
      ...
      foreach ($current_member->getScheduleBySummit($summit) as $schedule_event) {
        if (!$summit->isEventOnSchedule($schedule_event->getEvent()->getId())) {
          continue;
        }
        $schedule[] = SerializerRegistry::getInstance()
          ->getSerializer($schedule_event)
          ->serialize(
            SerializerUtils::getExpand(),
            SerializerUtils::getFields(),
            SerializerUtils::getRelations(),
      ...
      $response = new PagingResponse(count($schedule), count($schedule), 1, 1, $schedule);
  ...
  public function getAllBySummit($summit_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["first_name", "last_name", "id", "created", "last_edited"];
      },
  ...
  public function addEventRSVP($summit_id, $member_id, $event_id) {
    try {
      ...
      $rsvp = $this->summit_service->addRSVP(
        $summit,
        $current_member,
        $event_id,
        $this->validateEventUri($payload),
      );
  ...
  public function updateEventRSVP($summit_id, $member_id, $event_id) {
    try {
      ...
      $rsvp = $this->summit_service->updateRSVP(
        $summit,
        $current_member,
        $event_id,
        $this->validateEventUri($payload),
