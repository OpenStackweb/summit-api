 ...
 */
final class SpeakerService extends AbstractService implements ISpeakerService {
  ...
  public function addSpeaker(array $data, ?Member $creator = null, $send_email = true) {
    return $this->tx_service->transaction(function () use ($data, $creator, $send_email) {
      ...
      if (!empty($email)) {
        $formerRegistrationRequest = $this->speaker_registration_request_repository->getByEmail(
          $email,
        );
      ...
      if ($member_id == 0 && !empty($email)) {
        ...
        if (!is_null($member)) {
          Log::debug(
            sprintf("SpeakerService::addSpeaker: member %s found, setting it to speaker", $email),
          );
          $existent_speaker = $this->speaker_repository->getByMember($member);
          if (!is_null($existent_speaker)) {
            throw new ValidationException(
  ...
  public function addSpeakerBySummit(Summit $summit, array $data) {
    return $this->tx_service->transaction(function () use ($data, $summit) {
      ...
      $speaker->addSummitAssistance(
        PresentationSpeakerSummitAssistanceConfirmationRequestFactory::build(
          $summit,
          $speaker,
          $data,
        ),
  ...
  public function updateSpeakerBySummit(Summit $summit, PresentationSpeaker $speaker, array $data) {
    return $this->tx_service->transaction(function () use ($summit, $speaker, $data) {
      ...
      PresentationSpeakerSummitAssistanceConfirmationRequestFactory::populate(
        $summit_assistance,
        $data,
      );
  ...
  private function registerSpeaker(PresentationSpeaker $speaker, $email) {
    if ($this->speaker_registration_request_repository->existByEmail($email)) {
      throw new ValidationException(
        sprintf("email %s already has a Speaker Registration Request", $email),
      );
    }
    ...
    do {
      ...
      $this->speaker_registration_request_repository->existByHash(
        $registration_request->getConfirmationHash(),
      )
    );
   ...
   * @throws ValidationException
   */
  public function registerSummitPromoCodeByValue(
    ...
    return $this->tx_service->transaction(function () use ($speaker, $summit, $reg_code) {
      ...
      $existent_code = $this->registration_code_repository->getBySpeakerAndSummit(
        $speaker,
        $summit,
      );
      ...
      if (
        !is_null($existent_code) &&
        $reg_code !== $existent_code->getCode() &&
        $existent_code->isRedeemed()
      ) {
        throw new ValidationException(
          sprintf(
            "speaker has been already assigned to another registration code (%s) already redeemed!",
            $existent_code->getCode(),
          ),
        );
      }

      if (!is_null($existent_code) && $reg_code == $existent_code->getCode()) {
        return $existent_code;
      }

      // check if reg code is assigned already to another speaker ...
      if (
        $assigned_code = $this->registration_code_repository->getAssignedCode($reg_code, $summit)
      ) {
        if ($assigned_code->getSpeaker()->getId() != $speaker->getId()) {
          throw new ValidationException(
            sprintf(
              "there is another speaker with that code for this summit ( speaker id %s )",
              $assigned_code->getSpeaker()->getId(),
            ),
          );
        }
      }
  ...
  private function updateSpeakerRelations(PresentationSpeaker $speaker, array $data) {
    ...
    if (isset($data["other_presentation_links"]) && is_array($data["other_presentation_links"])) {
      ...
      foreach ($data["other_presentation_links"] as $link) {
        $speaker->addOtherPresentationLink(
          new SpeakerPresentationLink(trim($link["link"]), trim($link["title"])),
        );
      }
    }
    // languages
    ...
    if (isset($data["organizational_roles"]) && is_array($data["organizational_roles"])) {
      ...
      foreach ($data["organizational_roles"] as $org_role_id) {
        ...
        if (is_null($role)) {
          throw new ValidationException(
            trans(
              "validation_errors.SpeakerService.updateSpeakerRelations.InvalidOrganizationRole",
              [
                "role" => $role,
              ],
            ),
      ...
      if (isset($data["other_organizational_rol"])) {
        $role = $this->speaker_organizational_role_repository->getByRole(
          trim($data["other_organizational_rol"]),
        );
        if (is_null($role)) {
          // create it
    ...
    if (isset($data["active_involvements"]) && is_array($data["active_involvements"])) {
      ...
      foreach ($data["active_involvements"] as $involvement_id) {
        ...
        if (is_null($involvement)) {
          throw new ValidationException(
            trans(
              "validation_errors.SpeakerService.updateSpeakerRelations.InvalidActiveInvolvement",
              [
  ...
  public function merge(
    PresentationSpeaker $speaker_from,
    PresentationSpeaker $speaker_to,
    array $data,
  ) {
    return $this->tx_service->transaction(function () use ($speaker_from, $speaker_to, $data) {
      ...
      // bio
      if (!isset($data["bio"])) {
        throw new ValidationException("bio field is required");
      }
      $speaker_id = intval($data["bio"]);
      $speaker_to->setBio(
        $speaker_id == $speaker_from->getId() ? $speaker_from->getBio() : $speaker_to->getBio(),
      );

      // first_name
      if (!isset($data["first_name"])) {
        throw new ValidationException("first_name field is required");
      }
      $speaker_id = intval($data["first_name"]);
      $speaker_to->setFirstName(
        $speaker_id == $speaker_from->getId()
          ? $speaker_from->getFirstName()
          : $speaker_to->getFirstName(),
      );

      // last_name
      if (!isset($data["last_name"])) {
        throw new ValidationException("last_name field is required");
      }
      $speaker_id = intval($data["last_name"]);
      $speaker_to->setLastName(
        $speaker_id == $speaker_from->getId()
          ? $speaker_from->getLastName()
          : $speaker_to->getLastName(),
      );
      ...
      $speaker_to->setTitle(
        $speaker_id == $speaker_from->getId() ? $speaker_from->getTitle() : $speaker_to->getTitle(),
      );

      // irc
      if (!isset($data["irc"])) {
        throw new ValidationException("irc field is required");
      }
      $speaker_id = intval($data["irc"]);
      $speaker_to->setIrcHandle(
        $speaker_id == $speaker_from->getId()
          ? $speaker_from->getIrcHandle()
          : $speaker_to->getIrcHandle(),
      );

      // twitter
      if (!isset($data["twitter"])) {
        throw new ValidationException("twitter field is required");
      }
      $speaker_id = intval($data["twitter"]);
      $speaker_to->setTwitterName(
        $speaker_id == $speaker_from->getId()
          ? $speaker_from->getTwitterName()
          : $speaker_to->getTwitterName(),
      );

      // pic
      try {
        if (!isset($data["pic"])) {
          throw new ValidationException("pic field is required");
        }
        $speaker_id = intval($data["pic"]);
        $photo =
          $speaker_id == $speaker_from->getId()
            ? $speaker_from->getPhoto()
            : $speaker_to->getPhoto();
        if (!is_null($photo)) {
      ...
      try {
        if (!isset($data["registration_request"])) {
          throw new ValidationException("registration_request field is required");
        }
        $speaker_id = intval($data["registration_request"]);
        $registration_request =
          $speaker_id == $speaker_from->getId()
            ? $speaker_from->getRegistrationRequest()
            : $speaker_to->getRegistrationRequest();
      ...
      try {
        if (!isset($data["member"])) {
          throw new ValidationException("member field is required");
        }
        $speaker_id = intval($data["member"]);
        $member =
          $speaker_id == $speaker_from->getId()
            ? $speaker_from->getMember()
            : $speaker_to->getMember();
        if (!is_null($member)) {
  ...
  public function addSpeakerAssistance(Summit $summit, array $data) {
    return $this->tx_service->transaction(function () use ($data, $summit) {
      ...
      if (is_null($speaker)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.add_speaker_assistance_speaker_not_found", [
      ...
      if (!$speaker->isSpeakerOfSummit($summit)) {
        throw new ValidationException(
          trans("validation_errors.add_speaker_assistance_speaker_is_not_on_summit", [
            "speaker_id" => $speaker_id,
      ...
      if ($speaker->hasAssistanceFor($summit)) {
        throw new ValidationException(
          trans("validation_errors.add_speaker_assistance_speaker_already_has_assistance", [
            "speaker_id" => $speaker_id,
            "summit_id" => $summit->getId(),
          ]),
        );
  ...
  public function updateSpeakerAssistance(Summit $summit, $assistance_id, array $data) {
    return $this->tx_service->transaction(function () use ($summit, $assistance_id, $data) {
      ...
      if ($assistance->getSummitId() != $summit->getId()) {
        throw new ValidationException(
          trans("validation_errors.speaker_assistance_does_not_belongs_to_summit", [
  ...
  public function deleteSpeakerAssistance(Summit $summit, $assistance_id) {
    return $this->tx_service->transaction(function () use ($summit, $assistance_id) {
      ...
      if ($assistance->isConfirmed()) {
        throw new ValidationException(
          trans("validation_errors.speaker_assistance_delete_already_confirmed", [
            "assistance_id" => $assistance_id,
   ...
   * @throws \Exception
   */
  private function generateSpeakerAssistance(
    Summit $summit,
    PresentationSpeaker $speaker,
    ?Filter $filter,
   ...
   * @throws ValidationException
   */
  public function requestSpeakerEditPermission(
    int $requested_by_id,
    int $speaker_id,
    ...
    return $this->tx_service->transaction(function () use ($requested_by_id, $speaker_id) {
      ...
      $request = $this->speaker_edit_permisssion_repository->getBySpeakerAndRequestor(
        $speaker,
        $requestor,
      );
   ...
   * @throws ValidationException
   */
  public function getSpeakerEditPermission(
    int $requested_by_id,
    int $speaker_id,
  ): SpeakerEditPermissionRequest {
    return $this->tx_service->transaction(function () use ($requested_by_id, $speaker_id) {
   ...
   * @throws ValidationException
   */
  public function approveSpeakerEditPermission(
    string $token,
    int $speaker_id,
  ): SpeakerEditPermissionRequest {
   ...
   * @throws ValidationException
   */
  public function rejectSpeakerEditPermission(
    string $token,
    int $speaker_id,
  ): SpeakerEditPermissionRequest {
    return $this->tx_service->transaction(function () use ($token, $speaker_id) {
      ...
      function (
        ...
        try {
          $this->tx_service->transaction(function () use (
            ...
            $promo_code_strategy = $this->promo_code_strategy_factory->createStrategy(
              $summit,
              $payload,
            );
            ...
            if (!is_null($promo_code_strategy)) {
              Log::debug(
                sprintf("SpeakerService::send getting promo code for speaker id %s", $speaker_id),
              );
