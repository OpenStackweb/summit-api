...
class SummitOrder extends SilverstripeBaseModel implements IQREntity {
  ...
  public function canPubliclyEdit(): bool {
    if (empty($this->hash) || is_null($this->hash_creation_date)) {
      return false;
    }
    ...
    if ($this->hash_creation_date <= $eol) {
      Log::debug(
        sprintf("SummitOrder::canPubliclyEdit id %s ttl %s is void", $this->id, $ttl_minutes),
      );
      return false;
    }
    Log::debug(
      sprintf("SummitOrder::canPubliclyEdit id %s ttl %s is valid", $this->id, $ttl_minutes),
    );
    return true;
  }
  ...
  public function generateNumber(): string {
    $this->number = strtoupper(
      str_replace(".", "", uniqid($this->summit->getOrderQRPrefix() . "_", true)),
    );
    $this->generateQRCode();
    return $this->number;
  }
  ...
  public function setPaid(array $payload = null) {
    ...
    if (
      !is_null($payload) &&
      isset($payload["order_credit_card_type"]) &&
      isset($payload["order_credit_card_4numbers"])
    ) {
      Log::debug(
        sprintf(
  ...
  public function setCancelled(): void {
    ...
    if (in_array($this->status, $ignore_statuses)) {
      return;
    }
  ...
  public function calculateTicketsAndPromoCodesToReturn(): array {
    ...
    foreach ($this->tickets as $ticket) {
      ...
      Log::debug(
        sprintf(
          "SummitOrder::calculateTicketsAndPromoCodesToReturn order %s ticket %s",
          $this->id,
          $ticket->getId(),
        ),
      );
      ...
      if ($ticket->hasPromoCode()) {
        ...
        Log::debug(
          sprintf(
            "SummitOrder::calculateTicketsAndPromoCodesToReturn order %s ticket %s promo code %s",
            $this->id,
            $ticket->getId(),
            $code,
          ),
  ...
  public function getFinalAmount(): float {
    ...
    foreach ($taxes as $tax_id => $tax_detail) {
      ...
      Log::debug(
        sprintf(
          "SummitOrder::getFinalAmount id %s tax %s tax amount %s",
          $this->id,
          $tax->getName(),
          $tax_amount,
        ),
      ...
      Log::debug(
        sprintf(
          "SummitOrder::getFinalAmount id %s tax %s tax amount after rounding %s",
          $this->id,
          $tax->getName(),
          $tax_amount,
        ),
  ...
  public function getAppliedTaxes(): array {
    ...
    foreach ($this->tickets as $ticket) {
      foreach ($ticket->getAppliedTaxes() as $appliedTax) {
        ...
        $applied_taxes[$tax->getId()]["amount"] =
          $applied_taxes[$tax->getId()]["amount"] + $appliedTax->getAmount();
      }
    }
  ...
  public function isVoid(): bool {
    ...
    Log::debug(
      sprintf(
        "SummitOrder::isVoid status %s created %s eol %s",
        $this->status,
        $this->getCreatedUTC()->format("Y-m-d H:i:s"),
        $eol->format("Y-m-d H:i:s"),
      ),
    );
    return ($this->status == IOrderConstants::ErrorStatus ||
      $this->status == IOrderConstants::ReservedStatus) &&
      $this->getCreatedUTC() <= $eol;
  }
  ...
  public function isSingleOrder(): bool {
    ...
    if ($ticket->getOwnerEmail() != $this->getOwnerEmail()) {
      return false;
    }
  ...
  public function hasTicketOwner(SummitAttendee $attendee): bool {
    $criteria = Criteria::create();
    $criteria
      ->where(Criteria::expr()->eq("owner", $attendee))
