...
class SummitAttendeeRegistrationIncompleteReminderEmail extends AbstractSummitAttendeeTicketEmail {
  ...
  public function __construct(SummitAttendee $attendee, ?string $test_email_recipient = null) {
    ...
    if (empty($payload[IMailTemplatesConstants::owner_full_name])) {
      Log::warning(
        sprintf(
          "SummitAttendeeRegistrationIncompleteReminderEmail owner_full_name is empty setting email",
        ),
      );
    ...
    if (empty($payload[IMailTemplatesConstants::owner_first_name])) {
      Log::warning(
        sprintf(
          "SummitAttendeeRegistrationIncompleteReminderEmail owner_first_name is empty setting email",
        ),
      );
    ...
    if (empty($payload[IMailTemplatesConstants::owner_last_name])) {
      Log::warning(
        sprintf(
          "SummitAttendeeRegistrationIncompleteReminderEmail owner_last_name is empty setting email",
        ),
      );
    ...
    Log::debug(
      sprintf(
        "SummitAttendeeRegistrationIncompleteReminderEmail::__construct payload %s template %s",
        json_encode($payload),
        $template_identifier,
      ),
    );
