  ...
  public function handle(
    ...
    if (!empty($query)) {
      ...
      foreach ($query as $q) {
        ...
        if (
          strtolower($q[0]) === "q" ||
          strtolower($q[0]) === "access_token" ||
          strtolower($q[0]) === "token_type"
        ) {
          continue;
        }
        $key .= "." . implode("=", $q);
    ...
    if (!empty($cache_region) && !empty($param_id)) {
      ...
      if (!empty($cache_region_key) && $this->cache_service->exists($cache_region_key)) {
        //
        Log::debug(
          sprintf("CacheMiddleware::handle trying to get region %s data ...", $cache_region_key),
        );
        ...
        if (!empty($region_data)) {
          ...
          Log::debug(
            sprintf(
              "CacheMiddleware::handle got payload %s for region %s",
              json_encode($region),
              $cache_region_key,
            ),
          );
    ...
    if (empty($data) || empty($time) || $evict_cache) {
      ...
      Log::debug(
        sprintf(
          "CacheMiddleware::handle cache value not found for key %s , getting from api...",
          $key,
        ),
      );
      // normal flow ...
      ...
      if ($response instanceof JsonResponse && $response->getStatusCode() === 200) {
        ...
        if (!empty($cache_region_key)) {
          $region[$key] = $key;
          Log::debug(
            sprintf("CacheMiddleware::handle storing data for region %s", $cache_region_key),
          );
          $this->cache_service->setSingleValue(
            $cache_region_key,
            gzdeflate(json_encode($region), 9),
          );
        }
      }
    } else {
