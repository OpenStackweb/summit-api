...
class Election extends SilverstripeBaseModel
{
    ...
    public function isOpen():bool {
        return $this->isNominationsOpen() || $this->isOnVotingPeriod() || $this->isUpcomingVotingPeriod();
    }
    ...
    public function getStatus():string{
        ...
        else if($this->isOnVotingPeriod()){
            $status= Election::StatusElectionOpen;
        }
        else if($this->isUpcomingVotingPeriod()){
    ...
    public function canEditCandidateProfile(Candidate $candidate):bool{
        if($candidate->isGoldMember()) return true;
        $now = new DateTime('now', new DateTimeZone('UTC'));
        if(!is_null($this->nomination_deadline) && $now < $this->nomination_deadline) return true;
        if($this->isNominationsOpen()) return true;
        return false;
    }
    ...
    public function createCandidancy(Member $candidate):Candidate {
        ...
        if($this->isCandidate($candidate))
            throw new ValidationException(sprintf("Member %s is already a candidate.", $candidate->getId()));
    ...
    public function addNomination(Nomination $nomination){
        if($this->nominations->contains($nomination)) return;
     ...
     */
    public function setCandidateApplicationFormRelationshipToOpenstackLabel(string $candidate_application_form_relationship_to_openstack_label): void
    {
     ...
     */
    public function setCandidateApplicationFormExperienceLabel(string $candidate_application_form_experience_label): void
    {
     ...
     */
    public function setCandidateApplicationFormBoardsRoleLabel(string $candidate_application_form_boards_role_label): void
    {
     ...
     */
    public function setCandidateApplicationFormTopPriorityLabel(string $candidate_application_form_top_priority_label): void
    {
