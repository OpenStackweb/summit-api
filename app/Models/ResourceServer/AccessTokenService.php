...
final class AccessTokenService implements IAccessTokenService {
  ...
  private function doIntrospectionRequest($token_value) {
    try {
      ...
      $response = $client->request("POST", "{$auth_server_url}/oauth2/token/introspection", [
      ...
      if (!str_contains($content_type, "application/json")) {
        ...
        Log::warning(
          sprintf(
            "AccessTokenService::doIntrospectionRequest status %s content type %s body %s",
            $status,
            $content_type,
            $body,
          ),
      ...
      Log::warning(
        sprintf(
          "AccessTokenService::doIntrospectionRequest token %s code %s body %s",
          $token_value,
          $code,
          json_encode($body),
        ),
      ...
      throw new OAuth2InvalidIntrospectionResponse(
        sprintf("http code %s - body %s", $ex->getCode(), $body),
      );
    }
  }
}
