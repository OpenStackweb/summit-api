...
class PresentationSpeakerNotificationEmail extends AbstractSummitEmailJob {
  ...
  // metadata
  const EVENT_SLUG = "SUMMIT_SUBMISSIONS_PRESENTATION_SPEAKER_NOTIFICATION";
  const EVENT_NAME = "SUMMIT_SUBMISSIONS_PRESENTATION_SPEAKER_NOTIFICATION";
  const DEFAULT_TEMPLATE = "SUMMIT_SUBMISSIONS_PRESENTATION_SPEAKER_NOTIFICATION";
  ...
  public function __construct(PresentationSpeaker $speaker, Presentation $presentation) {
    ...
    if (empty($payload[IMailTemplatesConstants::speaker_email])) {
      Log::error(
        sprintf(
          "PresentationSpeakerNotificationEmail::__construct speaker %s has no email set",
          $speaker->getId(),
        ),
      );
