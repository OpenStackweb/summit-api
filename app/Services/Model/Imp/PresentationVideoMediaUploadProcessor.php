 ...
 */
final class PresentationVideoMediaUploadProcessor extends AbstractService implements
  IPresentationVideoMediaUploadProcessor {
  /**
   ...
   * @throws \Exception
   */
  public function processPublishedPresentationFor(
    int $summit_id,
    MuxCredentials $credentials,
    ?string $mountingFolder = null,
  ): int {
    $this->_configMux($credentials);

    Log::debug(
      sprintf(
        "PresentationVideoMediaUploadProcessor::processPublishedPresentationFor summit id %s mountingFolder %s",
        $summit_id,
        $mountingFolder,
      ),
    );
    ...
    foreach ($event_ids as $event_id) {
      Log::warning(
        sprintf(
          "PresentationVideoMediaUploadProcessor::processPublishedPresentationFor processing event %s",
          $event_id,
        ),
      );
  ...
  public function processEvent(
    ...
    ?MuxCredentials $credentials = null,
  ): bool {
    try {
      $this->_configMux($credentials);
      return $this->tx_service->transaction(function () use ($event_id, $mountingFolder) {
        try {
          ...
          if (is_null($event) || !$event instanceof Presentation) {
            Log::warning(
              sprintf(
                "PresentationVideoMediaUploadProcessor::processEvent event %s not found",
                $event_id,
              ),
            );
            return false;
          ...
          if (!$event->isPublished()) {
            Log::warning(
              sprintf(
                "PresentationVideoMediaUploadProcessor::processEvent event %s not published",
                $event_id,
              ),
            );
            return false;
          ...
          Log::debug(
            sprintf(
              "PresentationVideoMediaUploadProcessor::processEvent processing event %s (%s)",
              $event->getTitle(),
              $event_id,
            ),
          );
          ...
          if (!empty($event->getMuxAssetId())) {
            Log::warning(
              sprintf(
                "PresentationVideoMediaUploadProcessor::processEvent event %s already has assigned an asset id %s",
                $event_id,
                $event->getMuxAssetId(),
              ),
            );
          ...
          foreach ($event->getMediaUploads() as $mediaUpload) {
            if ($mediaUpload->getMediaUploadType()->isVideo()) {
              if ($has_video) {
                Log::warning(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processEvent event %s processing media upload %s (%s) already has a video processed!.",
                    $event_id,
                    $mediaUpload->getId(),
              ...
              Log::debug(
                sprintf(
                  "PresentationVideoMediaUploadProcessor::processEvent event %s processing media upload %s",
                  $event_id,
                  $mediaUpload->getId(),
                ),
              );
              ...
              $strategy = FileDownloadStrategyFactory::build(
                $mediaUpload->getMediaUploadType()->getPrivateStorageType(),
              );
              if (!is_null($strategy)) {
                $relativePath = $mediaUpload->getRelativePath(
                  IStorageTypesConstants::PrivateType,
                  $mountingFolder,
                );
                Log::debug(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processEvent event %s processing media upload %s relativePath %s",
                    $event_id,
                    $mediaUpload->getId(),
                    $relativePath,
                  ),
                ...
                if ($assetUrl == "#") {
                  Log::debug(
                    sprintf(
                      "PresentationVideoMediaUploadProcessor::processEvent event %s processing media upload %s got asset url %s is not valid",
                      $event_id,
                      $mediaUpload->getId(),
                      $assetUrl,
                    ),
                ...
                Log::debug(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processEvent event %s processing media upload %s got asset url %s",
                    $event_id,
                    $mediaUpload->getId(),
                    $assetUrl,
                  ),
                ...
                $input = new MuxInputSettings([
                  "url" => $assetUrl,
                ]);
                $createAssetRequest = new MuxCreateAssetRequest([
                  "input" => $input,
                  "playback_policy" => [MuxPlaybackPolicy::_PUBLIC],
                ]);
                // Ingest
                ...
                Log::debug(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processEvent event %s Playback URL: %s assset id %s",
                    $event_id,
                    $streaming_url,
                    $asset_id,
                  ),
  ...
  public function enableMP4Support(int $event_id, ?MuxCredentials $credentials = null): void {
    ...
    $this->tx_service->transaction(function () use ($event_id) {
      ...
      if (is_null($event) || !$event instanceof Presentation) {
        Log::warning(
          sprintf(
            "PresentationVideoMediaUploadProcessor::enableMP4Support event %s not found",
            $event_id,
          ),
        );
        return;
      ...
      if (!$event->isPublished()) {
        Log::warning(
          sprintf(
            "PresentationVideoMediaUploadProcessor::enableMP4Support event %s not published",
            $event_id,
          ),
        );
        return;
      ...
      Log::debug(
        sprintf(
          "PresentationVideoMediaUploadProcessor::enableMP4Support processing event %s (%s)",
          $event->getTitle(),
          $event_id,
        ),
      );
      ...
      if (empty($assetId)) {
        Log::warning(
          sprintf(
            "PresentationVideoMediaUploadProcessor::enableMP4Support event %s asset id is empty",
            $event_id,
          ),
        );
        return;
  ...
  private function _enableMP4Support(string $assetId) {
    try {
      $request = new MuxUpdateAssetMP4SupportRequest([
        "mp4_support" => MuxUpdateAssetMP4SupportRequest::MP4_SUPPORT_STANDARD,
      ]);
  ...
  private function _configMux(MuxCredentials $credentials) {
    ...
    try {
      ...
      $this->assets_api = new MuxAssetApi(new GuzzleHttpClient(), $config);

      $this->playback_api = new MuxPlaybackIDApi(new GuzzleHttpClient(), $config);
    } catch (\Exception $ex) {
      Log::error($ex);
      throw $ex;
    }
  ...
  public function processSummitEventsStreamURLs(
    ...
    $event_ids = [];
    $excerpt =
      sprintf("Starting MUX Assets Enabling MP4 Support Process Summit %s.", $summit_id) . PHP_EOL;
    ...
    try {
      ...
      foreach ($event_ids as $event_id) {
        Log::debug(
          sprintf(
            "PresentationVideoMediaUploadProcessor::processMuxAssetsFromStreamUrl processing event %s",
            $event_id,
          ),
        );
        try {
          $this->tx_service->transaction(function () use ($event_id, $credentials, &$excerpt) {
            try {
              ...
              if (is_null($event) || !$event instanceof Presentation) {
                Log::warning(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processMuxAssetsFromStreamUrl event %s not found",
                    $event_id,
                  ),
                );
                return false;
              ...
              if (!$event->isPublished()) {
                Log::warning(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processMuxAssetsFromStreamUrl event %s not published",
                    $event_id,
                  ),
                );
              ...
              if (!empty($event->getMuxAssetId())) {
                $excerpt .=
                  sprintf(
                    "event %s - mux asset id (%s) - already has enabled mp4 support.",
                    $event_id,
                    $event->getMuxAssetId(),
                  ) . PHP_EOL;
                return false;
              ...
              if (empty($stream_url)) {
                Log::warning(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processMuxAssetsFromStreamUrl event %s stream url does not match mux format (empty)",
                    $event_id,
                  ),
                );
                $excerpt .=
                  sprintf("event %s stream url does not match mux format (empty)", $event_id) .
                  PHP_EOL;
              }
              ...
              if (!preg_match(MUXUtils::MUX_STREAM_REGEX, $stream_url, $matches)) {
                Log::warning(
                  sprintf(
                    "PresentationVideoMediaUploadProcessor::processMuxAssetsFromStreamUrl event %s stream url does not match mux format (%s)",
                    $event_id,
                    $stream_url,
                  ),
                );
                $excerpt .=
                  sprintf(
                    "event %s stream url does not match mux format (%s)",
                    $event_id,
                    $stream_url,
                  ) . PHP_EOL;
                return false;
              ...
              if (!is_null($staticRenditions)) {
                $excerpt .=
                  sprintf(
                    "event %s - mux asset id (%s) - has already enabled mp4 support.",
                    $event_id,
                    $asset_id,
                  ) . PHP_EOL;
                return false;
              ...
              $excerpt .=
                sprintf(
                  "event %s - mux asset id (%s) - has been enabled mp4 support.",
                  $event_id,
                  $asset_id,
                ) . PHP_EOL;
            } catch (\Exception $ex) {
    ...
    if (count($event_ids) > 0) {
      Log::debug(
        sprintf(
          "PresentationVideoMediaUploadProcessor::processMuxAssetsFromStreamUrl triggering CreateVideosFromMUXAssetsForSummitJob",
        ),
      );
      // fire exporting
   ...
   * @throws \Exception
   */
  public function createVideosFromMUXAssets(
    int $summit_id,
    MuxCredentials $credentials,
    ?string $mail_to,
  ): int {
    ...
    $event_ids = [];
    $excerpt =
      sprintf("Starting Create Videos From MUX Assets Process Summit %s.", $summit_id) . PHP_EOL;
    try {
      $event_ids = $this->tx_service->transaction(function () use ($summit_id) {
        return $this->event_repository->getPublishedEventsIdsBySummit($summit_id);
      });
        ...
        $this->tx_service->transaction(function () use ($event_id, $credentials, &$excerpt) {
          try {
            ...
            if (is_null($event) || !$event instanceof Presentation) {
              Log::warning(
                sprintf(
                  "PresentationVideoMediaUploadProcessor::createVideosFromMUXAssets event %s not found",
                  $event_id,
                ),
              );
              return false;
            ...
            if (!$event->isPublished()) {
              Log::warning(
                sprintf(
                  "PresentationVideoMediaUploadProcessor::createVideosFromMUXAssets event %s not published",
                  $event_id,
                ),
              );
              $excerpt .= sprintf("event %s not published.", $event_id) . PHP_EOL;
            ...
            if ($staticRenditions->getStatus() != "ready") {
              $excerpt .=
                sprintf(
                  "event %s - transcoding not ready (%s)",
                  $event_id,
                  $staticRenditions->getStatus(),
                ) . PHP_EOL;
              return false;
            }
            ...
            $fileNames = ["low.mp4", "medium.mp4", "high.mp4"];
            ...
            foreach ($staticRenditions->getFiles() as $file) {
              ...
              if (
                array_search($bestFile->getName(), $fileNames) <
                array_search($file->getName(), $fileNames)
              ) {
                $bestFile = $file;
              }
            }
            ...
            $newVideo = PresentationVideoFactory::build([
              ...
              "external_url" => sprintf(
                "https://stream.mux.com/%s/%s",
                $event->getMuxPlaybackId(),
                $bestFile->getName(),
              ),
            ]);
