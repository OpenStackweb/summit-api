...
final class OAuth2SummitTicketApiController extends OAuth2ProtectedController {
  ...
  public function getAllBySummitExternal($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      ...
      if (is_null($filterVal)) {
        return $this->error400("missing owner_email parameter.");
      }
      ...
      $response = new PagingResponse(1, 1, 1, 1, $data);
  ...
  public function getAllBySummitCSV($summit_id) {
    ...
    return $this->_getAllCSV(
      ...
      function () use ($summit) {
        ...
        foreach (
          $summit->getOrderExtraQuestionsByUsage(
        ...
        if (count($diff) > 0) {
          throw new ValidationException(
            sprintf("columns %s are not allowed!", implode(",", $diff)),
          );
        }
        if (empty($columns)) {
      ...
      [
        ...
        "ticket_questions" => $summit->getOrderExtraQuestionsByUsage(
          SummitOrderExtraQuestionTypeConstants::TicketQuestionUsage,
        ),
      ],
    );
  ...
  public function getImportTicketDataTemplate($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      ...
      return $this->export("csv", "ticket-data-import-template", $template, [], []);
    });
  }
  ...
  public function getAttendeeBadge($summit_id, $ticket_id) {
    return $this->processRequest(function () use ($summit_id, $ticket_id) {
      ...
      $ticket = is_int($ticket_id)
        ? $this->repository->getById(intval($ticket_id))
        : $this->repository->getByNumber($ticket_id);
  ...
  public function printAttendeeBadge($summit_id, $ticket_id, $view_type) {
    return $this->processRequest(function () use ($summit_id, $ticket_id, $view_type) {
      ...
      $badge = $this->service->printAttendeeBadge(
        $summit,
        $ticket_id,
        $view_type,
        $current_member,
  ...
  public function canPrintAttendeeBadge($summit_id, $ticket_id, $view_type) {
    return $this->processRequest(function () use ($summit_id, $ticket_id, $view_type) {
      ...
      $badge = $this->service->canPrintAttendeeBadge(
        $summit,
        $ticket_id,
        $view_type,
        $current_member,
