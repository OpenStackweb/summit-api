 ...
 */
final class MemberEventScheduleSummitActionSyncWorkRequestUpdateStrategy implements
  ...
  public function process(AbstractCalendarSyncWorkRequest $request) {
    ...
    $pending_requests = $this->queue_manager->getSummitEventRequestFor(
      $calendar_sync_info->getId(),
      $summit_event_id,
    );
    if (
      count($pending_requests) > 0 ||
      !$request->getOwner()->isEventSynchronized($calendar_sync_info, $summit_event_id)
    ) {
