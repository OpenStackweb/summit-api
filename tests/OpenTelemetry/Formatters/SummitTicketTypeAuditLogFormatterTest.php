...
class SummitTicketTypeAuditLogFormatterTest extends TestCase
{
    ...
    public function testSubjectFormatWithoutContext(): void
    {
        $ticket_type = $this->createMockTicketType('Test Ticket', 50.00, 'USD', 100, 0);

        $formatter_creation = new SummitTicketTypeAuditLogFormatter(IAuditStrategy::EVENT_ENTITY_CREATION);
        $result = $formatter_creation->format($ticket_type, []);
