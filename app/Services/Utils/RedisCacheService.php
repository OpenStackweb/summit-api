...
class RedisCacheService implements ICacheService {
  ...
  public function __construct() {
    try {
      ...
      Log::error(
        sprintf(
          "RedisCacheService::__construct %s %s %s",
          $ex->getCode(),
          $ex->getMessage(),
          $metadata,
        ),
  ...
  private function retryOnConnectionError(
    ...
    do {
      try {
        ...
        Log::warning(
          sprintf(
            "RedisCacheService::retryOnConnectionError code %s msg %s metadata %s, trying to reconnect...",
            $ex->getCode(),
            $ex->getMessage(),
            $metadata,
          ),
        ...
        if ($times > $maxRetries) {
          Log::error(
            sprintf(
              "RedisCacheService::retryOnConnectionError max retries reached %s!.",
              $maxRetries,
            ),
          );
          Log::error($ex);
