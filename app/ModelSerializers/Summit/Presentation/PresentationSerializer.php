...
class PresentationSerializer extends SummitEventSerializer {
  ...
  protected function getMediaUploadsSerializerType(): string {
    ...
    if (
      !is_null($currentUser) &&
      ($currentUser->isAdmin() || $presentation->memberCanEdit($currentUser))
    ) {
  ...
  public function serialize(
    ...
    if (Cache::has($key) && $use_cache) {
      ...
      Log::debug(
        sprintf(
          "PresentationSerializer::serialize cache hit for presentation %s",
          $presentation->getId(),
        ),
      );
      if (!empty($expand)) {
          ...
          switch ($relation) {
            case "media_uploads":
              ...
              foreach ($presentation->getMediaUploads() as $mediaUpload) {
                $media_uploads[] = SerializerRegistry::getInstance()
                  ->getSerializer($mediaUpload, $this->getMediaUploadsSerializerType())
                  ->serialize(
        ...
        switch ($relation) {
          case "speakers":
            ...
            foreach ($presentation->getSpeakers() as $s) {
              $serialized_speaker = SerializerRegistry::getInstance()
            ...
            if (
              isset($values["moderator_speaker_id"]) &&
              intval($values["moderator_speaker_id"]) > 0
            ) {
              $values["moderator"] = SerializerRegistry::getInstance()
                ->getSerializer($presentation->getModerator(), $this->getSerializerType($relation))
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
                  AbstractSerializer::filterFieldsByPrefix($relations, $relation),
          ...
          case "creator":
            if ($presentation->getCreatorId() > 0) {
              ...
              $values["creator"] = SerializerRegistry::getInstance()
                ->getSerializer($presentation->getCreator(), $this->getSerializerType($relation))
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
                  AbstractSerializer::filterFieldsByPrefix($relations, $relation),
          ...
          case "selection_plan":
            if ($presentation->getSelectionPlanId() > 0) {
              ...
              $values["selection_plan"] = SerializerRegistry::getInstance()
                ->getSerializer($presentation->getSelectionPlan())
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
                  AbstractSerializer::filterFieldsByPrefix($relations, $relation),
          ...
          case "slides":
            $slides = [];
            foreach ($presentation->getSlides() as $slide) {
              $slide_values = SerializerRegistry::getInstance()
                ->getSerializer($slide)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "public_comments":
            $public_comments = [];
            foreach ($presentation->getPublicComments() as $comment) {
              $public_comments[] = SerializerRegistry::getInstance()
          ...
          case "links":
            $links = [];
            foreach ($presentation->getLinks() as $link) {
              $link_values = SerializerRegistry::getInstance()
                ->getSerializer($link)
                ->serialize(AbstractSerializer::filterExpandByPrefix($expand, $relation));
              if (empty($link_values["link"])) {
                continue;
              }
              $links[] = $link_values;
            }
          ...
          case "videos":
            $videos = [];
            foreach ($presentation->getVideos() as $video) {
              $video_values = SerializerRegistry::getInstance()
                ->getSerializer($video)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "actions":
            $actions = [];
            foreach ($presentation->getPresentationActions() as $action) {
              $actions[] = SerializerRegistry::getInstance()
