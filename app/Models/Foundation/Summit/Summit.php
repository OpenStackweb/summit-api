...
class Summit extends SilverstripeBaseModel {
   ...
   */
  public function setMarketingSiteOauth2ClientScopes(
    string $marketing_site_oauth2_client_scopes,
  ): void {
  ...
  public function setScheduleDefaultStartDate($schedule_default_start_date) {
    $this->schedule_default_start_date = $this->convertDateFromTimeZone2UTC(
      $schedule_default_start_date,
    );
  }
  ...
  public function setStartShowingVenuesDate($start_showing_venues_date) {
    $this->start_showing_venues_date = $this->convertDateFromTimeZone2UTC(
  ...
  public function addSpeakerAssistance(
    PresentationSpeakerSummitAssistanceConfirmationRequest $assistanceConfirmationRequest,
  ) {
    if ($this->speaker_assistances->contains($assistanceConfirmationRequest)) {
      return;
    }
  ...
  public function getLocalBeginAllowBookingDate(): ?DateTime {
    if (is_null($this->begin_allow_booking_date)) {
      return null;
    }
   ...
   * @return bool
   */
  public static function isPrimaryLocation(SummitAbstractLocation $location) {
   ...
   */
  public function getExtraQuestionTypeByExternalId(
    string $extra_question_type_external_id,
  ...
  public function getPresentations() {
    $query = $this->createQuery(
      "SELECT p from models\summit\Presentation p JOIN p.summit s WHERE s.id = :summit_id",
    );
  ...
  public function getPublishedPresentations() {
    $query = $this->createQuery(
      "SELECT p from models\summit\Presentation p JOIN p.summit s WHERE s.id = :summit_id and p.published = 1",
    );
    return $query->setParameter("summit_id", $this->getIdentifier())->getResult();
  }
   ...
   * @return array
   */
  public function getModeratedPresentationsBy(
   ...
   * @return array
   */
  public function getCreatedPresentations(
    PresentationSpeaker $speaker,
    SelectionPlan $selectionPlan = null,
  ) {
    $selection_plan_cond = "";
  ...
  public function recalculateTrackOrder(PresentationCategory $track, $new_order) {
    ...
    self::recalculateOrderForSelectable(
      $this->presentation_categories->matching($criteria),
      $track,
      $new_order,
    );
  }
  ...
  public function addCategoryGroup(PresentationCategoryGroup $track_group) {
    if ($this->category_groups->contains($track_group)) {
      return;
    }
  ...
  public function isEventInsideSummitDuration(SummitEvent $summit_event) {
    return $this->isTimeFrameInsideSummitDuration(
  ...
  public function isTimeFrameInsideSummitDuration(DateTime $start_date, DateTime $end_date) {
    ...
    return $start_date >= $summit_start_date &&
      $start_date <= $summit_end_date &&
      $end_date <= $summit_end_date &&
      $end_date >= $start_date;
  }
  ...
  public function isTimeFrameOnBookingPeriod(DateTime $start_date, DateTime $end_date): bool {
    ...
    return $start_date >= $this->convertDateFromUTC2TimeZone($this->begin_allow_booking_date) &&
      $start_date <= $this->convertDateFromUTC2TimeZone($this->end_allow_booking_date) &&
      $end_date <= $this->convertDateFromUTC2TimeZone($this->end_allow_booking_date) &&
      $end_date >= $start_date;
  }
   ...
   * @throws ValidationException
   */
  public static function allowToSee(SummitEvent $summit_event, Member $member = null) {
    $event_type = $summit_event->getType();

    if (is_null($event_type)) {
      throw new ValidationException(
        sprintf("event type is null for event id %s", $summit_event->getId()),
      );
    }

    if (!$event_type->isPrivate()) {
      return true;
    }
  ...
  public function setRegistrationSlugPrefix(string $registration_slug_prefix) {
    ...
    if ($slug_prefix != $this->registration_slug_prefix && $this->getPaidTicketsCount() > 0) {
      throw new ValidationException(
        "Cannot change the registration slug when there are paid tickets.",
      );
    }
  ...
  public function getPresentationsSubmittedCount() {
    try {
      ...
      $stmt = $this->prepareRawSQL($sql, [
        "summit_id" => $this->id,
        "status" => Presentation::STATUS_RECEIVED,
      ]);
      $res = $stmt->executeQuery();
      $res = $res->fetchFirstColumn();
      return count($res) > 0 ? $res[0] : 0;
  ...
  public function getDefaultTicketTypeCurrency(): string {
    ...
    foreach ($this->ticket_types as $ticket_type) {
      ...
      if ($ticket_type_currency != $default_currency) {
        throw new ValidationException(
          sprintf("All ticket types for summit %s should have same currency.", $this->getId()),
        );
      }
    }
  ...
  public function addRSVPTemplate(RSVPTemplate $template) {
    if ($this->rsvp_templates->contains($template)) {
      return;
    }
  ...
  public function getCurrentSelectionPlanByStatus($status) {
    ...
    switch (strtoupper($status)) {
      case SelectionPlan::STATUS_SUBMISSION:
        $criteria
          ->where(Criteria::expr()->lte("submission_begin_date", $now_utc))
          ->andWhere(Criteria::expr()->gte("submission_end_date", $now_utc));
        break;
      case SelectionPlan::STATUS_VOTING:
        $criteria
          ->where(Criteria::expr()->lte("voting_begin_date", $now_utc))
          ->andWhere(Criteria::expr()->gte("voting_end_date", $now_utc));
        break;
      case SelectionPlan::STATUS_SELECTION:
        $criteria
          ->where(Criteria::expr()->lte("selection_begin_date", $now_utc))
          ->andWhere(Criteria::expr()->gte("selection_end_date", $now_utc));
        break;
    }
    $selection_plan = $this->selection_plans->matching($criteria)->first();
    return $selection_plan === false ? null : $selection_plan;
  ...
  public function addTrackTagGroup(TrackTagGroup $track_tag_group) {
    if ($this->track_tag_groups->contains($track_tag_group)) {
      return $this;
    }
  ...
  public function setMeetingRoomBookingSlotLength(int $meeting_room_booking_slot_length): void {
    if ($meeting_room_booking_slot_length <= 0) {
      throw new ValidationException(
        "meeting_room_booking_slot_length should be greater than zero.",
      );
    }
   ...
   */
  public function addMeetingBookingRoomAllowedAttribute(
    SummitBookableVenueRoomAttributeType $type,
  ) {
    if ($this->meeting_booking_room_allowed_attributes->contains($type)) {
      return;
    }
   ...
   * @param SummitBookableVenueRoomAttributeType $type
   */
  public function removeMeetingBookingRoomAllowedAttribute(
    SummitBookableVenueRoomAttributeType $type,
  ) {
    if (!$this->meeting_booking_room_allowed_attributes->contains($type)) {
      return;
   ...
   * @return SummitBookableVenueRoomAttributeType|null
   */
  public function getBookableAttributeTypeByTypeName(
    string $type,
   ...
   */
  public function getMeetingBookingRoomAllowedAttributeValueById(
    int $id,
  ...
  public function setApiFeedType(string $api_feed_type): void {
    if (
      !empty($api_feed_type) &&
      !in_array($api_feed_type, ISummitExternalScheduleFeedType::ValidFeedTypes)
    ) {
  ...
  public function addBadgeAccessLevelType(SummitAccessLevelType $access_level): void {
    if ($this->badge_access_level_types->contains($access_level)) {
      return;
    }
  ...
  public function addTaxType(SummitTaxType $tax_type): void {
    if ($this->tax_types->contains($tax_type)) {
      return;
    }
  ...
  public function addFeatureType(SummitBadgeFeatureType $feature_type): void {
    if ($this->badge_features_types->contains($feature_type)) {
      return;
    }
  ...
  public function addBadgeType(SummitBadgeType $badge_type) {
    if ($this->badge_types->contains($badge_type)) {
      return;
    }
  ...
  public function isBookingPeriodOpen(): bool {
    ...
    if (!is_null($this->begin_allow_booking_date) && !is_null($this->end_allow_booking_date)) {
      return $now_utc >= $this->begin_allow_booking_date &&
        $now_utc <= $this->end_allow_booking_date;
    }
   ...
   */
  public function setRegistrationDisclaimerMandatory(
    bool $registration_disclaimer_mandatory,
  ): void {
  ...
  public function addSummitSponsor(Sponsor $sponsor) {
    if ($this->summit_sponsors->contains($sponsor)) {
      return;
    }
  ...
  public function addRefundPolicy(SummitRefundPolicyType $policy) {
    if ($this->refund_policies->contains($policy)) {
      return;
    }
   ...
   * @return SummitBadgeType|null
   */
  public function getRefundPolicyByDays(
    ...
    $criteria->where(
      Criteria::expr()->eq(
        ...
        intval($until_x_days_before_event_starts),
      ),
    );
    $policy = $this->refund_policies->matching($criteria)->first();
    return $policy === false ? null : $policy;
   ...
   */
  public function getRefundPolicyForRefundRequest(
    int $performed_n_days_before_event_starts,
  ): ?SummitRefundPolicyType {
    $criteria = Criteria::create();
    $criteria->where(
      Criteria::expr()->lte(
        "until_x_days_before_event_starts",
        intval($performed_n_days_before_event_starts),
      ),
    );
  ...
  public function getMandatoryOrderExtraQuestionsByUsage(string $usage) {
    ...
    $criteria->andWhere(
      Criteria::expr()->in("usage", [
        $usage,
        SummitOrderExtraQuestionTypeConstants::BothQuestionUsage,
      ]),
    );
  ...
  public function addOrderExtraQuestion(SummitOrderExtraQuestionType $extra_question) {
    if ($this->order_extra_questions->contains($extra_question)) {
      return;
    }
   ...
   */
  public function setRegistrationReminderEmailDaysInterval(
    int $registration_reminder_email_days_interval,
  ): void {
  ...
  public function setExternalRegistrationFeedType(string $external_registration_feed_type): void {
    if (
      !empty($external_registration_feed_type) &&
      !in_array(
        $external_registration_feed_type,
        ISummitExternalRegistrationFeedType::ValidFeedTypes,
      )
    ) {
      throw new ValidationException(
        sprintf("feed type %s is not valid!", $external_registration_feed_type),
      );
    }
   ...
   * @throws ValidationException
   */
  public function setScheduleDefaultEventDetailUrl(
    string $schedule_default_event_detail_url,
  ): void {
    if (
      !empty($schedule_default_event_detail_url) &&
      !str_contains($schedule_default_event_detail_url, ":event_id")
    ) {
      throw new ValidationException(
        "Property schedule_default_event_detail_url must contains at least replacement variable :event_id.",
      );
    }
  ...
  public function addScheduleShareableLink(PersonalCalendarShareInfo $link) {
    if ($this->schedule_shareable_links->contains($link)) {
      return;
    }
  ...
  public function getPaymentGateWayPerApp(
    ...
    Log::debug(
      sprintf(
        "Summit::getPaymentGateWayPerApp id %s application_type %s",
        $this->id,
        $application_type,
      ),
    );
    ...
    if (!in_array($application_type, IPaymentConstants::ValidApplicationTypes)) {
      throw new ValidationException(
        sprintf("Application Type %s is not valid.", $application_type),
      );
    }
    ...
    if (!$payment_profile && !is_null($build_default_payment_gateway_profile_strategy)) {
      ...
      Log::debug(
        sprintf(
          "Summit::getPaymentGateWayPerApp id %s application_type %s trying to get default settings",
          $this->id,
          $application_type,
        ),
      );
  ...
  public function addEmailEventFlow(SummitEmailEventFlow $email_event_flow) {
    if ($this->email_flows_events->contains($email_event_flow)) {
      return;
    }
  ...
  public function getEmailIdentifierPerEmailEventFlowSlug(string $eventSlug): ?string {
    Log::debug(
      sprintf(
        "Summit::getEmailIdentifierPerEmailEventFlowSlug id %s slug %s",
        $this->id,
        $eventSlug,
      ),
    );
    ...
    Log::debug(
      sprintf(
        "Summit::getEmailIdentifierPerEmailEventFlowSlug id %s slug %s trying to get default one",
        $this->id,
        $eventSlug,
      ),
    );
  ...
  public function getEmailRecipientPerEmailEventFlowSlug(string $eventSlug): ?string {
    Log::debug(
      sprintf(
        "Summit::getEmailRecipientPerEmailEventFlowSlug id %s slug %s",
        $this->id,
        $eventSlug,
      ),
    );
  ...
  public function seedDefaultAccessLevelTypes(): void {
    ...
    foreach ($defaults as $default) {
      if (!is_null($this->getBadgeAccessLevelTypeByName($default["name"]))) {
        continue;
      }
   ...
   */
  public function setSpeakerConfirmationDefaultPageUrl(
    string $speaker_confirmation_default_page_url,
  ): void {
  ...
  public function addSummitDocument(SummitDocument $doc) {
    if ($this->summit_documents->contains($doc)) {
      return;
    }
   ...
   */
  public function getSummitRegistrationInvitationByEmail(
    string $email,
   ...
   * @return SummitRegistrationInvitation|null
   */
  public function getSummitRegistrationInvitationById(
    int $invitation_id,
  ): ?SummitRegistrationInvitation {
    $criteria = Criteria::create();
   ...
   * @return SummitRegistrationInvitation|null
   */
  public function getSummitRegistrationInvitationByHash(
    string $hash,
  ): ?SummitRegistrationInvitation {
    $criteria = Criteria::create();
   ...
   * @return bool
   */
  public function canBuyRegistrationTicketByType(
    string $email,
    SummitTicketType $ticketType,
  ): bool {
  ...
  public function addRegistrationInvitation(SummitRegistrationInvitation $invitation) {
    if ($this->registration_invitations->contains($invitation)) {
      return;
    }
  ...
  public function addSubmissionInvitation(SummitSubmissionInvitation $invitation) {
    if ($this->submission_invitations->contains($invitation)) {
      return;
    }
   ...
   */
  public function add2SummitAdministratorPermissionGroup(
    SummitAdministratorPermissionGroup $group,
  ) {
    if ($this->permission_groups->contains($group)) {
      return;
    }
    $this->permission_groups->add($group);
  }

  public function removeFromSummitAdministratorPermissionGroup(
    SummitAdministratorPermissionGroup $group,
  ) {
    if (!$this->permission_groups->contains($group)) {
      return;
  ...
  public function addMediaUploadType(SummitMediaUploadType $type) {
    if ($this->media_upload_types->contains($type)) {
      return;
    }
  ...
  public function addFeaturedSpeaker(PresentationSpeaker $speaker): ?FeaturedSpeaker {
    if ($this->isFeaturedSpeaker($speaker)) {
      return null;
    }
  ...
  public function addTrackChair(Member $member, array $categories): ?SummitTrackChair {
    if (!$member->isOnGroup(IGroup::TrackChairs)) {
      throw new ValidationException(
        sprintf("Member %s does not belong to group %s", $member->getId(), IGroup::TrackChairs),
      );
    }
    ...
    foreach ($categories as $category) {
      ...
      if ($trackChair->isCategoryAllowed($category)) {
        throw new ValidationException(
          sprintf(
            "Category %s is already allowed for member %s",
            $category->getId(),
            $member->getId(),
          ),
        );
  ...
  public function isTrackChair(Member $member, PresentationCategory $category = null): bool {
    ...
    if ($this->isTrackChairAdmin($member)) {
      return true;
    }
    if ($this->isSummitAdmin($member)) {
      return true;
    }
    ...
    if (!is_null($category)) {
      $isTrackChair =
        $this->track_chairs
          ->matching($criteria)
          ->filter(function ($track_chair) use ($category) {
  ...
  public function hasPermissionOnSummit(Member $member): bool {
    return $this->permission_groups
      ->filter(function ($group) use ($member) {
  ...
  public function isSummitAdmin(Member $member): bool {
    ...
    return $this->hasPermissionOnSummit($member) &&
  ...
  public function addPresentationActionType(PresentationActionType $presentation_action_type) {
    if ($this->presentation_action_types->contains($presentation_action_type)) {
      return $this;
    }
   ...
   */
  public function setRegistrationSendQrAsImageAttachmentOnTicketEmail(
    bool $registration_send_qr_as_image_attachment_on_ticket_email,
  ): void {
   ...
   */
  public function setRegistrationSendTicketAsPdfAttachmentOnTicketEmail(
    bool $registration_send_ticket_as_pdf_attachment_on_ticket_email,
  ): void {
   ...
   */
  public function setRegistrationSendTicketEmailAutomatically(
    bool $registration_send_ticket_email_automatically,
  ): void {
   ...
   */
  public function setRegistrationSendOrderEmailAutomatically(
    bool $registration_send_order_email_automatically,
  ): void {
   ...
   */
  public function setAllowUpdateAttendeeExtraQuestions(
    bool $allow_update_attendee_extra_questions,
  ): void {
   ...
   */
  public function setAllowAutomaticReminderEmails(
    bool $registration_allow_automatic_reminder_emails,
  ): void {
  ...
  public function addScheduleSetting(SummitScheduleConfig $scheduleConfig) {
    if ($this->schedule_settings->contains($scheduleConfig)) {
      return;
    }
    $criteria = Criteria::create();
  ...
  public function addRegistrationCompany(Company $registrationCompany) {
    if ($this->registration_companies->contains($registrationCompany)) {
      return;
    }
  ...
  public function addAnnouncementSummitEmail(
    SpeakerAnnouncementSummitEmail $announcementSummitEmail,
  ) {
    if ($this->speakers_announcement_emails->contains($announcementSummitEmail)) {
      return;
    }
  ...
  public function removeAnnouncementSummitEmail(
    SpeakerAnnouncementSummitEmail $announcementSummitEmail,
  ) {
    if (!$this->speakers_announcement_emails->contains($announcementSummitEmail)) {
      return;
  ...
  public function isInviteOnlyRegistration(): bool {
    ...
    $hasWithInvitation =
      $this->getTicketTypesByAudience(SummitTicketType::Audience_With_Invitation)->count() > 0;
    $hasWithoutInvitation =
      $this->getTicketTypesByAudience(SummitTicketType::Audience_Without_Invitation)->count() > 0;
    if ($hasWithInvitation && !$hasAll && !$hasWithoutInvitation) {
      return true;
    }
    return false;
  }
   ...
   * @return int
   */
  public function getTicketCountByTypeAndOwnerEmail(
    SummitTicketType $ticketType,
    string $emailOwner,
  ): int {
    try {
  ...
  public function getBadgeViewTypeByName(string $name): ?SummitBadgeViewType {
    $res = $this->badge_view_types
      ->filter(function ($e) use ($name) {
        return $e instanceof SummitBadgeViewType &&
          strtolower($e->getName()) == strtolower(trim($name));
      })
      ->first();
    return $res === false ? null : $res;
  }
  ...
  public function addBadgeViewType(SummitBadgeViewType $viewType) {
    if ($this->badge_view_types->contains($viewType)) {
      return;
    }
    if (!is_null($this->getBadgeViewTypeByName($viewType->getName()))) {
      return;
    }
   ...
   */
  public function setRegistrationAllowedRefundRequestTillDate(
    ?DateTime $registration_allowed_refund_request_till_date,
  ): void {
    $this->registration_allowed_refund_request_till_date = $this->convertDateFromTimeZone2UTC(
  ...
  public function addSponsorshipType(SummitSponsorshipType $type) {
    if ($this->sponsorship_types->contains($type)) {
      return;
    }
   ...
   */
  public function getSelectionPlanExtraQuestionById(
    int $question_id,
   ...
   * @return SummitSelectionPlanExtraQuestionType|null
   */
  public function getSelectionPlanExtraQuestionByName(
    string $name,
  ): ?SummitSelectionPlanExtraQuestionType {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("name", trim($name)));
   ...
   * @return SummitSelectionPlanExtraQuestionType|null
   */
  public function getSelectionPlanExtraQuestionByLabel(
    string $label,
  ): ?SummitSelectionPlanExtraQuestionType {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("label", trim($label)));
  ...
  public function addSelectionPlanExtraQuestion(SummitSelectionPlanExtraQuestionType $question) {
    if ($this->selection_plan_extra_questions->contains($question)) {
      return;
    }
  ...
  public function hasRelatedActivities(PresentationCategory $track): bool {
    ...
    return $this->events
      ->matching($criteria->where($criteria->expr()->eq("category", $track)))
      ->count() > 0;
  }
  ...
  public function getMuxAllowedDomains(): array {
    if (empty($this->mux_allowed_domains)) {
      return [];
    }
   ...
   */
  public function getRegistrationFeedMetadataById(
    int $metadata_id,
  ): ?SummitRegistrationFeedMetadata {
    $criteria = Criteria::create();
  ...
  public function removeRegistrationFeedMetadata(SummitRegistrationFeedMetadata $metadata): void {
    if (!$this->registration_feed_metadata->contains($metadata)) {
      return;
    }
  ...
  public function addLeadReportSetting(SummitLeadReportSetting $lead_report_setting): void {
    if ($this->lead_report_settings->contains($lead_report_setting)) {
      return;
    }
  ...
  public function getLeadReportSettingsMetadata(?Sponsor $sponsor = null): array {
    $attendee_extra_questions = $this->getOrderExtraQuestionsByUsage(
    ...
    $default_report_setting_columns[
      SummitLeadReportSetting::AttendeeExtraQuestionsKey
    ...
    if (!is_null($sponsor)) {
      $default_report_setting_columns[SummitLeadReportSetting::SponsorExtraQuestionsKey] = $sponsor
        ->getExtraQuestions()
        ->map(function ($entity) {
          return [
            "id" => $entity->getId(),
