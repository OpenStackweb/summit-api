...
abstract class RetrieveSummitEventsStrategy {
  ...
  public function getEvents(array $params = []) {
    ...
    [$page, $per_page] = $this->getPaginationParams();
  ...
  protected function buildFilter() {
    ...
    if (FiltersParams::hasFilterParam()) {
      $filter = FilterParser::parse(FiltersParams::getFilterParam(), $this->getValidFilters());
    }
   ...
   * @return PagingResponse
   */
  abstract public function retrieveEventsFromSource(
  ...
  protected function getFilterValidatorRules(): array {
    return [
      ...
      "class_name" =>
        "sometimes|string|in:" . implode(",", [Presentation::ClassName, SummitEvent::ClassName]),
      "presentation_attendee_vote_date" => "sometimes|date_format:U",
      "votes_count" => "sometimes|integer",
      "selection_status" =>
        "sometimes|string|in:selected,accepted,rejected,alternate,lightning-accepted,lightning-alternate",
      ...
      "submission_source" =>
        "sometimes|string|in:" . implode(",", SummitEvent::ValidSubmissionSources),
    ];
  }
}
