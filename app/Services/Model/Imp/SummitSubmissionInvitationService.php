 ...
 */
final class SummitSubmissionInvitationService extends AbstractService implements
  ISummitSubmissionInvitationService {
  /**
  ...
  public function importInvitationData(Summit $summit, UploadedFile $csv_file): void {
    Log::debug(
      sprintf(
        "SummitSubmissionInvitationService::importInvitationData - summit %s",
        $summit->getId(),
      ),
    );
    ...
    foreach ($reader as $idx => $row) {
      try {
        Log::debug(
          sprintf(
            "SummitSubmissionInvitationService::importInvitationData processing row %s",
            json_encode($row),
          ),
        );
  ...
  public function add(Summit $summit, array $payload): SummitSubmissionInvitation {
    return $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      Log::debug(
        sprintf(
          "SummitSubmissionInvitationService::add adding invitation for email %s to summit %s",
          $email,
          $summit->getName(),
        ),
      );
   ...
   * @throws \Exception
   */
  private function setInvitationSpeaker(
    ...
    return $this->tx_service->transaction(function () use ($invitation, $email) {
      try {
        ...
        if (is_null($speaker)) {
          Log::debug(
            sprintf(
              "SummitSubmissionInvitationService::setInvitationSpeaker - creating speaker for %s",
              $email,
            ),
          );
      ...
      function (
        ...
        try {
          $this->tx_service->transaction(function () use (
            ...
            $res = $this->tx_service->transaction(function () use (
              $flow_event,
              $invitation_id,
              $payload,
            ) {
              Log::debug(
                sprintf(
                  "SummitSubmissionInvitationService::send processing invitation id  %s",
                  $invitation_id,
                ),
              );
              ...
              if (empty($invitation->getOtp())) {
                ...
                if (is_null($otp)) {
                  Log::warning(
                    sprintf(
                      "SummitSubmissionInvitationService::send can not generate OTP for invitation %s",
                      $invitation->getId(),
                    ),
                  );
                  return null;
                ...
                Log::debug(
                  sprintf(
                    "SummitSubmissionInvitationService::send got OTP %s for invitation %s",
                    json_encode($otp),
                    $invitation->getId(),
                  ),
                );
              ...
              if (isset($payload["selection_plan_id"])) {
                ...
                if (is_null($selection_plan)) {
                  Log::warning(
                    sprintf(
                      "SummitSubmissionInvitationService::send selection plan %s does not exists on summit %s",
                      $selection_plan_id,
                      $summit->getId(),
                    ),
                  );
                ...
                if (!$selection_plan->isAllowedMember($email)) {
                  Log::debug(
                    sprintf(
                      "SummitSubmissionInvitationService::send adding %s to selection plan %s",
                      $email,
                      $selection_plan_id,
                    ),
                  );
