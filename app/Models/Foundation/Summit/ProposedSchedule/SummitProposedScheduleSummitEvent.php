 ...
 */
class SummitProposedScheduleSummitEvent extends SilverstripeBaseModel implements
  IPublishableEventWithSpeakerConstraint {
  ...
  public function getDuration(): int {
    ...
    if (!$this->duration) {
      if (!is_null($this->start_date) && !is_null(!is_null($this->end_date))) {
        ...
        Log::debug(
          sprintf(
            "SummitProposedScheduleSummitEvent::getDuration overriding with summit event setting event id %s duration %s",
            $this->id,
            $this->duration,
          ),
        );
  ...
  public function getScheduleId() {
    try {
      return is_null($this->summit_proposed_schedule)
        ? 0
        : $this->summit_proposed_schedule->getId();
    } catch (\Exception $ex) {
      return 0;
    }
  }
