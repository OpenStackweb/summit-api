...
class RegisteredMemberOrderPaidMail extends AbstractSummitEmailJob {
  ...
  public function __construct(SummitOrder $order) {
    ...
    $payload[IMailTemplatesConstants::order_raw_amount] = FormatUtils::getNiceFloat(
      $order->getRawAmount(),
    );
    $payload[IMailTemplatesConstants::order_amount] = FormatUtils::getNiceFloat(
      $order->getFinalAmount(),
    );
    $payload[IMailTemplatesConstants::order_taxes] = FormatUtils::getNiceFloat(
      $order->getTaxesAmount(),
    );
    $payload[IMailTemplatesConstants::order_discount] = FormatUtils::getNiceFloat(
      $order->getDiscountAmount(),
    );
    $payload[IMailTemplatesConstants::order_refunded_amount] = FormatUtils::getNiceFloat(
      $order->getRefundedAmount(),
    );
    $payload[IMailTemplatesConstants::order_amount_adjusted] = FormatUtils::getNiceFloat(
      $order->getFinalAmountAdjusted(),
    );
    ...
    $payload[
      IMailTemplatesConstants::order_purchase_date
    ] = FormatUtils::getNiceDateMonthDayYearTextual(
      $summit->convertDateFromUTC2TimeZone($order->getCreatedUTC()),
    );
    ...
    foreach ($order->getTickets() as $ticket) {
      $ticket_dto = [
        ...
        IMailTemplatesConstants::net_selling_price => FormatUtils::getNiceFloat(
          $ticket->getNetSellingPrice(),
        ),
      ...
      if ($ticket->hasPromoCode()) {
        ...
        if ($promo_code instanceof SummitRegistrationDiscountCode) {
          ...
          $promo_code_dto[IMailTemplatesConstants::discount_amount] = FormatUtils::getNiceFloat(
    ...
    $payload[IMailTemplatesConstants::manage_orders_url] = sprintf(
      "%s/a/my-tickets",
    ...
    Log::debug(
      sprintf(
        "RegisteredMemberOrderPaidMail::__construct template_identifier %s",
        $template_identifier,
      ),
    );
