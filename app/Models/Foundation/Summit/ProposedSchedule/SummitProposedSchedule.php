...
class SummitProposedSchedule extends SilverstripeBaseModel {
   ...
   */
  public function getScheduledSummitEventById(
   ...
   * @return SummitProposedScheduleSummitEvent|null
   */
  public function getScheduledSummitEventByEvent(
    SummitEvent $event,
  ): ?SummitProposedScheduleSummitEvent {
    $criteria = Criteria::create();
  ...
  public function getScheduledSummitEventsByLocationAndDateRange(
    ...
    ?SummitAbstractLocation $location = null,
  ): array {
    $criteria = Criteria::create();
    if ($start_date != null) {
   ...
   * @return SummitProposedScheduleSummitEvent|null
   */
  public function getProposedPublishedEventBeforeThan(
   ...
   * @return SummitProposedScheduleSummitEvent|null
   */
  public function getProposedPublishedEventAfterThan(
    IPublishableEvent $event,
  ): ?SummitProposedScheduleSummitEvent {
    $date = $event->getStartDate();
    $location = $event->getLocation();
  ...
  public function addScheduledSummitEvent(IPublishableEvent $scheduled_event) {
    if (!$scheduled_event instanceof SummitProposedScheduleSummitEvent) {
      return;
    }
    if ($this->scheduled_summit_events->contains($scheduled_event)) {
      return;
    }
    ...
    if ($this->scheduled_summit_events->matching($criteria)->count() > 0) {
      throw new ValidationException(
        sprintf("Scheduled event %s already exists", $scheduled_event->getId()),
      );
    }
