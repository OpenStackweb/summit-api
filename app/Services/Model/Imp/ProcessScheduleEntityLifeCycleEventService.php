 ...
 */
final class ProcessScheduleEntityLifeCycleEventService extends AbstractService implements
  IProcessScheduleEntityLifeCycleEventService {
  const CacheTTL = 2;
  /**
  ...
  public function __construct(
    ...
    Log::debug(
      sprintf(
        "ProcessScheduleEntityLifeCycleEventService::__construct schedule.use_realtime_updates %s",
        $use_realtime_updates,
      ),
    );

    if ($use_realtime_updates) {
      Log::debug(
        "ProcessScheduleEntityLifeCycleEventService::__construct schedule.use_realtime_updates is enabled",
      );
  ...
  public function process(
    ...
    $this->tx_service->transaction(function () use (
      ...
      if ($entity_type === "PresentationSpeakerAssignment") {
        ...
        if (!$ps_assignment instanceof PresentationSpeakerAssignment) {
          return;
        }
        ...
        if ($presentation instanceof Presentation) {
          Log::debug(
            sprintf(
              "ProcessScheduleEntityLifeCycleEventService::process presentation %s from summit %s",
              $entity_id,
              $summit->getId(),
            ),
          );
      ...
      if ($entity_type === "Presentation" || $entity_type === "SummitEvent") {
        $cache_region_key = CacheRegions::getCacheRegionFor(
      ...
      if (!empty($cache_region_key) && $this->cache_service->exists($cache_region_key)) {
        Log::debug(
          sprintf(
            "ProcessScheduleEntityLifeCycleEventService::process will clear cache region %s",
            $cache_region_key,
          ),
        );
        ...
        if (!empty($region_data)) {
          ...
          Log::debug(
            sprintf(
              "ProcessScheduleEntityLifeCycleEventService::process got payload %s region %s",
              json_encode($region),
              $cache_region_key,
            ),
          );
          foreach ($region as $key => $val) {
            Log::debug(
              sprintf(
                "ProcessScheduleEntityLifeCycleEventService::process clearing cache key %s",
                $key,
              ),
            );
      ...
      if (is_null($this->rabbit_service)) {
        Log::debug(
          "ProcessScheduleEntityLifeCycleEventService::process rabbit service is disabled.",
        );
        return;
      }
      ...
      if ($entity_type === "PresentationSpeaker") {
        foreach ($this->summit_repository->getAll() as $summit) {
          ...
          if ($summit->getSpeaker($entity_id)) {
            Log::debug(
              sprintf(
                "ProcessScheduleEntityLifeCycleEventService::process publishing speaker %s to summit %s",
                $entity_id,
                $summit->getId(),
              ),
            );
      ...
      Log::debug(
        sprintf(
          "ProcessScheduleEntityLifeCycleEventService::process publishing summit %s entity id %",
          $summit_id,
          $entity_id,
        ),
      );
