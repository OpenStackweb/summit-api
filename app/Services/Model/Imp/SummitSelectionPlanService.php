 ...
 */
final class SummitSelectionPlanService extends AbstractService implements
  ISummitSelectionPlanService {
  /**
    ...
    return $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      if (!is_null($former_selection_plan)) {
        throw new ValidationException(
          trans("validation_errors.SummitSelectionPlanService.addSelectionPlan.alreadyExistName", [
  ...
  public function updateSelectionPlan(Summit $summit, $selection_plan_id, array $payload) {
    return $this->tx_service->transaction(function () use ($summit, $selection_plan_id, $payload) {
      ...
      if (isset($payload["name"])) {
        ...
        if (
          !is_null($former_selection_plan) &&
          $former_selection_plan->getId() != $selection_plan_id
        ) {
          throw new ValidationException(
            trans(
   ...
   * @throws EntityNotFoundException
   */
  public function addTrackGroupToSelectionPlan(
    Summit $summit,
    $selection_plan_id,
    $track_group_id,
  ) {
  ...
  public function markPresentationAsViewed(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!$selection_plan->isSelectionOpen()) {
        throw new ValidationException(
          sprintf("Selection period is not open for selection plan %s", $selection_plan->getId()),
        );
      }
      ...
      $isAuth =
        ...
        $summit->isTrackChair($current_member, $category);

      if (!$isAuth) {
        throw new ValidationException(
          sprintf(
            "Presentation %s has changed to track %s",
  ...
  public function addPresentationComment(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      return $presentation->addTrackChairComment(
        $current_member,
        trim($payload["body"]),
        boolval($payload["is_public"]),
      );
    });
  }
  ...
  public function createPresentationCategoryChangeRequest(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!$selection_plan->isAllowTrackChangeRequests()) {
        throw new ValidationException(
          sprintf(
            "Selection Plan %s does not allow track change requests",
  ...
  public function resolvePresentationCategoryChangeRequest(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!$selection_plan->isAllowTrackChangeRequests()) {
        throw new ValidationException(
          sprintf(
            "Selection Plan %s does not allow approve/reject track change requests",
            $selection_plan->getId(),
          ),
        );
      }
      ...
      $isAuth =
        $summit->isTrackChairAdmin($current_member) ||
        $summit->isTrackChair($current_member, $newCategory);
      ...
      if ($approved) {
        ...
        foreach ($presentation->getPendingCategoryChangeRequests() as $pending_request) {
          if ($pending_request->getId() == $change_request->getId()) {
            continue;
          }
          $pending_request->reject(
            $current_member,
            sprintf("Request ID %s was approved instead.", $change_request->getId()),
          );
        }
      } else {
  ...
  public function upsertAllowedPresentationActionType(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      } elseif ($new_order == null || $new_order === $current_order) {
        throw new ValidationException(
          "Presentation Action Type is already assigned to this Selection Plan in the order specified.",
        );
      }

      if ($new_order != null && $current_order != $new_order) {
        $selection_plan->recalculatePresentationActionTypeOrder(
  ...
  public function importAllowedMembers(
    ...
    Log::debug(
      sprintf(
        "SelectionPlanService::importAllowedMembers - summit %s selection plan %s",
        $summit->getId(),
        $selection_plan_id,
      ),
    );
    ...
    if (!$reader->hasColumn("email")) {
      throw new ValidationException("Email column is missing.");
    }

    $this->upload_strategy->save($csv_file, $path, $basename);

    ProcessSelectionPlanAllowedMemberData::dispatch(
      $summit->getId(),
      $selection_plan_id,
      $basename,
    );
  ...
  public function processAllowedMemberData(
    ...
    Log::debug(
      sprintf(
        "SelectionPlanService::processAllowedMemberData summit %s selection_plan_id %s filename %s",
        $summit_id,
        $selection_plan_id,
        $filename,
      ),
    ...
    $selection_plan = $this->tx_service->transaction(function () use (
      $summit_id,
      $selection_plan_id,
    ) {
    ...
    foreach ($reader as $row) {
      $this->tx_service->transaction(function () use ($selection_plan, $reader, $row) {
        Log::debug(
          sprintf(
            "SelectionPlanService::processAllowedMemberData processing row %s",
            json_encode($row),
          ),
        );
    ...
    Log::debug(
      sprintf(
        "SelectionPlanService::processAllowedMemberData deleting file %s from storage %s",
        $path,
        $this->download_strategy->getDriver(),
      ),
    );
