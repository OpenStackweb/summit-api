...
class SpeakerServiceBulkSendChunkingTest extends ProtectedApiTestCase
{
     ...
     * snapshot - a speaker that is deleted or stops matching the filter between two page reads
     * shifts every later row left by one and exactly one speaker is silently skipped. Every
    ...
    {
        Queue::fake();
        Config::set('emails.speakers_process_db_chunk_size', 1);
        ...
        try {
            $this->service()->triggerSendEmails(self::$summit, $this->basePayload(), null);
        } finally {
