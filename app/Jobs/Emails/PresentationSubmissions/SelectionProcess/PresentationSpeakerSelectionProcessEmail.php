...
abstract class PresentationSpeakerSelectionProcessEmail extends AbstractSummitEmailJob {
  ...
  public function __construct(
    ...
    foreach (
      ...
      if (
        ...
        $p->hasCreatedBy() &&
        !in_array($p->getCreatedBy()->getEmail(), $cc_email) &&
        $speaker->getEmail() != $p->getCreatedBy()->getEmail()
      ) {
        $cc_email[] = $p->getCreatedBy()->getEmail();
      }
      $payload[
        IMailTemplatesConstants::accepted_presentations
      ][] = SerializerRegistry::getInstance()
        ->getSerializer($p, IPresentationSerializerTypes::SpeakerEmails)
        ->serialize();
    }
    ...
    foreach (
      ...
      $payload[
        IMailTemplatesConstants::accepted_moderated_presentations
      ][] = SerializerRegistry::getInstance()
        ->getSerializer($p, IPresentationSerializerTypes::SpeakerEmails)
        ->serialize();
    }
    ...
    foreach (
      ...
      $payload[
        IMailTemplatesConstants::alternate_presentations
      ][] = SerializerRegistry::getInstance()
        ->getSerializer($p, IPresentationSerializerTypes::SpeakerEmails)
        ->serialize();
    }
    ...
    foreach (
      ...
      $payload[
        IMailTemplatesConstants::alternate_moderated_presentations
      ][] = SerializerRegistry::getInstance()
        ->getSerializer($p, IPresentationSerializerTypes::SpeakerEmails)
        ->serialize();
    }
    ...
    foreach (
      ...
      $payload[
        IMailTemplatesConstants::rejected_presentations
      ][] = SerializerRegistry::getInstance()
        ->getSerializer($p, IPresentationSerializerTypes::SpeakerEmails)
        ->serialize();
    }
    ...
    foreach (
      ...
      $payload[
        IMailTemplatesConstants::rejected_moderated_presentations
      ][] = SerializerRegistry::getInstance()
        ->getSerializer($p, IPresentationSerializerTypes::SpeakerEmails)
        ->serialize();
    }
    ...
    if (!is_null($promo_code)) {
      ...
      if (!is_null($promo_code->getValidUntilDate())) {
        $payload[IMailTemplatesConstants::promo_code_until_date] = $promo_code
          ->getValidUntilDate()
          ->format("Y-m-d H:i:s");
      }
    ...
    $payload[
      IMailTemplatesConstants::speaker_confirmation_link
    ] = $summit->getSpeakerConfirmationDefaultPageUrl();
    ...
    parent::__construct(
      ...
      $payload[IMailTemplatesConstants::speaker_email],
      null,
      $payload[IMailTemplatesConstants::cc_email] ?? null,
    );
  }
  ...
  public static function getEmailTemplateSchema(): array {
    ...
    $presentations_schema["properties"][
      IMailTemplatesConstants::selection_plan
    ] = $selection_plan_schema;
    ...
    $presentations_schema["properties"][IMailTemplatesConstants::speakers][
      "items"
    ] = $speaker_schema;
