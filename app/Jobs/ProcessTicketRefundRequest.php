  ...
  public function handle(
    ...
    try {
      $tx_service->transaction(function () use ($default_payment_gateway_strategy, $repository) {
        Log::debug(
          sprintf("ProcessTicketRefundRequest::handle processing ticket %s", $this->ticket_id),
        );
        $ticket = $repository->getByIdExclusiveLock($this->ticket_id);

        if (!$ticket instanceof SummitAttendeeTicket || !$ticket->isRefundRequested()) {
          return;
        }
        ...
        Log::debug(
          sprintf("ProcessTicketRefundRequest::handle got ticket %s", $ticket->getNumber()),
        );
        ...
        $amount_2_refund = ($rate / 100.0) * $ticket->getNetSellingPrice();
        ...
        $request = $ticket->refund(
          ...
          $amount_2_refund,
          $paymentGatewayRes,
          sprintf("* AUTOMATIC REFUND FROM POLICY %s.", $policy->getId()),
        );
