...
final class SummitRegistrationStatsSerializer extends SilverStripeSerializer {
  ...
  public function serialize(
    ...
    $values["total_payment_amount_collected"] = JsonUtils::toJsonMoney(
      $summit->getTotalPaymentAmountCollected($start_date, $end_date),
    );
    $values["total_refund_amount_emitted"] = JsonUtils::toJsonMoney(
      $summit->getTotalRefundAmountEmitted($start_date, $end_date),
    );
    ...
    $values["total_checked_in_attendees"] = $summit->getInPersonCheckedInAttendeesCount(
      $start_date,
      $end_date,
    );
    $values["total_virtual_attendees"] = $summit->getVirtualAttendeesCount($start_date, $end_date);

    $values["total_non_checked_in_attendees"] = $summit->getInPersonNonCheckedInAttendeesCount(
      $start_date,
      $end_date,
    );
    $values[
      "total_virtual_non_checked_in_attendees"
    ] = $summit->getVirtualNonCheckedInAttendeesCount($start_date, $end_date);
