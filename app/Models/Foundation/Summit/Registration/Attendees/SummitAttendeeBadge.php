...
class SummitAttendeeBadge extends SilverstripeBaseModel implements IQREntity {
  ...
  public function generateQRCode(): string {
    ...
    if (!empty($qr_codes_enc_key)) {
      Log::debug(
        sprintf(
          "SummitAttendeeBadge::generateQRCode encrypting qr_code %s for id %s",
          $this->qr_code,
          $this->id,
        ),
      );
    ...
    Log::debug(
      sprintf(
        "SummitAttendeeBadge::generateQRCode generated qr_code %s for id %s",
        $this->qr_code,
        $this->id,
      ),
    );
  ...
  public static function parseQRCode(string $qr_code): array {
    ...
    if (count($fields) != 4) {
      throw new ValidationException("Invalid Badge QR code.");
    }
  ...
  public function getAllFeatures() {
    ...
    foreach ($inherited_features as $inherited_feature) {
      if ($this->features->contains($inherited_feature)) {
        continue;
      }
