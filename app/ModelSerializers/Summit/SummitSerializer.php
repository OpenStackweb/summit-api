...
class SummitSerializer extends SilverStripeSerializer {
  protected static $array_mappings = [
    ...
    "RegistrationSendQrAsImageAttachmentOnTicketEmail" =>
      "registration_send_qr_as_image_attachment_on_ticket_email:json_boolean",
    "RegistrationSendTicketAsPdfAttachmentOnTicketEmail" =>
      "registration_send_ticket_as_pdf_attachment_on_ticket_email:json_boolean",
    "RegistrationSendTicketEmailAutomatically" =>
      "registration_send_ticket_email_automatically:json_boolean",
    "RegistrationSendOrderEmailAutomatically" =>
      "registration_send_order_email_automatically:json_boolean",
    "RegistrationAllowAutomaticReminderEmails" =>
      "registration_allow_automatic_reminder_emails:json_boolean",
    ...
    "RegistrationAllowedRefundRequestTillDate" =>
      "registration_allowed_refund_request_till_date:datetime_epoch",
  ...
  public function serialize(
    ...
    $values["page_url"] = empty($main_page)
      ? null
      : sprintf(
        "%s%s",
    ...
    $values["schedule_page_url"] = empty($schedule_page)
      ? null
      : sprintf(
        "%s%s",
        Config::get("server.assets_base_url", "https://www.openstack.org/"),
    ...
    $values["schedule_event_detail_url"] = empty($schedule_page)
      ? null
      : sprintf(
        "%s%s/%s",
        Config::get("server.assets_base_url", "https://www.openstack.org/"),
    ...
    if (in_array("ticket_types", $relations)) {
      ...
      foreach ($summit->getTicketTypes() as $ticket) {
        $ticket_types[] = SerializerRegistry::getInstance()
          ->getSerializer($ticket)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "ticket_types"));
      }
    ...
    if (in_array("badge_access_level_types", $relations)) {
      ...
      foreach ($summit->getBadgeAccessLevelTypes() as $accessLevelType) {
        $badge_access_level_types[] = SerializerRegistry::getInstance()
          ->getSerializer($accessLevelType)
          ->serialize(
            AbstractSerializer::filterExpandByPrefix($expand, "badge_access_level_types"),
          );
      }
    ...
    if (in_array("badge_view_types", $relations)) {
      ...
      foreach ($summit->getBadgeViewTypes() as $viewType) {
        $badge_view_types[] = SerializerRegistry::getInstance()
          ->getSerializer($viewType)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "badge_view_types"));
      }
    ...
    if (in_array("payment_profiles", $relations)) {
      ...
      foreach ($summit->getActivePaymentGateWayProfiles() as $profile) {
        $payment_profiles[] = SerializerRegistry::getInstance()
          ->getSerializer($profile, $this->getSerializerType())
      ...
      foreach ($values["payment_profiles"] as $payment_profile) {
        if (
          $payment_profile["application_type"] == IPaymentConstants::ApplicationTypeBookableRooms
        ) {
          $has_bookable_rooms_profile = true;
        }
        if (
          $payment_profile["application_type"] == IPaymentConstants::ApplicationTypeRegistration
        ) {
          $has_registration_profile = true;
        }
      }

      $build_default_payment_gateway_profile_strategy =
        $params["build_default_payment_gateway_profile_strategy"] ?? null;
      ...
      if (
        ...
        $values["payment_profiles"][] = SerializerRegistry::getInstance()
          ->getSerializer(
    ...
    if (in_array("tax_types", $relations)) {
      ...
      foreach ($summit->getTaxTypes() as $tax_type) {
        $tax_types[] = SerializerRegistry::getInstance()
          ->getSerializer($tax_type)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "tax_types"));
      }
    ...
    if (in_array("summit_documents", $relations)) {
      ...
      foreach ($summit->getSummitDocuments() as $document) {
        $summit_documents[] = SerializerRegistry::getInstance()
          ->getSerializer($document)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "summit_documents"));
      }
    ...
    if (in_array("meeting_booking_room_allowed_attributes", $relations)) {
      ...
      foreach ($summit->getMeetingBookingRoomAllowedAttributes() as $attr) {
        $meeting_booking_room_allowed_attributes[] = SerializerRegistry::getInstance()
          ->getSerializer($attr)
          ->serialize(
            AbstractSerializer::filterExpandByPrefix(
              $expand,
              "meeting_booking_room_allowed_attributes",
            ),
          );
    ...
    if (in_array("summit_sponsors", $relations)) {
      ...
      foreach ($summit->getPublishedSummitSponsors() as $sponsor) {
        $summit_sponsors[] = SerializerRegistry::getInstance()
          ->getSerializer($sponsor)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "summit_sponsors"));
      }
    ...
    if (in_array("locations", $relations)) {
      ...
      foreach ($summit->getLocations() as $location) {
        $locations[] = SerializerRegistry::getInstance()
          ->getSerializer($location)
          ->serialize(
    ...
    if (in_array("wifi_connections", $relations)) {
      ...
      foreach ($summit->getWifiConnections() as $wifi_connection) {
        $wifi_connections[] = SerializerRegistry::getInstance()
          ->getSerializer($wifi_connection)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "wifi_connections"));
      }
    ...
    if (in_array("selection_plans", $relations)) {
      ...
      foreach ($summit->getSelectionPlans() as $selection_plan) {
        $selection_plans[] = SerializerRegistry::getInstance()
          ->getSerializer($selection_plan)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "selection_plans"));
      }
    ...
    if (in_array("email_flows_events", $relations)) {
      ...
      foreach ($summit->getAllEmailFlowsEvents() as $email_flow_event) {
        $email_flows_events[] = SerializerRegistry::getInstance()
          ->getSerializer($email_flow_event)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "email_flows_events"));
      }
    ...
    if (in_array("presentation_action_types", $relations)) {
      ...
      foreach ($summit->getPresentationActionTypes() as $action) {
        $presentation_action_types[] = SerializerRegistry::getInstance()
          ->getSerializer($action)
          ->serialize(
            AbstractSerializer::filterExpandByPrefix($expand, "presentation_action_types"),
          );
      }
        ...
        switch ($relation) {
          ...
          case "tracks":
            ...
            foreach ($summit->getPresentationCategories() as $cat) {
              $presentation_categories[] = SerializerRegistry::getInstance()
                ->getSerializer($cat)
                ->serialize(AbstractSerializer::filterExpandByPrefix($expand, $relation));
            }
          ...
          case "track_groups":
            ...
            foreach ($summit->getCategoryGroups() as $group) {
              $track_groups[] = SerializerRegistry::getInstance()
                ->getSerializer($group)
                ->serialize(AbstractSerializer::filterExpandByPrefix($expand, $relation));
            }
          ...
          case "sponsors":
            ...
            foreach ($summit->getEventSponsors() as $company) {
              $sponsors[] = SerializerRegistry::getInstance()
                ->getSerializer($company)
                ->serialize(AbstractSerializer::filterExpandByPrefix($expand, $relation));
            }
          ...
          case "speakers":
            ...
            foreach ($summit->getSpeakers() as $speaker) {
              $speakers[] = SerializerRegistry::getInstance()
                ->getSerializer($speaker)
                ->serialize(
