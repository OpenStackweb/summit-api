...
trait RequestScopedCache {
  ...
  static function getScopeId(): string {
    ...
    if (is_null($requestId)) {
      ...
      Log::debug(sprintf("RequestScopedCache::getScopeId setting request id %s.", $requestId));
  ...
  function cache(string $key, Closure $callback) {
    ...
    if (!empty($res)) {
      ...
      Log::debug(
        sprintf(
          "RequestScopedCache::cache scope %s key %s cache hit res %s.",
          $scope,
          $key,
          $json_res,
        ),
    ...
    Log::debug(
      sprintf(
        "RequestScopedCache::cache scope %s key %s res %s adding to cache.",
        $scope,
        $key,
        $json,
      ),
