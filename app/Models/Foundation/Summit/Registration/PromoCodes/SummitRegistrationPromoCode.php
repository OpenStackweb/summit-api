...
class SummitRegistrationPromoCode extends SilverstripeBaseModel {
  ...
  public function hasQuantityAvailable(): bool {
    ...
    if ($quantity_available > 0 && $quantity_available <= $quantity_used) {
      return false;
    }
    return true;
  }
  ...
  public function validate(string $email, ?string $company) {
    Log::debug(
      sprintf(
        "SummitRegistrationPromoCode::validate - email: %s, company: %s",
        $email,
        $company ?? "",
      ),
    );
    ...
    if (!$this->isLive()) {
      throw new ValidationException(
        sprintf("The Promo Code %s is not a valid code.", $this->getCode()),
      );
    }
  }
  ...
  public function isLive(): bool {
    ...
    if (
      !is_null($this->valid_since_date) &&
      !is_null($this->valid_until_date) &&
      ($now_utc < $this->valid_since_date || $now_utc > $this->valid_until_date)
    ) {
      return false;
    }
    return true;
  ...
  public function addUsage(string $ownerEmail, int $usage = 1) {
    ...
    if (!$this->isInfinite() && $newVal > $quantity_available) {
      throw new ValidationException(
        sprintf("Promo code %s has reached max. usage (%s).", $this->code, $quantity_available),
      );
    }

    $this->quantity_used = $newVal;
    $this->setRedeemed(!$this->isInfinite() && $this->quantity_available == $quantity_used);
  }
  ...
  public function removeUsage(int $to_restore, ?string $owner_email = null) {
    ...
    if ($newVal < 0) {
      // we want to restore more than we used
      throw new ValidationException(
        sprintf(
    ...
    Log::info(
      sprintf("SummitRegistrationPromoCode::removeUsage quantity_used %s", $this->quantity_used),
    );
  ...
  public function canBeAppliedTo(SummitTicketType $ticketType): bool {
    Log::debug(
      sprintf("SummitRegistrationPromoCode::canBeAppliedTo Ticket type %s.", $ticketType->getId()),
    );
  ...
  public function addBadgeFeatureType(SummitBadgeFeatureType $feature_type) {
    if ($this->badge_features->contains($feature_type)) {
      return;
    }
