...
class SummitSelectedPresentationList extends SilverstripeBaseModel {
  ...
  public function canEdit(Member $member): bool {
    ...
    if ($this->list_type == self::Group) {
      return $this->category->getSummit()->isTrackChair($member, $this->category) ||
        $this->category->getSummit()->isTrackChairAdmin($member);
    }
  ...
  public function compareHash(?string $oldHash): bool {
    ...
    if (!empty($this->hash) && empty($oldHash)) {
      return false;
    }
  ...
  public function removeSelection(SummitSelectedPresentation $selection) {
    if (!$this->selected_presentations->contains($selection)) {
      return;
    }
   ...
   */
  public function getSelectionByPresentation(
  ...
  public function reorder(): void {
    ...
    $order = 1;
    foreach (
      $this->getSelectedPresentationsByCollection(SummitSelectedPresentation::CollectionMaybe)
      as $p
    ) {
      $p->setOrder($order);
      ++$order;
    ...
    $order = 1;
    foreach (
      $this->getSelectedPresentationsByCollection(SummitSelectedPresentation::CollectionPass)
      as $p
    ) {
      $p->setOrder($order);
      ++$order;
