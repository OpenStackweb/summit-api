...
class Member extends SilverstripeBaseModel {
  ...
  public function addLegalAgreement(LegalAgreement $legalAgreement) {
    if ($this->legal_agreements->contains($legalAgreement)) {
      return;
    }
  ...
  public function getAffiliationByOrgName(string $orgName): ?Affiliation {
    $res = $this->affiliations
      ->filter(function ($e) use ($orgName) {
  ...
  public function isOnExternalGroup(string $code): bool {
    ...
    if ($resource_server_ctx instanceof IResourceServerContext) {
      foreach ($resource_server_ctx->getCurrentUserGroups() as $group) {
        Log::debug(
          sprintf(
            "Member::isOnExternalGroup id %s code %s external group %s",
            $this->id,
            $code,
            $group["slug"],
          ),
        );
        if (isset($group["slug"]) && trim($group["slug"]) == trim($code)) {
          return true;
        }
      }
    }
  ...
  public function isOnGroup(string $code, $skip_external = false) {
    ...
    if ($this->isAdmin($skip_external)) {
      Log::debug(
        sprintf("Member::isOnGroup member %s group code %s isAdmin true", $this->id, $code),
      );
      return true;
    }
    ...
    if (!is_null($group)) {
      Log::debug(
        sprintf("Member::isOnGroup member %s group code %s belongs to group", $this->id, $code),
      );
      return true;
    }
    if (!$skip_external) {
      Log::debug(
        sprintf("Member::isOnGroup member %s group code %s check external ones", $this->id, $code),
      );
  ...
  public function addFavoriteSummitEvent(SummitEvent $event) {
    if ($this->isOnFavorite($event)) {
      throw new ValidationException(
        sprintf(
          "Event %s already belongs to member %s favorites.",
          $event->getId(),
          $this->getId(),
        ),
      );
  ...
  public function removeFavoriteSummitEvent(SummitEvent $event) {
    ...
    if (is_null($favorite)) {
      throw new ValidationException(
        sprintf(
          "Event %s does not belongs to member %s favorite.",
          $event->getId(),
          $this->getId(),
        ),
      );
  ...
  public function removeFromSchedule(SummitEvent $event) {
    ...
    if (is_null($schedule)) {
      throw new ValidationException(
        sprintf(
          "Event %s does not belongs to member %s schedule.",
          $event->getId(),
          $this->getId(),
        ),
      );
   ...
   */
  public function getScheduleSyncInfoByEvent(
    $summit_event_id,
  ...
  public function getReservationById(int $reservation_id): ?SummitRoomReservation {
    $criteria = Criteria::create()->where(Criteria::expr()->eq("id", $reservation_id));
  ...
  public function setFirstName(?string $first_name): void {
    ...
    if ($resource_server_ctx->getCurrentUserEmail() === $this->email) {
      ...
      $resource_server_ctx->updateAuthContextVar(
        IResourceServerContext::UserFirstName,
        $first_name,
      );
    }
  ...
  public function getSummitRegistrationOrderById(int $order_id): ?SummitOrder {
    $criteria = Criteria::create()->where(Criteria::expr()->eq("id", $order_id));
  ...
  public function addSummitRegistrationOrder(SummitOrder $summit_order) {
    if ($this->summit_registration_orders->contains($summit_order)) {
      return;
    }
  ...
  public function getSponsorBySummit(Summit $summit): ?Sponsor {
    $sponsor = $this->sponsor_memberships
      ->filter(function ($entity) use ($summit) {
  ...
  public function add2SummitAdministratorPermissionGroup(
    ...
    if ($this->summit_permission_groups->contains($group)) {
      return;
    }
  ...
  public function getAllAllowedSummitsIds(): array {
    return MemberSummitStrategyFactory::getMemberSummitStrategy($this)->getAllAllowedSummitIds();
  }
  ...
  public function hasPermissionForOnGroup(Summit $summit, string $groupSlug): bool {
    if (!SummitAdministratorPermissionGroup::isValidGroup($groupSlug)) {
      return false;
    }
  ...
  public function addElectionApplication(Nomination $application) {
    if ($this->election_applications->contains($application)) {
      return;
    }
  ...
  public function getLatestElectionApplications() {
    ...
    if (is_null($currentElection)) {
      return [];
    }
    $criteria = Criteria::create()->where(Criteria::expr()->eq("election", $currentElection));
  ...
  public function nominateCandidate(Member $candidate, Election $election): Nomination {
    ...
    if ($this->election_nominations->matching($criteria)->count() > 0) {
      throw new ValidationException(
        sprintf("You have already nominated %s.", $candidate->getFullName()),
      );
    }
  ...
  public function getElectionNominationsFor(Election $election) {
    $criteria = Criteria::create()->where(Criteria::expr()->eq("election", $election));
  ...
  public function addCandidateProfile(Candidate $candidate): void {
    if ($this->candidate_profiles->contains($candidate)) {
      return;
    }
   ...
   */
  public function getAcceptedPresentations(Summit $summit, ?Filter $filter = null) {
    $extraWhere = "";
    if (!is_null($filter)) {
   ...
   * @return array
   */
  public function getAcceptedPresentationIds(Summit $summit, ?Filter $filter = null) {
    $ids = [];
   ...
   * @return bool
   */
  public function hasAcceptedPresentations(Summit $summit, ?Filter $filter = null) {
   ...
   * @return bool
   */
  public function hasAlternatePresentations(Summit $summit, ?Filter $filter = null) {
   ...
   * @return array
   */
  public function getAlternatePresentations(Summit $summit, ?Filter $filter = null) {
    ...
    $query->setParameter("summit_id", $summit->getId())->setParameter("submitter_id", $this->id);
   ...
   * @return array
   */
  public function getAlternatePresentationIds(Summit $summit, ?Filter $filter = null) {
    $ids = [];
   ...
   * @return bool
   */
  public function hasRejectedPresentations(Summit $summit, ?Filter $filter = null) {
   ...
   * @return array
   */
  public function getRejectedPresentations(Summit $summit, ?Filter $filter = null) {
   ...
   * @return array
   */
  public function getRejectedPresentationIds(Summit $summit, ?Filter $filter = null) {
    $ids = [];
  ...
  public function isSummitAllowed(Summit $summit): bool {
    ...
    return MemberSummitStrategyFactory::getMemberSummitStrategy($this)->isSummitAllowed($summit);
  }
  ...
  public function isAuthzFor(Summit $summit, Sponsor $sponsor = null): bool {
    ...
    if (
      $this->isSponsorUser() &&
      !is_null($sponsor) &&
      $this->hasSponsorMembershipsFor($summit, $sponsor)
    ) {
      return true;
    }
    return false;
