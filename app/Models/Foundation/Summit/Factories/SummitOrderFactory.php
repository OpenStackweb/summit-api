...
final class SummitOrderFactory {
  ...
  public static function populate(Summit $summit, SummitOrder $order, array $payload): SummitOrder {
    ...
    if (
      isset($payload["owner"]) &&
      !is_null($payload["owner"]) &&
      $payload["owner"] instanceof Member
    ) {
      $owner = $payload["owner"];
      Log::debug(
        sprintf(
    ...
    } elseif (isset($payload["owner_company_id"]) && !is_null($payload["owner_company_id"])) {
      ...
      if ($ownerCompanyId > 0) {
        ...
        if (is_null($company)) {
          throw new ValidationException(
            sprintf(
              "Owner company with id %d not found as a registered company for summit %d",
              $ownerCompanyId,
              $summit->getId(),
            ),
          );
    ...
    if (
      isset($payload["billing_address_zip_code"]) &&
      !is_null($payload["billing_address_zip_code"])
    ) {
    ...
    if (
      isset($payload["billing_address_country"]) &&
      !is_null($payload["billing_address_country"])
    ) {
    ...
    if (count($extra_questions) > 0) {
      $mandatory_questions = $summit->getMandatoryOrderExtraQuestionsByUsage(
        SummitOrderExtraQuestionTypeConstants::OrderQuestionUsage,
      );

      if (count($extra_questions) < $mandatory_questions->count()) {
        throw new ValidationException(
          "You neglected to fill in all mandatory questions for the order.",
        );
      }

      $questions = $summit->getOrderExtraQuestionsByUsage(
        SummitOrderExtraQuestionTypeConstants::OrderQuestionUsage,
      );
          ...
          foreach ($extra_questions as $question_answer) {
            if (intval($question_answer["question_id"]) == $question->getId()) {
              ...
              if ($question->allowsValues() && !$question->allowValue($value)) {
                Log::warning(
                  sprintf("value %s is not allowed for question %s", $value, $question->getName()),
                );
