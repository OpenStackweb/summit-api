...
class FilterElement extends AbstractFilterElement {
  ...
  public static function mapValueSymbols(string $val): string {
    if (isset(self::$symbols[strtoupper($val)])) {
  ...
  public static function makeBetween($field, $value, $same_field_op = null) {
    ...
    if (count($value) != 2) {
      throw new \InvalidArgumentException("Value must be an array of 2 elements.");
    }
    return new self($field, $value, [">=", "<="], $same_field_op);
  }
  ...
  public function __toString(): string {
    return sprintf(
      "%s%s%s",
      $this->field,
      is_array($this->operator) ? json_encode($this->operator) : $this->operator,
