 ...
 */
final class SummitSelectedPresentationListService extends AbstractService implements
  ISummitSelectedPresentationListService {
  /**
   ...
   * @inheritDoc
   */
  public function getTeamSelectionList(
    Summit $summit,
   ...
   * @inheritDoc
   */
  public function getIndividualSelectionList(
    Summit $summit,
    int $selection_plan_id,
    int $track_id,
    int $owner_id,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_null($selection_list)) {
        // create it
        return $this->createIndividualSelectionList(
          $summit,
          $selection_plan_id,
          $track_id,
          $owner_id,
  ...
  public function reorderList(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      Log::debug(
        sprintf(
          "SummitSelectedPresentationListService::reorderList track %s list %s payload %s.",
          $track_id,
          $list_id,
          json_encode($payload),
        ),
      ...
      if (
        count($payload["presentations"]) > $category->getTrackChairAvailableSlots() &&
        trim($payload["collection"]) == SummitSelectedPresentation::CollectionSelected
      ) {
        throw new ValidationException(
          sprintf(
            "You can not add more presentations (%s).",
            $category->getTrackChairAvailableSlots(),
          ),
        );
      }
       ...
       */
      foreach (
        $selection_list->getSelectedPresentationsByCollection(trim($payload["collection"]))
        as $selection
      ) {
        ...
        if (is_null($selection)) {
          ...
          if ($selection_list->isGroup()) {
            $presentation->addTrackChairNotification(
              $current_member,
              "{member} added this presentation to the team list",
            );
          }
        }
  ...
  public function assignPresentationToMyIndividualList(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_null($selection_list)) {
        $selection_list = $this->createIndividualSelectionList(
          $summit,
          $selection_plan_id,
          $track_id,
          $current_member->getId(),
      ...
      if (
        ...
        $highest_order_in_list >= $available_slots
      ) {
        // will not add this presentation, list is full
        throw new ValidationException(
          sprintf(
            "Presentation Selection list is full. Currently at %s. Limit is %s.",
      ...
      if (
        !empty($former_collection) &&
        $former_collection != SummitSelectedPresentation::CollectionPass
      ) {
        Log::debug(
          sprintf(
   ...
   * @throws \Exception
   */
  public function removePresentationFromMyIndividualList(
    Summit $summit,
    int $selection_plan_id,
    int $track_id,
    int $presentation_id,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      $category = $summit->getPresentationCategory(intval($track_id));
      if (is_null($category) || !$category instanceof PresentationCategory) {
        throw new EntityNotFoundException("Track not found.");
      }
      ...
      if (is_null($selection_list)) {
        throw new EntityNotFoundException(
          sprintf(
            "Individual List not found for member %s and category %s",
            $current_member->getId(),
            $category->getId(),
          ),
        );
