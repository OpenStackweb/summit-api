 ...
 */
final class DoctrineSummitAttendeeRepository extends SilverStripeDoctrineRepository implements
  ISummitAttendeeRepository {
  /**
  ...
  protected function applyExtraJoins(QueryBuilder $query, ?Filter $filter = null) {
    ...
    if ($filter->hasFilter("features") || $filter->hasFilter("features_id")) {
      $query = $query->leftJoin("b.features", "bf")->leftJoin("bt.badge_features", "btf");
    }

    if ($filter->hasFilter("access_levels") || $filter->hasFilter("access_levels_ids")) {
  ...
  protected function getFilterMappings() {
    return [
      ...
      "first_name" => ["m.first_name :operator :value", "e.first_name :operator :value"],
      "has_member" => new DoctrineSwitchFilterMapping([
        "true" => new DoctrineCaseFilterMapping("true", "m.id is not null and m.id > 0"),
        "false" => new DoctrineCaseFilterMapping("false", "m.id is null"),
      ]),
      ...
      "has_checkin" => new DoctrineSwitchFilterMapping([
        "true" => new DoctrineCaseFilterMapping("true", "e.summit_hall_checked_in = 1"),
        "false" => new DoctrineCaseFilterMapping("false", "e.summit_hall_checked_in = 0"),
      ]),
      "tickets_count" => new DoctrineHavingFilterMapping(
        "",
        "t.owner",
        sprintf(
          "COALESCE(SUM(CASE WHEN (t.status = '%s' AND t.is_active = 1) THEN 1 ELSE 0 END), 0)  :operator :value",
          IOrderConstants::PaidStatus,
        ),
      ),
      ...
      "last_name" => ["m.last_name :operator :value", "e.surname :operator :value"],
      "full_name" => [
      ...
      "email" => [Filter::buildEmailField("m.email"), Filter::buildEmailField("e.email")],
      "external_order_id" => new DoctrineFilterMapping("t.external_order_id :operator :value"),
      "external_attendee_id" => new DoctrineFilterMapping(
        "t.external_attendee_id :operator :value",
      ),
      "presentation_votes_date" =>
        "pv.created:datetime_epoch|" . SilverstripeBaseModel::DefaultTimeZone,
      "presentation_votes_count" => new DoctrineHavingFilterMapping(
        "",
        "pv.voter",
        "count(pv.id) :operator :value",
      ),
      "presentation_votes_track_group_id" => new DoctrineFilterMapping("pcg.id :operator :value"),
      "features" => ["bf.name :operator :value", "btf.name :operator :value"],
      "features_id" => ["bf.id :operator :value", "btf.id :operator :value"],
      "access_levels" => "bac.name :operator :value",
      "access_levels_id" => "bac.id :operator :value",
      "summit_hall_checked_in_date" => Filter::buildDateTimeEpochField(
        "e.summit_hall_checked_in_date",
      ),
      ...
      "notes" => new DoctrineLeftJoinFilterMapping(
        "e.notes",
        "notes",
        "notes.content :operator :value",
      ),
      "has_notes" => new DoctrineSwitchFilterMapping([
        "true" => new DoctrineCaseFilterMapping("true", "SIZE(e.notes) > 0"),
        "false" => new DoctrineCaseFilterMapping("false", "SIZE(e.notes) = 0"),
      ]),
    ];
  }
  ...
  public function getBySummitAndEmailAndFirstNameAndLastNameAndExternalId(
    ...
    if (!empty($first_name)) {
      $query = $query
        ->andWhere("e.first_name = :first_name")
