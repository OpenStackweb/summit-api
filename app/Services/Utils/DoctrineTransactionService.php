...
final class DoctrineTransactionService implements ITransactionService {
  ...
  public function shouldReconnect(\Exception $e): bool {
    ...
    if ($e instanceof \PDOException) {
      switch (intval($e->getCode())) {
        ...
        case 2002:
          Log::warning(
            "DoctrineTransactionService::shouldReconnect:  php_network_getaddresses: getaddrinfo failed: nodename nor servname provided, or not known!",
          );
          return true;
      }
    }
  ...
  public function transaction(
    Closure $callback,
    int $isolationLevel = TransactionIsolationLevel::READ_COMMITTED,
  ) {
    $retry = 0;
    $done = false;
    $result = null;
    ...
    while (!$done and $retry < self::MaxRetries) {
      try {
        ...
        if (!$em->isOpen()) {
          Log::warning(
            "DoctrineTransactionService::transaction: entity manager is closed!, trying to re open...",
          );
        ...
        if ($this->shouldReconnect($ex)) {
          ...
          if ($retry === self::MaxRetries) {
            Log::warning(
              sprintf("DoctrineTransactionService::transaction Max Retry Reached %s", $retry),
            );
            Log::error($ex);
            throw $ex;
          }
