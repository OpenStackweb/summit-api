...
final class MUXApi implements IMUXApi {
  ...
  public function setCredentials(MuxCredentials $credentials): IMUXApi {
    try {
      ...
      $this->assets_api = new MuxAssetApi(new GuzzleHttpClient(), $this->config);

      $this->playback_api = new MuxPlaybackIDApi(new GuzzleHttpClient(), $this->config);

      $this->signing_key_api = new SigningKeysApi(new GuzzleHttpClient(), $this->config);
  ...
  public function createPlaybackRestriction(
    ...
    try {
      Log::debug(
        sprintf(
          "MUXApi::createPlaybackRestriction allowed_domains %s",
  ...
  public function deletePlaybackRestriction(string $playback_restriction_id): void {
    try {
      $this->playback_restriction_api->deletePlaybackRestriction($playback_restriction_id);
    } catch (\Exception $ex) {
      Log::error($ex);
      throw $ex;
    }
