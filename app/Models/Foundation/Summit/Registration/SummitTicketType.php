...
class SummitTicketType extends SilverstripeBaseModel implements ISummitTicketType {
  ...
  const SubTypes = [self::Subtype_Regular, self::Subtype_PrePaid];
  ...
  public function preRemoveHandler(LifecycleEventArgs $args) {
    if (
      Config::get("registration.validate_ticket_type_removal", true) &&
      $this->quantity_sold > 0
    ) {
      throw new ValidationException(
        sprintf("Can not delete ticket type %s because has sold tickets.", $this->getId()),
      );
    }
  }
  ...
  public function addAppliedTax(SummitTaxType $tax) {
    if ($this->applied_taxes->contains($tax)) {
      return;
    }
  ...
  public function isLive() {
    ...
    if (!is_null($this->sales_start_date) && $now_utc < $this->sales_start_date) {
      Log::warning(
        sprintf(
          "SummitTicketType::isLive ticket %s is not live (now_utc < sales_start_date)",
          $this->id,
        ),
      );
      return false;
    ...
    if (!is_null($this->sales_end_date) && $now_utc > $this->sales_end_date) {
      Log::warning(
        sprintf(
          "SummitTicketType::isLive ticket %s is not live (now_utc < sales_end_date)",
          $this->id,
        ),
      );
      return false;
  ...
  public function sell(int $qty = 1): int {
    ...
    if (!$unlimited) {
      if ($qty > $quantity_2_sell) {
        throw new ValidationException(
          sprintf("Can not sell more tickets than max. total available (%s).", $quantity_2_sell),
        );
      }
  ...
  public function restore(int $qty): int {
    ...
    Log::info(
      sprintf(
        "SummitTicketType::restore qty_2_restore %s final qty %s",
        $qty,
        $this->quantity_sold,
      ),
    );
