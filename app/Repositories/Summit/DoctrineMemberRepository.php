 ...
 */
final class DoctrineMemberRepository extends SilverStripeDoctrineRepository implements
  IMemberRepository {
  /**
   * @return string
   */
  ...
  protected function getFilterMappings() {
    ...
    return [
      ...
      "last_name" => new DoctrineFilterMapping("( LOWER(e.last_name) :operator LOWER(:value) )"),
      "full_name" => new DoctrineFilterMapping(
        "( CONCAT(LOWER(e.first_name), ' ', LOWER(e.last_name)) :operator LOWER(:value) )",
      ),
      "first_name" => new DoctrineFilterMapping("( LOWER(e.first_name) :operator LOWER(:value) )"),
      "email" => [
        Filter::buildEmailField("e.email"),
        Filter::buildEmailField("e.second_email"),
        Filter::buildEmailField("e.third_email"),
      ],
      "member_id" => new DoctrineFilterMapping("( e.id :operator :value )"),
      ...
      "summit_id" => "s.id",
      "group_slug" => new DoctrineJoinFilterMapping("e.groups", "g", "g.code :operator :value"),
      ...
      "presentations_selection_plan_id" => new DoctrineFilterMapping(
        "EXISTS ( 
          ...
          (!empty($extraSelectionPlanFilter) ? sprintf($extraSelectionPlanFilter, "51") : "") .
          ")",
      ),
      ...
      "has_accepted_presentations" => new DoctrineSwitchFilterMapping([
        "true" => new DoctrineCaseFilterMapping(
          ...
          sprintf(
            ...
            (!empty($extraSelectionStatusFilter)
              ? sprintf($extraSelectionStatusFilter, "12")
              : "") .
            " )",
        ),
      ...
      "has_alternate_presentations" => new DoctrineSwitchFilterMapping([
        "true" => new DoctrineCaseFilterMapping(
          "true",
          sprintf(
            'EXISTS (
            ...
            (!empty($extraSelectionStatusFilter)
              ? sprintf($extraSelectionStatusFilter, "21")
              : "") .
            " )",
        ),
        "false" => new DoctrineCaseFilterMapping(
      ...
      "has_rejected_presentations" => new DoctrineSwitchFilterMapping([
        "true" => new DoctrineCaseFilterMapping(
          "true",
          sprintf(
            'EXISTS (
              ...
              (!empty($extraSelectionStatusFilter)
                ? sprintf($extraSelectionStatusFilter, "31")
                : " ") .
              ' AND NOT EXISTS (
                                            SELECT ___sp31.id 
      ...
      "is_speaker" => new DoctrineSwitchFilterMapping([
        "true" => new DoctrineCaseFilterMapping(
          "true",
          '(
                                 EXISTS (
  ...
  public function getSubmittersBySummit(
    ...
    Log::debug(
      sprintf("DoctrineMemberRepository::getSubmittersBySummit summit %s", $summit->getId()),
    );
    $start = time();
    $res = $this->getParametrizedAllByPage(
      function () use ($summit) {
        return $this->getEntityManager()
          ->createQueryBuilder()
          ->distinct(true)
  ...
  public function getSubmittersIdsBySummit(
    ...
    return $this->getParametrizedAllIdsByPage(
      function () use ($summit) {
        return $this->getEntityManager()
          ->createQueryBuilder()
          ->distinct(true)
