...
class SummitEvent extends SilverstripeBaseModel implements IPublishableEvent {
  ...
  const AllowedFields = [self::FieldAbstract, self::FieldSocialDescription, self::FieldLevel];
    ...
    if ($this->type->isAllowsPublishingDates()) {
      ...
      if (is_null($start_date) || is_null($end_date)) {
        throw new ValidationException(
          "To publish this event you must define a start/end datetime.",
        );
      }
  ...
  public function setStartDate(DateTime $value): SummitEvent {
    Log::debug(
      sprintf("SummitEvent::setStartDate id %s value %s", $this->id, $value->getTimestamp()),
    );
    if (!$this->type->isAllowsPublishingDates()) {
  ...
  public function setEndDate(DateTime $value): SummitEvent {
    Log::debug(
      sprintf("SummitEvent::setEndDate id %s value %s", $this->id, $value->getTimestamp()),
    );
    if (!$this->type->isAllowsPublishingDates()) {
      throw new ValidationException("Type does not allows Publishing Period.");
    }
  ...
  public function getCurrentRSVPSubmissionSeatType(): string {
    ...
    if ($count_regular < intval($this->rsvp_max_user_number)) {
      return RSVP::SeatTypeRegular;
    }
    $count_wait = $this->getRSVPSeatTypeCount(RSVP::SeatTypeWaitList);
    if ($count_wait < intval($this->rsvp_max_user_wait_list_number)) {
      return RSVP::SeatTypeWaitList;
    }
  ...
  public function getDateNice(): string {
    ...
    $date_nice =
      date("l, F j, g:ia", strtotime($start_date)) . "-" . date("g:ia", strtotime($end_date));
    return $date_nice;
  }
  ...
  public function hasAccess(?Member $member): bool {
    ...
    Log::debug(
      sprintf(
        "SummitEvent::hasAccess member %s (%s) event %s.",
        $member->getId(),
        $member->getEmail(),
        $this->id,
      ),
    ...
    if ($this->summit->isPubliclyOpen()) {
      Log::debug(
        sprintf(
          "SummitEvent::hasAccess member %s (%s) event %s summit is public open.",
          $member->getId(),
          $member->getEmail(),
          $this->id,
        ),
    ...
    if ($member->isAdmin() || $this->summit->isSummitAdmin($member) || $member->isTester()) {
      Log::debug(
        sprintf(
          "SummitEvent::hasAccess member %s (%s) event %s is SuperAdmnin/Admin/SummitAdmin or Tester.",
          $member->getId(),
          $member->getEmail(),
          $this->id,
        ),
    ...
    if ($member->hasPaidTicketOnSummit($this->summit)) {
      if ($this->category->getAllowedAccessLevels()->count() > 0) {
        ...
        foreach ($member->getPaidSummitTickets($this->summit) as $ticket) {
          ...
          Log::debug(
            sprintf(
              "SummitEvent::hasAccess checking access levels for ticket %s ticket access levels (%s).",
              $ticket->getId(),
              implode(",", $ticketAccessLevelsIds),
            ),
          );
          if (count(array_intersect($eventAccessLevelsIds, $ticketAccessLevelsIds))) {
            return true;
          }
        }
        Log::debug(
          sprintf(
            "SummitEvent::hasAccess member %s (%s) event %s has no access.",
            $member->getId(),
            $member->getEmail(),
            $this->id,
          ),
      ...
      Log::debug(
        sprintf(
          "SummitEvent::hasAccess member %s (%s) event %s has a paid ticket.",
          $member->getId(),
          $member->getEmail(),
          $this->id,
        ),
  ...
  public function isMuxStream(): bool {
    ...
    if (preg_match("/(.*\.mux\.com)/i", $this->streaming_url)) {
      return true;
    }
    return false;
  }
  ...
  public function setStreamIsSecure(bool $stream_is_secure): void {
    Log::debug(
      sprintf(
        "SummitEvent::setStreamIsSecure summit %s event %s stream_is_secure %s",
        $this->summit->getId(),
        $this->id,
        $stream_is_secure,
      ),
  ...
  public function addAllowedTicketType(SummitTicketType $ticket_type) {
    if ($this->allowed_ticket_types->contains($ticket_type)) {
      return;
    }
  ...
  public function setSubmissionSource(string $submission_source): void {
    if (!in_array($submission_source, self::ValidSubmissionSources)) {
      throw new ValidationException(
        sprintf("%s is not a valid submission source.", $submission_source),
      );
    }
