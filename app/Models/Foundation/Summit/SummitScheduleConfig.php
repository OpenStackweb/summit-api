...
class SummitScheduleConfig extends SilverstripeBaseModel {
  ...
  const AllowedColorSource = [
    self::ColorSource_EventType,
    self::ColorSource_Track,
    self::ColorSource_TrackGroup,
  ];
  ...
  public function addPreFilter(SummitSchedulePreFilterElementConfig $filter): void {
    if ($this->pre_filters->contains($filter)) {
      return;
    }
    // check type
    $criteria = Criteria::create();
   ...
   * @throws ValidationException
   */
  public function recalculateFilterOrder(
    SummitScheduleFilterElementConfig $filter,
    int $new_order,
  ) {
   ...
   */
  public function setHidePastEventsWithShowAlwaysOnSchedule(
    bool $hide_past_events_with_show_always_on_schedule,
  ): void {
