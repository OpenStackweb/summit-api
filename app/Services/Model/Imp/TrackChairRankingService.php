 ...
 */
final class TrackChairRankingService extends AbstractService implements ITrackChairRankingService {
  /**
   * @inheritDoc
   */
  public function getTrackChairRatingType(
    ...
    return $this->tx_service->transaction(function () use (
      $selection_plan,
      $track_chair_rating_type_id,
    ) {
      $track_chair_rating_type = $selection_plan->getTrackChairRatingTypeById(
        $track_chair_rating_type_id,
      );
  ...
  public function updateTrackChairRatingType(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (isset($payload["name"])) {
        ...
        $track_chair_rating_type_by_name = $selection_plan->getTrackChairRatingTypeByName(
          $rating_type_name,
        );

        if (
          !is_null($track_chair_rating_type_by_name) &&
          $track_chair_rating_type_by_name->getId() != $track_chair_rating_type_id
        ) {
          throw new ValidationException("There is another rating type with the same name.");
        }
      }

      $track_chair_rating_type = PresentationTrackChairRatingTypeFactory::populate(
        $track_chair_rating_type,
        $payload,
      );

      if (
        isset($payload["order"]) &&
        intval($payload["order"]) != $track_chair_rating_type->getOrder()
      ) {
        $selection_plan->recalculateTrackChairRatingTypeOrder(
          $track_chair_rating_type,
          intval($payload["order"]),
        );
      }
      return $track_chair_rating_type;
    });
  ...
  public function updateTrackChairScoreType(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      $track_chair_score_type = $track_chair_rating_type->getScoreTypeById(
        $track_chair_score_type_id,
      );
      ...
      if (isset($payload["name"])) {
        ...
        $track_chair_score_type_by_name = $track_chair_rating_type->getScoreTypeByName(
          $score_type_name,
        );

        if (
          !is_null($track_chair_score_type_by_name) &&
          $track_chair_score_type_by_name->getId() != $track_chair_score_type_id
        ) {
          throw new ValidationException("There is another score type with the same name.");
        }
      }

      $track_chair_score_type = PresentationTrackChairScoreTypeFactory::populate(
        $track_chair_score_type,
        $payload,
      );

      if (
        isset($payload["score"]) &&
        intval($payload["score"]) != $track_chair_score_type->getScore()
      ) {
        $track_chair_rating_type->recalculateScoreTypeScore(
          $track_chair_score_type,
          intval($payload["score"]),
        );
      }
      return $track_chair_score_type;
    });
