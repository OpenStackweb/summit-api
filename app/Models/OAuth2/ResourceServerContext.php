...
final class ResourceServerContext implements IResourceServerContext {
  ...
  public function getCurrentUser(
    ...
    $user_email_verified = boolval(
      $this->getAuthContextVar(IResourceServerContext::UserEmailVerified),
    );
    ...
    if (is_null($member)) {
      ...
      $member = $this->tx_service->transaction(function () use ($user_email) {
        ...
        Log::debug(
          sprintf("ResourceServerContext::getCurrentUser getting user by email %s", $user_email),
        );
    ...
    if (is_null($member)) {
      // user exist on IDP but not in our local DB, proceed to create it
      Log::debug(
        sprintf(
    ...
    if (is_null($member)) {
      Log::warning(
        sprintf(
          "ResourceServerContext::getCurrentUser user not found %s (%s).",
          $user_external_id,
          $user_email,
        ),
      );
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if ($update_member_fields) {
        ...
        if (!empty($user_email)) {
          Log::debug(
            sprintf(
              "ResourceServerContext::getCurrentUser setting email for member %s",
              $member->getId(),
            ),
          );
        ...
        if (!empty($user_first_name)) {
          Log::debug(
            sprintf(
              "ResourceServerContext::getCurrentUser setting first name for member %s",
              $member->getId(),
            ),
          );
        ...
        if (!empty($user_last_name)) {
          Log::debug(
            sprintf(
              "ResourceServerContext::getCurrentUser setting last name for member %s",
              $member->getId(),
            ),
          );
  ...
  private function checkGroups(Member $member): Member {
    Log::debug(
      sprintf(
        "ResourceServerContext::checkGroups member %s %s",
        $member->getId(),
        $member->getEmail(),
      ),
    );
    ...
    foreach ($this->getCurrentUserGroups() as $idpGroup) {
      Log::debug(
        sprintf(
          "ResourceServerContext::checkGroups member %s %s group %s",
          $member->getId(),
          $member->getEmail(),
          json_encode($idpGroup),
        ),
      );
      $slug = $idpGroup["slug"] ?? "";
      Log::debug(
        sprintf(
          "ResourceServerContext::checkGroups member %s %s group slug %s",
          $member->getId(),
          $member->getEmail(),
          $slug,
        ),
      ...
      Log::debug(
        sprintf(
          "ResourceServerContext::checkGroups member %s %s slug %s",
          $member->getId(),
          $member->getEmail(),
          trim($idpGroup["slug"]),
        ),
