...
final class PresentationCategorySerializer extends SilverStripeSerializer {
  ...
  public function serialize(
    ...
    if (!$category instanceof PresentationCategory) {
      return [];
    }
      ...
      foreach ($exp_expand as $relation) {
        ...
        switch ($relation) {
          ...
          case "allowed_tags":
            ...
            foreach ($category->getAllowedTags() as $tag) {
              $allowed_tag = SerializerRegistry::getInstance()
                ->getSerializer($tag)
                ->serialize(null, [], ["none"]);
              $track_tag_group = $summit->getTrackTagGroupForTag($tag);
              if (!is_null($track_tag_group)) {
                $allowed_tag["track_tag_group"] = SerializerRegistry::getInstance()
                  ->getSerializer($track_tag_group)
                  ->serialize(null, [], ["none"]);
              }
          ...
          case "proposed_schedule_allowed_locations":
            ...
            foreach ($category->getProposedScheduleAllowedLocations() as $allowed_location) {
              $proposed_schedule_allowed_locations[] = SerializerRegistry::getInstance()
          ...
          case "parent":
            if ($category->hasParent()) {
              ...
              $values["parent"] = SerializerRegistry::getInstance()
                ->getSerializer($category->getParent())
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
                  AbstractSerializer::filterFieldsByPrefix($relations, $relation),
          ...
          case "subtracks":
            ...
            foreach ($category->getSubTracks() as $children) {
              $subtracks[] = SerializerRegistry::getInstance()
                ->getSerializer($children)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
                  AbstractSerializer::filterFieldsByPrefix($relations, $relation),
