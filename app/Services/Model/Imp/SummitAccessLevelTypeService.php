 ...
 */
final class SummitAccessLevelTypeService extends AbstractService
implements ISummitAccessLevelTypeService
{
    ...
    public function updateAccessLevelType(Summit $summit, int $level_id, array $data): SummitAccessLevelType
    {
        return $this->tx_service->transaction(function() use($summit, $level_id, $data){
            ...
            if(isset($data['name'])){
                ...
                if (!is_null($former_level) && $former_level->getId() != $level_id && $former_level->getSummitId() == $summit->getId() ) {
                    throw new ValidationException(sprintf("access level with name %s already exists!", $name));
                }
            }
