...
class OAuth2SummitNotificationsApiController extends OAuth2ProtectedController {
  ...
  public function getAllApprovedByUser($summit_id) {
    ...
    return $this->_getAll(
      ...
      null,
      null,
      function ($page, $per_page, $filter, $order, $applyExtraFilters) use (
        $current_member,
        $summit,
      ) {
  ...
  public function approveNotification($summit_id, $notification_id) {
    try {
      ...
      $notification = $this->push_notification_service->approveNotification(
        $summit,
  ...
  public function unApproveNotification($summit_id, $notification_id) {
    try {
      ...
      $notification = $this->push_notification_service->unApproveNotification(
        $summit,
        $this->resource_server_context->getCurrentUser(),
        $notification_id,
      );
  ...
  public function addPushNotification($summit_id) {
    try {
      ...
      $notification = $this->push_notification_service->addPushNotification(
        $summit,
        $this->resource_server_context->getCurrentUser(),
        $data->all(),
      );
      ...
      return $this->created(
        SerializerRegistry::getInstance()->getSerializer($notification)->serialize(),
      );
    } catch (ValidationException $ex1) {
      Log::warning($ex1);
      return $this->error412([$ex1->getMessage()]);
