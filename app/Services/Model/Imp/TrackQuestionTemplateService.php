 ...
 */
final class TrackQuestionTemplateService extends AbstractService implements
  ITrackQuestionTemplateService {
  /**
  ...
  public function addTrackQuestionTemplate(array $payload) {
    return $this->tx_service->transaction(function () use ($payload) {
      $former_track_question_template = $this->track_question_template_repository->getByName(
        $payload["name"],
      );
      ...
      $former_track_question_template = $this->track_question_template_repository->getByLabel(
        $payload["label"],
      );
      if (!is_null($former_track_question_template)) {
        throw new ValidationException(
  ...
  public function updateTrackQuestionTemplate($track_question_template_id, array $payload) {
    return $this->tx_service->transaction(function () use ($payload, $track_question_template_id) {
      ...
      if (
        !is_null($former_track_question_template) &&
        $former_track_question_template->getId() != $track_question_template_id
      ) {
        throw new ValidationException(
          trans(
      ...
      if ($track_question_template instanceof TrackMultiValueQuestionTemplate) {
        if (isset($payload["default_value_id"])) {
          $default_value = $track_question_template->getValueById(
            intval($payload["default_value_id"]),
          );
  ...
  public function updateTrackQuestionValueTemplate(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (isset($data["value"])) {
        $former_value = $track_question_template->getValueByValue($data["value"]);
        if (
          !is_null($former_value) &&
          $former_value->getId() != $track_question_value_template_id
        ) {
          throw new ValidationException(
            trans(
