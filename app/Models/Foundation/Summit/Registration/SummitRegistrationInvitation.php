...
class SummitRegistrationInvitation extends SilverstripeBaseModel {
  ...
  const AllowedStatus = [self::Status_Pending, self::Status_Accepted, self::Status_Rejected];
  ...
  public function generateConfirmationToken(): string {
    ...
    Log::debug(
      sprintf(
        "SummitRegistrationInvitation::generateConfirmationToken id %s token %s hash %s",
        $this->id,
        $this->token,
        $this->hash,
      ),
  ...
  public function cancelReservedOrders(): void {
    foreach ($this->orders as $order) {
      if ($order->isReserved()) {
        Log::debug(
          sprintf(
            "SummitRegistrationInvitation::cancelReservedOrders id %s cancelling order id %s",
            $this->id,
            $order->getId(),
          ),
        );
  ...
  public function getBoughtTicketTypesExcerpt(): array {
    Log::debug(
      sprintf("SummitRegistrationInvitation::getBoughtTicketTypesExcerpt id %s", $this->id),
    );
  ...
  public function getRemainingAllowedTicketTypes(): array {
    ...
    if (
      $this->isAccepted() &&
      $this->getAcceptanceCriteria() === self::AcceptanceCriteria_AnyTicketType
    ) {
      // if we already accepted and acceptance criteria is any ( OR ), then there are now more allowed ticket types
      Log::debug(
        sprintf(
          "SummitRegistrationInvitation::getRemainingAllowedTicketTypes id %s is accepted and acceptance criteria is any",
          $this->id,
        ),
      );
      return [];
    ...
    if ($invitation_ticket_types->count() === 0) {
      // we are allowed to purchase any ticket type with invitation
      $invitation_ticket_types = $this->summit->getTicketTypesByAudience(
  ...
  public function markAsAccepted(): void {
    ...
    Log::debug(
      sprintf(
        "SummitRegistrationInvitation::markAsAccepted %s bought_tickets %s",
        $this->id,
        json_encode($bought_tickets),
      ),
    );
  ...
  public function isTicketTypeAllowed(int $ticket_type_id): bool {
    ...
    if (
      is_null($ticket_type) ||
      $ticket_type->getAudience() !== SummitTicketType::Audience_With_Invitation
    ) {
      return false;
    }
  ...
  public function setAcceptanceCriteria(string $acceptance_criteria): void {
    if (!in_array($acceptance_criteria, self::AllowedAcceptanceCriteria)) {
      throw new ValidationException(
        sprintf("acceptance_criteria %s is not allowed.", $acceptance_criteria),
      );
    }
  ...
  public function setStatus(string $status): void {
    ...
    $this->action_date =
      $status == self::Status_Pending ? null : new \DateTime("now", new \DateTimeZone("UTC"));
