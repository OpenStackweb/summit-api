...
class PresentationMediaUpload extends PresentationMaterial {
  ...
  public function getRelativePath(
    string $storageType = IStorageTypesConstants::PublicType,
  ...
  public function getPath(
    ...
    if (empty($mountingFolder)) {
      $mountingFolder =
        $storageType == IStorageTypesConstants::PrivateType
    ...
    Log::debug(
      sprintf(
        "PresentationMediaUpload::getPath storageType %s mountingFolder %s",
        $storageType,
        $mountingFolder,
      ),
    );
    ...
    $format =
      $storageType == IStorageTypesConstants::PublicType
        ? "%s/%s/%s"
        : "%s/" . IStorageTypesConstants::PrivateType . "/%s/%s";
    ...
    return sprintf(
      ...
      sprintf(
        "%s-%s",
        $summit->getId(),
        filter_var($summit->getRawSlug(), FILTER_SANITIZE_ENCODED),
      ),
      sprintf(
        "%s-%s",
        $presentation->getId(),
        filter_var($presentation->getSlug(), FILTER_SANITIZE_ENCODED),
      ),
    );
  }
