...
final class AttendeeService extends AbstractService implements IAttendeeService {
  ...
  public function updateAttendee(Summit $summit, $attendee_id, array $data) {
    return $this->tx_service->transaction(function () use ($summit, $attendee_id, $data) {
      ...
      if (is_null($attendee)) {
        throw new EntityNotFoundException(
          sprintf("Attendee does not belongs to summit id %s.", $summit->getId()),
        );
      }
      ...
      if (isset($data["member_id"]) && intval($data["member_id"]) > 0) {
        ...
        if (!is_null($old_attendee) && $old_attendee->getId() != $attendee->getId()) {
          throw new ValidationException(
            sprintf(
              "Another attendee (%s) already exist for summit id %s and member id %s.",
              $old_attendee->getId(),
              $summit->getId(),
              $member->getIdentifier(),
            ),
      ...
      if (isset($data["email"])) {
        $old_attendee = $this->attendee_repository->getBySummitAndEmail(
          $summit,
          trim($data["email"]),
        );
        if (!is_null($old_attendee) && $old_attendee->getId() != $attendee->getId()) {
      ...
      if (isset($data["extra_questions"]) && !$attendee->hasAllowedExtraQuestions()) {
        Log::debug(
          sprintf(
            "SummitAttendeeService::updateAttendee attendee %s does not have allowed extra questions.",
            $attendee->getId(),
          ),
        );
  ...
  public function deleteAttendeeTicket(SummitAttendee $attendee, $ticket_id) {
    return $this->tx_service->transaction(function () use ($attendee, $ticket_id) {
      ...
      if (is_null($ticket)) {
        throw new EntityNotFoundException(
          sprintf(
            "ticket id %s does not belongs to attendee id %s",
            $ticket_id,
            $attendee->getId(),
          ),
        );
  ...
  public function updateRedeemedPromoCodes(Summit $summit, $page_nbr = 1) {
    return $this->tx_service->transaction(function () use ($summit, $page_nbr) {
      ...
      foreach ($attendees as $attendee) {
        if (!isset($attendee["promotional_code"])) {
          continue;
        }
   ...
   * @throws \Exception
   */
  public function reassignAttendeeTicket(
    Summit $summit,
    SummitAttendee $attendee,
    int $ticket_id,
    array $payload,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_null($new_owner)) {
        Log::debug(
          sprintf(
            "AttendeeService::reassignAttendeeTicket attendee %s does no exists .. creating it.",
            $attendee_email,
          ),
        );
        $attendee_payload = [
  ...
  public function doVirtualCheckin(Summit $summit, int $attendee_id): ?SummitAttendee {
    return $this->tx_service->transaction(function () use ($summit, $attendee_id) {
      Log::debug(
        sprintf(
          "AttendeeService::doVirtualCheckin summit id %s attendee id %s",
          $summit->getId(),
          $attendee_id,
        ),
      );
  ...
  public function doCheckIn(Summit $summit, string $qr_code): void {
    $this->tx_service->transaction(function () use ($summit, $qr_code) {
      Log::debug(
        sprintf("AttendeeService::doCheckIn summit id %s qr_code %s", $summit->getId(), $qr_code),
      );

      if (!str_starts_with($qr_code, $summit->getBadgeQRPrefix()) && $summit->hasQRCodesEncKey()) {
        Log::debug(
          sprintf(
            "AttendeeService::doCheckIn summit id %s qr_code %s decrypting",
            $summit->getId(),
            $qr_code,
          ),
        );
  ...
  public function updateAttendeesByMemberId(int $member_id): void {
    ...
    $attendees_2_delete = $this->tx_service->transaction(function () use ($member_id) {
      ...
      if (!$member instanceof Member) {
        Log::debug(
          sprintf("AttendeeService::updateAttendeesByMemberId member %s not found.", $member_id),
        );
    ...
    if (count($attendees_2_delete)) {
      Log::debug(
        sprintf(
          "AttendeeService::updateAttendeesByMemberId deleting %s attendees",
          count($attendees_2_delete),
        ),
      );
      $this->tx_service->transaction(function () use ($attendees_2_delete) {
        foreach ($attendees_2_delete as $attendee_id) {
          ...
          Log::debug(
            sprintf(
              "AttendeeService::updateAttendeesByMemberId deleting attendee %s",
              $attendee->getId(),
            ),
          );
          $this->attendee_repository->delete($attendee);
  ...
  public function resynchAttendeesStatusBySummit(int $summit_id): void {
    ...
    if (Cache::has($key)) {
      Log::debug(
        sprintf(
          "AttendeeService::resynchAttendeesStatusBySummit summit id %s already running",
          $summit_id,
        ),
      );
      return;
    ...
    try {
      ...
      do {
        $attendees_ids = $this->tx_service->transaction(function () use (
          $summit,
          $current_page,
          $filter,
          $maxPageSize,
          ...
          Log::debug(
            sprintf("AttendeeService::resynchAttendeesStatusBySummit page %s", $current_page),
          );
          return $this->attendee_repository->getAllIdsByPage(
            new PagingInfo($current_page, $maxPageSize),
            $filter,
          );
        });
    ...
    Log::debug(
      sprintf(
        "AttendeeService::resynchAttendeesStatusBySummit summit id %s processed %s attendees",
        $summit_id,
        $count,
      ),
    );
  ...
  public function upsertAttendeeNote(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      Log::debug(
        sprintf(
          "AttendeeService::upsertAttendeeNote summit id %s, attendee id %s, payload: %s",
          $summit->getId(),
          $attendee_id,
          json_encode($payload),
        ),
      ...
      if (is_null($attendee)) {
        throw new EntityNotFoundException(
          sprintf("Attendee does not belong to summit id %s.", $summit->getId()),
        );
      }
      ...
      if (!is_null($note_id)) {
        ...
        if (is_null($note)) {
          throw new EntityNotFoundException(
            sprintf(
              "Attendee note id %s does not belong to attendee id %s.",
              $note_id,
              $attendee_id,
            ),
          );
      ...
      if (is_null($note)) {
        Log::debug(
          sprintf(
            "AttendeeService::upsertAttendeeNote adding new note. Summit id %s, attendee id %s.",
            $summit->getId(),
            $attendee_id,
          ),
        );
        ...
        Log::debug(
          sprintf(
            "AttendeeService::upsertAttendeeNote updating note with id %s. Summit id %s, attendee id %s.",
            $note_id,
            $summit->getId(),
            $attendee_id,
          ),
  ...
  public function deleteAttendeeNote(Summit $summit, int $attendee_id, int $note_id): void {
    $this->tx_service->transaction(function () use ($summit, $attendee_id, $note_id) {
      Log::debug(
        sprintf(
          "AttendeeService::deleteAttendeeNote summit id %s, attendee id %s, note id %s",
          $summit->getId(),
          $attendee_id,
          $note_id,
        ),
      ...
      if (is_null($note)) {
        throw new EntityNotFoundException(
          sprintf("Attendee note id %s does not belong to attendee id %s.", $note_id, $attendee_id),
        );
      }
  ...
  public function processAttendeeCheckStatusUpdate(int $attendee_id): void {
    $this->tx_service->transaction(function () use ($attendee_id) {
      Log::debug(
        sprintf("AttendeeService::processAttendeeCheckStatusUpdate attendee id %s", $attendee_id),
      );
