 ...
 */
final class SummitService extends AbstractPublishService implements ISummitService {
  /**
   ...
   */
  public function addEventToMemberSchedule(
    Summit $summit,
    Member $member,
    $event_id,
    $check_rsvp = true,
   ...
   * @throws \Exception
   */
  public function removeEventFromMemberSchedule(
    Summit $summit,
    Member $member,
    $event_id,
    $check_rsvp = true,
   ...
   * @throws Exception
   */
  public function addMyEventFeedback(
    Member $member,
    Summit $summit,
    int $event_id,
    array $payload,
    ...
    return $this->tx_service->transaction(function () use ($member, $summit, $event_id, $payload) {
      ...
      if (!$event->isAllowFeedback()) {
        throw new ValidationException(
          sprintf("Event id %s does not allow feedback.", $event->getIdentifier()),
        );
      }
      ...
      if (!is_null($former_feedback)) {
        throw new ValidationException(
          sprintf("You already sent feedback for event id %s!.", $event->getIdentifier()),
        );
      }
   ...
   * @throws Exception
   */
  public function updateMyEventFeedback(
    Member $member,
    Summit $summit,
    int $event_id,
    array $payload,
    ...
    return $this->tx_service->transaction(function () use ($member, $summit, $event_id, $payload) {
      ...
      if (is_null($feedback)) {
        throw new ValidationException(
          sprintf("you dont have feedback for event id %s!.", $event->getIdentifier()),
        );
      }
   ...
   * @throws Exception
   */
  public function getMyEventFeedback(
    Member $member,
    Summit $summit,
    int $event_id,
  ): SummitEventFeedback {
  ...
  private function canPerformEventTypeTransition(
    SummitEventType $old_event_type,
    SummitEventType $event_type,
  ) {
    if ($old_event_type->getId() == $event_type->getId()) {
      return true;
    }
  ...
  private function saveOrUpdateEvent(Summit $summit, array $data, $event_id = null) {
    return $this->tx_service->transaction(function () use ($summit, $data, $event_id) {
      ...
      if (
        ...
        !$this->permissions_manager->canEditFields($current_member, "SummitEvent", $data)
      ) {
        throw new ValidationException(
          sprintf("user %s cant set requested summit event fields", $current_member->getEmail()),
        );
      }
      ...
      if (isset($data["type_id"])) {
        ...
        if (is_null($event_type)) {
          throw new EntityNotFoundException(
            sprintf("event type id %s does not exists!", $data["type_id"]),
          );
        }
      }
      ...
      if (isset($data["track_id"])) {
        ...
        if (is_null($track)) {
          throw new EntityNotFoundException(
            sprintf("track id %s does not exists!", $data["track_id"]),
          );
        }
      }
      ...
      if (isset($data["location_id"])) {
        ...
        if (is_null($location) && intval($data["location_id"]) > 0) {
          throw new EntityNotFoundException(
            sprintf("location id %s does not exists!", $data["location_id"]),
          );
        }
      }
      ...
      if (!is_null($event_id) && intval($event_id) > 0) {
        ...
        if (
          !is_null($event_type) &&
          !$this->canPerformEventTypeTransition($old_event_type, $event_type)
        ) {
          throw new ValidationException(
            sprintf(
      ...
      if (isset($data["created_by_id"])) {
        ...
        if (is_null($created_by) && intval($data["created_by_id"]) > 0) {
          throw new EntityNotFoundException(
            sprintf("member id %s does not exists!", $data["created_by_id"]),
          );
        }
      }
      ...
      if (isset($data["rsvp_template_id"])) {
        ...
        if (is_null($rsvp_template)) {
          throw new EntityNotFoundException(
            sprintf(
              "rsvp template id %s does not belongs to summit id %s",
              $data["rsvp_template_id"],
              $summit->getId(),
            ),
          );
        }
        ...
        if (!$rsvp_template->isEnabled()) {
          throw new ValidationException(
            sprintf("rsvp template id %s is not enabled", $data["rsvp_template_id"]),
          );
        }
      ...
      if (!is_null($location)) {
        if (!$event->hasType()) {
          throw new ValidationException(
            "To be able to set a location, event type must be set First.",
          );
        }
      ...
      $sponsors = $event_type->isUseSponsors() && isset($data["sponsors"]) ? $data["sponsors"] : [];
      ...
      if (isset($data["sponsors"])) {
        ...
        foreach ($sponsors as $sponsor_id) {
          ...
          if (is_null($sponsor)) {
            throw new EntityNotFoundException(sprintf("sponsor id %s", $sponsor_id));
          }
  ...
  private function saveOrUpdateSummitGroupEventData(
    SummitEvent $event,
    SummitEventType $event_type,
    array $data,
  ) {
    ...
    foreach ($data["groups"] as $group_id) {
      ...
      if (is_null($group)) {
        throw new EntityNotFoundException(sprintf("group id %s", $group_id));
      }
   ...
   * @throws ValidationException
   */
  private function saveOrUpdatePresentationData(
    SummitEvent $event,
    SummitEventType $event_type,
    array $data,
  ) {
    ...
    if ($event_type instanceof PresentationType && $event_type->isUseModerator()) {
      $shouldClearModerator =
        isset($data["moderator_speaker_id"]) && intval($data["moderator_speaker_id"]) == 0;
      $moderator_id = isset($data["moderator_speaker_id"])
      ...
      if ($shouldClearModerator) {
        $event->unsetModerator();
      }
  ...
  public function publishEvent(Summit $summit, $event_id, array $data): SummitEvent {
    return $this->tx_service->transaction(function () use ($summit, $data, $event_id) {
      ...
      if (!$event->hasType()) {
        throw new EntityNotFoundException(
          sprintf("Event type its not assigned to event id %s!", $event_id),
        );
      }
      ...
      if (is_null($event->getSummit())) {
        throw new EntityNotFoundException(
          sprintf("Summit its not assigned to event id %s!", $event_id),
        );
      }

      if ($event->getSummit()->getIdentifier() !== $summit->getIdentifier()) {
        throw new ValidationException(
          sprintf("Event %s does not belongs to summit id %s", $event_id, $summit->getIdentifier()),
        );
      }
      ...
      if ($type->isAllowsPublishingDates()) {
        ...
        if (is_null($start_datetime)) {
          throw new ValidationException(
            sprintf("start_date its not assigned to event id %s!", $event_id),
          );
        }

        if (is_null($end_datetime)) {
          throw new ValidationException(
            sprintf("end_date its not assigned to event id %s!", $event_id),
          );
        }
      }
    ...
    try {
      ...
      if (isset($external_order["attendees"])) {
        ...
        if (intval($summit->getSummitExternalId()) !== intval($summit_external_id)) {
          throw new ValidationException(
            "order %s does not belongs to current summit!",
            $external_order_id,
          );
        }
        ...
        if ($status !== "placed") {
          throw new ValidationException(
            sprintf("invalid order status %s for order %s", $status, $external_order_id),
          );
        }
        ...
        foreach ($external_order["attendees"] as $a) {
          ...
          if (is_null($ticket_type)) {
            throw new EntityNotFoundException(
              sprintf("external ticket type %s not found!", $ticket_external_id),
            );
          }
    ...
    return $this->tx_service->transaction(function () use ($request) {
      try {
        ...
        if (isset($external_order["attendees"])) {
          ...
          if (
            intval($request->getSummit()->getSummitExternalId()) !== intval($summit_external_id)
          ) {
            throw new ValidationException(
              "order %s does not belongs to current summit!",
          ...
          if (is_null($external_attendee)) {
            throw new EntityNotFoundException(
              sprintf("attendee %s not found!", $request->getExternalAttendeeId()),
            );
          }
          ...
          if (is_null($ticket_type)) {
            throw new EntityNotFoundException(
              sprintf("ticket type %s not found!", $ticket_external_id),
            );
          }
          ...
          if ($status !== "placed") {
            throw new ValidationException(
              sprintf(
                "invalid order status %s for order %s",
                $status,
                $request->getExternalOrderId(),
              ),
            );
          ...
          if (!is_null($old_attendee)) {
            throw new ValidationException("Attendee already exists for current summit.");
          }
   ...
   * @throws ValidationException
   */
  public function addEventAttachment(
    Summit $summit,
    $event_id,
    UploadedFile $file,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!$event instanceof SummitEventWithFile) {
        throw new ValidationException(
          sprintf("event id %s does not allow attachments!", $event_id),
        );
      }

      if (!in_array($file->extension(), $allowed_extensions)) {
        throw new ValidationException(
          "file does not has a valid extension ('png','jpg','jpeg','gif','pdf').",
        );
      }
   ...
   */
  public function getSummitScheduleEmptySpots(Summit $summit, Filter $filter) {
    return $this->tx_service->transaction(function () use ($summit, $filter) {
      $gaps = [];
      $order = new Order([
      ...
      foreach ($location_ids as $location_id) {
        foreach ($intervals as $interval) {
          ...
          $events_filter->addFilterCondition(
            FilterParser::buildFilter("summit_id", "==", $summit->getId()),
          );
          $events_filter->addFilterCondition(
            FilterParser::buildFilter("location_id", "==", intval($location_id)),
          );

          $events_filter->addFilterCondition(
            FilterParser::buildFilter("start_date", "<", $interval[1]->getTimestamp()),
          );
          $events_filter->addFilterCondition(
            FilterParser::buildFilter("end_date", ">", $interval[0]->getTimestamp()),
          );
  ...
  public function addSummit(array $data) {
    return $this->tx_service->transaction(function () use ($data) {
      ...
      if (!is_null($former_summit)) {
        throw new ValidationException(
          trans("validation_errors.SummitService.AddSummit.NameAlreadyExists", ["name" => $name]),
        );
      }
      ...
      if (!empty($slug)) {
        ...
        if (!is_null($old_summit)) {
          throw new ValidationException(
            sprintf("Slug %s already belongs to another summit (%s).", $slug, $old_summit->getId()),
          );
        }
      }
      ...
      if (!empty($registration_slug_prefix)) {
        ...
        $old_summit = $this->summit_repository->getByRegistrationSlugPrefix(
          Summit::formatSlug($registration_slug_prefix),
        );
        if (!is_null($old_summit)) {
          throw new ValidationException(
            sprintf(
    ...
    return $this->tx_service->transaction(function () use ($summit_id, $data) {
      if (isset($data["name"])) {
        ...
        if (!is_null($former_summit) && $former_summit->getId() != $summit_id) {
          throw new ValidationException(
            trans("validation_errors.SummitService.updateSummit.NameAlreadyExists", [
              "name" => $data["name"],
            ]),
          );
        }
      ...
      if (isset($data["active"])) {
        ...
        if ($active && !is_null($active_summit) && $active_summit->getId() != $summit_id) {
          throw new ValidationException(
            trans("validation_errors.SummitService.updateSummit.SummitAlreadyActive", [
      ...
      if (is_null($summit)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitService.updateSummit.SummitNotFound", [
            "summit_id" => $summit_id,
          ]),
        );
      }
  ...
  public function deleteSummit($summit_id) {
    return $this->tx_service->transaction(function () use ($summit_id) {
      ...
      if (is_null($summit)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitService.deleteSummit.SummitNotFound", [
            "summit_id" => $summit_id,
          ]),
        );
      }
   ...
   * @throws ValidationException
   */
  public function addSpeaker2Presentation(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_null($current_speaker)) {
        throw new EntityNotFoundException(
          sprintf("Member %s does not has a speaker profile.", $current_member_id),
        );
      }
      ...
      if (!$presentation->canEdit($current_speaker)) {
        throw new ValidationException(
          sprintf("Member %s can not edit presentation %s.", $current_member_id, $presentation_id),
        );
      }
      ...
      if (
        ...
        $selection_plan->isPrivate() &&
        !$selection_plan->containsMember($speaker->getEmail())
      ) {
   ...
   * @throws ValidationException
   */
  public function addModerator2Presentation(
    int $current_member_id,
    int $speaker_id,
    int $presentation_id,
  ): Presentation {
   ...
   * @throws ValidationException
   */
  public function removeModeratorFromPresentation(
    int $current_member_id,
    int $speaker_id,
    int $presentation_id,
  ): Presentation {
   ...
   * @throws EntityNotFoundException
   */
  public function addBookableRoomAttribute(
    Summit $summit,
    array $payload,
  ): SummitBookableVenueRoomAttributeType {
    return $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      if (!is_null($former_type)) {
        throw new ValidationException(
          sprintf(
            "bookable room attr type %s already exists on summit %s",
            $type_name,
            $summit->getId(),
          ),
        );
   ...
   * @throws EntityNotFoundException
   */
  public function updateBookableRoomAttribute(
    Summit $summit,
    int $type_id,
    array $payload,
  ): SummitBookableVenueRoomAttributeType {
   ...
   * @throws EntityNotFoundException
   */
  public function addBookableRoomAttributeValue(
    Summit $summit,
    int $type_id,
    array $payload,
  ): SummitBookableVenueRoomAttributeValue {
    return $this->tx_service->transaction(function () use ($summit, $type_id, $payload) {
      $type = $summit->getBookableAttributeTypeById($type_id);
      if (is_null($type)) {
        throw new EntityNotFoundException();
      ...
      if (!is_null($former_value)) {
        throw new ValidationException(
          sprintf(
            "bookable room attr value %s already exists on summit %s",
            $value_name,
            $summit->getId(),
          ),
        );
   ...
   * @throws EntityNotFoundException
   */
  public function updateBookableRoomAttributeValue(
    Summit $summit,
    int $type_id,
    int $value_id,
    array $payload,
   ...
   * @throws EntityNotFoundException
   */
  public function deleteBookableRoomAttributeValue(
    Summit $summit,
    int $type_id,
    int $value_id,
  ): void {
   ...
   * @throws ValidationException
   */
  public function addSummitSecondaryLogo(
    int $summit_id,
    UploadedFile $file,
    $max_file_size = 10485760,
  ) {
  ...
  public function unRSVPEvent(Summit $summit, Member $member, int $event_id) {
    return $this->tx_service->transaction(function () use ($summit, $member, $event_id) {
      ...
      if (is_null($rsvp)) {
        throw new ValidationException(
          sprintf("RSVP for event id %s does not exist for your member.", $event_id),
        );
      }
   ...
   * @throws Exception
   */
  public function revokeScheduleShareableLink(
    Summit $summit,
    Member $member,
  ): ?PersonalCalendarShareInfo {
    return $this->tx_service->transaction(function () use ($summit, $member) {
    ...
    return $this->tx_service->transaction(function () use ($summit, $cid) {
      ...
      foreach ($owner->getScheduleBySummit($summit) as $scheduled) {
        ...
        if ($timeZone->getName() == "UTC") {
          $vEvent->setUseUtc(true)->setUseTimezone(false);
        } else {
          $vEvent->setUseUtc(false)->setUseTimezone(true);
        }
  ...
  public function shareEventByEmail(Summit $summit, int $event_id, array $data): void {
    $this->tx_service->transaction(function () use ($summit, $event_id, $data) {
      ...
      ShareEventEmail::dispatch(trim($data["from"]), trim($data["to"]), $event_uri, $event);
    });
  }
  ...
  public function calculateFeedbackAverageForOngoingSummits(): void {
    ...
    foreach ($ongoing_summits as $summit) {
      Log::debug(
        sprintf(
          "SummitService::calculateFeedbackAverageForOngoingSummits processing summit %s",
          $summit->getId(),
        ),
      );
      ...
      foreach ($event_ids as $event_id) {
        ...
        $this->tx_service->transaction(function () use ($event_id) {
          try {
            Log::debug(
              sprintf(
                "SummitService::calculateFeedbackAverageForOngoingSummits processing event %s",
                $event_id,
              ),
            );
            $event = $this->event_repository->getById($event_id);
            if (is_null($event) || !$event instanceof SummitEvent) {
              Log::debug(
                sprintf(
                  "SummitService::calculateFeedbackAverageForOngoingSummits event %s not found",
                  $event_id,
                ),
              );
              return;
            ...
            Log::debug(
              sprintf(
                "SummitService::calculateFeedbackAverageForOngoingSummits new avg rate %s - old avg rate %s - for event id %s",
                $avg_rate,
                $old_avg_rate,
                $event->getId(),
              ),
   ...
   * @throws Exception
   */
  public function advanceSummit(
    int $summit_id,
    int $days,
    bool $negative = false,
    $check_summit_ends = true,
  ): void {
    $interval = new DateInterval(sprintf("P%sD", $days));

    Log::debug(
      sprintf(
        "SummitService::advanceSummit summit_id %s days %s negative %s check_summit_ends %s",
        $summit_id,
        $days,
        $negative,
        $check_summit_ends,
    ...
    $events_ids = $this->tx_service->transaction(function () use (
      $summit_id,
      $interval,
      $negative,
      $check_summit_ends,
      ...
      if (!is_null($summitBeginDate)) {
        Log::debug(
          sprintf(
            "SummitService::advanceSummit Current Summit begin date for summit %s is %s",
            $summit_id,
            $summitBeginDate->format("Ymd His"),
          ),
        );
        if ($negative) {
        ...
        Log::debug(
          sprintf(
            "SummitService::advanceSummit New Summit begin date for summit %s is %s",
            $summit_id,
            $summit->getBeginDate()->format("Ymd His"),
          ),
        );
      }
      ...
      if (!is_null($summitEndDate)) {
        Log::debug(
          sprintf(
            "SummitService::advanceSummit Current Summit end date for summit %s is %s",
            $summit_id,
            $summitEndDate->format("Ymd His"),
          ),
        );
        if ($negative) {
        ...
        Log::debug(
          sprintf(
            "SummitService::advanceSummit New Summit end date for summit %s is %s",
            $summit_id,
            $summit->getEndDate()->format("Ymd His"),
          ),
        );
      }
      ...
      if (!is_null($summitRegistrationBeginDate)) {
        Log::debug(
          sprintf(
            "SummitService::advanceSummit Current Summit registration begin date for summit %s is %s",
            $summit_id,
            $summitRegistrationBeginDate->format("Ymd His"),
          ),
        );
        if ($negative) {
        ...
        Log::debug(
          sprintf(
            "SummitService::advanceSummit New Summit registration begin date for summit %s is %s",
            $summit_id,
            $summit->getRegistrationBeginDate()->format("Ymd His"),
          ),
        );
      }
      ...
      if (!is_null($summitRegistrationEndDate)) {
        Log::debug(
          sprintf(
            "SummitService::advanceSummit Current Summit registration end date for summit %s is %s",
            $summit_id,
            $summitRegistrationEndDate->format("Ymd His"),
          ),
        );
        $summit->setRawRegistrationEndDate(clone $summitRegistrationEndDate->add($interval));
        Log::debug(
          sprintf(
            "SummitService::advanceSummit New Summit registration end date for summit %s is %s",
            $summit_id,
            $summit->getRegistrationEndDate()->format("Ymd His"),
          ),
        );
      }
      ...
      if (!is_null($summitReassignTicketTillDate)) {
        if ($negative) {
          $summit->setRawReassignTicketTillDate(
            clone $summitReassignTicketTillDate->sub($interval),
          );
        } else {
          $summit->setRawReassignTicketTillDate(
            clone $summitReassignTicketTillDate->add($interval),
          );
        }
      }
      ...
      if (!is_null($summitStartShowingVenuesDate)) {
        if ($negative) {
          $summit->setRawStartShowingVenuesDate(
            clone $summitStartShowingVenuesDate->sub($interval),
          );
        } else {
          $summit->setRawStartShowingVenuesDate(
            clone $summitStartShowingVenuesDate->add($interval),
          );
        }
      }
      ...
      if (!is_null($summitScheduleDefaultStartDate)) {
        if ($negative) {
          $summit->setRawScheduleDefaultStartDate(
            clone $summitScheduleDefaultStartDate->sub($interval),
          );
        } else {
          $summit->setRawScheduleDefaultStartDate(
            clone $summitScheduleDefaultStartDate->add($interval),
          );
        }
      }
    ...
    foreach ($events_ids as $event_id) {
      $this->tx_service->transaction(function () use ($summit_id, $event_id, $interval, $negative) {
        ...
        if (is_null($eventBeginDate) || is_null($eventEndDate)) {
          Log::debug(
            sprintf(
              "SummitService::advanceSummit summit id %s event id %s ( has not set dates but is published!), skipping it",
              $summit_id,
              $event->getId(),
            ),
          );
        ...
        Log::debug(
          sprintf(
            "SummitService::advanceSummit summit id %s event id %s current start date %s",
            $summit_id,
            $event->getId(),
            $eventBeginDate->format("Ymd His"),
          ),
        );
        if ($negative) {
        ...
        Log::debug(
          sprintf(
            "SummitService::advanceSummit summit id %s event id %s new start date %s",
            $summit_id,
            $event->getId(),
            $event->getStartDate()->format("Ymd His"),
          ),
        );

        Log::debug(
          sprintf(
            "SummitService::advanceSummit summit id %s event id %s current end date %s",
            $summit_id,
            $event->getId(),
            $eventEndDate->format("Ymd His"),
          ),
        );
        if ($negative) {
        ...
        Log::debug(
          sprintf(
            "SummitService::advanceSummit summit id %s event id %s new end date %s",
            $summit_id,
            $event->getId(),
            $event->getEndDate()->format("Ymd His"),
          ),
        );
  ...
  public function importEventData(Summit $summit, UploadedFile $csv_file, array $payload): void {
    ...
    Log::debug(
      sprintf("SummitService::importEventData trying to read file data from %s", $real_path),
    );
    $csv_data = \Illuminate\Support\Facades\File::get($real_path);

    if (empty($csv_data)) {
      Log::warning(
        sprintf("SummitService::importEventData file %s has empty content.", $real_path),
      );
    ...
    Log::debug(
      sprintf(
        "SummitService::importEventData uploading file %s to storage %s",
        $basename,
        $this->upload_strategy->getDriver(),
      ),
    );
    ...
    Log::debug(
      sprintf("SummitService::importEventData validating header %s", json_encode($header)),
    );
    ...
    if (!in_array("id", $header)) {
      ...
      $type_data_present = in_array("type_id", $header) || in_array("type", $header);
      ...
      $track_present = in_array("track_id", $header) || in_array("track", $header);
  ...
  public function processEventData(
    int $summit_id,
    string $filename,
    bool $send_speaker_email,
  ): void {
    Log::debug(
      sprintf("SummitService::processEventData summit %s filename %s", $summit_id, $filename),
    );
    ...
    if (!$this->download_strategy->exists($path)) {
      Log::warning(
        sprintf(
          "SummitService::processEventData file %s does not exists on storage %s.",
          $filename,
    ...
    foreach ($records as $idx => $row) {
      try {
        ...
        $event = $this->tx_service->transaction(function () use ($summit, $row, &$new_speakers) {
          Log::debug(
            sprintf("SummitService::processEventData processing row %s", json_encode($row)),
          );
          ...
          if (isset($row["id"]) && !empty($row["id"])) {
            Log::debug(
              sprintf("SummitService::processEventData trying to get event %s", $row["id"]),
            );
          ...
          if (isset($row["description"])) {
            ...
            Log::debug(
              sprintf("SummitService::processEventData setting description %s", $abstract),
            );
          ...
          if (isset($row["location"]) && !empty($row["location"])) {
            Log::debug(
              sprintf("SummitService::processEventData processing location %s", $row["location"]),
            );
          ...
          if (!is_null($event_type)) {
            $sponsors =
              $event_type->isUseSponsors() && isset($row["sponsors"]) ? $row["sponsors"] : "";
            ...
            if (isset($row["sponsors"])) {
              ...
              foreach ($sponsors as $sponsor_name) {
                ...
                if (is_null($sponsor)) {
                  throw new EntityNotFoundException(sprintf("sponsor %s", $sponsor_name));
                }
                $event->addSponsor($sponsor);
              }
            }
          ...
          if ($event instanceof Presentation) {
            Log::debug(
              sprintf(
                "SummitService::processEventData event %s is a presentation",
                $event->getId(),
              ),
            );
            ...
            if (
              !is_null($event_type) &&
              $event_type instanceof PresentationType &&
              $event_type->isUseSpeakers()
            ) {
              $speakers = isset($row["speaker_emails"]) ? $row["speaker_emails"] : "";
              Log::debug(
                sprintf(
                  "SummitService::processEventData event %s processing speaker_emails %s",
                  $event->getId(),
                  $speakers,
                ),
              );
              ...
              if (isset($row["speaker_fullnames"])) {
                $speakers_names = isset($row["speaker_fullnames"]) ? $row["speaker_fullnames"] : "";
                Log::debug(
                  sprintf(
                    "SummitService::processEventData event %s processing speaker_fullnames %s",
                    $event->getId(),
                    $speakers_names,
                  ),
                );
              ...
              if (isset($row["speaker_companies"])) {
                ...
                Log::debug(
                  sprintf(
                    "SummitService::processEventData event %s processing speaker_companies %s",
                    $event->getId(),
                    $speakers_companies,
                  ),
                );
              ...
              if (isset($row["speaker_titles"])) {
                $speakers_titles = isset($row["speaker_titles"]) ? $row["speaker_titles"] : "";
                Log::debug(
                  sprintf(
                    "SummitService::processEventData event %s processing speaker_titles %s",
                    $event->getId(),
                    $speakers_titles,
                  ),
                );
              ...
              if (count($speakers) > 0) {
                foreach ($speakers as $idx => $speaker_email) {
                  ...
                  Log::debug(
                    sprintf(
                      "SummitService::processEventData processing speaker email %s speaker fullname %s",
                      $speaker_email,
                      $speaker_full_name,
                    ),
                  );
                  ...
                  if (is_null($speaker)) {
                    Log::debug(
                      sprintf(
                        "SummitService::processEventData speaker %s fname %s lname %s does not exists",
                        $speaker_email,
                        $speaker_first_name,
                        $speaker_last_name,
                      ),
                    ...
                    Log::debug(
                      sprintf(
                        "SummitService::processEventData adding speaker %s",
                        json_encode($payload),
                      ),
                    );
                    ...
                    Log::debug(
                      sprintf(
                        "SummitService::processEventData speaker %s already exists, updating ",
                        $speaker_email,
                      ),
                    );
                    ...
                    if (
                      array_key_exists($idx, $speakers_companies) &&
                      !empty($speakers_companies[$idx])
                    ) {
                      $payload["company"] = $speakers_companies[$idx];
                    }

                    if (
                      array_key_exists($idx, $speakers_titles) &&
                      !empty($speakers_titles[$idx])
                    ) {
                      $payload["title"] = $speakers_titles[$idx];
                    }
                    ...
                    Log::debug(
                      sprintf(
                        "SummitService::processEventData updating speaker %s",
                        json_encode($payload),
                      ),
                    );
                  ...
                  if (!$event->isSpeaker($speaker)) {
                    ...
                    Log::debug(
                      sprintf(
                        "SummitService::processEventData adding speaker %s to event %s",
                        $speaker->getEmail(),
                        $event->getTitle(),
                      ),
                    );
            ...
            if (
              !is_null($event_type) &&
              $event_type instanceof PresentationType &&
              $event_type->isUseModerator() &&
              isset($row["moderator"])
            ) {
              $moderator_email = trim($row["moderator"]);

              if (
                $event_type->isModeratorMandatory() &&
                !$event->hasModerator() &&
                empty($moderator_email)
              ) {
              ...
              if (!empty($moderator_email)) {
                Log::debug(
                  sprintf(
                    "SummitService::processEventData processing moderator %s",
                    $moderator_email,
                  ),
                );
                ...
                if (is_null($moderator)) {
                  Log::debug(
                    sprintf(
                      "SummitService::processEventData moderator %s does not exists",
                      $moderator_email,
                    ),
                  );
                  $moderator = $this->speaker_service->addSpeaker(
            ...
            if (isset($row["selection_plan"])) {
              ...
              if (!is_null($selection_plan)) {
                Log::debug(
                  sprintf(
                    "SummitService::processEventData processing selection plan %s",
                    $row["selection_plan"],
                  ),
                );
                $track = $event->getCategory();
          ...
          if (isset($row["is_published"])) {
            ...
            if ($is_published) {
              if (!isset($row["start_date"])) {
                throw new ValidationException("start_date is required.");
              }
              if (!isset($row["end_date"])) {
                throw new ValidationException("end_date is required.");
              }
              if (!$event->isPublished()) {
                $event->publish();
              }
            ...
            $this->tx_service->transaction(function () use ($speaker, $event) {
              ...
              if ($speaker instanceof PresentationSpeaker) {
                if (!$speaker->hasMember()) {
                  Log::debug(
                    sprintf(
                      "SummitService::processEventData speaker %s has not member set, checking at idp",
                      $speaker->getEmail(),
                    ),
                  );
                  ...
                  if (is_null($user)) {
                    ...
                    Log::debug(
                      sprintf(
                        "SummitService::processEventData speaker %s user not found at idp, creating registration request",
                        $speaker->getEmail(),
                      ),
                    );
    ...
    Log::debug(
      sprintf(
        "SummitService::processEventData deleting file %s from cloud storage %s",
        $path,
        $this->download_strategy->getDriver(),
      ),
    );
  ...
  public function addFeaturedSpeaker(int $summit_id, int $speaker_id): ?FeaturedSpeaker {
    return $this->tx_service->transaction(function () use ($summit_id, $speaker_id) {
      ...
      if (!$this->speaker_repository->speakerBelongsToSummitSchedule($speaker_id, $summit_id)) {
        throw new ValidationException(
          sprintf("Speaker %s does not belongs to Summit %s schedule.", $speaker_id, $summit_id),
        );
      }
   ...
   * @throws Exception
   */
  public function updateFeaturedSpeaker(
    int $summit_id,
    int $speaker_id,
    array $payload,
  ): ?FeaturedSpeaker {
  ...
  public function addCompany(int $summit_id, int $company_id): ?Company {
    return $this->tx_service->transaction(function () use ($summit_id, $company_id) {
      ...
      if (!is_null($company)) {
        throw new ValidationException(
          sprintf("summit %s already has a company with id %s.", $summit_id, $company_id),
        );
      }
   ...
   * @throws Exception
   */
  public function migratePrivateStorage2PublicStorage(
    int $summit_id,
    int $media_upload_type_id,
    string $default_public_storage = IStorageTypesConstants::S3,
  ): int {
    Log::debug(
      sprintf(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      foreach ($media_upload_type->getMediaUploadsToDisplayOnSite() as $media_upload) {
        if (!$media_upload instanceof PresentationMediaUpload) {
          continue;
        }
        try {
          Log::debug(
            sprintf(
              "SummitService::migratePrivateStorage2PublicStorage processing media upload %s file %s",
              $media_upload->getId(),
              $media_upload->getFilename(),
            ),
          );
          $strategy = FileDownloadStrategyFactory::build(
            $media_upload_type->getPrivateStorageType(),
          );
          if (!is_null($strategy)) {
            ...
            $uploadStrategy = FileUploadStrategyFactory::build(
              $media_upload_type->getPublicStorageType(),
            );

            if (!is_null($uploadStrategy)) {
              $path = sprintf(
                "%s/%s",
                $media_upload->getPath(IStorageTypesConstants::PublicType),
                $media_upload->getFilename(),
              );
              Log::debug(
                sprintf(
                  "SummitService::migratePrivateStorage2PublicStorage uploading file %s to public storage type",
                  $path,
                ),
              );
  ...
  public function regenerateTemporalUrlsForMediaUploads(int $summit_id): void {
    Log::debug(
      sprintf(
        "SummitService::regenerateTemporalUrlsForMediaUploads processing summit %s",
        $summit_id,
      ),
    );
    ...
    $mediaUploadTypes = $this->tx_service->transaction(function () use ($summit_id) {
      ...
      foreach ($summit->getMediaUploadTypes() as $mediaUploadType) {
        if (
          $mediaUploadType->hasPublicStorageSet() &&
    ...
    foreach ($mediaUploadTypes as $mediaUploadType) {
      ...
      Log::debug(
        sprintf(
          "SummitService::regenerateTemporalUrlsForMediaUploads processing media upload type %s",
          $mediaUploadType->getId(),
        ),
      );
      do {
        $res = $this->presentation_media_upload_repository->getAllByPage(
          new PagingInfo($page, 100),
          $filter,
        );
        Log::debug(
          sprintf(
            "SummitService::regenerateTemporalUrlsForMediaUploads processing media upload type %s page %s got %s items",
            $mediaUploadType->getId(),
            $page,
            count($res->getItems()),
          ),
        ...
        foreach ($res->getItems() as $item) {
          if (!$item instanceof PresentationMediaUpload) {
            continue;
          }
          Log::debug(
            sprintf(
              "SummitService::regenerateTemporalUrlsForMediaUploads processing media upload %s",
              $item->getId(),
            ),
          );
          try {
            ...
            Log::debug(
              sprintf(
                "SummitService::regenerateTemporalUrlsForMediaUploads media upload %s trying to regenerate public url for %s",
                $item->getId(),
            ...
            if (!is_null($strategy)) {
              ...
              Log::debug(
                sprintf(
                  "SummitService::regenerateTemporalUrlsForMediaUploads media upload %s regenerated public url for %s",
                  $item->getId(),
                  $item->getRelativePath(),
                ),
              );
    ...
    Log::debug(
      sprintf(
        "SummitService::regenerateTemporalUrlsForMediaUploads processed summit %s",
        $summit_id,
      ),
    );
  }
  ...
  public function importRegistrationCompanies(Summit $summit, UploadedFile $csv_file): void {
    ...
    Log::debug(
      sprintf(
        "SummitService::importRegistrationCompanies trying to read file data from %s",
        $real_path,
      ),
    );
    $csv_data = \Illuminate\Support\Facades\File::get($real_path);
    ...
    if (empty($csv_data)) {
      Log::warning(
        sprintf(
          "SummitService::importRegistrationCompanies file %s has empty content.",
          $real_path,
        ),
      );
      throw new ValidationException("file content is empty!");
    ...
    Log::debug(
      sprintf(
        "SummitService::importRegistrationCompanies uploading file %s to storage %s",
        $basename,
        $this->upload_strategy->getDriver(),
      ),
    );
    ...
    Log::debug(
      sprintf(
        "SummitService::importRegistrationCompanies validating header %s",
        json_encode($header),
      ),
    );
  ...
  public function processRegistrationCompaniesData(int $summit_id, string $filename): void {
    Log::debug(
      sprintf(
        "SummitService::processRegistrationCompaniesData summit %s filename %s",
        $summit_id,
        $filename,
      ),
    );
    ...
    if (!$this->download_strategy->exists($path)) {
      Log::warning(
        sprintf(
          "SummitService::processRegistrationCompaniesData file %s does not exists on storage %s.",
          $filename,
          $this->download_strategy->getDriver(),
        ),
      );
    ...
    if (empty($csv_data)) {
      Log::warning(
        sprintf("SummitService::processRegistrationCompaniesData file %s is empty.", $filename),
      );
      throw new ValidationException(sprintf("file %s does not exists.", $filename));
    }
    ...
    Log::debug(
      sprintf(
        "SummitService::processRegistrationCompaniesData deleting file %s from cloud storage %s",
        $path,
        $this->download_strategy->getDriver(),
      ),
    );
  ...
  public function generateQREncKey(Summit $summit): string {
    ...
    $res = $this->tx_service->transaction(function () use ($summit) {
      if (!is_null($summit->getQRCodesEncKey())) {
        throw new ValidationException(
          "There is already a QR encryption key configured for this Summit",
        );
      }
  ...
  public function regenerateBadgeQRCodes(int $summit_id): void {
    ...
    do {
      Log::debug(
        sprintf(
          "SummitService::regenerateBadgeQRCodes summit %s processing page %s",
          $summit_id,
          $page,
        ),
      );
      $page_response = $this->summit_attendee_badge_repository->getBadgeIdsBySummit(
        $summit_id,
        new PagingInfo($page, 500),
      );
      $has_more = count($page_response->getItems()) > 0;

      if (!$has_more) {
        Log::debug(
          sprintf("SummitService::regenerateBadgeQRCodes summit %s no more pages", $summit_id),
        );
        continue;
      }
      ...
      foreach ($page_response->getItems() as $page_response_item) {
        ...
        Log::debug(
          sprintf(
            "SummitService::regenerateBadgeQRCodes summit %s processing badge %s",
            $summit_id,
            $attendee_badge_id,
          ),
        );
        ...
        $this->tx_service->transaction(function () use ($attendee_badge_id) {
          try {
            $attendee_badge = $this->summit_attendee_badge_repository->getById(
              intval($attendee_badge_id),
            );
            if (!$attendee_badge instanceof SummitAttendeeBadge) {
              Log::warning(
                sprintf(
                  "SummitService::regenerateBadgeQRCodes badge %s not found",
                  $attendee_badge_id,
                ),
              );
              return;
    ...
    Log::debug(
      sprintf(
        "SummitService::regenerateBadgeQRCodes summit %s processed %s badges",
        $summit_id,
        $count,
      ),
    );
  ...
  public function generateMUXPrivateKey(int $summit_id): void {
    $this->tx_service->transaction(function () use ($summit_id) {
      try {
        ...
        $this->mux_api->setCredentials(
          new MuxCredentials($summit->getMuxTokenId(), $summit->getMuxTokenSecret()),
        );
        ...
        Log::debug(
          sprintf(
            "SummitService::generateMUXPrivateKey summit %s key %s",
            $summit_id,
            json_encode($key),
          ),
        );
    ...
    $this->tx_service->transaction(function () use ($summit_id) {
      try {
        ...
        if (count($allowed_domains) > 0) {
          ...
          $playback_restriction = $this->mux_api->createPlaybackRestriction($allowed_domains);

          Log::debug(
            sprintf(
              "SummitService::generateMuxPlaybackRestriction summit %s playback_restriction %s",
              $summit_id,
              json_encode($playback_restriction),
            ),
          );
   ...
   * @throws Exception
   */
  public function addRegistrationFeedMetadata(
    Summit $summit,
    array $payload,
  ): SummitRegistrationFeedMetadata {
    return $this->tx_service->transaction(function () use ($summit, $payload) {
  ...
  public function updateRegistrationFeedMetadata(
    Summit $summit,
    int $metadata_id,
    array $payload,
  ): SummitRegistrationFeedMetadata {
    return $this->tx_service->transaction(function () use ($summit, $metadata_id, $payload) {
      ...
      if (isset($payload["key"])) {
        ...
        if (!is_null($formerMetadata) && $formerMetadata->getId() != $metadata->getId()) {
          throw new ValidationException(
            sprintf("Metadata with key %s already exists", $payload["key"]),
          );
        }
      }
  ...
  public function addLeadReportSettings(Summit $summit, array $payload): SummitLeadReportSetting {
    return $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      if (!is_null($former_setting)) {
        throw new ValidationException(
          sprintf("Lead report settings already exists for summit %s", $summit->getId()),
        );
      }
   ...
   * @throws Exception
   */
  public function updateLeadReportSettings(
    Summit $summit,
    array $payload,
