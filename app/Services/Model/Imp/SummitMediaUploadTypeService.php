 ...
 */
final class SummitMediaUploadTypeService extends AbstractService implements
  ISummitMediaUploadTypeService {
  /**
      ...
      if (isset($data["name"])) {
        ...
        if (!is_null($media_upload)) {
          throw new ValidationException(
            sprintf(
              "Media upload name %s already exists at summit %s",
              $data["name"],
              $summit->getId(),
            ),
          );
        ...
        foreach ($data["presentation_types"] as $event_type_id) {
          ...
          if (is_null($presentation_type) || !$presentation_type instanceof PresentationType) {
            throw new EntityNotFoundException(
              sprintf("Presentation Type %s not found", $event_type_id),
            );
          }
  ...
  public function update(
    ...
    return $this->tx_service->transaction(function () use ($summit, $media_upload_type_id, $data) {
      ...
      if (is_null($media_upload)) {
        throw new EntityNotFoundException(
          sprintf(
            "Media upload %s not found at summit %s",
  ...
  public function addToPresentationType(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_null($presentation_type) || !$presentation_type instanceof PresentationType) {
        throw new EntityNotFoundException(
          sprintf("Presentation Type %s not found", $presentation_type_id),
        );
      }
  ...
  public function cloneMediaUploadTypes(Summit $fromSummit, Summit $toSummit): Summit {
    return $this->tx_service->transaction(function () use ($fromSummit, $toSummit) {
      foreach ($fromSummit->getMediaUploadTypes() as $mediaUploadType) {
        if ($toSummit->getMediaUploadTypeByName($mediaUploadType->getName())) {
          continue;
        }
        ...
        foreach ($mediaUploadType->getPresentationTypes() as $presentationType) {
          ...
          if (is_null($newPresentationType)) {
            ...
            $newPresentationType->setAreSpeakersMandatory(
              $presentationType->isAreSpeakersMandatory(),
            );
            $newPresentationType->setUseSpeakers($presentationType->isUseSpeakers());
            $newPresentationType->setAreSponsorsMandatory(
              $presentationType->isAreSponsorsMandatory(),
            );
            ...
            $newPresentationType->setShouldBeAvailableOnCfp(
              $presentationType->isShouldBeAvailableOnCfp(),
            );
            $newPresentationType->setIsModeratorMandatory(
              $presentationType->isModeratorMandatory(),
            );
