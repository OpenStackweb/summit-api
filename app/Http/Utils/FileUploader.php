...
final class FileUploader implements IFileUploader {
  ...
  public function build(UploadedFile $file, string $path, bool $is_image = false) {
    ...
    try {
      ...
      Log::debug(
        sprintf(
          "FileUploader::build: folder_name %s client original name %s",
          $path,
          $client_original_name,
        ),
      );

      $local_path = Storage::disk("public")->putFileAs($path, $file, $client_original_name);

      Log::debug(sprintf("FileUploader::build: saved to local path %s", $local_path));

      Log::debug(
        sprintf("FileUploader::build: invoking folder service findOrMake folder_name %s", $path),
      );
