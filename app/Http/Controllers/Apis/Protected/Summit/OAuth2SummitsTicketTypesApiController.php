...
final class OAuth2SummitsTicketTypesApiController extends OAuth2ProtectedController {
  ...
  public function getAllBySummit($summit_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["id", "name", "external_id", "audience"];
      },
      function ($filter) {
        if ($filter instanceof Filter) {
          $filter->addFilterCondition(
            FilterElement::makeEqual("audience", SummitTicketType::Audience_All),
          );
        }
        return $filter;
      },
  ...
  public function getAllowedBySummitAndCurrentMember($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      ...
      if ($filter->hasFilter("promo_code")) {
        ...
        Log::debug(
          sprintf(
            "OAuth2SummitsTicketTypesApiController::getAllowedBySummitAndCurrentMember promo_code %s",
            $promocode_val,
          ),
        );
      }

      $ticket_types = $this->ticket_type_service->getAllowedTicketTypes(
        $summit,
        $member,
        $promocode_val,
      );
  ...
  public function updateTicketTypeBySummit($summit_id, $ticket_type_id) {
    return $this->processRequest(function () use ($summit_id, $ticket_type_id) {
      ...
      $ticket_type = $this->ticket_type_service->updateTicketType(
        $summit,
        $ticket_type_id,
        $payload,
      );
    ...
    return $this->processRequest(function () use ($summit_id, $currency_symbol) {
      ...
      if (!in_array($currency_symbol, SummitTicketType::AllowedCurrencies)) {
        throw new ValidationException(
          sprintf("Currency symbol %s is not allowed.", $currency_symbol),
        );
      }
