 ...
 */
final class SummitBadgeTypeService extends AbstractService implements ISummitBadgeTypeService {
  ...
  public function addBadgeType(Summit $summit, array $data): SummitBadgeType {
    return $this->tx_service->transaction(function () use ($summit, $data) {
      ...
      if (!is_null($viewType)) {
        $badge_type->addAllowedViewType($viewType);
      }
      return $badge_type;
    });
  }
   ...
   * @throws ValidationException
   */
  public function updateBadgeType(
    Summit $summit,
    int $badge_type_id,
    array $data,
  ): SummitBadgeType {
    return $this->tx_service->transaction(function () use ($summit, $badge_type_id, $data) {
      ...
      if (isset($data["is_default"])) {
        // trying to set a default ( true or false)
        $is_default = boolval($data["is_default"]);

        if ($is_default && !$badge_type->isDefault()) {
          // is we want to set it as default ...
          if (!is_null($formerDefault)) {
        ...
        if (
          !$is_default &&
          (is_null($formerDefault) || $badge_type->getId() == $formerDefault->getId())
        ) {
          // force default
          throw new ValidationException("You must define a default badge type.");
        }
  ...
  public function addViewTypeToBadgeType(
    ...
    return $this->tx_service->transaction(function () use ($summit, $badge_type_id, $view_type_id) {
      Log::debug(
        sprintf(
          "SummitBadgeTypeService::addViewTypeToBadgeType summit %s badge_type_id %s view_type_id %s",
          $summit->getId(),
          $badge_type_id,
          $view_type_id,
        ),
  ...
  public function removeViewTypeFromBadgeType(
    ...
    return $this->tx_service->transaction(function () use ($summit, $badge_type_id, $view_type_id) {
      Log::debug(
        sprintf(
          "SummitBadgeTypeService::removeViewTypeFromBadgeType summit %s badge_type_id %s view_type_id %s",
          $summit->getId(),
          $badge_type_id,
          $view_type_id,
        ),
