...
class SummitBookableVenueRoom extends SummitVenueRoom {
  ...
  public function validateReservation(SummitRoomReservation $reservation): bool {
    ...
    if ($summit->isBookingPeriodEnded()) {
      throw new ValidationException(
        sprintf("Booking period ended for summit %s.", $summit->getName()),
      );
    }
    ...
    if (!($local_start_time <= $local_start_date && $local_end_date <= $local_end_time)) {
      throw new ValidationException(
        sprintf(
  ...
  public function getAvailableSlots(\DateTime $from_day): array {
    ...
    $test_date = $test_date->setTimezone($summit->getTimeZone())->setTime(0, 0, 0, 0);
    ...
    $local_start_datetime
      ->setTimezone($summit->getTimeZone())
      ->setTime(
    ...
    $criteria
      ->where(
        Criteria::expr()->gte(
          "start_datetime",
          $summit->convertDateFromTimeZone2UTC($local_start_datetime),
        ),
      )
      ->andWhere(
        Criteria::expr()->lte(
          "end_datetime",
          $summit->convertDateFromTimeZone2UTC($local_end_datetime),
        ),
      )
      ->andWhere(
        Criteria::expr()->notIn("status", [
    ...
    while ($local_start_datetime <= $local_end_datetime) {
      ...
      if ($current_time_slot_end <= $local_end_datetime) {
        $availableSlots[
          $local_start_datetime->format("Y-m-d H:i:s") .
            "|" .
            $current_time_slot_end->format("Y-m-d H:i:s")
        ] = true;
      }
    ...
    foreach ($reservations as $reservation) {
      ...
      $availableSlots[
        $summit
          ->convertDateFromUTC2TimeZone($reservation->getStartDatetime())
          ->format("Y-m-d H:i:s") .
          "|" .
          $summit
            ->convertDateFromUTC2TimeZone($reservation->getEndDatetime())
            ->format("Y-m-d H:i:s")
      ] = false;
    }
