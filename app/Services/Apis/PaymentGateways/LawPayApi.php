...
final class LawPayApi implements IPaymentGatewayAPI {
  ...
  public function __construct(array $config) {
    ...
    Log::debug(sprintf("LawPayApi::__construct config %s", json_encode($config)));
  }
  ...
  public function postProcessOrder(SummitOrder $order, array $payload = []): SummitOrder {
    Log::debug(
      sprintf(
        "LawPayApi::postProcessOrder order %s payload %s",
        $order->getId(),
        json_encode($payload),
      ),
    );
   ...
   * @throws ValidationException
   */
  private static function handleChargeIOError(ChargeIO_InvalidRequestError $ex): void {
   ...
   * @throws Exception
   */
  private static function handleGenericException(Exception $ex): void {
    Log::error($ex);
    throw $ex;
  }
  ...
  public function generatePayment(array $payload): array {
    ...
    try {
      ChargeIO::setCredentials(new ChargeIO_Credentials($this->public_key, $this->secret_key));
      // convert to cents
      ...
      if (!empty($this->account_id)) {
        Log::debug(
          sprintf("LawPayApi::generatePayment setting merchant account id %s.", $this->account_id),
        );
  ...
  public function processCallback(LaravelRequest $request): array {
    try {
      ...
      Log::debug(sprintf("LawPayApi::processCallback requestContent %s", $requestContent));
  ...
  public function isSuccessFullPayment(array $payload): bool {
    if (
      ...
      in_array($payload["type"], [
        ILawPayApiEventType::TransactionAuthorized,
        ILawPayApiEventType::TransactionCompleted,
      ])
    ) {
      return true;
    }
  ...
  public function refundPayment(
    ...
    try {
      ...
      Log::debug(
        sprintf(
          "LawPayApi::refundPayment refund requested for cart_id %s amount %s response %s",
          $cart_id,
          $amount,
          $res,
        ),
    ...
    try {
      ...
      if (!$this->canAbandon($charge->status)) {
        throw new CartAlreadyPaidException(
          sprintf("cart id %s has status %s", $cart_id, $charge->status),
        );
      }
      ...
      Log::warning(
        sprintf(
          "LawPayApi::abandonCart cart_id %s code %s message %s",
          $cart_id,
          $ex->getCode(),
          $ex->getMessage(),
        ),
  ...
  public function canAbandon(string $status): bool {
    return in_array($status, [ILawPayApiChargeStatus::Pending]);
  }
  ...
  public function getCartStatus(string $cart_id): ?string {
    ...
    try {
      ...
      Log::warning(
        sprintf(
          "LawPayApi::getCartStatus cart_id %s code %s message %s",
          $cart_id,
          $ex->getCode(),
          $ex->getMessage(),
        ),
  ...
  public function isSucceeded(string $status): bool {
    return in_array($status, [
      ILawPayApiChargeStatus::Authorized,
      ILawPayApiChargeStatus::Completed,
    ]);
  }
  ...
  public function createWebHook(string $webhook_endpoint_url): array {
    ...
    try {
      ...
      Log::debug(
        sprintf(
          "LawPayApi::createWebHook found merchant %s",
          json_encode($current_merchant->attributes),
        ),
      );
      ...
      Log::warning(
        sprintf("LawPayApi::createWebHook code %s message %s", $ex->getCode(), $ex->getMessage()),
      );
      return [];
    }
  }
