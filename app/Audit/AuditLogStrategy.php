...
class AuditLogStrategy {
  ...
  private function resolveAuditableEntity($subject) {
    ...
    if (
      $subject instanceof PresentationAction ||
      $subject instanceof PresentationExtraQuestionAnswer
    ) {
    ...
    try {
      ...
      switch ($event_type) {
        case self::EVENT_COLLECTION_UPDATE:
          ...
          $child_entity_formatter =
            $child_entity != null ? ChildEntityFormatterFactory::build($child_entity) : null;
