 ...
 */
final class DoctrineSummitMetricRepository extends SilverStripeDoctrineRepository implements
  ISummitMetricRepository {
  /**
   * @inheritDoc
   */
  ...
  public function getNonAbandoned(
    ...
    if (!is_null($source_id) && $source_id > 0) {
      if ($type == ISummitMetricType::Event || $type == ISummitMetricType::Poster) {
        $query = $query
          ->leftJoin(SummitEventAttendanceMetric::class, "sam", "WITH", "e.id = sam.id")
          ->join("sam.event", "evt")
      ...
      if ($type == ISummitMetricType::Sponsor) {
        $query = $query
          ->leftJoin(SummitSponsorMetric::class, "sm", "WITH", "e.id = sm.id")
          ->join("sm.sponsor", "sp")
