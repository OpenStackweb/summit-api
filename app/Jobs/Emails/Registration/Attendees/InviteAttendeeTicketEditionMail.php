...
class InviteAttendeeTicketEditionMail extends AbstractSummitAttendeeTicketEmail {
  ...
  public function __construct(
    ...
    if (!is_null($summit_reassign_ticket_till_date)) {
      $payload[
        IMailTemplatesConstants::summit_reassign_ticket_till_date
      ] = $summit_reassign_ticket_till_date->format("l j F Y h:i A T");
    }

    if (empty($payload[IMailTemplatesConstants::owner_full_name])) {
      Log::warning(
        sprintf("InviteAttendeeTicketEditionMail owner_full_name is empty setting email"),
      );
    ...
    if (empty($payload[IMailTemplatesConstants::owner_first_name])) {
      Log::warning(
        sprintf("InviteAttendeeTicketEditionMail owner_first_name is empty setting email"),
      );
      $payload[IMailTemplatesConstants::owner_first_name] =
        $payload[IMailTemplatesConstants::owner_email];
    }

    if (empty($payload[IMailTemplatesConstants::owner_last_name])) {
      Log::warning(
        sprintf("InviteAttendeeTicketEditionMail owner_last_name is empty setting email"),
      );
      $payload[IMailTemplatesConstants::owner_last_name] =
        $payload[IMailTemplatesConstants::owner_email];
    }
    ...
    $payload[
      IMailTemplatesConstants::summit_virtual_site_oauth2_client_id
    ] = $summit->getVirtualSiteOAuth2ClientId();
    $payload[
      IMailTemplatesConstants::summit_marketing_site_oauth2_client_id
    ] = $summit->getMarketingSiteOAuth2ClientId();
    $payload[
      IMailTemplatesConstants::summit_marketing_site_oauth2_scopes
    ] = $summit->getMarketingSiteOauth2ClientScopes();
    ...
    $payload[IMailTemplatesConstants::ticket_raw_amount] = FormatUtils::getNiceFloat(
      $ticket->getRawCost(),
    );
    ...
    $payload[IMailTemplatesConstants::ticket_discount] = FormatUtils::getNiceFloat(
      $ticket->getDiscount(),
    );
    $payload[IMailTemplatesConstants::ticket_taxes] = FormatUtils::getNiceFloat(
      $ticket->getTaxesAmount(),
    );
    $payload[IMailTemplatesConstants::ticket_amount] = FormatUtils::getNiceFloat(
    ...
    if (!is_null($promo_code)) {
      ...
      if ($promo_code instanceof SummitRegistrationDiscountCode) {
        ...
        $payload[IMailTemplatesConstants::promo_code_discount_amount] = FormatUtils::getNiceFloat(
          $promo_code->getAmount(),
        );
      }
    }
    ...
    Log::debug(
      sprintf(
        "InviteAttendeeTicketEditionMail::__construct payload %s template %s",
        json_encode($payload),
        $template_identifier,
      ),
    );
