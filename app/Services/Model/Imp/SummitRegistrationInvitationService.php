 ...
 */
final class SummitRegistrationInvitationService extends AbstractService implements
  ISummitRegistrationInvitationService {
  /**
   ...
   */
  public function importInvitationData(
    Summit $summit,
    ...
    Log::debug(
      sprintf(
        "SummitRegistrationInvitationService::importInvitationData - summit %s",
        $summit->getId(),
      ),
    );
    ...
    foreach ($reader as $idx => $row) {
      try {
        Log::debug(
          sprintf(
            "SummitRegistrationInvitationService::importInvitationData processing row %s",
            json_encode($row),
          ),
        );
        if (isset($row["allowed_ticket_types"]) && is_string($row["allowed_ticket_types"])) {
          $row["allowed_ticket_types"] = empty($row["allowed_ticket_types"])
            ? []
            : explode("|", $row["allowed_ticket_types"]);
        }
        ...
        $row["acceptance_criteria"] =
          ...
          SummitRegistrationInvitation::AcceptanceCriteria_AllTicketTypes;
  ...
  public function add(Summit $summit, array $payload): SummitRegistrationInvitation {
    return $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      Log::debug(
        sprintf("SummitRegistrationInvitationService::add trying to add email %s", $email),
      );
      $allowed_ticket_types = $payload["allowed_ticket_types"] ?? [];
      $former_invitation = $summit->getSummitRegistrationInvitationByEmail($email);
      if (!is_null($former_invitation)) {
        throw new ValidationException(
          sprintf("Email %s already has been invited for summit %s", $email, $summit->getId()),
        );
      }
      ...
      foreach ($allowed_ticket_types as $ticket_type_id) {
        ...
        Log::debug(
          sprintf(
            "SummitRegistrationInvitationService::add trying to add ticket %s for invitation email %s",
            $ticket_type_id,
            $email,
          ),
        );
      ...
      Log::debug(
        sprintf(
          "SummitRegistrationInvitationService::add adding invitation for email %s to summit %s",
          $email,
          $summit->getName(),
        ),
      );
   ...
   * @throws \Exception
   */
  private function setInvitationMember(
    ...
    return $this->tx_service->transaction(function () use ($invitation, $email) {
      try {
        ...
        if (is_null($member)) {
          ...
          Log::debug(
            sprintf(
              "SummitRegistrationInvitationService::setInvitationMember - trying to get member %s from user api",
              $email,
            ),
          );
          ...
          if (!is_null($user)) {
            ...
            try {
              ...
              Log::warning($ex);
              // race condition lost
              $member = $this->member_repository->getByExternalIdExclusiveLock(
                intval($external_id),
              );
              $invitation = $this->invitation_repository->getByIdExclusiveLock(
                $invitation->getId(),
              );
            }
          }
    ...
    return $this->tx_service->transaction(function () use ($summit, $payload, $invitation_id) {
      ...
      if (is_null($invitation)) {
        throw new EntityNotFoundException(
          sprintf("Invitation %s not found at Summit %s", $invitation_id, $summit->getId()),
        );
      }
   ...
   * @throws \Exception
   */
  public function getInvitationByToken(
    Member $current_member,
    string $token,
  ): SummitRegistrationInvitation {
    return $this->tx_service->transaction(function () use ($current_member, $token) {
      $invitation = $this->invitation_repository->getByHashExclusiveLock(
        SummitRegistrationInvitation::HashConfirmationToken($token),
      );
      ...
      Log::debug(
        sprintf("got invitation %s for email %s", $invitation->getId(), $invitation->getEmail()),
      );
   ...
   * @throws \Exception
   */
  public function getInvitationBySummitAndToken(
    Summit $summit,
    string $token,
  ): SummitRegistrationInvitation {
   ...
   * @throws \Exception
   */
  public function rejectInvitationBySummitAndToken(
    Summit $summit,
    string $token,
  ): SummitRegistrationInvitation {
    return $this->tx_service->transaction(function () use ($summit, $token) {
   ...
   * @throws \Exception
   */
  public function getInvitationByEmail(
    Summit $summit,
    string $email,
  ): ?SummitRegistrationInvitation {
    ...
    $this->_sendEmails(
      ...
      function ($summit, $paging_info, $filter, $resetPage) {
        ...
        if ($filter->hasFilter("is_sent")) {
          ...
          Log::debug(
            sprintf("SummitRegistrationInvitationService::send is_sent filter value %b", $is_sent),
          );
          if (!$is_sent && is_callable($resetPage)) {
            ...
            Log::debug(
              "SummitRegistrationInvitationService::send resetting page bc is_sent filter is false",
            );
            $resetPage();
          }
        }
      ...
      function (
        ...
        try {
          $this->tx_service->transaction(function () use (
            ...
            $invitation = $this->tx_service->transaction(function () use (
              $flow_event,
              $invitation_id,
            ) {
              Log::debug(
                sprintf(
                  "SummitRegistrationInvitationService::send processing invitation id  %s",
                  $invitation_id,
                ),
              );

              $invitation = $this->invitation_repository->getByIdExclusiveLock(
                intval($invitation_id),
              );
              ...
              if ($invitation->isRejected()) {
                Log::warning(
                  sprintf(
                    "SummitRegistrationInvitationService::send invitation %s is already rejected",
                    $invitation_id,
                  ),
                );
                return null;
              ...
              while (true) {
                ...
                $former_invitation = $this->invitation_repository->getByHashAndSummit(
                  $invitation->getHash(),
                  $summit,
                );
                if (
                  is_null($former_invitation) ||
                  $former_invitation->getId() == $invitation->getId()
                ) {
                  break;
                }
              }
            ...
            if (
              $flow_event == ReInviteSummitRegistrationEmail::EVENT_SLUG &&
              !is_null($invitation)
            ) {
