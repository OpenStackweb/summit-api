...
final class PaymentGatewayWebHookController extends JsonController {
  ...
  public function genericConfirm($application_type, LaravelRequest $request) {
    try {
      Log::debug(
        sprintf(
          "PaymentGatewayWebHookController::genericConfirm application_type %s ",
          $application_type,
        ),
      );
      ...
      if (is_null($paymentGatewayApi)) {
        Log::debug(
          sprintf(
            "PaymentGatewayWebHookController::genericConfirm application_type %s profile payment not found.",
            $application_type,
          ),
        );
        return $this->error412([
          sprintf("application_type %s profile payment not found.", $application_type),
        ]);
      }
      ...
      if (is_null($service)) {
        Log::debug(
          sprintf(
            "PaymentGatewayWebHookController::genericConfirm application_type %s service not found.",
            $application_type,
          ),
        );
        return $this->error412([
          sprintf("application_type %s service not found.", $application_type),
        ]);
      }

      $service->processPayment(
        $paymentGatewayApi->buildPaymentGatewayApi()->processCallback($request),
      );
  ...
  public function confirm($summit_id, $application_type, LaravelRequest $request) {
    try {
      Log::debug(
        sprintf(
          "PaymentGatewayWebHookController::confirm summit %s application_type %s ",
          $summit_id,
          $application_type,
        ),
      );
      ...
      if (is_null($summit) || !$summit instanceof Summit) {
        Log::debug(
          sprintf("PaymentGatewayWebHookController::confirm summit %s not found.", $summit_id),
        );
        return $this->error412([
          sprintf("application_type %s summit not found.", $application_type),
        ]);
      }

      // get api
      $paymentGatewayApi = $summit->getPaymentGateWayPerApp(
        $application_type,
      ...
      if (is_null($paymentGatewayApi)) {
        Log::debug(
          sprintf(
            "PaymentGatewayWebHookController::confirm summit %s profile payment not found.",
            $summit_id,
          ),
        );
        return $this->error412([
      ...
      if (is_null($service)) {
        Log::debug(
          sprintf(
            "PaymentGatewayWebHookController::confirm summit %s service not found.",
            $summit_id,
          ),
        );
        return $this->error412([
