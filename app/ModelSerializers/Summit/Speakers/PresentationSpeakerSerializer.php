  ...
  public function serialize(
    ...
    if (!is_null($summit)) {
      ...
      if (in_array("moderated_presentations", $relations)) {
        $values["moderated_presentations"] = $speaker->getModeratedPresentationIds(
          $summit->getId(),
          $published,
        );
      }
        ...
        if (in_array("badge_features", $relations)) {
          ...
          foreach ($member->getPaidSummitTicketsBySummitId($summit_id) as $ticket) {
            foreach ($ticket->getBadgeFeatures() as $feature) {
              if (in_array($feature->getId(), $already_processed_features)) {
                continue;
              }
    ...
    if (in_array("accepted_presentations", $relations) && !is_null($summit)) {
      $accepted_presentation_ids = $speaker->getAcceptedPresentationIds(
        $summit,
      ...
      $moderated_accepted_presentation_ids = $speaker->getAcceptedPresentationIds(
        $summit,
      ...
      $values["accepted_presentations"] = array_merge(
        $accepted_presentation_ids,
        $moderated_accepted_presentation_ids,
      );
    }

    if (in_array("alternate_presentations", $relations) && !is_null($summit)) {
      $alternate_presentation_ids = $speaker->getAlternatePresentationIds(
        $summit,
        PresentationSpeaker::RoleSpeaker,
        false,
        [],
      ...
      $moderated_alternate_presentation_ids = $speaker->getAlternatePresentationIds(
        $summit,
        PresentationSpeaker::RoleModerator,
        false,
        [],
      ...
      $values["alternate_presentations"] = array_merge(
        $alternate_presentation_ids,
        $moderated_alternate_presentation_ids,
      );
    }

    if (in_array("rejected_presentations", $relations) && !is_null($summit)) {
      $rejected_presentation_ids = $speaker->getRejectedPresentationIds(
        $summit,
        PresentationSpeaker::RoleSpeaker,
        false,
        [],
      ...
      $moderated_rejected_presentation_ids = $speaker->getRejectedPresentationIds(
        $summit,
        PresentationSpeaker::RoleModerator,
        false,
        [],
      ...
      $values["rejected_presentations"] = array_merge(
        $rejected_presentation_ids,
        $moderated_rejected_presentation_ids,
      );
    }
    ...
    if (in_array("affiliations", $relations)) {
      ...
      if ($speaker->hasMember()) {
        ...
        foreach ($member->getCurrentAffiliations() as $affiliation) {
          $affiliations[] = SerializerRegistry::getInstance()
    ...
    if (in_array("languages", $relations)) {
      ...
      foreach ($speaker->getLanguages() as $language) {
        $languages[] = SerializerRegistry::getInstance()
          ->getSerializer($language)
          ->serialize(
    ...
    if (in_array("other_presentation_links", $relations)) {
      ...
      foreach ($speaker->getOtherPresentationLinks() as $link) {
        $other_presentation_links[] = SerializerRegistry::getInstance()
          ->getSerializer($link)
          ->serialize(
    ...
    if (in_array("areas_of_expertise", $relations)) {
      ...
      foreach ($speaker->getAreasOfExpertise() as $exp) {
        $areas_of_expertise[] = SerializerRegistry::getInstance()
          ->getSerializer($exp)
          ->serialize(
    ...
    if (in_array("travel_preferences", $relations)) {
      ...
      foreach ($speaker->getTravelPreferences() as $tp) {
        $travel_preferences[] = SerializerRegistry::getInstance()
          ->getSerializer($tp)
          ->serialize(
    ...
    if (in_array("active_involvements", $relations)) {
      ...
      foreach ($speaker->getActiveInvolvements() as $ai) {
        $active_involvements[] = SerializerRegistry::getInstance()
          ->getSerializer($ai)
          ->serialize(
    ...
    if (in_array("organizational_roles", $relations)) {
      ...
      foreach ($speaker->getOrganizationalRoles() as $or) {
        $organizational_roles[] = SerializerRegistry::getInstance()
          ->getSerializer($or)
          ->serialize(
        ...
        switch ($relation) {
          case "presentations":
            ...
            foreach ($speaker->getPresentations($summit_id, $published) as $p) {
              $presentations[] = SerializerRegistry::getInstance()
                ->getSerializer($p)
                ->serialize(
            ...
            foreach ($speaker->getModeratedPresentations($summit_id, $published) as $p) {
              $moderated_presentations[] = SerializerRegistry::getInstance()
                ->getSerializer($p)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "member":
            if ($speaker->hasMember()) {
              ...
              $values["member"] = SerializerRegistry::getInstance()
                ->getSerializer($speaker->getMember(), $this->getMemberSerializerType())
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
                  AbstractSerializer::filterFieldsByPrefix($relations, $relation),
          ...
          case "accepted_presentations":
            $accepted_presentations = [];
            foreach (
              $speaker->getAcceptedPresentations(
                $summit,
                PresentationSpeaker::RoleSpeaker,
                true,
                [],
            ...
            foreach (
              ...
              $moderated_accepted_presentations[] = SerializerRegistry::getInstance()
                ->getSerializer($p)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "alternate_presentations":
            $alternate_presentations = [];
            foreach (
              $speaker->getAlternatePresentations(
                $summit,
                PresentationSpeaker::RoleSpeaker,
                false,
                [],
            ...
            foreach (
              ...
              $moderated_alternate_presentations[] = SerializerRegistry::getInstance()
                ->getSerializer($p)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "rejected_presentations":
            $rejected_presentations = [];
            foreach (
              $speaker->getRejectedPresentations(
                $summit,
                PresentationSpeaker::RoleSpeaker,
                false,
                [],
            ...
            foreach (
              ...
              $moderated_rejected_presentations[] = SerializerRegistry::getInstance()
                ->getSerializer($p)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
