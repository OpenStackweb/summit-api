...
trait InsertSummitTestData {
  ...
  protected static function insertSummitTestData() {
    ...
    self::$summit_permission_group_repository = EntityManager::getRepository(
      SummitAdministratorPermissionGroup::class,
    );
    ...
    self::$default_selection_plan->setPresentationCreatorNotificationEmailTemplate(
      "PRESENTATION_CREATOR_EMAIL_TEMPLATE",
    );
    self::$default_selection_plan->setPresentationModeratorNotificationEmailTemplate(
      "PRESENTATION_MODERATOR_EMAIL_TEMPLATE",
    );
    self::$default_selection_plan->setPresentationSpeakerNotificationEmailTemplate(
      "PRESENTATION_SPEAKER_EMAIL_TEMPLATE",
    );
    ...
    self::$default_selection_plan2->setPresentationCreatorNotificationEmailTemplate(
      "PRESENTATION_CREATOR_EMAIL_TEMPLATE",
    );
    self::$default_selection_plan2->setPresentationModeratorNotificationEmailTemplate(
      "PRESENTATION_MODERATOR_EMAIL_TEMPLATE",
    );
    self::$default_selection_plan2->setPresentationSpeakerNotificationEmailTemplate(
      "PRESENTATION_SPEAKER_EMAIL_TEMPLATE",
    );
    $submission_begin_date = new DateTime("now", self::$summit->getTimeZone());
    $submission_end_date = (clone $submission_begin_date)->add(new DateInterval("P14D"));
    ...
    self::$default_selection_plan->addPresentationActionType(
      self::$default_presentation_action_type,
    );
    ...
    for ($i = 0; $i < 20; $i++) {
      ...
      $presentation->setAbstract(
        sprintf("Presentation Abstract %s %s Votable", $i, str_random(16)),
      );
      $presentation->setCategory(self::$defaultTrack);
      $presentation->setProgress(Presentation::PHASE_COMPLETE);
      $presentation->setStatus(Presentation::STATUS_RECEIVED);
    ...
    self::$summit_permission_group->setTitle(
      sprintf("DEFAULT PERMISSION GROUP %s", str_random(16)),
    );
  ...
  protected static function clearSummitTestData() {
    ...
    self::$default_media_file_type = self::$media_file_type_repository->find(
      self::$default_media_file_type->getId(),
    );
    self::$summit_permission_group = self::$summit_permission_group_repository->find(
      self::$summit_permission_group->getId(),
    );
