  ...
  private function _sendEmails(
    ...
    do {
      $ids = $this->tx_service->transaction(function () use (
        ...
        return $getIdsBySummit(
          $summit,
          new PagingInfo($page, MaxPageSize),
          $filter,
          function () use ($caller, &$page) {
            $page = 0;
      ...
      if (!count($ids)) {
        ...
        Log::debug(
          sprintf("%s::send summit id %s page is empty, ending processing.", $caller, $summit_id),
        );
        break;
      }
      ...
      foreach ($ids as $subject_id) {
        try {
          if (in_array($subject_id, $exclude_ids)) {
            Log::debug(
              sprintf(
                "%s::send summit id %s %s id %s is excluded",
                $caller,
                $summit->getId(),
                $subject,
                $subject_id,
    ...
    if (
      !empty($outcome_email_recipient) &&
      !is_null($sendEmailExcerpt) &&
      is_callable($sendEmailExcerpt)
    ) {
