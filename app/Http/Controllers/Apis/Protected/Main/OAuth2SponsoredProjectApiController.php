...
final class OAuth2SponsoredProjectApiController extends OAuth2ProtectedController {
        ...
        if ($filter instanceof Filter) {
          if (is_numeric($id)) {
            $filter->addFilterCondition(
              FilterElement::makeEqual("sponsored_project_id", intval($id)),
            );
          } else {
  ...
  public function getSponsorshipType($id, $sponsorship_type_id) {
    Log::debug(
      sprintf(
        "OAuth2SponsoredProjectApiController::getSponsorshipType id %s sponsorship_type_id %s",
        $id,
        $sponsorship_type_id,
      ),
    );
  ...
  public function addSponsorshipType($id) {
    ...
    return $this->_add(
      ...
      function ($payload, $id) {
        return $this->service->addProjectSponsorshipType(
          $id,
  ...
  public function updateSponsorshipType($id, $sponsorship_type_id) {
    ...
    return $this->_update(
      ...
      function ($sponsorship_type_id, $payload, $project_id) {
        return $this->service->updateProjectSponsorshipType(
          $project_id,
          $sponsorship_type_id,
          HTMLCleaner::cleanData($payload, ["description"]),
        );
      ...
      function ($filter) use ($id, $sponsorship_type_id) {
        if ($filter instanceof Filter) {
          ...
          if (is_numeric($sponsorship_type_id)) {
            $filter->addFilterCondition(
              FilterElement::makeEqual("sponsorship_type_id", intval($sponsorship_type_id)),
            );
          } else {
            $filter->addFilterCondition(
              FilterElement::makeEqual("sponsorship_type_slug", $sponsorship_type_id),
            );
          }
        }
        return $filter;
  ...
  public function deleteSupportingCompanies($id, $sponsorship_type_id, $company_id) {
    return $this->_delete(
      $company_id,
      function ($id, $project_id, $sponsorship_type_id) {
        $this->service->removeCompanyToProjectSponsorshipType(
          $project_id,
          $sponsorship_type_id,
          $id,
        );
