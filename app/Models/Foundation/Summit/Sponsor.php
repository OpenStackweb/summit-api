...
class Sponsor extends SilverstripeBaseModel implements IOrderable {
  ...
  public function addUserInfoGrant(SponsorUserInfoGrant $grant) {
    if ($this->user_info_grants->contains($grant)) {
      return;
    }
  ...
  public function addSocialNetwork(SponsorSocialNetwork $social_network): void {
    if ($this->social_networks->contains($social_network)) {
      return;
    }
   ...
   */
  public function setCarouselAdvertiseImageAltText(
    string $carousel_advertise_image_alt_text,
  ): void {
  ...
  public function addExtraQuestion(ExtraQuestionType $extra_question): void {
    if ($this->extra_questions->count() >= self::MaxExtraQuestionCount) {
      throw new ValidationException(
        sprintf(
          "Sponsor %s cannot have more than %s extra questions.",
          $this->id,
          self::MaxExtraQuestionCount,
        ),
      );
    }
  ...
  public function recalculateQuestionOrder(
    SummitSponsorExtraQuestionType $question,
    int $new_order,
  ) {
  ...
  public function clearLeadReportSetting() {
    if (is_null($this->lead_report_setting)) {
      return;
    }
