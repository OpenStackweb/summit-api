...
final class StripeApi implements IPaymentGatewayAPI {
  ...
  public function getCreditCardInfo(array $payload): array {
    ...
    if (!isset($payload["charges"]) || !isset($payload["charges"]["data"])) {
      return [];
    }
  ...
  public function generatePayment(array $payload): array {
    ...
    if (isset($payload["receipt_email"]) && $this->send_email_receipt) {
      Log::debug(
        sprintf(
          "StripeApi::generatePayment setting receipt_email to %s",
    ...
    Log::debug(
      sprintf("StripeApi::generatePayment creating payment intent %s", json_encode($request)),
    );
  ...
  public function processCallback(LaravelRequest $request): array {
    try {
      ...
      WebhookSignature::verifyHeader($requestContent, $signature, $this->webhook_secret_key);
  ...
  public function isSuccessFullPayment(array $payload): bool {
    if (isset($payload["event_type"]) && $payload["event_type"] == "payment_intent.succeeded") {
      return true;
    }
  ...
  public function getPaymentError(array $payload): ?string {
    if (
      ...
      $payload["event_type"] == "payment_intent.payment_failed"
    ) {
  ...
  public function refundPayment(
    ...
    try {
      Log::debug(
        sprintf(
          "StripeApi::refundPayment calling cart_id %s amount %s currency %s",
          $cart_id,
          $amount,
          $currency,
        ),
      ...
      Log::debug(
        sprintf(
          "StripeApi::refundPayment refund requested for cart_id %s amount %s response %s",
          $cart_id,
          $amount,
          $res,
        ),
  ...
  public function abandonCart(string $cart_id) {
    ...
    if (
      ...
      throw new CartAlreadyPaidException(
        sprintf("cart id %s has status %s", $cart_id, $intent->status),
      );
    }
  ...
  public function getCartStatus(string $cart_id): ?string {
    ...
    try {
      ...
      Log::warning(
        sprintf(
          "StripeApi::getCartStatus cart_id %s code %s message %s",
          $cart_id,
          $ex->getCode(),
          $ex->getMessage(),
        ),
  ...
  public function createWebHook(string $webhook_endpoint_url): array {
    ...
    Log::debug(
      sprintf(
        "StripeApi::createWebHook webhook_endpoint_url %s res %s",
  ...
  public function getCartCreditCardInfo(string $cart_id): ?array {
    ...
    try {
      ...
      Log::warning(
        sprintf(
          "StripeApi::getCartCreditCardInfo cart_id %s code %s message %s",
          $cart_id,
          $ex->getCode(),
          $ex->getMessage(),
        ),
