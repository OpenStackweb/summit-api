 ...
 */
final class SummitPromoCodeService extends AbstractService implements ISummitPromoCodeService {
  /**
  ...
  public function addPromoCode(Summit $summit, array $data, Member $current_user = null) {
    $promo_code = $this->tx_service->transaction(function () use ($summit, $data, $current_user) {
      ...
      if (!is_null($old_promo_code)) {
        throw new ValidationException(
          sprintf(
            "Promo code %s already exits on Summit %s.",
            trim($data["code"]),
            $summit->getId(),
          ),
        );
      ...
      $promo_code = SummitPromoCodeFactory::build(
        $summit,
        $data,
        $this->getPromoCodeParams($summit, $data),
      );
    ...
    if (isset($data["ticket_types_rules"])) {
      $promo_code = $this->tx_service->transaction(function () use (
        $summit,
        $data,
        $current_user,
        $promo_code,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (isset($data["code"])) {
        ...
        if (!is_null($old_promo_code) && $old_promo_code->getId() != $promo_code_id) {
          throw new ValidationException(
            sprintf(
              "Promo Code %s already exits on summit id %s for promo code id %s.",
              trim($data["code"]),
              $summit->getId(),
              $old_promo_code->getId(),
            ),
          );
        }
      }
      ...
      $badge_features_apply_to_all_tix_retroactively =
        $data["badge_features_apply_to_all_tix_retroactively"] ?? false;

      if ($badge_features_apply_to_all_tix_retroactively) {
        Log::debug(
          sprintf(
            "SummitPromoCodeService::updatePromoCode summit %s promo code %s triggering retro actively apply features",
            $summit->getId(),
            $promo_code_id,
          ),
        );
  ...
  public function deletePromoCode(Summit $summit, $promo_code_id) {
    $this->tx_service->transaction(function () use ($promo_code_id, $summit) {
      ...
      if (is_null($promo_code)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.promo_code_delete_code_not_found", [
      ...
      if ($promo_code->isRedeemed()) {
        throw new ValidationException(
          trans("validation_errors.promo_code_delete_already_redeemed"),
        );
      }
  ...
  public function sendPromoCodeMail(Summit $summit, $promo_code_id) {
    $this->tx_service->transaction(function () use ($promo_code_id, $summit) {
      ...
      if (is_null($promo_code)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.promo_code_email_code_not_found", [
            "promo_code_id" => $promo_code_id,
            "summit_id" => $summit->getId(),
          ]),
        );
   ...
   * @throws ValidationException
   */
  public function addPromoCodeTicketTypeRule(
    Summit $summit,
    int $promo_code_id,
    int $ticket_type_id,
    array $data,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if ($promo_code instanceof SummitRegistrationDiscountCode) {
        ...
        if ($ticket_type->isFree()) {
          throw new ValidationException(
            sprintf(
              "Ticket Type %s (Free) can not be added on added on Discount Code.",
              $ticket_type_id,
            ),
          );
        }
   ...
   * @throws ValidationException
   */
  public function removePromoCodeTicketTypeRule(
    Summit $summit,
    int $promo_code_id,
    int $ticket_type_id,
  ): SummitRegistrationPromoCode {
   ...
   * @throws \Exception
   */
  public function addPromoCodeBadgeFeature(
    Summit $summit,
    int $promo_code_id,
    int $badge_feature_id,
  ): SummitRegistrationPromoCode {
    return $this->tx_service->transaction(function () use (
      $summit,
      $promo_code_id,
   ...
   * @throws ValidationException
   */
  public function removePromoCodeBadgeFeature(
    Summit $summit,
    int $promo_code_id,
    int $badge_feature_id,
  ): SummitRegistrationPromoCode {
  ...
  public function importPromoCodes(
    ...
    foreach ($reader as $idx => $row) {
      try {
        ...
        Log::debug(
          sprintf("SummitPromoCodeService::importPromoCodes processing row %s", json_encode($row)),
        );
        $code = trim($row["code"]);
   ...
   * @throws ValidationException
   */
  public function importSponsorPromoCodes(
    Summit $summit,
    UploadedFile $csv_file,
    ?Member $current_user = null,
  ): void {
    Log::debug(
      sprintf("SummitPromoCodeService::importSponsorPromoCodes - summit %s", $summit->getId()),
    );
    ...
    foreach ($reader as $idx => $row) {
      try {
        ...
        Log::debug(
          sprintf(
            "SummitPromoCodeService::importSponsorPromoCodes processing row %s",
            json_encode($row),
          ),
        );
        $code = trim($row["code"]);
  ...
  public function reApplyPromoCode(int $promo_code_id): void {
    Log::debug(
      sprintf("SummitPromoCodeService::reApplyPromoCode promo code id %s", $promo_code_id),
    );
    ...
    do {
      $ticket_ids = $this->tx_service->transaction(function () use ($filter, $page, $maxPageSize) {
        return $this->ticket_repository->getAllIdsByPage(
      ...
      if (!count($ticket_ids)) {
        ...
        Log::debug(
          sprintf(
            "SummitPromoCodeService::reApplyPromoCode promo code id %s page is empty, ending processing.",
            $promo_code_id,
          ),
        );
        break;
      ...
      foreach ($ticket_ids as $ticket_id) {
        try {
          Log::debug(
            sprintf("SummitPromoCodeService::reApplyPromoCode processing ticket %s", $ticket_id),
          );
          ...
          $this->tx_service->transaction(function () use ($ticket_id, $promo_code) {
            ...
            if (!$ticket->hasBadge()) {
              throw new ValidationException(
                sprintf("Ticket %s does not has a Badge set.", $ticket_id),
              );
            }

            Log::debug(
              sprintf(
                "SummitPromoCodeService::reApplyPromoCode reapplying promo code %s to ticket %s",
                $promo_code->getId(),
                $ticket_id,
              ),
            );
   ...
   * @throws \Exception
   */
  public function addPromoCodeSpeaker(
   ...
   * @throws \Exception
   */
  public function removePromoCodeSpeaker(
    SummitRegistrationPromoCode $promo_code,
    int $speaker_id,
  ): SummitRegistrationPromoCode {
    return $this->tx_service->transaction(function () use ($promo_code, $speaker_id) {
  ...
  public function preValidatePromoCode(
    ...
    $this->tx_service->transaction(function () use ($summit, $owner, $promo_code_value, $filter) {
      ...
      if (is_null($ticket_type)) {
        throw new EntityNotFoundException(
          sprintf("Ticket Type %s not found on summit %s.", $ticket_type_id, $summit->getId()),
        );
      }
      ...
      $validator = PromoCodeValidationStrategyFactory::createStrategy(
        $ticket_type,
        $ticket_type_subtype,
        $qty,
        $owner,
      ...
      if (
        ...
        $promo_code->getSummitId() != $summit->getId() ||
        !$validator->isValid($promo_code)
      ) {
        throw new ValidationException(
   ...
   * @inheritDoc
   */
  public function triggerSendSponsorPromoCodes(
    Summit $summit,
    array $payload,
    $filter = null,
  ): void {
   ...
   * @inheritDoc
   */
  public function sendSponsorPromoCodes(
    int $summit_id,
    array $payload,
    Filter $filter = null,
  ): void {
      ...
      function ($summit, $paging_info, $filter, $resetPage) {
        ...
        if ($filter->hasFilter("email_sent")) {
          ...
          if (!$email_sent && is_callable($resetPage)) {
            ...
            Log::debug(
              "SummitPromoCodesService::send resetting page bc email_sent filter is false",
            );
            $resetPage();
          }
        }
        ...
        try {
          $this->tx_service->transaction(function () use (
            ...
            $promo_code = $this->tx_service->transaction(function () use (
              ...
              Log::debug(
                sprintf("SummitPromoCodeService::send processing promocode id  %s", $promocode_id),
              );
