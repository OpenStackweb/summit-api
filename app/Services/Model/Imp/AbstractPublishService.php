...
abstract class AbstractPublishService extends AbstractService {
  ...
  public function __construct(
    ISummitEventPublishRepository $event_repository,
  ...
  public function updateDuration(
    ...
    return $this->tx_service->transaction(function () use ($data, $summit, $publishable_event) {
      Log::debug(
        sprintf(
          "AbstractPublishService::updateDuration data %s summit %s event %s",
          json_encode($data),
          $summit->getId(),
          $publishable_event->getId(),
        ),
    ...
    if (isset($data["start_date"]) && isset($data["end_date"])) {
      ...
      if (!$publishable_event->hasType()) {
        throw new ValidationException(
          "To be able to set schedule dates event type must be set First.",
        );
      }
  ...
  protected function validateBlackOutTimesAndTimes(
    ...
    Log::debug(
      sprintf(
        "AbstractPublishService::validateBlackOutTimesAndTimes event %s",
    ...
    if (!$eventType->isAllowsPublishingDates()) {
      return;
    }
    ...
    if (count($conflict_events) > 0) {
      ...
      foreach ($conflict_events as $c_event) {
        $is_blackout_applied_2_conflicting_event = $c_event
          ->getType()
          ->isBlackoutAppliedTo($c_event);
        Log::debug(
          sprintf(
            ...
            is_null($current_event_location)
              ? false
              : $current_event_location->isOverrideBlackouts(),
        ...
        if (
          $publishable_event instanceof IPublishableEventWithSpeakerConstraint &&
