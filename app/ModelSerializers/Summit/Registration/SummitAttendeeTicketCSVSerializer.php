  ...
  public function serialize(
    ...
    if (isset($params["features_types"])) {
      ...
      foreach ($params["features_types"] as $features_type) {
        if (!$features_type instanceof SummitBadgeFeatureType) {
          continue;
        }
        $values[$features_type->getName()] = in_array($features_type->getName(), $ticket_features)
          ? "1"
          : "0";
      }
    }
    ...
    if (isset($params["ticket_questions"])) {
      foreach ($params["ticket_questions"] as $question) {
        ...
        if (!is_null($ticket_owner)) {
          ...
          Log::debug(
            sprintf(
              "SummitAttendeeTicketCSVSerializer::serialize question %s value %s",
              $question->getId(),
              $value,
            ),
          );
    ...
    foreach ($ticket->getAppliedTaxes() as $appliedTax) {
      $values[
        sprintf("%s_rate", strtolower($appliedTax->getTax()->getName()))
      ] = $appliedTax->getRate();
      $values[
        sprintf("%s_price", strtolower($appliedTax->getTax()->getName()))
