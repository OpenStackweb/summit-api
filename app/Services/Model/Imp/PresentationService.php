 ...
 */
final class PresentationService extends AbstractService implements IPresentationService {
    ...
    $video = $this->tx_service->transaction(function () use ($presentation_id, $video_data) {
      ...
      if ($presentation->hasVideos()) {
        throw new ValidationException(
          sprintf("presentation %s already has a video!", $presentation_id),
        );
      }

      if (!isset($video_data["name"])) {
        $video_data["name"] = $presentation->getTitle();
      }
  ...
  public function updateVideo($presentation_id, $video_id, array $video_data) {
    $video = $this->tx_service->transaction(function () use (
      $presentation_id,
      $video_id,
      $video_data,
    ) {
  ...
  public function submitPresentation(Summit $summit, array $data) {
    return $this->tx_service->transaction(function () use ($summit, $data) {
      Log::debug(
        sprintf(
          "PresentationService::submitPresentation summit %s payload %s",
          $summit->getId(),
          json_encode($data),
        ),
      );
      ...
      if (is_null($current_selection_plan)) {
        throw new ValidationException(
          trans("validation_errors.PresentationService.submitPresentation.NotValidSelectionPlan"),
        );
      }
      ...
      if (!$current_selection_plan->isAllowNewPresentations()) {
        throw new ValidationException(
          sprintf(
            "Selection Plan %s does not allow new submissions",
      ...
      if (is_null($current_speaker)) {
        throw new ValidationException(
          trans("validation_errors.PresentationService.submitPresentation.NotValidSpeaker"),
        );
      }
      ...
      foreach (
        $current_speaker->getPresentationsBySelectionPlanAndRole(
          $current_selection_plan,
          PresentationSpeaker::ROLE_CREATOR,
        )
        as $p
      ) {
        if (isset($presentations[$p->getId()])) {
          continue;
        }
      ...
      foreach (
        $current_speaker->getPresentationsBySelectionPlanAndRole(
          $current_selection_plan,
          PresentationSpeaker::ROLE_MODERATOR,
        )
        as $p
      ) {
        if (isset($presentations[$p->getId()])) {
      ...
      foreach (
        $current_speaker->getPresentationsBySelectionPlanAndRole(
          $current_selection_plan,
          PresentationSpeaker::ROLE_SPEAKER,
        )
        as $p
      ) {
        if (isset($presentations[$p->getId()])) {
      ...
      if ($count >= $limit) {
        throw new ValidationException(
          trans("validation_errors.PresentationService.submitPresentation.limitReached", [
            "limit" => $limit,
          ]),
        );
      }
   ...
   * @throws \Exception
   */
  private function saveOrUpdatePresentation(
    Summit $summit,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_null($event_type)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.PresentationService.saveOrUpdatePresentation.eventTypeNotFound", [
      ...
      if (is_null($track)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.PresentationService.saveOrUpdatePresentation.trackNotFound", [
  ...
  public function updatePresentationSubmission(Summit $summit, $presentation_id, array $data) {
    return $this->tx_service->transaction(function () use ($summit, $presentation_id, $data) {
      Log::debug(
        sprintf(
          "PresentationService::updatePresentationSubmission presentation %s payload %s",
          $presentation_id,
          json_encode($data),
        ),
      );
      ...
      $current_selection_plan->checkPresentationAllowedQuestions($data);
      $current_selection_plan->checkPresentationAllowedEdtiableQuestions(
        $data,
  ...
  public function deletePresentation(Summit $summit, $presentation_id) {
    return $this->tx_service->transaction(function () use ($summit, $presentation_id) {
      ...
      if (is_null($current_speaker)) {
        throw new EntityNotFoundException(
          sprintf("member %s does not has a speaker profile", $member->getId()),
        );
      }
      ...
      if (!$presentation->canEdit($current_speaker)) {
        throw new ValidationException(
          sprintf("member %s can not edit presentation %s", $member->getId(), $presentation_id),
        );
      }
  ...
  public function completePresentationSubmission(Summit $summit, $presentation_id) {
    return $this->tx_service->transaction(function () use ($summit, $presentation_id) {
      ...
      if (!$presentation->fulfilMediaUploadsConditions()) {
        throw new ValidationException(
          sprintf(
            "Presentation %s is not allowed to mark as completed because does not fulfil media uploads conditions.",
            $presentation_id,
          ),
        );
      }
      ...
      if (!$presentation->fulfilSpeakersConditions()) {
        throw new ValidationException(
          sprintf(
            "Presentation %s is not allowed to mark as completed because does not fulfil speakers conditions.",
            $presentation_id,
          ),
        );
      }
    ...
    $slide = $this->tx_service->transaction(function () use (
      ...
      if ($hasFile) {
        if (!in_array($fileInfo->getFileExt(), $allowed_extensions)) {
          throw new ValidationException(
            sprintf(
              "file does not has a valid extension '(%s)'.",
        ...
        $slideFile = $this->file_uploader->build(
          $file,
          sprintf(
            "summits/%s/presentations/%s/slides",
            $presentation->getSummitId(),
            $presentation_id,
          ),
          false,
        );
  ...
  public function addLinkTo($presentation_id, array $link_data) {
    $link = $this->tx_service->transaction(function () use ($presentation_id, $link_data) {
      $presentation = $this->presentation_repository->getById($presentation_id);
  ...
  public function addMediaUploadTo(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      Log::debug(
        sprintf(
          "PresentationService::addMediaUploadTo summit %s presentation %s",
          $summit->getId(),
          $presentation_id,
        ),
      );
      ...
      if (is_null($mediaUploadType)) {
        throw new EntityNotFoundException(
          sprintf("Media Upload Type %s not found.", $media_upload_type_id),
        );
      }
      ...
      if (!$mediaUploadType->isPresentationTypeAllowed($presentation->getType())) {
        throw new ValidationException(
          sprintf(
            "Presentation Type %s is not allowed on Media Upload %s",
            $presentation->getTypeId(),
      ...
      if (
        $maxUploadsQty != 0 &&
        $presentation->getMediaUploadsCountByType($mediaUploadType) == $maxUploadsQty
      ) {
        throw new ValidationException(
          sprintf(
            "Presentation %s has reached the maximum media uploads qty allowed for the type %s.",
            $presentation_id,
            $mediaUploadType->getName(),
          ),
        );
      }
      ...
      if ($mediaUploadType->getMaxSize() < $fileInfo->getSize()) {
        throw new ValidationException(
          sprintf(
            "Max Size is %s MB (%s).",
            $mediaUploadType->getMaxSizeMB(),
            $fileInfo->getSize() / 1024,
          ),
        );
      }
      ...
      if (!$mediaUploadType->isValidExtension($fileInfo->getFileExt())) {
        throw new ValidationException(
          sprintf(
            "File Extension %s is not valid (%s).",
            $fileInfo->getFileExt(),
            $mediaUploadType->getValidExtensions(),
          ),
        );
      }
      ...
      if (!$presentation->isCompleted()) {
        Log::debug(
          sprintf(
            "PresentationService::addMediaUploadTo presentation %s is not complete",
            $presentation_id,
          ),
        );
        ...
        if ($type instanceof PresentationType) {
          ...
          Log::debug(
            "PresentationService::addMediaUploadTo presentation {$presentation_id} got summitMediaUploadCount {$summitMediaUploadCount}",
          );

          if ($summitMediaUploadCount == 0) {
            Log::debug(
              "PresentationService::addMediaUploadTo presentation {$presentation_id} marking as PHASE_UPLOADS (no mandatories uploads)",
            );
            ...
            foreach (
              $presentationMandatoryUploadsCountByType
              as $presentationMediaUploadTypeId => $uploadsCount
            ) {
              if (
                array_key_exists(
                  $presentationMediaUploadTypeId,
                  $summitMandatoryMediaUploadTypes,
                ) &&
                $uploadsCount <
                  $summitMandatoryMediaUploadTypes[
                    $presentationMediaUploadTypeId
            ...
            if ($mandatoryIsCompleted) {
              Log::debug(
                "PresentationService::addMediaUploadTo presentation {$presentation_id} marking as PHASE_UPLOADS (mandatories completed)",
              );
              $presentation->setProgress(Presentation::PHASE_UPLOADS);
            }
          }
  ...
  public function updateMediaUploadFrom(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      Log::debug(
        sprintf(
          "PresentationService::updateMediaUploadFrom summit %s presentation %s",
          $summit->getId(),
          $presentation_id,
        ),
      );
      ...
      if (!$canEdit) {
        throw new ValidationException(
          sprintf("Media Upload Type %s is not editable.", $mediaUploadType->getName()),
        );
      }
  ...
  public function deleteMediaUpload(
    ...
    $this->tx_service->transaction(function () use (
      ...
      Log::debug(
        sprintf(
          "PresentationService::deleteMediaUpload summit %s presentation %s",
          $summit->getId(),
          $presentation_id,
        ),
      );
  ...
  public function castAttendeeVote(
    ...
    return $this->tx_service->transaction(function () use ($summit, $member, $presentation_id) {
      Log::debug(
        sprintf(
          "PresentationService::castAttendeeVote summit %s member %s presentation %s",
          $summit->getId(),
          $member->getId(),
          $presentation_id,
        ),
      ...
      $presentation = $this->presentation_repository->getById($presentation_id);
      if (
        is_null($presentation) ||
        !$presentation instanceof Presentation ||
        !$presentation->hasAccess($member)
      ) {
        throw new EntityNotFoundException("Presentation not found.");
      }
      ...
      if (is_null($attendee)) {
        throw new ValidationException(
          sprintf("Current Member is not an attendee at Summit %s.", $summit->getId()),
        );
      }
      ...
      foreach ($currentTrack->getGroups() as $currentTrackGroup) {
        Log::debug(
          sprintf(
            "PresentationService::castAttendeeVote processing track group %s",
        ...
        if (!$currentTrackGroup->isAttendeeVotingPeriodOpen()) {
          throw new ValidationException(
            sprintf(
              "Attendee Voting Period for track group %s is closed.",
        ...
        if (!$currentTrackGroup->canEmitAttendeeVote($attendee)) {
          throw new ValidationException(
            sprintf(
              "You Reached the Max. allowed votes for Track Group %s [%s]",
              $currentTrackGroup->getName(),
  ...
  public function addTrackChairScore(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!$selectionPlan->isSelectionOpen()) {
        throw new ValidationException(
          sprintf("Selection Period is over for Selection Plan %s", $selection_plan_id),
        );
      }
      ...
      if (is_null($summit_track_chair)) {
        throw new ValidationException(
          sprintf("Can't find a track chair for current member at Summit %s.", $summit->getId()),
        );
      }
      ...
      $track_chair_score = $summit_track_chair->getScoreByRatingTypeAndPresentation(
        $score_type->getType(),
        $presentation,
      );
  ...
  public function processMediaUpload(
    ...
    $this->tx_service->transaction(function () use (
      ...
      Log::debug(
        sprintf(
          "PresentationService::processMediaUpload summit id %s media upload type id %s public path %s private path %s file name %s path %s",
          $summit_id,
          $media_upload_type_id,
          $public_path,
          $private_path,
      ...
      if (!is_null($strategy)) {
        Log::debug(
          sprintf(
            "PresentationService::processMediaUpload saving file %s to private storage",
            $file_name,
          ),
        );
        $strategy->saveFromPath($localPath, $private_path, $file_name);
      }
      ...
      if (!is_null($strategy)) {
        Log::debug(
          sprintf(
            "PresentationService::processMediaUpload saving file %s to public storage",
            $file_name,
          ),
        );
        $options = $mediaUploadType->isUseTemporaryLinksOnPublicStorage() ? [] : "public";
        $strategy->saveFromPath($localPath, $public_path, $file_name, $options);
      }
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (isset($data["order"])) {
        ...
        if ($current_order != $new_order) {
          $presentation->updateSpeakerOrder($speaker, $new_order);
        }
      }
