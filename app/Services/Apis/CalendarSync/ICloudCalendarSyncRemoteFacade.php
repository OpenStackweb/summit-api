 ...
 */
final class ICloudCalendarSyncRemoteFacade extends AbstractCalendarSyncRemoteFacade {
  ...
  private function getEventInfo($calendar_url, $event_url) {
    ...
    try {
      ...
      $events_response = $this->client->getEventsBy($calendar_url, [$relative_resource_url]);
  ...
  public function addEvent(MemberEventScheduleSummitActionSyncWorkRequest $request) {
    try {
      ...
      $res = $this->client->createEvent($calendar_url, $vo);

      $etag = $res->getETag();
      $vcard = "";

      if (empty($etag)) {
        [$etag, $vcard] = $this->getEventInfo($calendar_url, $res->getResourceUrl());
      }
  ...
  public function deleteEvent(
    ...
    try {
      if (empty($schedule_sync_info->getEtag())) {
        return false;
      }
  ...
  public function createCalendar(
    ...
    try {
      ...
      $this->sync_calendar_info->setUserPrincipalUrl(
        str_replace(Config::get("apple_api.base_caldav_server"), $real_server, $user_ppal_url),
      );
