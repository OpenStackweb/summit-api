...
class RegionalSupportSerializer extends SilverStripeSerializer {
  protected static $allowed_relations = ["supported_channel_types"];
  ...
  public function serialize(
    ...
    if (!$regional_support instanceof RegionalSupport) {
      return [];
    }
    ...
    if (in_array("supported_channel_types", $relations)) {
      ...
      foreach ($regional_support->getSupportedChannelTypes() as $channel_type) {
        $res[] = SerializerRegistry::getInstance()->getSerializer($channel_type)->serialize();
      }
