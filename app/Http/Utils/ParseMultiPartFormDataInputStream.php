...
final class ParseMultiPartFormDataInputStream {
  ...
  private function blocks($array) {
    ...
    foreach ($array as $key => $value) {
      ...
      foreach ($block["parameters"] as $key => $val) {
        if (
          key_exists($key, $results["parameters"]) &&
          is_array($results["parameters"][$key]) &&
          is_array($val)
        ) {
  ...
  private function file_stream($data) {
    ...
    if ($idx === false) {
      Log::warning(
        "ParseMultiPartFormDataInputStream.file_stream(): Could not locate header separator in data:",
      );
      Log::warning($data);
    } else {
  ...
  function parse_parameter(&$params, $parameter, $value) {
    if (strpos($parameter, "[") !== false) {
      ...
      if (preg_match('/^([^[]*)\[([^]]*)\](.*)$/', $parameter, $match)) {
        ...
        Log::warning(
          "ParseMultiPartFormDataInputStream.parse_parameter() Parameter name regex failed: '" .
            $parameter .
            "'",
        );
      }
