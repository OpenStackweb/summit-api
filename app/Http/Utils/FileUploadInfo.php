...
final class FileUploadInfo {
   ...
   * @param $size
   */
  private function __construct($file, $fileName, $fileExt, $filePath, $size) {
    $this->file = $file;
  ...
  public static function build(LaravelRequest $request, array $payload): ?FileUploadInfo {
    ...
    if (is_null($file) && isset($payload["filepath"]) && !empty($payload["filepath"])) {
      Log::debug(
        sprintf(
          "FileUploadInfo::build build file is present on as %s storage (%s)",
      ...
      if (!$disk->exists($payload["filepath"])) {
        throw new ValidationException(
          sprintf(
            "file provide on filepath %s does not exists on %s storage.",
            self::getStorageDriver(),
            $payload["filepath"],
          ),
        );
      ...
      Log::debug(
        sprintf(
          "FileUploadInfo::build build file %s storage (%s) size %s",
          self::getStorageDriver(),
          $payload["filepath"],
          $size,
        ),
      ...
      $fileExt = pathinfo($fileName, PATHINFO_EXTENSION);
      $file = self::isLocal()
        ? new UploadedFile($disk->path($payload["filepath"]), $fileName)
        : null;
    }
