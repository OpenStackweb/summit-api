...
SQL;
        ...
        string $groupBy, PagingInfo $pagingInfo, ?DateTime $startDate  = null, ?DateTime $endDate = null): PagingResponse
    {
        ...
        try {
            ...
            $sql = <<<SQL
SELECT COUNT(SummitAttendee.ID) as qty, DATE_FORMAT(SummitAttendee.SummitHallCheckedInDate, :check_in_date_format) AS label 
FROM SummitAttendee
WHERE
...
SQL;
            ...
            $stmt->execute(['summit_id' => $this->id, 'check_in_date_format' => $date_format]);
            $res = $stmt->fetchAll(\PDO::FETCH_COLUMN);

            $total = count($res) > 0 ? intval($res[0]) : 0;

            $sql .= " LIMIT {$limit_from},{$per_page}";
            $stmt = $this->prepareRawSQL($sql);
            $stmt->execute(['summit_id' => $this->id, 'check_in_date_format' => $date_format]);
            $res = $stmt->fetchAll();
            $res = count($res) > 0 ? $res : [];
