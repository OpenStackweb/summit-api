 ...
 */
final class DoctrineSummitEventRepository extends SilverStripeDoctrineRepository implements
  ISummitEventRepository {
  private static $forbidden_classes = [SummitGroupEvent::ClassName];
  ...
  public function getPublishedOnSameTimeFrame(IPublishableEvent $event): array {
    ...
    foreach (self::$forbidden_classes as $forbidden_class) {
      $query = $query->andWhere("not e INSTANCE OF :forbidden_class" . $idx);
  ...
  public function getAllByPage(
    ...
    if (!is_null($filter)) {
      $filter->apply2Query(
        $query,
        $this->getCustomFilterMappings($current_member_id, $current_track_id),
      );
      if ($filter->hasFilter("actions")) {
        ...
        $query = $query
          ->innerJoin("p.selection_plan", "sp_i")
  ...
  protected function getCustomFilterMappings(int $current_member_id, int $current_track_id) {
    return [
      ...
      "submission_status" => new DoctrineSwitchFilterMapping([
        ...
        "NonReceived" => new DoctrineCaseFilterMapping("NonReceived", "p.status is null"),
      ]),
      ...
      "track_group_id" => new DoctrineJoinFilterMapping("c.groups", "cg", "cg.id :operator :value"),
      "selection_plan_id" => new DoctrineFilterMapping("(selp.id :operator :value)"),
      ...
      "sponsor_id" => new DoctrineFilterMapping("(sprs.id :operator :value)"),
      "sponsor" => new DoctrineFilterMapping("(sprs.name :operator :value)"),
      ...
      "track_chairs_status" => new DoctrineSwitchFilterMapping([
        ...
        "selected" => new DoctrineCaseFilterMapping(
          "selected",
          "sspl.list_type = 'Individual' and sspl.list_class = 'Session' and ssp.collection = 'selected' and ssp_member.id = " .
            $current_member_id,
        ),
        "maybe" => new DoctrineCaseFilterMapping(
          "maybe",
          "sspl.list_type = 'Individual' and sspl.list_class = 'Session' and ssp.collection = 'maybe' and ssp_member.id = " .
            $current_member_id,
        ),
        "pass" => new DoctrineCaseFilterMapping(
          "selected",
          "sspl.list_type = 'Individual' and sspl.list_class = 'Session' and ssp.collection = 'pass' and ssp_member.id = " .
            $current_member_id,
        ),
      ]),
      "viewed_status" => new DoctrineSwitchFilterMapping([
        "seen" => new DoctrineCaseFilterMapping(
          "seen",
          sprintf(
            "exists (select vw1 from models\summit\PresentationTrackChairView vw1 inner join vw1.presentation p1 join vw1.viewer v1 where p1.id = p.id and v1.id = %s)",
            $current_member_id,
          ),
        ),
        "unseen" => new DoctrineCaseFilterMapping(
          "unseen",
          sprintf(
            "not exists (select vw1 from models\summit\PresentationTrackChairView vw1 inner join vw1.presentation p1 join vw1.viewer v1 where p1.id = p.id and v1.id = %s)",
            $current_member_id,
          ),
        ),
      ...
      "presentation_attendee_vote_date" =>
        "av.created:datetime_epoch|" . SilverstripeBaseModel::DefaultTimeZone,
      "votes_count" => new DoctrineHavingFilterMapping(
        "",
        "av.presentation",
        "count(av.id) :operator :value",
      ),
