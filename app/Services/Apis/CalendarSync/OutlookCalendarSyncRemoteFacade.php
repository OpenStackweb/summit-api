 ...
 */
final class OutlookCalendarSyncRemoteFacade extends AbstractCalendarSyncRemoteFacade {
  /**
  ...
  public function createCalendar(
    ...
    try {
      $res = $this->client->createCalendar(new CalendarVO($request->getCalendarName()));
    ...
    try {
      ...
      do {
        ...
        if ($res instanceof ErrorResponse) {
          log::warning(
            sprintf(
              "OutlookCalendarSyncRemoteFacade::deleteCalendar: error deleting calendar id %s",
              $calendar_id,
            ),
          );
          ...
          log::info(
            sprintf(
              "OutlookCalendarSyncRemoteFacade::deleteCalendar: renaming calendar id %s",
              $calendar_id,
            ),
          );
          $this->client->updateCalendar($calendar_id, new CalendarVO(md5(uniqid(mt_rand(), true))));
          $exp = $this->getSleepInterval() * (pow(2, $counter) - 1);
          log::info(
            sprintf(
              "OutlookCalendarSyncRemoteFacade::deleteCalendar: retrying calendar id %s on %s ms",
              $calendar_id,
              $exp,
            ),
          );
        ...
        if ($counter == self::MaxRetriesAttempt) {
          log::warning(
            sprintf(
              "OutlookCalendarSyncRemoteFacade::deleteCalendar: error deleting calendar id %s (MaxRetriesAttempt reached !)",
              $calendar_id,
            ),
          );
          break;
