 ...
 */
final class OAuth2SummitPromoCodesApiTest extends ProtectedApiTestCase {
  ...
  private function getPromoCodeBySummitAndFilter(
    int $summit_id,
    string $filter,
    string $expand = "",
  ) {
      ...
      "filter" => [
        ...
        implode("||", [
          self::$default_sponsors_promo_codes[
            count(self::$default_sponsors_promo_codes) - 1
          ]->getId(),
          self::$default_sponsors_promo_codes[
            count(self::$default_sponsors_promo_codes) - 3
          ]->getId(),
        ]),
        "email_sent==0",
      ],
