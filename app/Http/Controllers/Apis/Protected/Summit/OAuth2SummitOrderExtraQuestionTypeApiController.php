 ...
 */
final class OAuth2SummitOrderExtraQuestionTypeApiController extends OAuth2ProtectedController {
  ...
  public function getMetadata($summit_id) {
    ...
    return $this->ok($this->repository->getQuestionsMetadata($summit));
  }
  ...
  protected function getFilterValidatorRules(): array {
    return [
      ...
      "class" =>
        "sometimes|required|string|in:" .
        implode(",", ExtraQuestionTypeConstants::AllowedQuestionClass),
  ...
  protected function addChild(Summit $summit, array $payload): IEntity {
    return $this->service->addOrderExtraQuestion(
      $summit,
  ...
  public function updateQuestionValue($summit_id, $question_id, $value_id) {
    return $this->processRequest(function () use ($summit_id, $question_id, $value_id) {
      ...
      $payload = $this->getJsonPayload(
        ExtraQuestionTypeValueValidationRulesFactory::buildForUpdate(),
      );

      $value = $this->service->updateOrderExtraQuestionValue(
        $summit,
        $question_id,
        $value_id,
        $payload,
  ...
  public function getSubQuestionRules($summit_id, $question_id) {
    return $this->processRequest(function () use ($summit_id, $question_id) {
      ...
      $response = new PagingResponse(count($rules), count($rules), 1, 1, $rules);
  ...
  public function addSubQuestionRule($summit_id, $question_id) {
    return $this->processRequest(function () use ($summit_id, $question_id) {
      ...
      $sub_question_rule = $this->service->addSubQuestionRule(
        $summit,
        intval($question_id),
        $payload,
      );
  ...
  public function updateSubQuestionRule($summit_id, $question_id, $rule_id) {
    return $this->processRequest(function () use ($summit_id, $question_id, $rule_id) {
      ...
      $sub_question_rule = $this->service->updateSubQuestionRule(
        $summit,
        intval($question_id),
        intval($rule_id),
        $payload,
      ...
      function ($filter) use ($summit) {
        if ($filter instanceof Filter) {
          ...
          $filter->addFilterCondition(
            FilterElement::makeEqual("class", ExtraQuestionTypeConstants::QuestionClassMain),
          );
          $filter->addFilterCondition(
