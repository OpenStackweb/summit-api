...
final class OAuth2SummitTrackTagGroupsApiController extends OAuth2ProtectedController {
  ...
  public function getAllowedTags($summit_id) {
    ...
    try {
      ...
      if (Request::has("order")) {
        $order = OrderParser::parse(Request::input("order"), ["tag", "id"]);
      }
      ...
      return $this->ok($data->toArray(Request::input("expand", ""), $fields, $relations));
    } catch (EntityNotFoundException $ex1) {
      Log::warning($ex1);
      return $this->error404();
    } catch (ValidationException $ex2) {
  ...
  public function updateTrackTagGroup($summit_id, $track_tag_group_id) {
    try {
      ...
      $track_tag_group = $this->track_tag_group_service->updateTrackTagGroup(
        $summit,
        $track_tag_group_id,
        $data->all(),
      );
  ...
  public function seedTagTrackGroupOnTrack($summit_id, $track_tag_group_id, $track_id) {
    try {
      ...
      $this->track_tag_group_service->seedTagTrackGroupTagsOnTrack(
        $summit,
        $track_tag_group_id,
        $track_id,
      );
