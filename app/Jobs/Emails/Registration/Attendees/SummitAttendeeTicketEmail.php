...
class SummitAttendeeTicketEmail extends AbstractSummitAttendeeTicketEmail {
  ...
  public function __construct(
    ...
    $payload[IMailTemplatesConstants::promo_code] = $ticket->hasPromoCode()
      ? $ticket->getPromoCode()->getCode()
      : "";
    ...
    if ($summit->isRegistrationSendQrAsImageAttachmentOnTicketEmail()) {
      Log::debug(
        sprintf(
          "SummitAttendeeTicketEmail::__construct adding QR as attachment for summit %s",
          $summit->getId(),
        ),
      );
      $attachments[] = [
        ...
        "content" => base64_encode(
          QrCode::format("png")->size(250, 250)->generate($ticket->getQRCode()),
        ),
    ...
    if ($summit->isRegistrationSendTicketAsPdfAttachmentOnTicketEmail()) {
      Log::debug(
        sprintf(
          "SummitAttendeeTicketEmail::__construct adding Ticket PDF as attachment for summit %s",
          $summit->getId(),
        ),
      );
    ...
    Log::debug(
      sprintf(
        "SummitAttendeeTicketEmail::__construct payload %s template %s",
        json_encode($payload),
        $template_identifier,
      ),
    );
