 ...
 */
final class DoctrineSummitAttendeeTicketRepository extends SilverStripeDoctrineRepository implements
  ISummitAttendeeTicketRepository {
  /**
   * @return string
   */
  ...
  protected function applyExtraSelects(
    ...
    $query = $query->addSelect(
      "COALESCE(SUM(rr.refunded_amount),0) AS HIDDEN HIDDEN_REFUNDED_AMOUNT",
    );
    $query = $query->addSelect(
      "( (e.raw_cost - e.discount) - COALESCE(SUM(rr.refunded_amount),0) ) AS HIDDEN HIDDEN_FINAL_AMOUNT_ADJUSTED",
    );
  ...
  protected function getFilterMappings() {
    return [
      ...
      "has_owner" => new DoctrineSwitchFilterMapping([
        "1" => new DoctrineCaseFilterMapping("true", "a is not null"),
        "0" => new DoctrineCaseFilterMapping("false", "a is null"),
      ]),
      "has_order_owner" => new DoctrineSwitchFilterMapping([
        "1" => new DoctrineCaseFilterMapping("true", "ord_m is not null"),
        "0" => new DoctrineCaseFilterMapping("false", "ord_m is null"),
      ]),
      "has_badge" => new DoctrineSwitchFilterMapping([
        "1" => new DoctrineCaseFilterMapping("true", "b is not null"),
        "0" => new DoctrineCaseFilterMapping("false", "b is null"),
      ]),
      "owner_status" => "a.status",
      "final_amount" => "(e.raw_cost - e.discount) :operator :value",
      "is_printable" => new DoctrineSwitchFilterMapping([
        "1" => new DoctrineCaseFilterMapping(
          "true",
      ...
      "has_badge_prints" => new DoctrineSwitchFilterMapping([
        "1" => new DoctrineCaseFilterMapping("true", "SIZE(prt) > 0"),
        "0" => new DoctrineCaseFilterMapping("false", "SIZE(prt) = 0"),
      ]),
  ...
  protected function getOrderMappings() {
    return [
      ...
      "owner_email" =>
        "COALESCE(LOWER(m.email), LOWER(m.second_email), LOWER(m.third_email), LOWER(a.email))",
      "owner_company" => "COALESCE(a.company_name, a_c.name)",
      "promo_code" => "pc.code",
   ...
   */
  public function getByExternalOrderIdAndExternalAttendeeId(
    $external_order_id,
   ...
   * @return SummitAttendeeTicket|null
   */
  public function getByExternalAttendeeIdExclusiveLock(
    Summit $summit,
    string $external_attendee_id,
  ): ?SummitAttendeeTicket {
    $query = $this->getEntityManager()
      ->createQueryBuilder()
      ->select("e")
  ...
  protected function applyExtraFilters(QueryBuilder $query) {
    $query = $query
      ...
      ->setParameter("cancelled", IOrderConstants::CancelledStatus);
    return $query;
  }
}
