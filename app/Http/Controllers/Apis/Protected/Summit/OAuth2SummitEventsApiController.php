...
final class OAuth2SummitEventsApiController extends OAuth2ProtectedController {
  ...
  private function getSerializerType(): string {
    ...
    Log::debug(
      sprintf(
        "OAuth2SummitEventsApiController::getSerializerType client id %s app_type %s scope %s has current user %b %s %s ",
        $clientId,
        $application_type,
        implode(" ", $scope),
    ...
    if (
      ...
      (!is_null($current_user) && ($current_user->isAdmin() || $current_user->isSummitAdmin()))
    ) {
      Log::debug(
        sprintf(
          "OAuth2SummitEventsApiController::getSerializerType app_type %s has current user %b PRIVATE",
          $application_type,
          !is_null($current_user),
        ),
      );
    ...
    Log::debug(
      sprintf(
        "OAuth2SummitEventsApiController::getSerializerType app_type %s has current user %b PUBLIC",
        $application_type,
        !is_null($current_user),
      ),
    );
  ...
  public function getEvents($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      $strategy = new RetrieveAllSummitEventsBySummitStrategy(
        $this->repository,
    ...
    return $this->processRequest(function () use ($summit_id) {
      ...
      return $this->export("csv", $filename, $list["data"], [
        ...
        "start_date" => new EpochCellFormatter(
          EpochCellFormatter::DefaultFormat,
          $summit->getTimeZone(),
        ),
  ...
  public function getScheduledEvents($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      ...
      $strategy = new RetrievePublishedSummitEventsBySummitStrategy(
        $this->repository,
        $this->event_repository,
        $this->resource_server_context,
      );
  ...
  public function getAllPresentations($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      $strategy = new RetrieveAllSummitPresentationsStrategy(
        $this->repository,
        $this->event_repository,
        $this->resource_server_context,
      );
  ...
  public function getVoteablePresentation($summit_id, $presentation_id) {
    return $this->processRequest(function () use ($summit_id, $presentation_id) {
      ...
      if (!$event->hasAccess($current_user)) {
        Log::debug(
          sprintf(
            "OAuth2SummitEventsApiController::getVoteablePresentation summit id %s presentation id %s user has no access.",
            $summit_id,
            $presentation_id,
          ),
        );
      ...
      return SerializerRegistry::getInstance()
        ->getSerializer($event, SerializerRegistry::SerializerType_Private)
        ->serialize(
          SerializerUtils::getExpand(),
          SerializerUtils::getFields(),
          SerializerUtils::getRelations(),
  ...
  private function _getSummitEvent($summit_id, $event_id, $published = true) {
    ...
    $event = $published
      ? $summit->getScheduleEvent(intval($event_id))
      : $summit->getEvent(intval($event_id));
  ...
  public function updateEvent($summit_id, $event_id) {
    return $this->processRequest(function () use ($summit_id, $event_id) {
      Log::debug(
        sprintf(
          "OAuth2SummitEventsApiController::updateEvent summit id %s event id %s",
          $summit_id,
          $event_id,
        ),
      );
      ...
      $isTrackChair =
        $summit->isTrackChairAdmin($current_member) ||
        $summit->isTrackChair($current_member, $event->getCategory());
      ...
      if (is_null($rules)) {
        $rules = $isTrackChair
          ? SummitEventValidationRulesFactory::buildForTrackChair($payload, true)
          : null;
      }
      ...
      if ($isAdmin) {
        Log::debug(
          sprintf(
            "OAuth2SummitEventsApiController::updateEvent summit id %s event id %s updating event",
            $summit_id,
            $event_id,
          ),
        );
        ...
        Log::debug(
          sprintf(
            "OAuth2SummitEventsApiController::updateEvent summit id %s event id %s updating duration",
            $summit_id,
            $event_id,
          ),
        );
  ...
  public function getEventFeedback($summit_id, $event_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["created", "owner_id", "owner_full_name", "rate", "id"];
      },
      function ($filter) {
        return $filter;
      },
      ...
      function () {
        return new Order([OrderElement::buildDescFor("created")]);
      },
      null,
      function ($page, $per_page, $filter, $order, $applyExtraFilters) use ($event) {
        return $this->event_feedback_repository->getByEvent(
          $event,
          new PagingInfo($page, $per_page),
          call_user_func($applyExtraFilters, $filter),
          $order,
  ...
  public function getMyEventFeedback($summit_id, $member_id, $event_id) {
    return $this->processRequest(function () use ($summit_id, $event_id) {
      ...
      $feedback = $this->service->getMyEventFeedback($current_member, $summit, intval($event_id));
  ...
  public function deleteMyEventFeedback($summit_id, $member_id, $event_id) {
    return $this->processRequest(function () use ($summit_id, $event_id) {
      ...
      $this->service->deleteMyEventFeedback($current_member, $summit, intval($event_id));
  ...
  public function getUnpublishedEvents($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      $strategy = new RetrieveAllUnPublishedSummitEventsStrategy(
        $this->repository,
        $this->event_repository,
        $this->resource_server_context,
      );
  ...
  public function getScheduleEmptySpots($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      ...
      $response = new PagingResponse(count($gaps), count($gaps), 1, 1, $gaps);
  ...
  public function getScheduledEventJWT($summit_id, $event_id) {
    return $this->processRequest(function () use ($summit_id, $event_id) {
      Log::debug(
        sprintf(
          "OAuth2SummitEventsApiController::getScheduledEventJWT summit id %s event id %s",
          $summit_id,
          $event_id,
        ),
      );
      ...
      return SerializerRegistry::getInstance()
        ->getSerializer($event, IPresentationSerializerTypes::SecureStream)
        ->serialize(
          SerializerUtils::getExpand(),
          SerializerUtils::getFields(),
          SerializerUtils::getRelations(),
