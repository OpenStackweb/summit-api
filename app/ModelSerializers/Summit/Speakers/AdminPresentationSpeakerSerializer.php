    ...
    if (!is_null($summit)) {
      if (in_array("summit_assistance", $relations)) {
        ...
        if ($summit_assistance) {
          $values["summit_assistance"] = SerializerRegistry::getInstance()
            ->getSerializer($summit_assistance)
            ->serialize(
      ...
      if (in_array("registration_code", $relations)) {
        ...
        if (!is_null($registration_code)) {
          $values["registration_code"] = SerializerRegistry::getInstance()
            ->getSerializer($registration_code)
            ->serialize(
      ...
      if (in_array("all_moderated_presentations", $relations)) {
        $values["all_moderated_presentations"] = $speaker->getModeratedPresentationIds(
          $summit->getId(),
          false,
        );
      }
      ...
      if (in_array("summit_assistances", $relations)) {
        ...
        foreach ($speaker->getSummitAssistances() as $assistance) {
          $summit_assistances[] = SerializerRegistry::getInstance()
            ->getSerializer($assistance)
            ->serialize(
      ...
      if (in_array("registration_codes", $relations)) {
        ...
        foreach ($speaker->getAssignedPromoCodes() as $promo_code) {
          $registration_codes[] = SerializerRegistry::getInstance()
