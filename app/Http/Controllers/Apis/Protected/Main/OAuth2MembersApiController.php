...
final class OAuth2MembersApiController extends OAuth2ProtectedController {
        ...
        if (
          ...
          (!is_null($current_member) &&
            ($current_member->isAdmin() ||
              $current_member->isSummitAdmin() ||
              $current_member->isTrackChairAdmin()))
        ) {
  ...
  public function updateMyMember() {
    return $this->processRequest(function () {
      ...
      return $this->updated(
        SerializerRegistry::getInstance()
          ->getSerializer($me, SerializerRegistry::SerializerType_Private)
          ->serialize(
            SerializerUtils::getExpand(),
            SerializerUtils::getFields(),
            SerializerUtils::getRelations(),
  ...
  public function updateAffiliation($member_id, $affiliation_id) {
    return $this->processRequest(function () use ($member_id, $affiliation_id) {
      ...
      $affiliation = $this->member_service->updateAffiliation(
        $member,
        intval($affiliation_id),
        $payload,
      );
