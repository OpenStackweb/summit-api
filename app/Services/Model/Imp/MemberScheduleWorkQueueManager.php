 ...
 */
final class MemberScheduleWorkQueueManager implements ICalendarSyncWorkRequestQueueManager {
  /**
   * @var array
   */
  ...
  private function generateKey($type, $calendar_id, $event_id = null) {
    $sub_type = is_null($event_id)
      ? MemberCalendarScheduleSummitActionSyncWorkRequest::SubType
      : MemberEventScheduleSummitActionSyncWorkRequest::SubType;
  ...
  public function getSummitEventRequestFor($calendar_id, $event_id, $type = null) {
    ...
    foreach ($types as $t) {
      ...
      if (
        isset($this->registered_requests[$key]) &&
        !isset($this->registered_requests_2_delete[$key])
      ) {
  ...
  public function registerRequestForDelete(AbstractCalendarSyncWorkRequest $request) {
    $key = $this->getKey($request);
    if (isset($this->registered_requests_2_delete[$key])) {
      return false;
    }
   ...
   * @return bool
   */
  public function unRegisterRequestForDelete(
    MemberScheduleSummitActionSyncWorkRequest $request,
    $type = null,
  ) {
