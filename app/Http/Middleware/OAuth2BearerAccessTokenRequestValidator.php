...
class OAuth2BearerAccessTokenRequestValidator {
  ...
  public function handle($request, Closure $next) {
    ...
    try {
      ...
      $auth_header = isset($this->headers["authorization"])
        ? $this->headers["authorization"]
        : null;
      if (!is_null($auth_header) && !empty($auth_header)) {
        $access_token_value = BearerAccessTokenAuthorizationHeaderParser::getInstance()->parse(
          $auth_header,
        );
      } else {
      ...
      if (
        ...
        (is_null($origin) ||
          empty($origin) ||
          str_contains($token_info->getAllowedOrigins(), $origin) === false)
      ) {
        //check origins
        throw new OAuth2ResourceServerException(
          403,
          ...
          sprintf(
            "invalid origin %s - allowed ones (%s)",
            $origin,
            $token_info->getAllowedOrigins(),
          ),
        );
      }
      //check scopes
      ...
      if (!is_null($token_info->getUserId())) {
        Log::debug(
          sprintf(
            "OAuth2BearerAccessTokenRequestValidator::handle user id is not null (%s)",
