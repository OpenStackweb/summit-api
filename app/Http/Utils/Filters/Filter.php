...
final class Filter {
   ...
   * @param array $ops
   */
  public function __construct(array $filters = [], $originalExp = null, array $ops = []) {
  ...
  private static function _validate($field, $val, $rule, $messages) {
    $validation = Validator::make([$field => $val], [$field => $rule], $messages);
    if ($validation->fails()) {
  ...
  private function applyCondition(FilterElement $filter, string $mapping, int &$param_idx): string {
    ...
    Log::debug(
      sprintf(
        "Filter::applyCondition mapping %s op %s value %s",
        $mapping_parts[0],
        json_encode($op),
        json_encode($value),
      ),
    ...
    if (count($mapping_parts) > 1) {
      ...
      $value = $filter->getValue();
      Log::debug(
        sprintf("Filter::applyCondition converted converted value %s", json_encode($value)),
      );
    }
    ...
    if (is_array($value)) {
      ...
      foreach ($value as $idx => $val) {
        ...
        $inner_condition .= sprintf(
          "%s %s :%s %s ",
          self::cleanMapping($mapping_parts[0]),
          $cond,
          sprintf(self::ParamPrefix, $param_idx),
          $sameOp,
        );
  ...
  private function getMainOp(int $idx): string {
    ...
    if (count($this->ops) == 0) {
      return Filter::MainOperatorAnd;
    }
    if (count($this->ops) - 1 < $idx) {
      return Filter::MainOperatorAnd;
    }
    return $this->ops[$idx];
  }
  ...
  public function apply2Query(QueryBuilder $query, array $mappings) {
    ...
    foreach ($this->filters as $idx => $filter) {
      ...
      if ($filter instanceof FilterElement && isset($mappings[$filter->getField()])) {
        ...
        Log::debug(
          sprintf(
            "Filter::apply2Query single filter idx %s field %s mapping %s",
            $idx,
            $filter->getField(),
            json_encode($mapping),
          ),
        ...
        if ($mapping instanceof IQueryApplyable) {
          Log::debug(
            sprintf(
              "Filter::apply2Query single filter idx %s field %s mapping is IQueryApplyable",
              $idx,
              $filter->getField(),
            ),
          );
          ...
          Log::debug(
            sprintf(
              "Filter::apply2Query single filter idx %s field %s mapping is array",
              $idx,
              $filter->getField(),
            ),
          );
          ...
          Log::debug(
            sprintf(
              "Filter::apply2Query single filter idx %s field %s mapping is raw",
              $idx,
              $filter->getField(),
            ),
          );
        ...
        foreach ($filter as $e) {
          if ($e instanceof FilterElement && isset($mappings[$e->getField()])) {
            ...
            if ($mapping instanceof IQueryApplyable) {
              Log::debug(
                sprintf(
                  "Filter::apply2Query single filter idx %s field %s mapping is OR",
                  $idx,
                  $e->getField(),
                ),
              );
  ...
  public static function convertToDateTime($value, ?string $strTimeZone = null) {
    ...
    Log::debug(
      sprintf("Filter::convertToDateTime original date value %s", $datetime->format("Y-m-d H:i:s")),
    );
    if (!is_null($timezone)) {
      $datetime = $datetime->setTimezone($timezone);
    }
    Log::debug(
      sprintf("Filter::convertToDateTime final date %s", $datetime->format("Y-m-d H:i:s")),
    );
