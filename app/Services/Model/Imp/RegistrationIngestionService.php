 ...
 */
final class RegistrationIngestionService extends AbstractService implements
  IRegistrationIngestionService {
  /**
   * @var IMemberService
   */
    ...
    try {
      return $this->tx_service->transaction(function () use (
        ...
        if (is_null($ticket_type)) {
          ...
          Log::debug(
            sprintf(
              "RegistrationIngestionService::ingestSummit: ticket class %s does not exists",
              $ticket_class["id"],
            ),
          );
          $ticket_type = SummitTicketTypeFactory::build($summit, [
        ...
        $company = isset($external_attendee_profile["company"])
          ? trim($external_attendee_profile["company"])
          : "";
          ...
          try {
            ...
            if (!is_null($user)) {
              ...
              try {
                ...
                Log::warning($ex);
                $attendee_owner = $this->member_repository->getByExternalIdExclusiveLock(
                  intval($external_id),
                );
              }
            }
        ...
        if (is_null($attendee)) {
          Log::debug(
            sprintf(
              "RegistrationIngestionService::ingestSummit attendee %s does not exists",
              $attendee_email,
            ),
          );
        ...
        $order = $this->order_repository->getByExternalIdAndSummitLockExclusive(
          $summit,
          $external_order["id"],
        );
        if (is_null($order)) {
          ...
          Log::debug(
            sprintf(
              "RegistrationIngestionService::ingestSummit: order %s does not exists",
              $external_order["id"],
            ),
          );
            ...
            try {
              ...
              if (!is_null($user)) {
                ...
                try {
                  ...
                  // race condition lost, try to get it
                  Log::warning($ex);
                  $owner = $this->member_repository->getByExternalIdExclusiveLock(
                    intval($external_id),
                  );
                }
              }
        ...
        if (is_null($ticket)) {
          ...
          $ticket->setBoughtDate(
            new \DateTime($external_attendee["created"] ?? "now", new \DateTimeZone("UTC")),
          );
        ...
        if (!is_null($external_promo_code)) {
          Log::debug(
            sprintf(
              "RegistrationIngestionService::ingestSummit processing promo code %s",
              json_encode($external_promo_code),
            ),
          );
          // has promo code
          ...
          if (is_null($promo_code)) {
            ...
            $promo_code_params = [
              "class_name" =>
                $external_promo_code["promotion_type"] == "discount"
        ...
        foreach ($answers as $answerDTO) {
          ...
          if (is_null($question)) {
            Log::debug(
              sprintf(
                "RegistrationIngestionService::ingestSummit question not found ( external id %s )",
            ...
            Log::debug(
              sprintf(
                "RegistrationIngestionService::ingestSummit creating question ( external id %s )",
                $external_question_id,
              ),
            );
          ...
          Log::debug(
            sprintf("RegistrationIngestionService::ingestSummit added answer %s", $answer),
          );
        }
        ...
        if ($refunded) {
          try {
            if ($ticket->canRefund($ticket->getFinalAmount())) {
              $ticket->refund(null, $ticket->getFinalAmount(), null, null, false);
            }
          } catch (Exception $ex) {
            Log::warning($ex);
          }
        ...
        Log::debug(
          sprintf(
            "RegistrationIngestionService::ingestSummit processed attendee %s",
            $external_attendee["id"],
          ),
        );
  ...
  public function ingestSummit(Summit $summit): void {
    try {
      ...
      Log::debug(
        sprintf("RegistrationIngestionService::ingestSummit ingesting summit %s", $summit_id),
      );
      ...
      if (!$summit->hasDefaultBadgeType()) {
        throw new ValidationException(
          sprintf("summit %s has not default badge type set", $summit_id),
        );
      }
      ...
      do {
        Log::debug(
          sprintf(
            "RegistrationIngestionService::ingestSummit getting external attendees page %s",
            $page,
          ),
        );
        $response = $feed->getAttendees(
          $page,
          $summit->getExternalRegistrationFeedLastIngestDate(),
        );
        ...
        Log::debug(
          sprintf(
            "RegistrationIngestionService::ingestSummit response got %s records on page",
            $response->pageCount(),
          ),
        );
        Log::debug(
          sprintf(
            "RegistrationIngestionService::ingestSummit response %s",
            $response->__toString(),
          ),
        );
