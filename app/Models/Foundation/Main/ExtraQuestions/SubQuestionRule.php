    ...
    implements IOrderable
{
    ...
    {
        if(!in_array($visibility_condition, ExtraQuestionTypeConstants::AllowedSubQuestionRuleVisibilityCondition))
            throw new ValidationException(sprintf("Visibility Condition  %s is not valid value.", $visibility_condition));
    ...
    {
        if(empty($this->answer_values)) return [];
    ...
    {
        if(!in_array($answer_values_operator, ExtraQuestionTypeConstants::AllowedSubQuestionRuleAnswerValuesOperator))
            throw new ValidationException(sprintf("Answer Values Operator %s is not valid value.", $answer_values_operator));
    ...
    public function isSubQuestionVisible(?ExtraQuestionAnswer $answer):bool{
        $initial_condition = $this->answer_values_operator === ExtraQuestionTypeConstants::SubQuestionRuleAnswerValuesOperator_And ? true : false;
        if(is_null($answer)){
            if($this->visibility_condition === ExtraQuestionTypeConstants::SubQuestionRuleVisibilityCondition_Equal)
            ...
            switch ($this->visibility_condition){
                case ExtraQuestionTypeConstants::SubQuestionRuleVisibilityCondition_Equal:{
                    foreach ($this->getAnswerValues() as $rule_val) {
                        if ($this->answer_values_operator === ExtraQuestionTypeConstants::SubQuestionRuleAnswerValuesOperator_And)
