...
final class SwiftAdapter implements FilesystemAdapter, PublicUrlGenerator {
  ...
  public function directoryExists(string $path): bool {
    try {
      return $this->container->listObjects(["prefix" => $path])->valid();
    } catch (\Exception $e) {
  ...
  public function fileSize(string $path): FileAttributes {
    ...
    if (null === $fileAttributes->fileSize()) {
      throw UnableToRetrieveMetadata::fileSize(
        $path,
        sprintf('Invalid file size "%s".', $object->contentLength),
      );
    }
  ...
  private function normalizeObject(StorageObject $object): FileAttributes {
    ...
    return new FileAttributes($object->name, $fileSize, null, $lastModified, $mimeType);
  }
}
