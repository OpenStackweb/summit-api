...
final class AdminPresentationCSVSerializer extends AdminPresentationSerializer {
  ...
  public function serialize(
    ...
    if (
      isset($params["current_user"]) &&
      $params["current_user"] instanceof Member &&
      $params["current_user"]->isAdmin()
    ) {
    ...
    foreach ($presentation->getMediaUploads() as $mediaUpload) {
      if ($mediaUpload->getMediaUploadType()->isVideo()) {
        $media_upload_csv = SerializerRegistry::getInstance()
          ->getSerializer($mediaUpload, $serializerType)
          ->serialize(AbstractSerializer::filterExpandByPrefix($expand, "media_uploads"));
        if (!isset($media_upload_csv["private_url"]) || !isset($media_upload_csv["filename"])) {
          Log::warning(
            sprintf(
              "AdminPresentationCSVSerializer::serialize can not process media upload %s",
              json_encode($media_upload_csv),
            ),
          );
          continue;
        }
        if (in_array("video", $fields)) {
          $values["video"] = sprintf(
            '=HYPERLINK("%s";"%s")',
            $media_upload_csv["private_url"],
            $media_upload_csv["filename"],
          );
        }
        ...
        if (in_array("public_video", $fields)) {
          $values["public_video"] = sprintf(
            '=HYPERLINK("%s";"%s")',
            $media_upload_csv["public_url"],
            $media_upload_csv["filename"],
          );
        }
      }
    ...
    if (in_array("extra_questions", $fields)) {
      ...
      foreach ($presentation->getExtraQuestionAnswers() as $answer) {
        ...
        $values["extra_questions"] =
          $values["extra_questions"] . str_replace(",", "", (string) $answer);
      }
    }
    ...
    if (in_array("presentation_flags", $fields)) {
      ...
      foreach ($presentation->getPresentationActions() as $action) {
        ...
        $values["presentation_flags"] =
          $values["presentation_flags"] . str_replace(",", "", (string) $action);
      }
    }
