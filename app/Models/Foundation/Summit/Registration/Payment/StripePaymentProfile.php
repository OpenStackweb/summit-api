...
class StripePaymentProfile extends PaymentGatewayProfile {
  ...
  public function setLiveWebHookData(array $webhook_data): void {
    Log::debug(
      sprintf("StripePaymentProfile::setLiveWebHookData payload %s", json_encode($webhook_data)),
    );
  ...
  public function setTestWebHookData(array $webhook_data): void {
    Log::debug(
      sprintf("StripePaymentProfile::setTestWebHookData payload %s", json_encode($webhook_data)),
    );
  ...
  public function buildWebHook(): void {
    try {
      ...
      throw new ValidationException(
        "Can not create the Stripe Webhook, please review your provided credentials.",
      );
    }
  }
  ...
  protected function clearTestWebHook(): void {
    try {
      ...
      if ($this->existsWebHookTest()) {
        ...
        Log::debug(
          sprintf(
            "StripePaymentProfile::clearTestWebHook deleting webhook %s",
            $this->getTestWebhookId(),
          ),
        );
        $api->deleteWebHookById($this->getTestWebhookId());
        Log::debug(
          sprintf(
            "StripePaymentProfile::clearTestWebHook webhook %s deleted",
            $this->getTestWebhookId(),
          ),
        );
  ...
  protected function clearLiveWebHook(): void {
    try {
      ...
      if ($this->existsWebHookLive()) {
        ...
        Log::debug(
          sprintf(
            "StripePaymentProfile::clearLiveWebHook deleting webhook %s",
            $this->getTestWebhookId(),
          ),
        );
        ...
        Log::debug(
          sprintf(
            "StripePaymentProfile::clearLiveWebHook webhook %s deleted",
            $this->getTestWebhookId(),
          ),
        );
