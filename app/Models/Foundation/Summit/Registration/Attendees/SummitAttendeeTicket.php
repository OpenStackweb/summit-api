 ...
 */
class SummitAttendeeTicket extends SilverstripeBaseModel implements IQREntity {
  use FinancialTrait;
  ...
  public function generateNumber(): string {
    ...
    $this->number = strtoupper(
      str_replace(".", "", uniqid($summit->getTicketQRPrefix() . "_", true)),
    );
    return $this->number;
  }
  ...
  public function generateQRCode(): string {
    ...
    $fields = [$this->order->getSummit()->getTicketQRPrefix(), $this->number];
  ...
  public function hasPendingRefundRequests(): bool {
    $criteria = Criteria::create();
    $criteria->where(
      Criteria::expr()->eq("status", ISummitRefundRequestConstants::RefundRequestedStatus),
    );
  ...
  public function refund(
    ...
    if (!$this->canRefund($amount)) {
      throw new ValidationException(sprintf("Can not request a refund on Ticket %s.", $this->id));
    }
  ...
  public function applyTaxes(array $taxes): void {
    foreach ($taxes as $tax) {
      if (!$tax instanceof SummitTaxType) {
        continue;
      }
      if (!$tax->mustApplyTo($this->ticket_type)) {
        continue;
      }
  ...
  public function getFinalAmount(): float {
    ...
    Log::debug(
      sprintf(
        "SummitAttendeeTicket::getFinalAmount id %s amount %s taxes %s",
        $this->id,
        $amount,
        $taxes,
      ),
  ...
  public function getBadgeFeaturesNames(): array {
    ...
    foreach ($this->badge->getType()->getBadgeFeatures() as $feature) {
      if (in_array($feature->getName(), $res)) {
        continue;
      }
      $res[] = $feature->getName();
    }
  ...
  public function getBadgeFeatures(): array {
    ...
    foreach ($this->badge->getType()->getBadgeFeatures() as $feature) {
      if (key_exists($feature->getId(), $res)) {
        continue;
      }
      $res[$feature->getId()] = $feature;
    }
  ...
  public function canEditTicket(Member $member): bool {
    ...
    if ($member->isOnGroup(IGroup::BadgePrinters)) {
      return true;
    }
    // i am ticket owner
    if ($this->hasOwner() && $this->owner->getEmail() == $member->getEmail()) {
      return true;
    }
    // i am order owner
    if ($this->order->getOwnerEmail() == $member->getEmail()) {
      return true;
    }
    return false;
  }
   ...
   * @throws ValidationException
   */
  public static function parseQRCode(string $qr_code): array {
