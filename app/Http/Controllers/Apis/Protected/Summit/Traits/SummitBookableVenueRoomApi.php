...
trait SummitBookableVenueRoomApi {
      ...
      function ($filter) use ($summit) {
        if ($filter instanceof Filter) {
          $filter->addFilterCondition(
            FilterParser::buildFilter("class_name", "==", SummitBookableVenueRoom::ClassName),
          );
        }
        return $filter;
      },
    ...
    return $this->_getAll(
      ...
      function () {
        return [
          ...
          "end_datetime" =>
            "sometimes|required_with:start_datetime|date_format:U|after:start_datetime",
        ];
      },
      function () {
        return [
    ...
    return $this->_getAllCSV(
      ...
      function () use ($summit) {
        return [
          ...
          "approved_payment_date" => new EpochCellFormatter(
            EpochCellFormatter::DefaultFormat,
            $summit->getTimeZone(),
          ),
        ];
  ...
  public function getBookableVenueRoomAvailability($summit_id, $room_id, $day) {
    return $this->processRequest(function () use ($summit_id, $room_id, $day) {
      ...
      try {
        ...
        $from_day = is_numeric($day)
          ? new \DateTime("@$day")
          : // if its a string then we need to be aware of time zone
          \DateTime::createFromFormat("Y-m-d", $day, $summit->getTimeZone())->setTime(0, 0, 0);
      } catch (\Exception $ex) {
      ...
      foreach ($slots_definitions as $slot_label => $is_free) {
        ...
        $list[] = SerializerRegistry::getInstance()
          ->getSerializer(
            new SummitBookableVenueRoomAvailableSlot(
              $room,
              $summit->convertDateFromTimeZone2UTC(
                new \DateTime($dates[0], $summit->getTimeZone()),
              ),
              $summit->convertDateFromTimeZone2UTC(
                new \DateTime($dates[1], $summit->getTimeZone()),
              ),
              $is_free,
            ),
          )
      ...
      $response = new PagingResponse(count($list), count($list), 1, 1, $list);
  ...
  public function createBookableVenueRoomReservation($summit_id, $room_id) {
    return $this->processRequest(function () use ($summit_id, $room_id) {
      ...
      $reservation = $this->location_service->addBookableRoomReservation(
        $summit,
        intval($room_id),
        $payload,
      );
  ...
  public function createOfflineBookableVenueRoomReservation($summit_id, $room_id) {
    return $this->processRequest(function () use ($summit_id, $room_id) {
      ...
      $reservation = $this->location_service->addOfflineBookableRoomReservation(
        $summit,
        intval($room_id),
        $payload,
      );
  ...
  public function updateBookableVenueRoomReservation($summit_id, $room_id, $reservation_id) {
    return $this->processRequest(function () use ($summit_id, $room_id, $reservation_id) {
      ...
      $reservation = $this->location_service->updateBookableRoomReservation(
        $summit,
        intval($room_id),
  ...
  public function cancelMyBookableVenueRoomReservation($summit_id, $reservation_id) {
    return $this->processRequest(function () use ($summit_id, $reservation_id) {
      ...
      $reservation = $this->location_service->cancelReservation(
        $summit,
        $current_member,
        $reservation_id,
      );
      ...
      return $this->updated(
        SerializerRegistry::getInstance()->getSerializer($reservation)->serialize(),
      );
    });
  }
  ...
  public function updateVenueBookableRoom($summit_id, $venue_id, $room_id) {
    return $this->processRequest(function () use ($summit_id, $venue_id, $room_id) {
      ...
      $room = $this->location_service->updateVenueBookableRoom(
        $summit,
        $venue_id,
        $room_id,
        $payload,
  ...
  public function refundBookableVenueRoomReservation($summit_id, $room_id, $reservation_id) {
    return $this->processRequest(function () use ($summit_id, $room_id, $reservation_id) {
      ...
      $reservation = $this->location_service->refundReservation(
        $room,
        intval($reservation_id),
        $amount,
      );
