...
trait TimeDurationRestrictedEvent {
  ...
  private function _setStartDate(DateTime $value, ?Summit $summit) {
    ...
    if (!is_null($end_date)) {
      ...
      Log::debug(
        sprintf(
          "TimeDurationRestrictedEvent::setStartDate id %s setting new duration %s",
          $this->id,
          $newDuration,
        ),
      );
    ...
    Log::debug(
      sprintf(
        "TimeDurationRestrictedEvent::setStartDate id %s start_date %s",
        $this->id,
  ...
  private function _setEndDate(DateTime $value, ?Summit $summit) {
    ...
    if (!is_null($start_date)) {
      ...
      Log::debug(
        sprintf(
          "TimeDurationRestrictedEvent::setEndDate id %s newDuration %s",
          $this->id,
          $newDuration,
        ),
      );
    ...
    Log::debug(
      sprintf(
        "SummitEvent::setEndDate id %s end_date %s",
        $this->id,
  ...
  private function _setDuration(
    ...
    if ($duration_in_seconds > 0 && $duration_in_seconds < self::MIN_EVENT_MINUTES * 60) {
      throw new ValidationException(
        sprintf("Duration should be greater than %s minutes.", self::MIN_EVENT_MINUTES),
      );
    }
    ...
    if (!$isAdmin) {
      ...
      if ($type instanceof PresentationType) {
        ...
        if ($min_duration > 0 && $duration_in_seconds < $min_duration * 60) {
          throw new ValidationException(
            sprintf("Duration must be greater than or equal to %s minutes.", $min_duration),
          );
        }
        $max_duration = $type->getMaxDuration();
        if ($max_duration > 0 && $duration_in_seconds > $max_duration * 60) {
          throw new ValidationException(
            sprintf("Duration must be less than or equal to %s minutes.", $max_duration),
          );
        }
      }
    }
