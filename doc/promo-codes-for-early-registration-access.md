...
- Inherited `addTicketTypeRule()` / `removeTicketTypeRuleForTicketType()` on the discount variant — the discount-specific overrides were removed (commit `497839bea`). Domain-authorized discount codes now use the parent `SummitRegistrationDiscountCode` behavior: adding a rule also writes the ticket type into `allowed_ticket_types` (parent's dual-write). This keeps the discounted type applicable, since `canBeAppliedTo()` requires `allowed_ticket_types` membership; the old override created rules whose ticket type was absent from `allowed_ticket_types`, leaving them non-applicable.
...
4. ~~Adding a `ticket_types_rule` on the discount variant fails if the ticket type is not already in `allowed_ticket_types`~~ — SUPERSEDED by commit `497839bea`: the discount variant no longer overrides `addTicketTypeRule()`/`removeTicketTypeRuleForTicketType()`. Adding a rule uses the inherited parent behavior, which also adds the ticket type to `allowed_ticket_types` (required for `canBeAppliedTo()` to apply the discount). The "Review Follow-ups" entries that implemented this override are historical and no longer reflect the code.
