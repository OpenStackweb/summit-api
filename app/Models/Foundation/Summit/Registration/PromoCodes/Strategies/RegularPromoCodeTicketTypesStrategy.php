...
class RegularPromoCodeTicketTypesStrategy implements IPromoCodeAllowedTicketTypesStrategy {
  ...
  public function getTicketTypes(): array {
    ...
    foreach (
      $this->summit->getTicketTypesByAudience(SummitTicketType::Audience_All)
      as $ticket_type
    ) {
    ...
    if (!is_null($invitation)) {
      ...
      $invitation_ticket_types = array_map(function ($type) {
        return $this->applyPromo2TicketType($type);
    ...
    foreach (
      $this->summit->getTicketTypesByAudience(SummitTicketType::Audience_Without_Invitation)
      as $ticket_type
    ) {
      if (!$ticket_type->canSell()) {
        Log::debug(
