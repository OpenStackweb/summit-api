...
final class OAuth2SummitOrdersApiController extends OAuth2ProtectedController {
    ...
    return $this->processRequest(function () use ($summit_id) {
      ...
      return $this->created(
        SerializerRegistry::getInstance()
          ->getSerializer($order, ISummitOrderSerializerTypes::ReservationType)
          ->serialize(
  ...
  public function getAllBySummit($summit_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["id", "number", "status", "owner_name", "owner_email", "owner_company", "created"];
      },
  ...
  public function cancelRefundRequestTicket($order_id, $ticket_id) {
    return $this->processRequest(function () use ($order_id, $ticket_id) {
      ...
      $ticket = $this->service->cancelRequestRefundTicket(
        intval($order_id),
        intval($ticket_id),
        $current_user,
        trim($payload["notes"] ?? ""),
  ...
  public function requestRefundMyTicket($order_id, $ticket_id) {
    return $this->processRequest(function () use ($order_id, $ticket_id) {
      ...
      $ticket = $this->service->requestRefundTicket(
        $current_user,
        intval($order_id),
        intval($ticket_id),
      );
  ...
  public function assignAttendee($order_id, $ticket_id) {
    return $this->processRequest(function () use ($order_id, $ticket_id) {
      ...
      $ticket = $this->service->ownerAssignTicket(
        $current_user,
        intval($order_id),
        intval($ticket_id),
        $payload,
  ...
  public function updateTicket($summit_id, $order_id, $ticket_id) {
    return $this->processRequest(function () use ($summit_id, $order_id, $ticket_id) {
      ...
      $ticket = $this->service->updateTicket(
        $summit,
        intval($order_id),
        intval($ticket_id),
        $payload,
  ...
  public function getMyTicketsByOrderId($order_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["id", "number", "status", "owner_first_name", "owner_last_name", "owner_name"];
      },
      function ($filter) use ($owner, $order_id) {
        if ($filter instanceof Filter) {
          ...
          $filter->addFilterCondition(
            FilterElement::makeEqual("status", IOrderConstants::PaidStatus),
          );
        }
        return $filter;
      },
  ...
  public function getAllRefundApprovedRequests($summit_id, $order_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["id", "created", "action_date", "ticket_id"];
      },
      function ($filter) use ($summit, $order_id) {
        if ($filter instanceof Filter) {
          $filter->addFilterCondition(
            FilterElement::makeEqual("class_name", SummitAttendeeTicketRefundRequest::ClassName),
          );
          $filter->addFilterCondition(FilterElement::makeEqual("summit_id", $summit->getId()));
          $filter->addFilterCondition(FilterElement::makeEqual("order_id", intval($order_id)));
          $filter->addFilterCondition(
            FilterElement::makeEqual("status", ISummitRefundRequestConstants::ApprovedStatus),
          );
        }
        return $filter;
      },
