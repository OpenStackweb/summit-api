 ...
 */
final class SummitSponsorService extends ExtraQuestionTypeService implements ISummitSponsorService {
  /**
  ...
  public function addSponsor(Summit $summit, array $payload): Sponsor {
    return $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      $featured_event_id = isset($payload["featured_event_id"])
        ? intval($payload["featured_event_id"])
        : 0;
   ...
   * @return File
   */
  public function addSponsorSideImage(
    Summit $summit,
    int $sponsor_id,
    UploadedFile $file,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!in_array($file->extension(), IFileConstants::ValidImageExtensions)) {
        throw new ValidationException(
          sprintf(
            ...
            implode(",", IFileConstants::ValidImageExtensions),
          ),
        );
      }
      ...
      $photo = $this->file_uploader->build(
        $file,
        sprintf("summits/%s/sponsors/%s", $summit->getId(), $summit_sponsor->getId()),
        true,
      );
   ...
   * @return File
   */
  public function addSponsorHeaderImage(
    Summit $summit,
    int $sponsor_id,
    UploadedFile $file,
    $max_file_size = IFileConstants::MaxImageSizeInBytes,
   ...
   * @return File
   */
  public function addSponsorHeaderImageMobile(
    Summit $summit,
    int $sponsor_id,
    UploadedFile $file,
    $max_file_size = IFileConstants::MaxImageSizeInBytes,
   ...
   * @return SponsorAd
   */
  public function updateSponsorAd(
    Summit $summit,
    int $sponsor_id,
    int $ad_id,
    array $payload,
   ...
   * @throws ValidationException
   */
  public function addSponsorAdImage(
    Summit $summit,
    int $sponsor_id,
    int $ad_id,
    UploadedFile $file,
    ...
    return $this->tx_service->transaction(function () use (
      ...
      $photo = $this->file_uploader->build(
        $file,
        sprintf(
          "summits/%s/sponsors/%s/ads/%s",
          $summit->getId(),
          $summit_sponsor->getId(),
          $ad->getId(),
        ),
        true,
      );
   ...
   * @throws ValidationException
   */
  public function addSponsorMaterial(
    Summit $summit,
    int $sponsor_id,
    array $payload,
  ): SponsorMaterial {
    return $this->tx_service->transaction(function () use ($summit, $sponsor_id, $payload) {
      ...
      if (!is_null($former_material)) {
        throw new ValidationException(
          sprintf(
            "Material name %s already exists on sponsor %s.",
            $former_material->getName(),
            $summit_sponsor->getId(),
          ),
        );
      }
   ...
   * @throws ValidationException
   */
  public function updateSponsorMaterial(
    Summit $summit,
    int $sponsor_id,
    int $material_id,
    array $payload,
   ...
   * @throws ValidationException
   */
  public function addSponsorSocialNetwork(
    Summit $summit,
    int $sponsor_id,
    array $payload,
   ...
   * @throws ValidationException
   */
  public function updateSponsorSocialNetwork(
    Summit $summit,
    int $sponsor_id,
   ...
   * @throws ValidationException
   */
  public function deleteSponsorSocialNetwork(
    Summit $summit,
    int $sponsor_id,
    int $social_network_id,
  ): void {
   ...
   * @inheritDoc
   */
  public function addSponsorExtraQuestion(
    Summit $summit,
    int $sponsor_id,
    array $payload,
   ...
   * @inheritDoc
   */
  public function updateSponsorExtraQuestion(
    Summit $summit,
    int $sponsor_id,
   ...
   * @throws \Exception
   */
  public function addLeadReportSettings(
    Summit $summit,
    int $sponsor_id,
    array $payload,
