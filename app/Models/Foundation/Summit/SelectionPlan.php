...
class SelectionPlan extends SilverstripeBaseModel {
  ...
  public function addPresentationAllowedEditableQuestion(string $type): void {
    if (!Presentation::isAllowedEditableField(trim($type))) {
      throw new ValidationException(
        sprintf("Presentation question %s is not edit allowed.", $type),
      );
    }
    ...
    if ($this->allowed_editable_presentation_questions->matching($criteria)->count() > 0) {
      throw new ValidationException(
        sprintf("Presentation Question %s is already allowed to edit.", $type),
      );
    }
  ...
  public function hasTrack(PresentationCategory $track) {
    foreach ($this->category_groups as $track_group) {
      if ($track_group->hasCategory($track->getIdentifier())) {
        return true;
      }
    }
    return false;
   ...
   * @throws ValidationException
   */
  public function recalculateQuestionOrder(
    SummitSelectionPlanExtraQuestionType $question,
    int $new_order,
  ): void {
    $criteria = Criteria::create();
  ...
  public function getEventTypeById(int $eventTypeId): ?SummitEventType {
    ...
    $event_type = $this->event_types
      ->filter(function ($e) use ($eventTypeId) {
   ...
   */
  public function setPresentationCreatorNotificationEmailTemplate(
    string $presentation_creator_notification_email_template,
  ): void {
   ...
   */
  public function setPresentationModeratorNotificationEmailTemplate(
    string $presentation_moderator_notification_email_template,
  ): void {
   ...
   */
  public function setPresentationSpeakerNotificationEmailTemplate(
    string $presentation_speaker_notification_email_template,
  ): void {
   ...
   * @throws ValidationException
   */
  public function createTeamSelectionList(
    PresentationCategory $track,
  ): SummitSelectedPresentationList {
    $team_selection_list = $this->getSelectionListByTrackAndTypeAndOwner(
      $track,
    ...
    if (is_null($team_selection_list)) {
      Log::debug(
        sprintf(
          "SelectionPlan::createSelectionLists adding team list for track %s selection plan %s",
          $track->getId(),
          $this->getId(),
        ),
      );
   ...
   * @throws ValidationException
   */
  public function createIndividualSelectionList(
    PresentationCategory $track,
    Member $member,
  ): SummitSelectedPresentationList {
    if (!is_null($member)) {
      $individual_selection_list = $this->getSelectionListByTrackAndTypeAndOwner(
        $track,
      ...
      if (is_null($individual_selection_list)) {
        ...
        Log::debug(
          sprintf(
            "SelectionPLan::createSelectionLists adding individual list for track %s and member %s",
            $track->getId(),
            $member->getId(),
          ),
        );
        $individual_selection_list->setName(
          sprintf(
            "%s Individual Selection List for %s",
            $member->getFullName(),
            $track->getTitle(),
          ),
        );
  ...
  public function addTrackChairRatingType(PresentationTrackChairRatingType $ratingType): void {
    if ($this->track_chair_rating_types->contains($ratingType)) {
      return;
    }
   ...
   * @throws ValidationException
   */
  public function recalculateTrackChairRatingTypeOrder(
   ...
   */
  public function setSubmissionLockDownPresentationStatusDate(
    ?DateTime $submission_lock_down_presentation_status_date,
  ): void {
    $this->submission_lock_down_presentation_status_date = $this->convertDateFromTimeZone2UTC(
   ...
   * @return AssignedSelectionPlanExtraQuestionType|null
   */
  public function getAssignedExtraQuestion(
    SummitSelectionPlanExtraQuestionType $question,
  ): ?AssignedSelectionPlanExtraQuestionType {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("question_type", $question));
  ...
  public function getPresentationActionType(PresentationActionType $type): ?PresentationActionType {
    ...
    $presentation_action_type_assignment = $this->allowed_presentation_action_types
      ->matching($criteria)
      ->first();
    if (
      $presentation_action_type_assignment === false ||
      !$presentation_action_type_assignment instanceof AllowedPresentationActionType
    ) {
      return null;
    }
   ...
   * @return int
   */
  public function getPresentationActionTypeOrder(
   ...
   * @param PresentationActionType $presentation_action_type
   */
  public function addPresentationActionType(
    PresentationActionType $presentation_action_type,
  ): void {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("type", $presentation_action_type));
    if ($this->allowed_presentation_action_types->matching($criteria)->count() > 0) {
      return;
    }
    $order = $this->getPresentationActionTypesMaxOrder();
    $allowed_presentation_action_type = new AllowedPresentationActionType(
      $presentation_action_type,
      $this,
      $order + 1,
    );
   ...
   * @param PresentationActionType $presentation_action_type
   */
  public function removePresentationActionType(
    PresentationActionType $presentation_action_type,
  ): void {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("type", $presentation_action_type));
    ...
    self::resetOrderForSelectable(
      $this->allowed_presentation_action_types,
      AllowedPresentationActionType::class,
    );
  }
   ...
   * @throws ValidationException
   */
  public function recalculatePresentationActionTypeOrder(
    PresentationActionType $presentation_action_type,
    int $new_order,
  ): void {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("type", $presentation_action_type));
    $selection_plan_assignment = $this->allowed_presentation_action_types
      ->matching($criteria)
      ->first();
    if ($selection_plan_assignment === false) {
      return;
    }
    self::recalculateOrderForSelectable(
      $this->allowed_presentation_action_types,
      $selection_plan_assignment,
      $new_order,
      AllowedPresentationActionType::class,
    );
  }
   ...
   * @return bool
   */
  public function isAllowedPresentationActionType(
    PresentationActionType $presentation_action_type,
  ): bool {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("type", $presentation_action_type));
  ...
  public function addAllowedMember(string $email): ?SelectionPlanAllowedMember {
    if ($this->is_hidden) {
      throw new ValidationException(
        sprintf("Members cannot be added to selection plan %s because it's hidden.", $this->id),
      );
    }
    ...
    if ($this->allowed_members->matching($criteria)->count() > 0) {
      throw new ValidationException(
        sprintf("Email %s already allowed for selection plan %s.", $email, $this->id),
      );
    }
  ...
  public function removeAllowedMember(SelectionPlanAllowedMember $allowedMember): void {
    if (!$this->allowed_members->contains($allowedMember)) {
      return;
    }
  ...
  public function isAllowedMember(string $email): bool {
    if ($this->getType() === self::PublicType || $this->getType() === self::HiddenType) {
      return true;
    }
  ...
  public function containsMember(string $email): bool {
    return $this->allowed_members
      ->filter(function ($element) use ($email) {
  ...
  public function checkPresentationAllowedQuestions(array $payload): void {
    ...
    foreach ($allowed_fields as $field) {
      ...
      if (isset($payload[$field]) && !$this->isAllowedPresentationQuestion($field)) {
        throw new ValidationException(
          sprintf("Field %s is not allowed on Selection Plan %s", $field, $this->name),
        );
      }
    }
  }
   ...
   * @throws ValidationException
   */
  public function checkPresentationAllowedEdtiableQuestions(
    array $payload,
    array $former_data,
  ): void {
    ...
    foreach ($allowed_fields as $field) {
      Log::debug(
        sprintf(
          "SelectionPlan::checkPresentationAllowedEdtiableQuestions Selection Plan %s checking Presentation Field %s if its editable...",
          $this->id,
          $field,
        ),
      );
      if (
        isset($payload[$field]) &&
        isset($former_data[$field]) &&
        !self::areFieldsEqual($payload[$field], $former_data[$field]) &&
        !$this->isAllowedEditablePresentationQuestion($field)
      ) {
        Log::debug(
          sprintf(
            "SelectionPlan::checkPresentationAllowedEdtiableQuestions current field %s ( %s ).",
            $field,
            json_encode($payload[$field]),
          ),
        );
        Log::debug(
          sprintf(
            "SelectionPlan::checkPresentationAllowedEdtiableQuestions former field %s ( %s ).",
            $field,
            json_encode($former_data[$field]),
          ),
        );
        throw new ValidationException(
          sprintf("Field %s is not allowed for edition on Selection Plan %s.", $field, $this->name),
        );
      }
    }
  }
