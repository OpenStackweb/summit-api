...
abstract class AbstractTask {
  abstract public function run(array $formerState): array;
...
final class SagaFactory {
  ...
  public function build(Member $owner, Summit $summit, array $payload): Saga {
    Log::debug(
      sprintf(
        "SagaFactory::build - summit id %s payload %s",
        $summit->getId(),
        json_encode($payload),
      ),
    );
    ...
    if (count($tickets) === 1) {
      ...
      if (!empty($promo_code_val)) {
        ...
        Log::debug(
          sprintf(
            "SagaFactory::build - summit id %s, promo_code %s",
            $summit->getId(),
            $promo_code_val,
          ),
        );
  ...
  private function buildPrePaidSaga(Member $owner, Summit $summit, array $payload): Saga {
    ...
    return Saga::start()
      ...
      ->addTask(
        new AutoAssignPrePaidTicketTask(
          $owner,
          $summit,
          $payload,
...
final class ReserveOrderTask extends AbstractTask {
  ...
  public function run(array $formerState): array {
    ...
    return $this->tx_service->transaction(function () {
      ...
      if (
        !is_null($this->owner) &&
        strtolower($this->owner->getEmail()) != strtolower($owner_email)
      ) {
      ...
      $shouldAutoAssignFirstTicket =
        !$this->owner->hasPaidRegistrationOrderForSummit($this->summit) && count($tickets) > 1;
      ...
      if (is_null($payment_gateway)) {
        throw new ValidationException(
          sprintf("Payment configuration is not set for summit %s", $this->summit->getId()),
        );
      }
      ...
      foreach ($tickets as $ticket_dto) {
        Log::debug(
          sprintf(
            "ReserveOrderTask::run Processing ticket #%s payload %s",
            $index,
            json_encode($ticket_dto),
          ),
        );
        ...
        $shouldAutoAssignInvitationTicket =
          $hasPendingInvitation && $invitation->isTicketTypeAllowed($type_id);
        ...
        $promo_code = !empty($promo_code_value)
          ? $this->summit->getPromoCodeByCode($promo_code_value)
          : null;
        ...
        if (!empty($attendee_email)) {
          ...
          Log::debug(
            sprintf("ReserveOrderTask::run - processing attendee_email %s", $attendee_email),
          );
          // assign attendee
          ...
          if (is_null($attendee) && isset($local_attendees[$attendee_email])) {
            Log::debug(
              sprintf(
                "ReserveOrderTask::run - attendee_email %s not fund in repo getting it from local tx",
                $attendee_email,
              ),
            );
          ...
          $attendee_owner =
            $this->owner->getEmail() === $attendee_email
              ? $this->owner
          ...
          if (is_null($attendee)) {
            Log::debug(
              sprintf(
                "ReserveOrderTask::run - creating attendee %s for summit %s",
                $attendee_email,
                $this->summit->getId(),
              ),
            );
            $attendee = SummitAttendeeFactory::build(
              $this->summit,
              [
      ...
      if (is_null($this->owner)) {
        Log::debug(
          sprintf("ReserveOrderTask::run is null trying to get owner by email %s", $owner_email),
        );
...
final class ApplyPromoCodeTask extends AbstractTask {
  ...
  public function run(array $formerState): array {
    ...
    foreach ($promo_codes_usage as $promo_code_value => $info) {
      $this->tx_service->transaction(function () use (
        ...
        if (
          !$promo_code instanceof SummitRegistrationPromoCode ||
          PromoCodesUtils::isPrePaidPromoCode($promo_code)
        ) {
          throw new EntityNotFoundException(
            sprintf("The Promo Code “%s” is not a valid code.", $promo_code_value),
          );
        }
        ...
        if ($promo_code->getSummitId() != $this->summit->getId()) {
          throw new EntityNotFoundException(
            sprintf(
              "Promo Code %s not found on summit %s.",
              $promo_code->getCode(),
              $this->summit->getId(),
            ),
          );
        ...
        foreach ($info["types"] as $ticket_type_id) {
          ...
          if (is_null($ticket_type)) {
            throw new ValidationException(
              sprintf(
                "Ticket Type %s not found on summit %s.",
                $ticket_type_id,
                $this->summit->getId(),
              ),
            );
        ...
        $this->lock_service->lock(
          "promocode." . $promo_code->getId() . ".usage.lock",
          function () use ($promo_code, $qty, $owner_email) {
  ...
  public function undo() {
    ...
    foreach ($promo_codes_usage as $code => $info) {
      Log::debug(
        sprintf(
          "ApplyPromoCodeTask::undo undoing promo code %s info %s owner_email %s",
          $code,
          json_encode($info),
          $owner_email,
        ),
      ...
      $this->tx_service->transaction(function () use ($code, $info, $owner_email) {
        ...
        if (!isset($info["redeem"])) {
          return;
        }

        $this->lock_service->lock(
          "promocode." . $promo_code->getId() . ".usage.lock",
          function () use ($promo_code, $info, $owner_email) {
...
final class ReserveTicketsTask extends AbstractTask {
  ...
  public function run(array $formerState): array {
    ...
    $this->tx_service->transaction(function () {
      ...
      $ticket_types = $this->ticket_type_repository->getByIdsExclusiveLock(
        $this->summit,
        $ticket_types_ids,
      );
      ...
      foreach ($ticket_types as $ticket_type) {
        ...
        if (!$ticket_type->canSell()) {
          throw new ValidationException(
            sprintf(
              "The ticket “%s” is not available. Please go back and select a different ticket.",
              $ticket_type->getName(),
            ),
          );
        }
        ...
        $this->lock_service->lock(
          ...
          function () use ($ticket_type, $reservations) {
...
final class PreProcessReservationTask extends AbstractTask {
   ...
   * @param array $payload
   */
  public function __construct(Summit $summit, array $payload) {
    $this->payload = $payload;
    $this->summit = $summit;
  }
  ...
  public function run(array $formerState): array {
    ...
    foreach ($tickets as $ticket_dto) {
      ...
      $promo_code_value = isset($ticket_dto["promo_code"])
        ? strtoupper(trim($ticket_dto["promo_code"]))
        : null;
...
final class PreOrderValidationTask extends AbstractTask {
  ...
  public function run(array $formerState): array {
    // pre checks
    $this->tx_service->transaction(function () {
      $extra_questions = isset($this->payload["extra_questions"])
        ? $this->payload["extra_questions"]
        : [];
      // check if we have at least a default badge template
      if (!$this->summit->hasDefaultBadgeType()) {
        throw new ValidationException(
          sprintf("Summit %s has not default badge type set", $this->summit->getId()),
        );
      }
      // check if we are on registration period
      if (!$this->summit->isRegistrationPeriodOpen()) {
        throw new ValidationException(
          sprintf("Summit %s registration period is closed", $this->summit->getId()),
        );
      }
      $owner_email = $this->payload["owner_email"];
      ...
      $mandatory_per_order = $this->summit->getMandatoryOrderExtraQuestionsByUsage(
      ...
      if ($mandatory_per_order->count() > 0) {
        ...
        foreach ($mandatory_per_order as $question) {
          ...
          foreach ($extra_questions as $question_answer) {
            if (
              $question_answer["question_id"] == $question->getId() &&
              !empty($question_answer["answer"])
            ) {
              $found = true;
              break;
            }
...
final class AutoAssignPrePaidTicketTask extends AbstractTask {
  ...
  public function run(array $formerState): array {
    ...
    return $this->tx_service->transaction(function () {
      if (is_null($this->owner)) {
        throw new ValidationException("Ticket owner can't be null.");
      }
      ...
      $promo_code_val = $ticket_dto["promo_code"] ?? null;
      if (empty($promo_code_val)) {
        throw new ValidationException("Promo code is required.");
      }

      $type_id = $ticket_dto["type_id"];
      $order = $this->lock_service->lock(
        "ticket_type." . $type_id . "promo_code." . $promo_code_val . ".sell.lock",
        function () use ($promo_code_val, $type_id) {
          ...
          if (empty($attendee_first_name)) {
            $attendee_first_name =
              $this->payload["owner_first_name"] ?? $this->owner->getFirstName();
          }
          // last name
          $attendee_last_name = $ticket_dto["attendee_last_name"] ?? null;
          if (empty($attendee_last_name)) {
          ...
          if (!$order->isOfflineOrder()) {
            throw new ValidationException(
              sprintf("Order %s is not an offline order.", $order->getId()),
            );
          }

          Log::debug(
            sprintf(
              "AutoAssignPrePaidTicketTask::run - processing attendee_email %s",
              $attendee_email,
            ),
          );
          // assign attendee
          ...
          if (is_null($attendee)) {
            Log::debug(
              sprintf(
                "AutoAssignPrePaidTicketTask::run - creating attendee %s for summit %s",
                $attendee_email,
                $this->summit->getId(),
              ),
            );
 ...
 * @package App\Services\Model
 */
final class SummitOrderService extends AbstractService implements ISummitOrderService {
  /**
   * @var IMemberRepository
   */
  private $member_repository;
  ...
  public function reserve(?Member $owner, Summit $summit, array $payload): SummitOrder {
    Log::debug(
      sprintf(
        "SummitOrderService::reserve owner %s summit %s payload %s",
        !is_null($owner) ? $owner->getId() : "N/A",
        $summit->getId(),
        json_encode($payload),
      ),
    );
    ...
    try {
      ...
      $owner = $this->tx_service->transaction(function () use ($owner, $payload) {
        ...
        if (isset($payload["owner_first_name"]) && !empty($payload["owner_first_name"])) {
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::reserve setting first name %s to member %s",
              $first_name,
              $owner->getId(),
            ),
          );
        ...
        if (isset($payload["owner_last_name"]) && !empty($payload["owner_last_name"])) {
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::reserve setting last name %s to member %s",
              $last_name,
              $owner->getId(),
            ),
          );
        ...
        Event::dispatch(
          new MemberUpdated($owner->getId(), $owner->getEmail(), $first_name, $last_name, $company),
        );
      ...
      if (!is_null($owner) && $owner instanceof Member) {
        Log::debug(
          sprintf(
            "SummitOrderService::reserve owner %s %s %s",
            $owner->getId(),
            $owner->getFirstName(),
            $owner->getLastName(),
          ),
  ...
  public function checkout(Summit $summit, string $order_hash, array $payload): SummitOrder {
    return $this->tx_service->transaction(function () use ($summit, $order_hash, $payload) {
      ...
      if (
        is_null($order) ||
        !$order instanceof SummitOrder ||
        $summit->getId() != $order->getSummitId()
      ) {
      ...
      if (is_null($payment_gateway)) {
        throw new ValidationException(
          sprintf("Payment configuration is not set for summit %s.", $summit->getId()),
        );
      }
  ...
  public function revokeTicket(
    ...
    return $this->tx_service->transaction(function () use ($current_user, $order_id, $ticket_id) {
      ...
      if (!$ticket->hasOwner()) {
        throw new ValidationException(
          "You attempted to assign or reassign a ticket that you don’t have permission to assign.",
        );
      }
      ...
      if ($ticket->getTicketType()->getAudience() === SummitTicketType::Audience_With_Invitation) {
        ...
        if ($attendee->getEmail() === $order->getOwnerEmail()) {
          if (
            $summit->getTicketCountByTypeAndOwnerEmail(
              $ticket->getTicketType(),
              $attendee->getEmail(),
            ) === 1
          ) {
            throw new ValidationException(
              "You can not reassign this ticket. please contact support.",
            );
          }
        }
      }
  ...
  public function ownerAssignTicket(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if ($ticket->hasOwner()) {
        Log::debug(
          sprintf("SummitOrderService::ownerAssignTicket - ticket %s has owner", $ticket->getId()),
        );
        if ($summit->hasReassignTicketLimit()) {
          $now = new \DateTime("now", new \DateTimeZone("UTC"));
          if ($now > $summit->getReassignTicketTillDate()) {
      ...
      $attendee = SummitAttendeeFactory::populate($summit, $attendee, $normalize_payload, $member);
  ...
  public function reSendOrderEmail(int $order_id): SummitOrder {
    return $this->tx_service->transaction(function () use ($order_id) {
      ...
      if (!$order->hasOwner()) {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::reSendOrderEmail - trying to get email %s from db",
            $ownerEmail,
          ),
        );
        ...
        if (!is_null($member)) {
          ...
          Log::debug(
            sprintf("SummitOrderService::reSendOrderEmail - member %s found at db", $ownerEmail),
          );
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::reSendOrderEmail trying to get external user %s",
            $ownerEmail,
          ),
        );
        ...
        if (is_null($user)) {
          ...
          $this->sendSummitOrderOwnerInvitationEmail(
            $order,
            $this->member_service->emitRegistrationRequest(
              $ownerEmail,
  ...
  private function sendExistentSummitOrderOwnerEmail(SummitOrder $order) {
    Log::debug(
      sprintf(
        "SummitOrderService::sendExistentSummitOrderOwnerEmail for order %s",
        $order->getId(),
      ),
    );
   ...
   */
  private function sendSummitOrderOwnerInvitationEmail(
    SummitOrder $order,
    array $user_registration_request,
  ) {
    Log::debug(
      sprintf(
        "SummitOrderService::sendSummitOrderOwnerInvitationEmail for order %s",
        $order->getId(),
      ),
    );
    UnregisteredMemberOrderPaidMail::dispatch(
      $order,
      $user_registration_request["set_password_link"],
    );
  }
  ...
  public function requestRefundOrder(Member $current_user, int $order_id): SummitOrder {
    return $this->tx_service->transaction(function () use ($current_user, $order_id) {
      ...
      if (!$order->getSummit()->canEmitRefundRequests($current_user)) {
        throw new ValidationException(
          sprintf("Refund Period is over for Summit %s.", $order->getSummit()->getName()),
        );
      }
  ...
  public function refundTicket(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      $request = $ticket->refund($currentUser, $amount_2_refund, $paymentGatewayRes, $notes);
  ...
  public function cancel(Summit $summit, string $order_hash): SummitOrder {
    return $this->tx_service->transaction(function () use ($summit, $order_hash) {
      Log::debug(
        sprintf("SummitOrderService::cancel summit %s order %s", $summit->getId(), $order_hash),
      );
      $order = $this->order_repository->getByHashLockExclusive($order_hash);
   ...
   * @return void
   */
  private function restoreTicketsPromoCodes(
    Summit $summit,
    $tickets_to_return,
    $promo_codes_to_return,
  ): void {
    ...
    foreach ($tickets_to_return as $ticket_type_id => $qty) {
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::restoreTicketsPromoCodes compensating ticket type %s on %s usages",
          $ticket_type_id,
          $qty,
        ),
      );
    ...
    foreach ($promo_codes_to_return as $code => $value) {
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::restoreTicketsPromoCodes compensating promo code %s on %s usages",
          $code,
          $qty,
        ),
      );
  ...
  public function processPayment(array $payload, ?Summit $summit = null): void {
    $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      if (
        is_null($order) ||
        !$order instanceof SummitOrder ||
        (!is_null($summit) && $order->getSummitId() != $summit->getId())
      ) {
        throw new EntityNotFoundException(
    ...
    foreach ($orders as $order) {
      $this->tx_service->transaction(function () use ($order) {
        try {
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::confirmOrdersOlderThanNMinutes processing order %s",
              $order->getId(),
            ),
          );
          $summit = $order->getSummit();
          ...
          if (is_null($payment_gateway)) {
            Log::warning(
              sprintf(
                "SummitOrderService::confirmOrdersOlderThanNMinutes Payment configuration is not set for summit %s",
                $summit->getId(),
              ),
            );
            return;
  ...
  private function processInvitation(SummitOrder $order): SummitOrder {
    ...
    if (is_null($invitation) || $invitation->isAccepted()) {
      Log::debug(
        sprintf(
          "SummitOrderService::processInvitation invitation for email %s does not exists or its already accepted.",
          $order->getOwnerEmail(),
        ),
      );
      return $order;
    ...
    Log::debug(
      sprintf(
        "SummitOrderService::processInvitation trying mark invitation for email %s as accepted.",
        $order->getOwnerEmail(),
      ),
    );
    ...
    if (!is_null($attendee)) {
      CopyInvitationTagsToAttendees::dispatch(
        $summit->getId(),
        $invitation->getId(),
        $attendee->getId(),
      );
  ...
  public function revokeReservedOrdersOlderThanNMinutes(int $minutes, int $max = 100): void {
    Log::debug(
      sprintf(
        "SummitOrderService::revokeReservedOrdersOlderThanNMinutes minutes %s max %s",
        $minutes,
        $max,
      ),
    );
    ...
    Log::debug(
      sprintf(
        "SummitOrderService::revokeReservedOrdersOlderThanNMinutes got %s orders to revoke",
        count($orders),
      ),
    );
  ...
  public function processOrder2Revoke(int $order_id): void {
    $this->tx_service->transaction(function () use ($order_id) {
      try {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::processOrder2Revoke processing order %s summit %s",
            $order->getId(),
            $summit->getId(),
          ),
        );
        ...
        if (is_null($payment_gateway)) {
          Log::warning(
            sprintf(
              "SummitOrderService::processOrder2Revoke Payment configuration is not set for summit %s",
              $summit->getId(),
            ),
          );
          return;
        ...
        Log::warning(
          sprintf(
            "SummitOrderService::processOrder2Revoke cancelling order reservation %s create at %s",
            $order->getNumber(),
            $order->getCreated()->format("Y-m-d h:i:sa"),
          ),
        );
        ...
        if (!empty($cart_id)) {
          ...
          if (!is_null($status)) {
            if (!$payment_gateway->canAbandon($status)) {
              Log::warning(
                sprintf(
                  "SummitOrderService::processOrder2Revoke reservation %s created at %s can not be cancelled external status %s",
                  $order->getId(),
                  $order->getCreated()->format("Y-m-d h:i:sa"),
                  $status,
                ),
        ...
        Log::warning(
          sprintf(
            "SummitOrderService::processOrder2Revoke order %s got cancelled",
            $order->getId(),
          ),
        );
      } catch (\Exception $ex) {
  ...
  public function renderTicketByFormat(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_integer($ticket_id)) {
        Log::debug(
          sprintf(
            "SummitOrderService::renderTicketByFormattrying to get ticket by id %s",
            $ticket_id,
          ),
        );
        $ticket = $this->ticket_repository->getByIdExclusiveLock(intval($ticket_id));
      ...
      if (is_null($ticket) && is_null($current_user)) {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::renderTicketByFormat trying to get ticket by hash %s",
            $ticket_id,
          ),
        );
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::renderTicketByFormat ticket id %s ticket status %s",
          $ticket->getId(),
          $ticket->getStatus(),
        ),
      );
  ...
  public function updateOrder(Summit $summit, int $order_id, array $payload): SummitOrder {
    return $this->tx_service->transaction(function () use ($summit, $order_id, $payload) {
      ...
      if (!empty($owner_email)) {
        Log::debug(sprintf("SummitOrderService::updateOrder new owner provided %s", $owner_email));
  ...
  public function deleteOrder(Summit $summit, int $order_id) {
    $this->tx_service->transaction(function () use ($summit, $order_id) {
      Log::debug(
        sprintf(
          "SummitOrderService::deleteOrder summit %s order id %s",
          $summit->getId(),
          $order_id,
        ),
      );
  ...
  public function addAttendeeBadgeFeature(
    ...
    return $this->tx_service->transaction(function () use ($summit, $ticket_id, $feature_id) {
      ...
      if ($badge->hasFeature($feature_type)) {
        throw new ValidationException(
          sprintf("Badge already has feature %s.", $feature_type->getName()),
        );
      }
  ...
  public function removeAttendeeBadgeFeature(
    ...
    return $this->tx_service->transaction(function () use ($summit, $ticket_id, $feature_id) {
      ...
      if (!$badge->hasFeature($feature_type)) {
        ...
        throw new ValidationException(
          sprintf("Badge does not have feature %s.", $feature_type->getName()),
        );
      }
   ...
   * @throws \Exception
   */
  private function getAttendeeBadge(
    Summit $summit,
    $ticket_id,
    Member $requestor,
  ): ?SummitAttendeeBadge {
    return $this->tx_service->transaction(function () use ($summit, $ticket_id, $requestor) {
      ...
      if (is_null($ticket) && is_string($ticket_id)) {
        ...
        if (is_null($ticket)) {
          $ticket = $this->ticket_repository->getByExternalAttendeeIdExclusiveLock(
            $summit,
            strval($ticket_id),
          );
        }
      ...
      if (!$ticket instanceof SummitAttendeeTicket || !$ticket->isActive()) {
        throw new EntityNotFoundException("Ticket not found.");
      }
   ...
   * @throws ValidationException
   */
  private function checkPrintingRights(
    Member $requestor,
    SummitAttendeeBadge $badge,
    SummitBadgeViewType $viewType,
  ): bool {
    // check rules
  ...
  public function getTicket(Summit $summit, $ticket_id): ?SummitAttendeeTicket {
    return $this->tx_service->transaction(function () use ($summit, $ticket_id) {
      Log::debug(
        sprintf(
          "SummitOrderService::getTicket summit %s ticket id %s",
          $summit->getId(),
          $ticket_id,
        ),
      );
  ...
  public function addTickets(Summit $summit, int $order_id, array $payload): SummitOrder {
    return $this->tx_service->transaction(function () use ($summit, $order_id, $payload) {
      ...
      $ticket_type = $this->ticket_type_repository->getByIdExclusiveLock(
        intval($payload["ticket_type_id"]),
      );

      if (
        is_null($ticket_type) ||
        !$ticket_type instanceof SummitTicketType ||
        $ticket_type->getSummitId() != $summit->getId()
      ) {
  ...
  private function createTicketsForOrder(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (is_null($default_badge_type)) {
        ...
        throw new ValidationException(
          sprintf("summit %s does not has a default badge type", $summit->getId()),
        );
      }
      ...
      for ($i = 0; $i < $ticket_qty; $i++) {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::createTicketsForOrder ticket number %s",
            $ticket->getNumber(),
          ),
        );
        ...
        if (!empty($promo_code)) {
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::createTicketsForOrder applying promo code %s",
              $pc->getCode(),
            ),
          );
          ...
          if (!$pc->canBeAppliedToOrder($order)) {
            throw new ValidationException(
              sprintf("Promo code %s is not valid for this order.", $pc->getCode()),
            );
          }
  ...
  public function updateTicketById(
    ...
    return $this->tx_service->transaction(function () use ($current_user, $ticket_id, $payload) {
      ...
      if (!$ticket->canEditTicket($current_user)) {
        throw new ValidationException(
          sprintf("Ticket %s can not be edited by current member", $ticket_id),
        );
      }
      ...
      if (!is_null($attendee)) {
        ...
        SummitAttendeeFactory::populate(
          ...
          $attendee,
          $payload,
          !empty($email) ? $this->member_repository->getByEmail($email) : null,
        );
        $attendee->addTicket($ticket);
        $attendee->updateStatus();
        if ($summit->isRegistrationSendTicketEmailAutomatically()) {
  ...
  public function updateTicket(
    ...
    [$ticket, $shouldSendInvitationEmail] = $this->tx_service->transaction(function () use (
      $summit,
      $order_id,
      $ticket_id,
      $payload,
    ...
    if (
      ...
      $summit->isRegistrationSendTicketEmailAutomatically() &&
      $ticket->hasOwner()
    ) {
  ...
  public function updateTicketsByOrderHash(string $order_hash, array $payload): SummitOrder {
    return $this->tx_service->transaction(function () use ($order_hash, $payload) {
      Log::debug(
        sprintf("SummitOrderService::updateTicketsByOrderHash order hash %s", $order_hash),
      );
      ...
      foreach ($tickets as $ticket_payload) {
        ...
        if (is_null($attendee) && !empty($email)) {
          Log::debug(
            sprintf("SummitOrderService::updateTicketsByOrderHash attendee does not exists"),
          );
          // try to create it
          ...
          if (is_null($attendee)) {
            Log::debug(
              sprintf(
                "SummitOrderService::updateTicketsByOrderHash creating new attendee for email %s",
                $email,
              ),
            );
            $attendee = new SummitAttendee();
  ...
  public function processTicketData(int $summit_id, string $filename) {
    ...
    Log::debug(
      sprintf("SummitOrderService::processTicketData summit %s filename %s", $summit_id, $filename),
    );
    ...
    $badge_data_present =
      $reader->hasColumn("badge_type_id") || $reader->hasColumn("badge_type_name");
    ...
    foreach ($reader as $idx => $row) {
      $this->tx_service->transaction(function () use (
        ...
        Log::debug(
          sprintf("SummitOrderService::processTicketData processing row %s", json_encode($row)),
        );
        ...
        if ($ticket_data_present) {
          Log::debug(
            "SummitOrderService::processTicketData - has ticket data present ... trying to get ticket",
          );
          ...
          if ($reader->hasColumn("number")) {
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData trying to get ticket by number %s",
                $row["number"],
              ),
            );
          ...
          if (is_null($ticket) && $reader->hasColumn("id")) {
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData trying to get ticket by id %s",
                $row["id"],
              ),
            );
        ...
        if ($attendee_data_present) {
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData - has attendee data present ... trying to get attendee %s",
              $attendee_email,
            ),
          );
          ...
          if (is_null($attendee) && !empty($attendee_email)) {
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData - attendee %s does not exists",
                $attendee_email,
              ),
            );
            ...
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData creating attendee with payload %s",
                json_encode($payload),
              ),
            );
        ...
        if (!is_null($attendee)) {
          if (is_null($ticket)) {
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData ticket is null, trying to create a new one",
              ),
            );
            ...
            if ($reader->hasColumn("ticket_type_name")) {
              Log::debug(
                sprintf(
                  "SummitOrderService::processTicketData trying to get ticket type by name %s",
                  $row["ticket_type_name"],
                ),
              );
              $ticket_type = $this->ticket_type_repository->getByType(
                $summit,
                $row["ticket_type_name"],
              );
            }
            ...
            if ($reader->hasColumn("ticket_promo_code")) {
              Log::debug(
                sprintf(
                  "SummitOrderService::processTicketData trying to get promo code by code %s",
                  $row["ticket_promo_code"],
                ),
              );
            ...
            if ($reader->hasColumn("promo_code_id")) {
              Log::debug(
                sprintf(
                  "SummitOrderService::processTicketData trying to get promo code by id %s",
                  $row["promo_code_id"],
                ),
              );
            ...
            if (is_null($ticket_type) && $reader->hasColumn("ticket_type_id")) {
              Log::debug(
                sprintf(
                  "SummitOrderService::processTicketData trying to get ticket type by id %s",
                  $row["ticket_type_id"],
                ),
              );
            ...
            if (!is_null($promo_code)) {
              Log::debug(
                sprintf(
                  "SummitOrderService::processTicketData adding promo code by code %s to offline order",
                  $promo_code->getId(),
                ),
              );
            ...
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData ticket exists. trying to re assign it ...",
              ),
            );
            ...
            if ($ticket->hasOwner() && $ticket->getOwnerEmail() != $attendee->getEmail()) {
              Log::debug(
                sprintf(
                  "SummitOrderService::processTicketData - reasigning ticket to attendee %s",
                  $attendee->getEmail(),
                ),
              );
            ...
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData assigning ticket %s to attendee %s",
                $ticket->getNumber(),
                $attendee->getEmail(),
              ),
            );
            ...
            if ($summit->isRegistrationSendTicketEmailAutomatically()) {
              Log::debug(
                sprintf(
                  "SummitOrderService::processTicketData sending invitation email to attendee %s",
                  $attendee->getEmail(),
                ),
              );
              $attendee->sendInvitationEmail($ticket);
        ...
        if (is_null($ticket)) {
          Log::warning(
            "SummitOrderService::processTicketData ticket is null stop current row processing.",
          );
          return;
        }
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::processTicketData - got ticket %s (%s)",
            $ticket->getId(),
            $ticket->getNumber(),
          ),
        );
        ...
        if (!$badge_data_present) {
          Log::warning(
            "SummitOrderService::processTicketData badge data is not present stop current row processing.",
          );
          return;
        }
        ...
        if ($reader->hasColumn("badge_type_id")) {
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData trying to get badge type by id %s",
              $row["badge_type_id"],
            ),
          );
        ...
        if (is_null($badge_type) && $reader->hasColumn("badge_type_name")) {
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData trying to get badge type by name %s",
              $row["badge_type_name"],
            ),
          );
        ...
        if (!is_null($badge_type)) {
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData - got badge type %s (%s)",
              $badge_type->getId(),
              $badge_type->getName(),
            ),
          );
        ...
        if (!$ticket->hasBadge()) {
          ...
          if (!is_null($badge_type)) {
            Log::warning(
              "SummitOrderService::processTicketData badge type is null stop current row processing.",
            );
            return;
          }
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData - ticket %s (%s) has not badge ... creating it",
              $ticket->getId(),
              $ticket->getNumber(),
            ),
          );
        ...
        foreach ($summit->getBadgeFeaturesTypes() as $featuresType) {
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData processing badge type feature %s for ticket %s",
              $feature_name,
              $ticket->getId(),
            ),
          );
          if (!$reader->hasColumn($feature_name)) {
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData badge type feature %s does not exists as column",
                $feature_name,
              ),
            );
            continue;
          ...
          if (!$mustAdd) {
            Log::debug(
              sprintf(
                "SummitOrderService::processTicketData badge type feature %s not set for ticket %s",
                $feature_name,
                $ticket->getId(),
              ),
            );
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData - ticket %s (%s) - trying to add new features to ticket badge (%s)",
              $ticket->getId(),
              $ticket->getNumber(),
              $feature_name,
            ),
          ...
          if (is_null($feature)) {
            Log::warning(
              sprintf(
                "SummitOrderService::processTicketData feature %s does not exist on summit %s",
                $feature,
                $summit->getId(),
              ),
            );
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::processTicketData badge type feature %s set for ticket %s",
              $feature_name,
              $ticket->getId(),
            ),
          );
    ...
    Log::debug(
      sprintf(
        "SummitOrderService::processTicketData deleting file %s from storage %s",
        $path,
        $this->download_strategy->getDriver(),
      ),
    );
  ...
  public function createOfflineOrder(Summit $summit, array $payload): SummitOrder {
    $order = $this->tx_service->transaction(function () use ($summit, $payload) {
      Log::debug(
        sprintf(
          "SummitOrderService::createOfflineOrder summit %s payload %s",
          $summit->getId(),
          json_encode($payload),
        ),
      );
      ...
      if (isset($payload["owner_id"])) {
        Log::debug(
          sprintf(
            "SummitOrderService::createOfflineOrder trying to get member by id %s",
            $payload["owner_id"],
          ),
        );
      ...
      if (is_null($owner) && isset($payload["owner_email"])) {
        Log::debug(
          sprintf(
            "SummitOrderService::createOfflineOrder trying to get member by email %s",
            $payload["owner_email"],
          ),
        );
        // if not try by email
      ...
      if (is_null($attendee) && isset($payload["owner_email"])) {
        Log::debug(
          sprintf(
            "SummitOrderService::createOfflineOrder trying to get attendee by email %s",
            $payload["owner_email"],
          ),
        );
        $attendee = $this->attendee_repository->getBySummitAndEmail(
          $summit,
          trim($payload["owner_email"]),
        );
        if (!is_null($attendee)) {
          Log::debug(
            sprintf(
              "SummitOrderService::createOfflineOrder found attendee %s (%s).",
              $attendee->getId(),
              $attendee->getEmail(),
            ),
          );
      ...
      if (is_null($attendee)) {
        ...
        //first name
        $first_name = isset($payload["owner_first_name"])
          ? trim($payload["owner_first_name"])
          : null;
        if (empty($first_name) && !is_null($owner) && !is_null($owner->getFirstName())) {
        ...
        if (empty($surname) && !is_null($owner) && !is_null($owner->getLastName())) {
      ...
      Log::debug(
        sprintf("SummitOrderService::createOfflineOrder order number %s", $order->getNumber()),
      );
    ...
    return $this->tx_service->transaction(function () use ($order) {
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::createOfflineOrder order number %s mark as paid",
          $order->getNumber(),
        ),
      );
      return $order;
  ...
  public function ingestExternalTicketData(Summit $summit, array $payload): void {
    ...
    IngestSummitExternalRegistrationData::dispatch($summit->getId(), $email_to);
  }
  ...
  public function processSummitOrderReminders(Summit $summit): void {
    Log::debug(
      sprintf("SummitOrderService::processSummitOrderReminders summit %s", $summit->getId()),
    );

    if ($summit->isEnded()) {
      Log::warning(
        sprintf(
          "SummitOrderService::processSummitOrderReminders - summit %s has ended already",
          $summit->getId(),
        ),
      );
      return;
    ...
    do {
      ...
      $orders = $this->tx_service->transaction(function () use ($summit, $page) {
        return $this->order_repository->getAllOrderThatNeedsEmailActionReminder(
          $summit,
          new PagingInfo($page, 100),
        );
      });
      ...
      foreach ($orders->getItems() as $order) {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::processSummitOrderReminders - summit %s order %s",
            $summit->getId(),
            $order->getId(),
          ),
        );
        ...
        try {
          ...
          if (
            $order_tickets->count() != 1 ||
            $order_tickets->first()->getOwnerEmail() !== $order->getOwnerEmail()
          ) {
        ...
        foreach ($order_tickets as $ticket) {
          try {
            if (!$ticket->isActive()) {
              Log::warning(
                sprintf(
                  "SummitOrderService::processSummitOrderReminders - summit %s order %s skipping ticket %s (not active)",
                  $summit->getId(),
                  $order->getId(),
                  $ticket->getId(),
                ),
  ...
  public function processOrderReminder(SummitOrder $order): void {
    $this->tx_service->transaction(function () use ($order) {
      ...
      if ($summit->isEnded()) {
        Log::warning(
          sprintf(
            "SummitOrderService::processOrderReminder - summit %s has ended already",
            $summit->getId(),
          ),
        );
        return;
      ...
      if (!$order->isPaid()) {
        Log::warning(
          sprintf(
            "SummitOrderService::processOrderReminder - order %s no need email reminder",
            $order->getId(),
          ),
        );
        return;
      ...
      foreach ($order->getTickets() as $ticket) {
        if (!$ticket->isActive()) {
          Log::warning(
            sprintf(
              "SummitOrderService::processOrderReminder - order %s skipping ticket %s ( NOT ACTIVE ).",
              $order->getId(),
              $ticket->getId(),
            ),
          );
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::processOrderReminder - last_action_date %s  utc_now %s",
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::processOrderReminder - last action date plus %s days %s  utc_now %s",
          $days_interval,
          $last_action_date->format("Y-m-d H:i:s"),
          $utc_now->format("Y-m-d H:i:s"),
        ),
      ...
      if ($last_action_date <= $utc_now) {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::processOrderReminder - sending reminder email for order %s",
            $order->getId(),
          ),
        );
  ...
  public function processTicketReminder(SummitAttendeeTicket $ticket): void {
    $this->tx_service->transaction(function () use ($ticket) {
      if (!$ticket->hasOwner()) {
        Log::warning(
          sprintf(
            "SummitOrderService::processTicketReminder ticket %s no need email reminder ( no owner )",
            $ticket->getId(),
          ),
        );
        return;
      ...
      if (!$ticket->isPaid()) {
        Log::warning(
          sprintf(
            "SummitOrderService::processTicketReminder ticket %s no need email reminder (not paid )",
            $ticket->getId(),
          ),
        );
        return;
      ...
      if (!$ticket->hasTicketType()) {
        Log::warning(
          sprintf(
            "SummitOrderService::processTicketReminder  ticket %s no need email reminder ( no type )",
            $ticket->getId(),
          ),
        );
        return;
      ...
      if ($attendee->isComplete()) {
        Log::warning(
          sprintf(
            "SummitOrderService::processTicketReminder  ticket %s no need email reminder",
            $ticket->getId(),
          ),
        );
        return;
      ...
      if ($summit->isEnded()) {
        Log::warning(
          sprintf(
            "SummitOrderService::processTicketReminder - summit %s has ended already",
            $summit->getId(),
          ),
        );
        return;
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::processTicketReminder days_interval is %s for summit %s",
          $days_interval,
          $summit->getId(),
        ),
      );
      ...
      Log::debug(
        sprintf(
          "SummitOrderService::processTicketReminder last_action_date %s now %s",
          $last_action_date->format("Y-m-d H:i:s"),
          $utc_now->format("Y-m-d H:i:s"),
        ),
      );
      if ($last_action_date <= $utc_now) {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::processTicketReminder sending reminder email for ticket %s",
            $ticket->getId(),
          ),
        );
        // regenerate hash
   ...
   * @return SummitAttendeeTicket|null
   */
  public function getMyTicketByOrderHash(
    Summit $summit,
    string $order_hash,
  ): ?SummitAttendeeTicket {
    return $this->tx_service->transaction(function () use ($summit, $order_hash) {
      ...
      if (!$ticket->canPubliclyEdit()) {
        Log::debug(
          sprintf(
            "SummitOrderService::getMyTicketByOrderHash regenerating hash for ticket %s",
            $ticket->getId(),
          ),
        );
        $ticket->generateHash();
  ...
  public function processOrderPaymentConfirmation(int $orderId): void {
    $this->tx_service->transaction(function () use ($orderId) {
      Log::debug(
        sprintf(
          "SummitOrderService::processOrderPaymentConfirmation: trying to get order id %s",
          $orderId,
        ),
      );
      ...
      if (is_null($order) || !$order instanceof SummitOrder) {
        Log::warning(
          sprintf(
            "SummitOrderService::processOrderPaymentConfirmation: order %s not found.",
            $orderId,
          ),
        );
      }
      ...
      if (!$order->hasOwner()) {
        ...
        Log::debug(
          sprintf(
            "SummitOrderService::processOrderPaymentConfirmation: trying to get email %s from db",
            $ownerEmail,
          ),
        );
        $member = $this->member_repository->getByEmail($ownerEmail);
        ...
        if (!is_null($member)) {
          ...
          Log::debug(
            sprintf(
              "SummitOrderService::processOrderPaymentConfirmation: member %s found at db",
              $ownerEmail,
            ),
          );
          $order->setOwner($member);
          ...
          if ($shouldSendTicketEmail && !$order->isOfflineOrder()) {
            Log::debug(
              "SummitOrderService::processOrderPaymentConfirmation: sending email to attendees",
            );
        ...
        if (is_null($user)) {
          ...
          if ($shouldSendOrderEmail && !$order->isOfflineOrder()) {
            Log::debug(
              "SummitOrderService::processOrderPaymentConfirmation - sending email to owner (NON REGISTERED)",
            );
            $this->sendSummitOrderOwnerInvitationEmail(
              $order,
              $this->member_service->emitRegistrationRequest(
  ...
  private function sendAttendeesInvitationEmail(SummitOrder $order): void {
    Log::debug(
      sprintf("SummitOrderService::sendAttendeesInvitationEmail order %s", $order->getId()),
    );
    ...
    foreach ($order->getTickets() as $ticket) {
      try {
        Log::debug(
          sprintf(
            "SummitOrderService::sendAttendeesInvitationEmail order %s ticket %s",
            $order->getId(),
            $ticket->getNumber(),
          ),
        );
        if (!$ticket->hasOwner()) {
          Log::debug(
            sprintf(
              "SummitOrderService::sendAttendeesInvitationEmail ticket %s has not owner set",
              $ticket->getNumber(),
            ),
          );
          continue;
