 ...
 */
final class SummitProposedScheduleAllowedLocationService extends AbstractService implements
  ISummitProposedScheduleAllowedLocationService {
  /**
    ...
    $this->tx_service->transaction(function () use ($track, $allowed_location_id) {
      ...
      if (is_null($allowed_location)) {
        throw new EntityNotFoundException(
          sprintf("Allowed Location %s not found", $allowed_location_id),
        );
      }
  ...
  public function addAllowedDayToProposedLocation(
    ...
    return $this->tx_service->transaction(function () use ($track, $allowed_location_id, $payload) {
      ...
      Log::debug(
        sprintf(
          "SummitProposedScheduleAllowedLocationService::addAllowedDayToProposedLocation day epoch %s",
          $day,
        ),
      );
      ...
      Log::debug(
        sprintf(
          "SummitProposedScheduleAllowedLocationService::addAllowedDayToProposedLocation day %s",
      ...
      Log::debug(
        sprintf(
          "SummitProposedScheduleAllowedLocationService::addAllowedDayToProposedLocation localDay %s",
      ...
      if (!is_null($opening_hour)) {
        [$hour, $minute] = Time::getHourAndMinutesFromInt($opening_hour);
        Log::debug(
          sprintf(
            "SummitProposedScheduleAllowedLocationService::addAllowedDayToProposedLocation opening_hour %s %s",
            $hour,
            $minute,
          ),
        );
        ...
        Log::debug(
          sprintf(
            "SummitProposedScheduleAllowedLocationService::addAllowedDayToProposedLocation start_local_date %s",
            $start_local_date->format("Y-m-d H:i:s"),
          ),
        );

        if (
          !$summit->dayIsOnSummitPeriod(
            $summit->convertDateFromTimeZone2UTC($start_local_date),
            false,
          )
        ) {
          throw new ValidationException(
      ...
      if (!is_null($closing_hour)) {
        [$hour, $minute] = Time::getHourAndMinutesFromInt($closing_hour);
        Log::debug(
          sprintf(
            "SummitProposedScheduleAllowedLocationService::addAllowedDayToProposedLocation closing_hour %s %s",
            $hour,
            $minute,
          ),
        );
        ...
        Log::debug(
          sprintf(
            "SummitProposedScheduleAllowedLocationService::addAllowedDayToProposedLocation end_local_date %s",
        ...
        if (
          !$summit->dayIsOnSummitPeriod(
            $summit->convertDateFromTimeZone2UTC($end_local_date),
            false,
          )
        ) {
          throw new ValidationException(
      ...
      return $allowed_location->addAllowedTimeFrame($day, $opening_hour, $closing_hour);
    });
  }
