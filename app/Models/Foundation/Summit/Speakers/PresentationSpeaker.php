...
class PresentationSpeaker extends SilverstripeBaseModel {
  ...
  public function addPresentation(Presentation $presentation) {
    if ($this->hasPresentationAssigned($presentation)) {
      return;
    }
  ...
  public function getAssignedPromoCodes() {
    $assignments = $this->createQuery(
      "SELECT o FROM models\summit\AssignedPromoCodeSpeaker o
  ...
  public function presentations($summit_id, $published_ones = true) {
    return $this->presentations
      ->filter(function (PresentationSpeakerAssignment $ps_assignment) use (
        $published_ones,
        $summit_id,
      ) {
  ...
  public function getPresentationsBySelectionPlanAndRole(SelectionPlan $selectionPlan, $role) {
    if ($role == self::ROLE_SPEAKER) {
      $res = $this->presentations
        ->filter(function (PresentationSpeakerAssignment $ps_assignment) use ($selectionPlan) {
          $presentation = $ps_assignment->getPresentation();
          if (is_null($presentation)) {
            return false;
          }
          if ($presentation->getSelectionPlanId() != $selectionPlan->getId()) {
            return false;
          }
          if ($presentation->getSummit()->getId() != $selectionPlan->getSummitId()) {
            return false;
          }
          if ($presentation->getModeratorId() == $this->getId()) {
            return false;
          }
          if ($presentation->getCreatorId() == $this->getMemberId()) {
            return false;
          }
          return true;
        })
  ...
  public function getPresentationsBySummitAndRole(Summit $summit, $role) {
    if ($role == self::ROLE_SPEAKER) {
      $res = $this->presentations
        ->filter(function (PresentationSpeakerAssignment $ps_assignment) use ($summit) {
          $presentation = $ps_assignment->getPresentation();
          if ($presentation->getSummit()->getId() != $summit->getId()) {
            return false;
          }
          if ($presentation->getModeratorId() == $this->getId()) {
            return false;
  ...
  public function hasPublishedRegularPresentations(
    ...
    return count(
      $this->getPublishedRegularPresentations($summit, $role, $include_sub_roles, $excluded_tracks),
    ) > 0;
  }
  ...
  public function getPublishedRegularPresentationIds(
    ...
    $ids = [];
    $alternatePresentations = $this->getPublishedRegularPresentations(
      $summit,
      $role,
      $include_sub_roles,
      $excluded_tracks,
  ...
  public function hasPublishedLightningPresentations(
    ...
    return count(
      $this->getPublishedLightningPresentations(
        $summit,
        $role,
        $include_sub_roles,
        $excluded_tracks,
  ...
  public function getAcceptedPresentations(
    ...
    if ($exclude_privates_tracks) {
      $private_track_groups = $this->createQuery(
        "SELECT pg from models\summit\PrivatePresentationCategoryGroup pg 
            JOIN pg.summit s
      ...
      foreach ($private_track_groups as $private_track_group) {
        ...
        if (count($current_private_tracks) == 0) {
          continue;
        }
  ...
  public function getAcceptedPresentationIds(
    ...
    $ids = [];
    $acceptedPresentations = $this->getAcceptedPresentations(
      $summit,
      $role,
      $include_sub_roles,
      $excluded_tracks,
  ...
  public function hasAlternatePresentations(
    ...
    return count(
      $this->getAlternatePresentations(
        $summit,
        $role,
        $include_sub_roles,
        $excluded_tracks,
  ...
  public function getAlternatePresentations(
    ...
    if ($include_sub_roles && $role == PresentationSpeaker::RoleModerator) {
      $presentations = $this->getAlternatePresentations(
        $summit,
        PresentationSpeaker::RoleSpeaker,
        $include_sub_roles,
        $excluded_tracks,
  ...
  public function getAlternatePresentationIds(
    ...
    $ids = [];
    $alternatePresentations = $this->getAlternatePresentations(
      $summit,
      $role,
      $include_sub_roles,
      $excluded_tracks,
  ...
  public function getRejectedPresentations(
    ...
    if ($include_sub_roles && $role == PresentationSpeaker::RoleModerator) {
      $presentations = $this->getUnacceptedPresentations(
        $summit,
        PresentationSpeaker::RoleSpeaker,
        true,
        $excluded_tracks,
  ...
  public function getRejectedPresentationIds(
    ...
    $ids = [];
    $rejectedPresentations = $this->getRejectedPresentations(
      $summit,
      $role,
      $include_sub_roles,
      $excluded_tracks,
  ...
  public function getUnacceptedPresentations(
    ...
    if ($exclude_privates_tracks) {
      ...
      foreach ($private_track_groups as $private_track_group) {
        ...
        $private_tracks = array_merge(
          $private_tracks,
          array_values($current_private_tracks->toArray()),
        );
      }
    }
    ...
    $query->setParameter("summit_id", $summit->getId())->setParameter("speaker_id", $this->id);
  ...
  public function getPublishedPresentationsByType(
    ...
    if ($exclude_privates_tracks) {
      ...
      foreach ($private_track_groups as $private_track_group) {
        $current_private_tracks = $private_track_group->getCategories();
        if ($current_private_tracks->count() == 0) {
          continue;
        }
        $private_tracks = array_merge($private_tracks, array_values($current_private_tracks));
      }
  ...
  public function moderated_presentations($summit_id, $published_ones = true) {
    return $this->moderated_presentations->filter(function ($p) use ($published_ones, $summit_id) {
  ...
  public function getPresentation($presentation_id): ?Presentation {
    $res = $this->presentations
      ->filter(function ($p) use ($presentation_id) {
  ...
  public function getAllPresentations($published_ones = true) {
    return $this->presentations(null, $published_ones)
      ->map(function ($entity) {
        return $entity;
      })
      ->toArray();
  ...
  public function getModeratedPresentations($summit_id, $published_ones = true) {
    return $this->moderated_presentations($summit_id, $published_ones)
      ->map(function ($entity) {
        return $entity;
      })
      ->toArray();
  ...
  public function getAllModeratedPresentations($published_ones = true) {
    return $this->moderated_presentations(null, $published_ones)
      ->map(function ($entity) {
        return $entity;
      })
      ->toArray();
   ...
   * @return $this
   */
  public function addSummitAssistance(
    PresentationSpeakerSummitAssistanceConfirmationRequest $assistance,
  ) {
    if ($this->summit_assistances->contains($assistance)) {
      return $this;
    }
  ...
  public function canBeEditedBy(Member $member): bool {
    Log::debug(
      sprintf(
        "PresentationSpeaker::canBeEditedBy member %s speaker member id %s",
        $member->getId(),
        $this->getMemberId(),
      ),
    );
    ...
    if ($member->isSummitAdmin()) {
      return true;
    }
    if ($this->getMemberId() == $member->getId()) {
      return true;
    }
    $criteria = Criteria::create();
    $criteria
  ...
  public function addAnnouncementSummitEmail(
    ...
    if ($this->announcement_summit_emails->contains($announcementSummitEmail)) {
      return;
    }
