...
final class SummitEventSecureStreamSerializer extends SilverStripeSerializer {
  ...
  public function serialize(
    ...
    if (Cache::tags(sprintf("secure_streams_%s", $summit->getId()))->has($key)) {
      $values = json_decode(
        Cache::tags(sprintf("secure_streams_%s", $summit->getId()))->get($key),
        true,
      );
      Log::debug(
        sprintf(
          "SummitEventSecureStreamSerializer::serialize cache hit for event %s",
          $event->getId(),
        ),
      );
      return $values;
    ...
    Cache::tags(sprintf("secure_streams_%s", $summit->getId()))->put(
      $key,
      json_encode($values),
      $stream_duration - self::TTL_SKEW,
    );
