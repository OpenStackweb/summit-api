 ...
 */
final class SummitEventTypeService extends AbstractService implements ISummitEventTypeService {
  /**
    ...
    $event_type = $this->tx_service->transaction(function () use ($summit, $payload) {
      ...
      if ($summit->hasEventType($type)) {
        throw new ValidationException(
          sprintf("Event type %s already exist on summit id %s.", $type, $summit->getId()),
        );
      }
      ...
      if (is_null($event_type)) {
        throw new ValidationException(sprintf("class_name %s is invalid.", $payload["class_name"]));
      }
  ...
  public function deleteEventType(Summit $summit, $event_type_id) {
    return $this->tx_service->transaction(function () use ($event_type_id, $summit) {
      ...
      if ($event_type->isDefault()) {
        throw new ValidationException(
          sprintf(
            "event type id %s is a default one and is not allowed to be deleted",
            $event_type_id,
          ),
        );
      }
      ...
      if (count($summit_events) > 0) {
        throw new ValidationException(
          sprintf(
            "event type id %s could not be deleted bc its assigned to published events on summit id %s",
            $event_type_id,
            $summit->getId(),
          ),
        );
