 ...
 */
final class SummitTicketTypeService extends AbstractService implements ISummitTicketTypeService {
  /**
   * @var IEventbriteAPI
   */
   ...
   */
  public function __construct(IEventbriteAPI $eventbrite_api, ITransactionService $tx_service) {
   ...
   */
  private static function getTicketTypeParams(Summit $summit, array $data): array {
    if (isset($data["badge_type_id"])) {
      $badge_type = $summit->getBadgeTypeById(intval($data["badge_type_id"]));
      if (is_null($badge_type)) {
        throw new EntityNotFoundException(
          sprintf("badge_type_id %s not found", $data["badge_type_id"]),
        );
      }
    ...
    $ticket_type = $this->tx_service->transaction(function () use ($summit, $data) {
      ...
      if (!is_null($former_ticket_type)) {
        throw new ValidationException(
          trans("validation_errors.SummitTicketTypeService.addTicketType.NameAlreadyExists"),
          [
            "name" => trim($data["name"]),
      ...
      $ticket_type = SummitTicketTypeFactory::build(
        $summit,
        self::getTicketTypeParams($summit, $data),
      );
      ...
      if ($summit->hasTicketTypes()) {
        ...
        if (!empty($currency) && !empty($summit_currency) && $summit_currency != $currency) {
          throw new ValidationException(
            sprintf("Ticket type should have same currency as summit (%s).", $summit_currency),
          );
        }
      }
    ...
    return $this->tx_service->transaction(function () use ($summit, $ticket_type_id, $data) {
      if (isset($data["name"])) {
        ...
        if (!is_null($former_ticket_type) && $former_ticket_type->getId() != $ticket_type_id) {
          throw new ValidationException(
            trans("validation_errors.SummitTicketTypeService.updateTicketType.NameAlreadyExists"),
            [
              "name" => trim($data["name"]),
              "summit_id" => $summit->getId(),
            ],
      ...
      if (is_null($ticket_type)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitTicketTypeService.updateTicketType.TicketTypeNotFound", [
      ...
      return SummitTicketTypeFactory::populate(
        $ticket_type,
        self::getTicketTypeParams($summit, $data),
      );
    });
  ...
  public function deleteTicketType(Summit $summit, $ticket_type_id) {
    return $this->tx_service->transaction(function () use ($summit, $ticket_type_id) {
      ...
      if (is_null($ticket_type)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitTicketTypeService.deleteTicketType.TicketTypeNotFound", [
            "ticket_type_id" => $ticket_type_id,
            "summit_id" => $summit->getId(),
          ]),
        );
    ...
    return $this->tx_service->transaction(function () use ($summit) {
      ...
      do {
        ...
        foreach ($response as $ticket_class) {
          Log::debug(
            sprintf(
              "SummitTicketTypeService::seedSummitTicketTypesFromEventBrite external ticket class %s",
              json_encode($ticket_class),
            ),
          );
          ...
          if (!is_null($old_ticket_type)) {
            ...
            $old_ticket_type->setDescription(
              isset($ticket_class["description"]) ? trim($ticket_class["description"]) : "",
            );
          ...
          $new_ticket_type->setDescription(
            isset($ticket_class["description"]) ? trim($ticket_class["description"]) : "",
          );
        ...
        foreach ($res as $ticket_type) {
          Event::dispatch(
            new SummitTicketTypeInserted($ticket_type->getId(), $ticket_type->getSummitId()),
          );
        }
