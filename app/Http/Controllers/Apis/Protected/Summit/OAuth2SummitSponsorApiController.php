...
final class OAuth2SummitSponsorApiController extends OAuth2ProtectedController {
    ...
    if (!is_null($summit)) {
      ...
      if (!is_null($current_member)) {
        ...
        if ($current_member->isSponsorUser()) {
          ...
          $filter->addFilterCondition(FilterElement::makeEqual("sponsor_id", $sponsor_ids, "OR"));
        }
      }
    }
   ...
   * @return \Illuminate\Http\JsonResponse|mixed
   */
  public function addSponsorCarouselAdvertiseImage(
    ...
    return $this->processRequest(function () use ($request, $summit_id, $sponsor_id) {
      ...
      $image = $this->service->addSponsorCarouselAdvertiseImage(
        $summit,
        intval($sponsor_id),
        $file,
      );
  ...
  public function addAdImage(LaravelRequest $request, $summit_id, $sponsor_id, $ad_id) {
    return $this->processRequest(function () use ($request, $summit_id, $sponsor_id, $ad_id) {
      ...
      $image = $this->service->addSponsorAdImage(
        $summit,
        intval($sponsor_id),
        intval($ad_id),
        $file,
  ...
  public function updateMaterial($summit_id, $sponsor_id, $material_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id, $material_id) {
      ...
      $payload = $this->getJsonPayload(
        SponsorMaterialValidationRulesFactory::buildForUpdate(),
        true,
      );

      $material = $this->service->updateSponsorMaterial(
        $summit,
        intval($sponsor_id),
        intval($material_id),
        $payload,
  ...
  public function addSocialNetwork($summit_id, $sponsor_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id) {
      ...
      $payload = $this->getJsonPayload(
        SponsorSocialNetworkValidationRulesFactory::buildForAdd(),
        true,
      );

      $social_network = $this->service->addSponsorSocialNetwork(
        $summit,
        intval($sponsor_id),
        $payload,
      );
  ...
  public function updateSocialNetwork($summit_id, $sponsor_id, $social_network_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id, $social_network_id) {
      ...
      $payload = $this->getJsonPayload(
        SponsorSocialNetworkValidationRulesFactory::buildForUpdate(),
        true,
      );

      $social_network = $this->service->updateSponsorSocialNetwork(
        $summit,
        intval($sponsor_id),
        intval($social_network_id),
        $payload,
      );
  ...
  public function deleteSocialNetwork($summit_id, $sponsor_id, $social_network_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id, $social_network_id) {
      ...
      $this->service->deleteSponsorSocialNetwork(
        $summit,
        intval($sponsor_id),
        intval($social_network_id),
      );
  ...
  public function getExtraQuestions($summit_id, $sponsor_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["id", "name", "label", "order", "type"];
      },
      function ($filter) use ($sponsor) {
        if ($filter instanceof Filter) {
          $filter->addFilterCondition(FilterElement::makeEqual("sponsor_id", $sponsor->getId()));
  ...
  public function getMetadata($summit_id) {
    ...
    return $this->ok($this->sponsor_extra_question_repository->getQuestionsMetadata());
  }
  ...
  public function addExtraQuestion($summit_id, $sponsor_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id) {
      ...
      $payload = $this->getJsonPayload(
        SponsorExtraQuestionValidationRulesFactory::buildForAdd(),
        true,
      );

      $extra_question = $this->service->addSponsorExtraQuestion(
        $summit,
        intval($sponsor_id),
        $payload,
      );
  ...
  public function updateExtraQuestion($summit_id, $sponsor_id, $extra_question_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id, $extra_question_id) {
      ...
      $payload = $this->getJsonPayload(
        SponsorExtraQuestionValidationRulesFactory::buildForUpdate(),
        true,
      );

      $extra_question = $this->service->updateSponsorExtraQuestion(
        $summit,
        intval($sponsor_id),
  ...
  public function deleteExtraQuestion($summit_id, $sponsor_id, $extra_question_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id, $extra_question_id) {
      ...
      $this->service->deleteSponsorExtraQuestion(
        $summit,
        intval($sponsor_id),
        intval($extra_question_id),
      );
    ...
    return $this->processRequest(function () use (
      ...
      return $this->_delete(
        ...
        function ($value_id, $summit, $sponsor_id, $extra_question_id) {
          $this->service->deleteExtraQuestionValue(
            $summit,
            intval($sponsor_id),
            intval($extra_question_id),
            intval($value_id),
  ...
  public function addLeadReportSettings($summit_id, $sponsor_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id) {
      ...
      $payload = $this->getJsonPayload(
        LeadReportSettingsValidationRulesFactory::buildForAdd(),
        true,
      );
  ...
  public function updateLeadReportSettings($summit_id, $sponsor_id) {
    return $this->processRequest(function () use ($summit_id, $sponsor_id) {
      ...
      $payload = $this->getJsonPayload(
        LeadReportSettingsValidationRulesFactory::buildForUpdate(),
        true,
      );
