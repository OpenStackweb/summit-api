...
/**
 * Covers ProcessAttendeesEmailRequestJob::handle()'s resume activation: on a retry (attempts() >
 * 1) with a dispatched_at already stamped in the payload, handle() must add resume_since =
 * dispatched_at before calling send(). Attendees have no should_resend concept (unlike speakers -
 * see Task 3's Context for Implementer), so this covers only the resume_since half of the
 * speaker precedent.
 *
 ...
 *
 * Class ProcessAttendeesEmailRequestJobResumeTest
 */
final class ProcessAttendeesEmailRequestJobResumeTest extends TestCase
{
    private function jobWithAttempts(array $payload, int $attempts): ProcessAttendeesEmailRequestJob
    {
        $job = new ProcessAttendeesEmailRequestJob(new Summit(), $payload, null);
    ...
    public function testHandleOnSecondAttemptSetsResumeSince(): void
    {
        $payload = [
        ...
        $service = Mockery::mock(IAttendeeService::class);
        $service->shouldReceive('send')->once()->withArgs(function ($summit_id, $sentPayload) {
    ...
    public function testHandleOnSecondAttemptWithoutDispatchedAtSetsNoResumeSince(): void
    {
        ...
        // re-run instead. Same deploy-window gap the speaker precedent documents.
        $payload = [
            'email_flow_event' => 'SUMMIT_REGISTRATION_GENERIC_ATTENDEE_EMAIL',
            'attendees_ids'    => [11, 22],
        ];
     ...
     * execution of the same chunk, which the resume_since check cannot protect against.
     */
    public function testTimeoutStaysStrictlyBelowRetryAfterForEveryQueueConnection(): void
    {
        $job = new ProcessAttendeesEmailRequestJob(new Summit(), [], null);
