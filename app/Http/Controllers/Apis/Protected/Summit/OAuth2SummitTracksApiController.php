...
final class OAuth2SummitTracksApiController extends OAuth2ProtectedController {
  ...
  public function getAllBySummitCSV($summit_id) {
    ...
    try {
      ...
      if (Request::has("order")) {
        $order = OrderParser::parse(Request::input("order"), ["id", "code", "title", "order"]);
      }
  ...
  public function getTrackAllowedTagsBySummit($summit_id, $track_id) {
    try {
      ...
      foreach ($res["data"] as $allowed_tag) {
        ...
        $res["data"][$i]["track_tag_group"] = SerializerRegistry::getInstance()
  ...
  public function copyTracksToSummit($summit_id, $to_summit_id) {
    try {
      $from_summit = SummitFinderStrategyFactory::build(
        $this->summit_repository,
        $this->resource_server_context,
      )->find($summit_id);
      if (is_null($from_summit)) {
        return $this->error404();
      }
      ...
      $response = new PagingResponse(count($tracks), count($tracks), 1, 1, $tracks);
  ...
  public function updateTrackBySummit($summit_id, $track_id) {
    try {
      ...
      $validation = Validator::make(
        $data->all(),
        SummitTrackValidationRulesFactory::buildForUpdate(),
      );
  ...
  public function addSubTrack($summit_id, $track_id, $child_track_id) {
    return $this->processRequest(function () use ($summit_id, $track_id, $child_track_id) {
      ...
      $track = $this->track_service->addSubTrack(
        $summit,
        intval($track_id),
        intval($child_track_id),
        $payload,
      );
