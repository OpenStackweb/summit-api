 ...
 */
final class ScheduleService extends AbstractPublishService implements IScheduleService {
  /**
  ...
  public function __construct(
    ISummitProposedScheduleRepository $schedule_repository,
  ...
  private function isAuthorizedUser(
    ...
    if ($summit->isSummitAdmin($member)) {
      return true;
    }
    if (!is_null($category)) {
   ...
   */
  private function checkTransitionTime(
    IPublishableEvent $event,
    SummitProposedSchedule $schedule,
  ): void {
    /*
    ...
    if (!is_null($transition_time)) {
      // check immediate previous one event
      ...
      if (!is_null($prevProposedScheduledEvent)) {
        ...
        if (
          ($event->getStartDate()->getTimestamp() -
            $prevProposedScheduledEvent->getEndDate()->getTimestamp()) /
            60 <
          $transition_time
        ) {
          throw new ValidationException(
            "There must be a transition time of at least {$transition_time} " .
              "minutes between the end of the previous event {$prevProposedScheduledEvent->getSummitEventId()} ( {$prevProposedScheduledEvent->getLocalEndDate()->format(
                "Y-m-d H:i:s",
              )} ) " .
              "and the start of the current one {$event->getSummitEventId()} ( {$event->getLocalStartDate()->format(
                "Y-m-d H:i:s",
              )} ).",
          );
        }
    ...
    if (!is_null($nextProposedScheduledEvent)) {
      Log::debug(
        sprintf(
          "ScheduleService::checkTransitionTime nextProposedScheduledEvent %s",
      ...
      if (!is_null($transition_time)) {
        ...
        if (
          $nextProposedScheduledEvent->getStartDate() <= $event->getEndDate() ||
          ($nextProposedScheduledEvent->getStartDate()->getTimestamp() -
            $event->getEndDate()->getTimestamp()) /
            60 <
            $transition_time
        ) {
          throw new ValidationException(
            "There must be a transition time of at least {$transition_time} " .
              "minutes between the end of the current event {$event->getSummitEventId()} ( {$event->getLocalEndDate()->format(
                "Y-m-d H:i:s",
              )} ) and the start of the next " .
              "one {$nextProposedScheduledEvent->getSummitEventId()} ( {$nextProposedScheduledEvent->getLocalStartDate()->format(
                "Y-m-d H:i:s",
              )} ).",
          );
        }
  ...
  public function publishProposedActivityToSource(
    ...
    $schedule = $this->tx_service->transaction(function () use (
      $source,
      $presentation_id,
      $payload,
    ) {
      ...
      $schedule = $this->schedule_repository->getBySourceAndSummitId(
        $source,
        $event->getSummitId(),
      );
      ...
      if ($event instanceof Presentation) {
        ...
        if (!$selection_plan instanceof SelectionPlan) {
          throw new EntityNotFoundException(
            "presentation id {$presentation_id} does not have a selection plan",
          );
        }

        if (!$selection_plan->isAllowProposedSchedules()) {
          throw new ValidationException(
            "selection plan id {$selection_plan->getId()} does not allow proposed schedules",
          );
        }
      }
   ...
   * @throws ValidationException
   */
  protected function validateBlackOutTimesAndTimes(
    IPublishableEvent $publishable_event,
    ...
    if (
      $allowed_location instanceof SummitProposedScheduleAllowedLocation &&
      $allowed_location->hasTimeFrameRestrictions()
    ) {
  ...
  public function publishProposedActivity(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!$event instanceof SummitEvent) {
        throw new EntityNotFoundException(
          sprintf("event id %s does not exists!", $presentation_id),
        );
      }
      ...
      if ($schedule->hasLockFor($category)) {
        throw new ValidationException(
          "This track is locked, its has been already submited for review.",
        );
      }
  ...
  public function unPublishProposedActivity(string $source, int $presentation_id): void {
    $this->tx_service->transaction(function () use ($source, $presentation_id) {
      ...
      if (is_null($schedule_event)) {
        throw new EntityNotFoundException(
          sprintf("schedule event for event id %s does not exists!", $presentation_id),
        );
      }
  ...
  public function publishAll(
    ...
    Log::debug(
      sprintf(
        "ScheduleService::publishAll summit id %s filter %s",
        $summit_id,
    ...
    do {
      Log::debug(
        sprintf(
          "ScheduleService::publishAll summit id %s filter %s processing page %s",
          $summit_id,
          is_null($filter) ? "" : $filter->__toString(),
          $page,
        ),
      ...
      $ids = $this->tx_service->transaction(function () use (
        ...
        if (isset($payload["event_ids"])) {
          Log::debug(
            sprintf(
              "ScheduleService::publishAll summit id %s event_ids %s",
              $summit_id,
              json_encode($payload["event_ids"]),
            ),
          );
          $res = [];
          foreach ($payload["event_ids"] as $event_id) {
            $proposed = $this->proposed_events_repository->getBySummitSourceAndEventId(
              $summit_id,
              $source,
              $event_id,
            );
        ...
        Log::debug(
          sprintf("ScheduleService::publishAll summit id %s getting by filter", $summit_id),
        );
        if (is_null($filter)) {
          $filter = new Filter();
        }
        ...
        return $this->proposed_events_repository->getAllIdsByPage(
          new PagingInfo($page, $maxPageSize),
          $filter,
        );
      });
      ...
      Log::debug(
        sprintf(
          "ScheduleService::publishAll summit id %s filter %s page %s got %s records",
          $summit_id,
          is_null($filter) ? "" : $filter->__toString(),
          $page,
          count($ids),
      ...
      if (!count($ids)) {
        ...
        Log::debug(
          sprintf(
            "ScheduleService::publishAll summit id %s page is empty, ending processing.",
            $summit_id,
          ),
        );
        break;
      ...
      foreach ($ids as $proposed_id) {
        try {
          $this->tx_service->transaction(function () use (
            ...
            Log::debug(
              sprintf("ScheduleService::publishAll processing proposed id  %s", $proposed_id),
            );
            ...
            if (!$proposed instanceof SummitProposedScheduleSummitEvent) {
              Log::debug(
                sprintf(
                  "ScheduleService::publishAll skipping processing of proposed_id %s.",
                  $proposed_id,
                ),
              );
              return;
  ...
  public function send2Review(
    ...
    Log::debug(
      sprintf(
        "ScheduleService::send2Review summit id %s track id %s payload %s",
        $summit->getId(),
        $track_id,
        json_encode($payload),
      ),
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!$track instanceof PresentationCategory) {
        throw new EntityNotFoundException(
          "Can not find a presentation category with id {$track_id} for summit id {$summit->getId()}.",
        );
      }
      ...
      if (!$schedule instanceof SummitProposedSchedule) {
        throw new EntityNotFoundException(
          "Can not find a proposed schedule for summit id {$summit->getId()} and source {$source}.",
        );
      }
      ...
      if (!$track_chair instanceof SummitTrackChair) {
        throw new EntityNotFoundException(
          "Can not find a track chair for current member {$member->getId()}.",
        );
      }

      if (!$summit->isTrackChair($member, $track)) {
        throw new ValidationException(
          "Current member is not a track chair for track id {$track->getId()}.",
        );
      }
  ...
  public function removeReview(
    ...
    Log::debug(
      sprintf(
        "ScheduleService::removeReview summit id %s track id %s payload %s",
        $summit->getId(),
        $track_id,
        json_encode($payload),
      ),
