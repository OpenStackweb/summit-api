 ...
 */
final class SummitTrackService extends AbstractService implements ISummitTrackService {
  /**
  ...
  public function addTrack(Summit $summit, array $data) {
    return $this->tx_service->transaction(function () use ($summit, $data) {
      if (!empty($data["code"])) {
        ...
        if (!is_null($former_track)) {
          throw new ValidationException(
            sprintf(
              "track id %s already has code %s assigned on summit id %s",
              $former_track->getId(),
              $data["code"],
              $summit->getId(),
            ),
      ...
      if (!is_null($former_track)) {
        throw new ValidationException(
          sprintf(
            "track id %s already has title %s assigned on summit id %s",
            $former_track->getId(),
            $data["name"],
            $summit->getId(),
          ),
      ...
      if (isset($data["allowed_tags"])) {
        foreach ($data["allowed_tags"] as $tag_value) {
          ...
          if (is_null($tackTagGroupAllowedTag)) {
            throw new ValidationException(
              sprintf(
                "allowed_tags : tag value %s is not allowed on current track tag groups for summit %s",
                $tag_value,
                $summit->getId(),
              ),
            );
    ...
    return $this->tx_service->transaction(function () use ($summit, $track_id, $data) {
      ...
      if (isset($data["allowed_access_levels"])) {
        ...
        foreach ($data["allowed_access_levels"] as $access_level_id) {
          Log::debug(
            sprintf(
              "SummitTrackService::UpdateTrack %s trying to add access level %s",
              $track_id,
              $access_level_id,
            ),
          );
          ...
          Log::debug(
            sprintf(
              "SummitTrackService::UpdateTrack %s added access level %s",
              $track_id,
              $access_level_id,
            ),
          );
  ...
  public function deleteTrack(Summit $summit, $track_id) {
    return $this->tx_service->transaction(function () use ($summit, $track_id) {
      ...
      if (count($summit_events) > 0) {
        throw new ValidationException(
          sprintf(
            "track id %s could not be deleted bc its assigned to published events on summit id %s",
            $track_id,
            $summit->getId(),
          ),
        );
  ...
  public function copyTracks(Summit $from_summit, Summit $to_summit) {
    return $this->tx_service->transaction(function () use ($from_summit, $to_summit) {
      if ($from_summit->getId() == $to_summit->getId()) {
        throw new ValidationException(
          trans("validation_errors.SummitTrackService.copyTracks.SameSummit"),
        );
      }
  ...
  public function addTrackExtraQuestion($track_id, $question_id) {
    return $this->tx_service->transaction(function () use ($track_id, $question_id) {
      ...
      if (is_null($track)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitTrackService.addTrackExtraQuestion.TrackNotFound", [
            "track_id" => $track_id,
          ]),
        );
      }
      ...
      if (is_null($track_question_template)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitTrackService.addTrackExtraQuestion.QuestionNotFound", [
  ...
  public function removeTrackExtraQuestion($track_id, $question_id) {
    return $this->tx_service->transaction(function () use ($track_id, $question_id) {
      ...
      if (is_null($track)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitTrackService.removeTrackExtraQuestion.TrackNotFound", [
            "track_id" => $track_id,
          ]),
        );
      }
      ...
      if (is_null($track_question_template)) {
        throw new EntityNotFoundException(
          trans("not_found_errors.SummitTrackService.removeTrackExtraQuestion.QuestionNotFound", [
            "question_id" => $question_id,
          ]),
        );
      }
  ...
  public function addTrackIcon(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (!in_array($file->extension(), $allowed_extensions)) {
        throw new ValidationException(
          "file does not has a valid extension ('png','jpg','jpeg','gif').",
        );
      }
