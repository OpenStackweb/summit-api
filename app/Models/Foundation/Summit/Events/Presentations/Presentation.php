...
class Presentation extends SummitEvent implements IPublishableEventWithSpeakerConstraint {
  ...
  public static function isAllowedField(string $type): bool {
    return in_array($type, Presentation::AllowedFields) || SummitEvent::isAllowedField($type);
  }
  ...
  public static function getAllowedEditableFields(): array {
    return array_merge(
      Presentation::AllowedEditableFields,
      SummitEvent::getAllowedEditableFields(),
    );
  }
  ...
  public function updateSpeakerOrder(PresentationSpeaker $speaker, int $order) {
    ...
    self::recalculateOrderForSelectable(
      ...
      $speaker_assignment,
      $order,
      PresentationSpeakerAssignment::class,
    );
  }
  ...
  public function getMediaUploadsCountByType(SummitMediaUploadType $media_upload_type): int {
    $res = $this->materials->filter(function ($element) use ($media_upload_type) {
      return $element instanceof PresentationMediaUpload &&
        $element->getMediaUploadTypeId() == $media_upload_type->getId();
    });
    return $res->count();
  }
  ...
  public function getSelectionStatus() {
    ...
    $session_sel = $this->createQuery(
      "SELECT sp from models\summit\SummitSelectedPresentation sp 
            JOIN sp.list l
    ...
    if (count($session_sel) > 1) {
      throw new ValidationException(
        sprintf("presentation %s has more than 1 (one) selection.", $this->id),
      );
    }
  ...
  public function canEdit(PresentationSpeaker $speaker): bool {
    if ($this->getCreatedById() == $speaker->getMemberId()) {
      return true;
    }
    if ($this->getModeratorId() == $speaker->getId()) {
      return true;
    }
    if ($this->isSpeaker($speaker)) {
      return true;
    }
    return false;
  }
  ...
  public function memberCanEdit(Member $member): bool {
    if ($this->getCreatedById() == $member->getId()) {
      return true;
    }
  ...
  public function fulfilMediaUploadsConditions(): bool {
    ...
    if ($summitMediaUploadCount > $this->getMediaUploadsMandatoryCount()) {
      return false;
    }
    return true;
  }
  ...
  public function fulfilSpeakersConditions(): bool {
    ...
    if ($type->isUseModerator()) {
      ...
      if ($type->isModeratorMandatory() && $min > $count) {
        return false;
      }
      if ($count > $max) {
        return false;
    ...
    if ($type->isUseSpeakers()) {
      ...
      if ($type->isAreSpeakersMandatory() && $min > $count) {
        return false;
      }
      if ($count > $max) {
        return false;
  ...
  public function isSubmitted(): bool {
    return $this->progress == Presentation::PHASE_COMPLETE &&
      $this->status == Presentation::STATUS_RECEIVED;
  }
  ...
  public function addTrackChairComment(Member $commenter, string $body, $is_public = true) {
    $comment = SummitPresentationComment::createComment($commenter, $this, $body, $is_public);
  ...
  public function addTrackChairNotification(Member $commenter, string $body) {
    $comment = SummitPresentationComment::createNotification($commenter, $this, $body);
  ...
  public function isGroupSelected($list_class = SummitSelectedPresentationList::Session): bool {
    return $this->createQuery(
      "SELECT COUNT(sp.id) from models\summit\SummitSelectedPresentation sp 
            JOIN sp.list l
            JOIN sp.presentation p
            WHERE p.id = :presentation_id 
            AND sp.collection = :collection
      ...
      ->setParameter("collection", SummitSelectedPresentation::CollectionSelected)
      ->setParameter("list_type", SummitSelectedPresentationList::Group)
      ->setParameter("list_class", $list_class)
   ...
   */
  public function getSelectionByMemberAndListClass(
    Member $member,
    $list_class = SummitSelectedPresentationList::Session,
  ): ?SummitSelectedPresentation {
    $criteria = Criteria::create();
    $criteria->where(Criteria::expr()->eq("member", $member));
    $selection = $this->selected_presentations
      ->matching($criteria)
      ->filter(function ($election) use ($list_class) {
   ...
   * @return string|null
   */
  public function getSelectionCurrentSelectionType(
    Member $member,
    $list_class = SummitSelectedPresentationList::Session,
  ): ?string {
  ...
  public function getSelectionMembers(string $collection_type): array {
    $selections = $this->selected_presentations
      ->filter(function ($selection) use ($collection_type) {
  ...
  public function hasMemberSelectionFor(Member $member, string $collection_type): bool {
    return $this->selected_presentations
      ->filter(function ($selection) use ($collection_type, $member) {
        $list = $selection->getList();
        return $list->getListType() === SummitSelectedPresentationList::Individual &&
          $selection->getCollection() == $collection_type &&
          $selection->getMemberId() == $member->getId();
      })
      ->count() > 0;
  }
  ...
  public function getVotesTotalPoints(): int {
    $res = $this->createQuery(
      "SELECT SUM(v.vote) from models\summit\PresentationVote v 
  ...
  public function getVotesCount(): int {
    $res = $this->createQuery(
      "SELECT COUNT(v.id) from models\summit\PresentationVote v 
            JOIN v.presentation p
            WHERE p.id = :presentation_id",
    )
      ->setParameter("presentation_id", $this->id)
  ...
  public function getVotesAverage(): float {
    $res = $this->createQuery(
      "SELECT AVG(v.vote) from models\summit\PresentationVote v 
            JOIN v.presentation p
            WHERE p.id = :presentation_id",
    )
      ->setParameter("presentation_id", $this->id)
  ...
  public function getPopularityScore(): float {
    ...
    return $this->getSelectorsCount() * $weight_select +
  ...
  public function addCategoryChangeRequest(
    ...
    if ($this->category_changes_requests->matching($criteria)->count() > 0) {
      throw new ValidationException(
        sprintf("There is already a pending category change request for this category."),
      );
    }

    if ($this->category->getId() == $newCategory->getId()) {
      throw new ValidationException(
        "This presentation already belongs to %s.",
  ...
  public function getRemainingSelectionsForMember(Member $member): int {
    $list = $this->category->getSelectionListByTypeAndOwner(
      SummitSelectedPresentationList::Individual,
      $member,
    );
    if (is_null($list) || !$list instanceof SummitSelectedPresentationList) {
   ...
   * @return PresentationAction|null
   */
  public function setCompletionByType(
    bool $complete,
   ...
   */
  public function setActionByType(
    PresentationActionType $presentation_action_type,
  ): PresentationAction {
    $action = $this->getActionByType($presentation_action_type);
    if (is_null($action)) {
      Log::debug(
        "Presentation::setActionByType creating new presentation action for type {$presentation_action_type->getLabel()}",
      );
  ...
  private function actionSort($a, $b) {
    if (!$a instanceof PresentationAction) {
      return 0;
    }
    if (!$b instanceof PresentationAction) {
      return 0;
    }
  ...
  public function getPresentationActions() {
    ...
    $res = $this->actions
      ->filter(function ($e) use ($selection_plan) {
        if (!$e instanceof PresentationAction) {
          return false;
        }
  ...
  public function getExtraQuestionAnswerByQuestion(
    SummitSelectionPlanExtraQuestionType $question,
  ): ?PresentationExtraQuestionAnswer {
    $answer = $this->extra_question_answers
      ->matching(Criteria::create()->where(Criteria::expr()->eq("question", $question)))
      ->first();
  ...
  public function addExtraQuestionAnswer(ExtraQuestionAnswer $answer): void {
    ...
    if ($this->extra_question_answers->contains($answer)) {
      return;
    }
  ...
  public function clearMediaUploads(): void {
    ...
    if ($mediaUploads->count()) {
      ...
      foreach ($mediaUploads as $mediaUpload) {
        ...
        Log::debug(
          sprintf(
            "Presentation::clearMediaUploads marking as to be deleted %s/%s ",
            $privatePath,
      ...
      Log::debug(
        sprintf(
          "Presentation::clearMediaUploads got payload %s, triggering background job ...",
          json_encode($payload),
        ),
      );
  ...
  private function getVotesRange(?int $begin_voting_date = null, ?int $end_voting_date = null) {
    ...
    if ($begin_voting_date != null) {
      $begin_voting_date = Carbon::createFromTimestamp(
        $begin_voting_date,
    ...
    if ($end_voting_date != null) {
      $end_voting_date = Carbon::createFromTimestamp(
        $end_voting_date,
        new \DateTimeZone(SilverstripeBaseModel::DefaultTimeZone),
      );
  ...
  public function getVoters($begin_voting_date = null, $end_voting_date = null): ArrayCollection {
    return $this->getVotesRange($begin_voting_date, $end_voting_date)->map(function (
      $attendeeVote,
    ) {
  ...
  public function castAttendeeVote(SummitAttendee $attendee): PresentationAttendeeVote {
    Log::debug(
      sprintf(
        "Presentation::castAttendeeVote attendee %s presentation %s",
        $attendee->getId(),
        $this->getId(),
      ),
    );
    ...
    if (
      $this->attendees_votes
        ->matching(Criteria::create()->where(Criteria::expr()->eq("voter", $attendee)))
        ->count() > 0
    ) {
      throw new ValidationException(
        sprintf("Attendee %s already vote on presentation %s", $attendee->getEmail(), $this->id),
      );
    }
  ...
  public function addTrackChairScore(PresentationTrackChairScore $score): void {
    if ($this->track_chairs_scores->contains($score)) {
      return;
    }
  ...
  public function getTrackChairAvgScoresPerRakingType(): array {
    Log::debug(
      sprintf("Presentation::getTrackChairAvgScoresPerRakingType presentation %s", $this->getId()),
    );
  ...
  public function canChangeAnswerValue(ExtraQuestionType $q): bool {
    if (!$q instanceof SummitSelectionPlanExtraQuestionType) {
      return false;
    }
    if (!$this->hasSelectionPlan()) {
      return false;
  ...
  public function getReviewStatus(): string {
    ...
    $submission_complete = in_array($presentation_status, [
      self::ReviewStatusReceived,
      self::ReviewStatusAccepted,
    ]);
    ...
    $submission_accepted = in_array($selection_status, [
      self::SelectionStatus_Alternate,
      self::SelectionStatus_Accepted,
    ]);
  ...
  public function getReviewStatusNice(): string {
    ...
    if ($review_status == self::ReviewStatusNoSubmitted) {
      return "Not Submitted";
    }
    if ($review_status == self::ReviewStatusInReview) {
      return "In Review";
    }
