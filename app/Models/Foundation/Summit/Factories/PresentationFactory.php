...
final class PresentationFactory {
  ...
  public static function populate(
    ...
    if (isset($payload["attendees_expected_learnt"])) {
      $presentation->setAttendeesExpectedLearnt(
        html_entity_decode($payload["attendees_expected_learnt"]),
      );
    }

    $presentation->setAttendingMedia(
      isset($payload["attending_media"])
        ? filter_var($payload["attending_media"], FILTER_VALIDATE_BOOLEAN)
        : 0,
    );
    ...
    if (isset($payload["disclaimer_accepted"]) && !empty($payload["disclaimer_accepted"])) {
      ...
      if ($disclaimer_accepted && !$presentation->isDisclaimerAccepted()) {
        $presentation->setDisclaimerAcceptedDate(new \DateTime("now", new \DateTimeZone("UTC")));
      }
    }
    ...
    if (isset($payload["links"])) {
      if (count($payload["links"]) > Presentation::MaxAllowedLinks) {
        throw new ValidationException(
          trans("validation_errors.PresentationService.saveOrUpdatePresentation.MaxAllowedLinks", [
    ...
    if ($presentation->hasSelectionPlan() && count($extra_questions)) {
      ...
      if (!$res) {
        throw new ValidationException(
          "You neglected to fill in all mandatory questions for the Presentation.",
        );
      }
    }
    return $presentation;
