...
final class FilterParser {
  ...
  public static function parse($filters, $allowed_fields = []) {
    ...
    foreach (
      $filters
      as $filter // parse Main Filters ( 1st grade )
    ) {
      // check main operator
      ...
      if (count($or_filters) > 1) {
        ...
        foreach ($or_filters as $of) {
          ...
          [$field, $op, $value] = self::filterExpresion($of);
          ...
          if (!in_array($op, $allowed_fields[$field])) {
            throw new FilterParserException(
              sprintf("%s op is not allowed for filter by field %s", $op, $field),
            );
          }
        ...
        [$field, $op, $value] = self::filterExpresion($filter);
  ...
  public static function filterExpresion(string $exp) {
    ...
    if (!preg_match("/\[\]|\(\)|>=|<=|<>|==|=\@|\@\@|<|>/i", $exp, $matches)) {
      throw new FilterParserException(
        sprintf("Invalid filter format %s (should be [:FIELD_NAME:OPERAND:VALUE]).", $exp),
      );
    }
    ...
    Log::debug(
      sprintf(
        "FilterParser::filterExpresion field %s op %s value %s",
        $field,
        $op,
        json_encode($value),
      ),
