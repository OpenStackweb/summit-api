...
class ImportEventSpeakerEmail extends AbstractSummitEmailJob {
  ...
  public function __construct(
    ...
    $payload = [];
    $payload[IMailTemplatesConstants::creator_full_name] = is_null($creator)
      ? ""
      : $creator->getFullName();
    $payload[IMailTemplatesConstants::creator_email] = is_null($creator)
      ? ""
      : $creator->getEmail();
    ...
    $payload[IMailTemplatesConstants::selection_plan_name] = is_null($selection_plan)
      ? ""
      : $selection_plan->getName();
    $payload[IMailTemplatesConstants::presentation_edit_link] = $presentation->getEditLink();
    $payload[IMailTemplatesConstants::until_date] = is_null($selection_plan)
      ? ""
      : $selection_plan->getSubmissionEndDate()->format("d F, Y");
    $payload[IMailTemplatesConstants::selection_process_link] = sprintf(
      "%s/app/%s/selection_process",
      $speaker_management_base_url,
      $summit->getRawSlug(),
    );
    $payload[
      IMailTemplatesConstants::speaker_management_link
    ] = EmailUtils::getSpeakerManagementLink($summit);
