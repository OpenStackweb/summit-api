 ...
 */
abstract class ExtraQuestionType extends SilverstripeBaseModel implements IOrderable {
  /**
  ...
  public function addValue(ExtraQuestionTypeValue $value) {
    if (!$this->allowsValues()) {
      throw new ValidationException(
        sprintf("%s type does not allow multiple values.", $this->type),
      );
    }
  ...
  public function allowValue(string $value): bool {
    if (empty($value) && !$this->isMandatory()) {
      return true;
    }
  ...
  public function addSubQuestionRule(SubQuestionRule $rule): void {
    if ($this->sub_question_rules->contains($rule)) {
      return;
    }
  ...
  public function addParentRule(SubQuestionRule $rule): void {
    if ($this->parent_rules->contains($rule)) {
      return;
    }
  ...
  public function isAnswered(ExtraQuestionAnswerSet $answers): bool {
    ...
    foreach ($this->parent_rules as $parent_rule) {
      if (
        !$parent_rule->isSubQuestionVisible(
          $answers->getAnswerFor($parent_rule->getParentQuestion()),
        )
      ) {
