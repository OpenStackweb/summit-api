...
class SummitMediaUploadMigratePrivateToPublicStorage extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            $this->info(sprintf("SummitMediaUploadMigratePrivateToPublicStorage::handle processing summit %s media upload type %s",  $summit_id, $media_upload_type_id));
            $start  = time();

            $res = $this->summit_service->migratePrivateStorage2PublicStorage(intval($summit_id), intval($media_upload_type_id));

            $end = time();
            $delta = $end - $start;
            $this->info(sprintf("SummitMediaUploadMigratePrivateToPublicStorage::handle execution call processed %s in %s seconds", $res, $delta));
