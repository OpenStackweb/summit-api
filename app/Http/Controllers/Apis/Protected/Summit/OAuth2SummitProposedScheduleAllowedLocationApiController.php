 ...
 */
final class OAuth2SummitProposedScheduleAllowedLocationApiController extends
  OAuth2ProtectedController {
  ...
  private function isCurrentUserAuth(Summit $summit, PresentationCategory $track): bool {
    ...
    if ($summit->isSummitAdmin($current_member)) {
      return true;
    }
    if ($summit->isTrackChair($current_member, $track)) {
      return true;
    }
    return false;
  }
  ...
  public function getAllAllowedLocationByTrack($summit_id, $track_id) {
    ...
    if (is_null($track) || !$track->isChairVisible()) {
      return $this->error404();
    }
  ...
  public function addAllowedLocationToTrack($summit_id, $track_id) {
    return $this->processRequest(function () use ($summit_id, $track_id) {
      ...
      $payload = $this->getJsonPayload(
        SummitProposedScheduleAllowedLocationValidationRulesFactory::buildForAdd(),
      );

      return $this->created(
        SerializerRegistry::getInstance()
          ->getSerializer($this->service->addProposedLocationToTrack($track, $payload))
          ->serialize(
  ...
  public function getAllowedLocationFromTrack($summit_id, $track_id, $location_id) {
    return $this->processRequest(function () use ($summit_id, $track_id, $location_id) {
      ...
      return $this->ok(
        SerializerRegistry::getInstance()
          ->getSerializer($allowed_location)
          ->serialize(
            SerializerUtils::getExpand(),
            SerializerUtils::getFields(),
            SerializerUtils::getRelations(),
  ...
  public function addTimeFrame2AllowedLocation($summit_id, $track_id, $location_id) {
    return $this->processRequest(function () use ($summit_id, $track_id, $location_id) {
      ...
      $payload = $this->getJsonPayload(
        SummitProposedScheduleAllowedDayValidationRulesFactory::buildForAdd(),
      );

      return $this->created(
        SerializerRegistry::getInstance()
          ->getSerializer(
            $this->service->addAllowedDayToProposedLocation($track, intval($location_id), $payload),
          )
          ->serialize(
            SerializerUtils::getExpand(),
            SerializerUtils::getFields(),
   ...
   * @return mixed
   */
  public function getTimeFrameFromAllowedLocation(
    $summit_id,
    $track_id,
    $location_id,
    $time_frame_id,
   ...
   * @return mixed
   */
  public function removeTimeFrameFromAllowedLocation(
    $summit_id,
    $track_id,
    $location_id,
    $time_frame_id,
    ...
    return $this->processRequest(function () use (
      ...
      $this->service->deleteAllowedDayToProposedLocation(
        $track,
        intval($location_id),
        intval($time_frame_id),
      );
   ...
   * @return mixed
   */
  public function updateTimeFrameFromAllowedLocation(
    $summit_id,
    $track_id,
    $location_id,
    $time_frame_id,
    ...
    return $this->processRequest(function () use (
      ...
      $payload = $this->getJsonPayload(
        SummitProposedScheduleAllowedDayValidationRulesFactory::buildForUpdate(),
      );

      return $this->updated(
        SerializerRegistry::getInstance()
          ->getSerializer(
            $this->service->updateAllowedDayToProposedLocation(
              $track,
              intval($location_id),
              intval($time_frame_id),
              $payload,
