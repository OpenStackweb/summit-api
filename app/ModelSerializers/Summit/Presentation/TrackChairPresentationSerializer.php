...
class TrackChairPresentationSerializer extends AdminPresentationSerializer {
  ...
  public function serialize(
    ...
    if ($summit_track_chair) {
      ...
      if (in_array("selected", $fields)) {
        $values["selected"] = $presentation->hasMemberSelectionFor(
          $summit_track_chair->getMember(),
          SummitSelectedPresentation::CollectionSelected,
        );
      }
      if (in_array("maybe", $fields)) {
        $values["maybe"] = $presentation->hasMemberSelectionFor(
          $summit_track_chair->getMember(),
          SummitSelectedPresentation::CollectionMaybe,
        );
      }
      if (in_array("pass", $fields)) {
        $values["pass"] = $presentation->hasMemberSelectionFor(
          $summit_track_chair->getMember(),
          SummitSelectedPresentation::CollectionPass,
        );
      }
    }
      ...
      foreach (explode(",", $expand) as $relation) {
        ...
        switch ($relation) {
          case "selectors":
            $selectors = [];
            foreach ($presentation->getSelectors() as $m) {
              $selectors[] = SerializerRegistry::getInstance()
                ->getSerializer($m)
                ->serialize(
          ...
          case "likers":
            $likers = [];
            foreach ($presentation->getLikers() as $m) {
              $likers[] = SerializerRegistry::getInstance()
                ->getSerializer($m)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "passers":
            $passers = [];
            foreach ($presentation->getPassers() as $m) {
              $passers[] = SerializerRegistry::getInstance()
                ->getSerializer($m)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "viewers":
            $viewers = [];
            foreach ($presentation->getMemberViewers() as $m) {
              $viewers[] = SerializerRegistry::getInstance()
                ->getSerializer($m)
                ->serialize(
                  AbstractSerializer::filterExpandByPrefix($expand, $relation),
                  AbstractSerializer::filterFieldsByPrefix($fields, $relation),
          ...
          case "comments":
            $comments = [];
            foreach ($presentation->getComments() as $comment) {
              $comments[] = SerializerRegistry::getInstance()
          ...
          case "category_changes_requests":
            $category_changes_requests = [];
            foreach ($presentation->getCategoryChangeRequests() as $request) {
              $category_changes_requests[] = SerializerRegistry::getInstance()
          ...
          case "track_chair_scores":
            if (!is_null($summit_track_chair)) {
              $track_chair_scores = [];
              foreach ($presentation->getTrackChairScoresBy($summit_track_chair) as $score) {
                $track_chair_scores[] = SerializerRegistry::getInstance()
                  ->getSerializer($score)
                  ->serialize(
                    AbstractSerializer::filterExpandByPrefix($expand, $relation),
                    AbstractSerializer::filterFieldsByPrefix($fields, $relation),
