 ...
 */
final class MemberService extends AbstractService implements IMemberService {
  // in secs
    ...
    return $this->tx_service->transaction(function () use ($member, $affiliation_id, $data) {
      ...
      if (is_null($affiliation)) {
        throw new EntityNotFoundException(
          sprintf(
            "affiliation id %s does not belongs to member id %s",
            $affiliation_id,
            $member->getId(),
          ),
        );
      ...
      if (isset($data["organization_id"])) {
        ...
        if (is_null($org)) {
          throw new EntityNotFoundException(
            sprintf("organization id %s not found", $data["organization_id"]),
          );
        }
  ...
  public function deleteRSVP(Member $member, $rsvp_id) {
    return $this->tx_service->transaction(function () use ($member, $rsvp_id) {
      ...
      if (is_null($rsvp)) {
        throw new EntityNotFoundException(
          sprintf("rsvp id %s does not belongs to member id %s", $rsvp_id, $member->getId()),
        );
      }
    ...
    return $this->tx_service->transaction(function () use ($member, $data) {
      ...
      if ($affiliation->isCurrent() && $affiliation->getEndDate() != null) {
        throw new ValidationException(
          sprintf("in order to set affiliation as current end_date should be null"),
        );
      }
  ...
  public function registerExternalUserById($user_external_id): Member {
    $member = $this->tx_service->transaction(function () use ($user_external_id) {
      ...
      if (is_null($user_data) || !isset($user_data["email"])) {
        Log::warning(
          sprintf(
            "MemberService::registerExternalUserById user_external_id %s does not exists.",
            $user_external_id,
          ),
        );
        throw new EntityNotFoundException(
      ...
      Log::debug(
        sprintf(
          "MemberService::registerExternalUserById trying to get user by external id %s",
          $user_external_id,
        ),
      );
      ...
      if (is_null($member)) {
        Log::debug(
          sprintf("MemberService::registerExternalUserById trying to get user by email %s", $email),
        );
      ...
      if (is_null($member)) {
        Log::debug(
          sprintf(
            "MemberService::registerExternalUserById %s does not exists , creating it ...",
            $email,
          ),
        );
        ...
        $member = MemberFactory::populateFromExternalProfile(
          $member,
          $user_external_id,
          $user_data,
        );
      ...
      Log::debug(
        sprintf(
          "MemberService::registerExternalUserById trying to get former registration request by email %s",
          $email,
        ),
      );
      ...
      if (!is_null($request) && $request->hasSpeaker()) {
        Log::debug(
          sprintf(
            "MemberService::registerExternalUserById got former registration request by email %s",
            $email,
          ),
        );
        ...
        if (!is_null($speaker)) {
          if (!$speaker->hasMember()) {
            Log::debug(
              sprintf(
                "MemberService::registerExternalUserById setting current member to speaker %s",
                $speaker->getId(),
              ),
            );
  ...
  public function deleteExternalUserById($user_external_id): void {
    $this->tx_service->transaction(function () use ($user_external_id) {
      Log::debug(
        sprintf(
          "MemberService::deleteExternalUserById trying to get user by external id %s",
          $user_external_id,
        ),
      );
      $member = $this->member_repository->getByExternalIdExclusiveLock(intval($user_external_id));
      ...
      Log::debug(
        sprintf(
          "MemberService::deleteExternalUserById deleting user %s (%s)",
          $member->getId(),
          $member->getEmail(),
        ),
      );
  ...
  public function synchronizeGroups(Member $member, array $groups): Member {
    return $this->tx_service->transaction(function () use ($member, $groups) {
      $val = $this->cache_service->getSingleValue(
        sprintf("member_%s_sync_groups", $member->getId()),
      );

      if (!empty($val)) {
        Log::debug(
          sprintf(
            "MemberService::synchronizeGroups member %s email %s synch already done",
            $member->getId(),
            $member->getEmail(),
          ),
        );
      ...
      Log::debug(
        sprintf(
          "MemberService::synchronizeGroups member %s email %s",
          $member->getId(),
          $member->getEmail(),
        ),
      );
      ...
      foreach ($member->getGroups() as $group) {
        ...
        if (!in_array($group->getCode(), $groups)) {
          ...
          if (
            $group->getCode() == IGroup::Administrators &&
            in_array(IGroup::SuperAdmins, $groups)
          ) {
            continue;
          }
          ...
          if (
            in_array($group->getCode(), [
              IGroup::FoundationMembers,
              IGroup::CommunityMembers,
              IGroup::TrackChairs,
              IGroup::Sponsors,
            ])
          ) {
            Log::debug(
              sprintf(
                "MemberService::synchronizeGroups skipping group %s removal",
                $group->getCode(),
              ),
            );
            continue;
          ...
          Log::debug(
            sprintf(
              "MemberService::synchronizeGroups member %s email %s marking group %s to remove (external) dues is not on member current groups",
              $member->getId(),
              $member->getEmail(),
              $group->getCode(),
            ),
      ...
      foreach ($groups2Remove as $externalGroup) {
        if (
          $externalGroup->getCode() === IGroup::SuperAdmins &&
          $member->belongsToGroup(IGroup::Administrators)
        ) {
          $group = $this->group_repository->getBySlug(IGroup::Administrators);
          if (!is_null($group)) {
            Log::debug(
              sprintf(
                "MemberService::synchronizeGroups member %s email %s removing from group %s due is also a super admin",
                $member->getId(),
                $member->getEmail(),
                $group->getCode(),
              ),
        ...
        Log::debug(
          sprintf(
            "MemberService::synchronizeGroups member %s email %s removing from group %s",
            $member->getId(),
            $member->getEmail(),
            $externalGroup->getCode(),
          ),
        );
      ...
      foreach ($groups as $code) {
        Log::debug(
          sprintf(
            "MemberService::synchronizeGroups member %s email %s processing group code %s",
            $member->getId(),
            $member->getEmail(),
            $code,
          ),
        ...
        if (!$member->belongsToGroup($code)) {
          ...
          if (is_null($group)) {
            ...
            Log::debug(
              sprintf(
                "MemberService::synchronizeGroups member %s email %s - group %s does not exists!, .. creating it ",
                $member->getId(),
                $member->getEmail(),
                $code,
              ),
          ...
          Log::debug(
            sprintf(
              "MemberService::synchronizeGroups member %s email %s adding to group %s",
              $member->getId(),
              $member->getEmail(),
              $code,
            ),
        ...
        if ($code === IGroup::SuperAdmins) {
          Log::debug(
            sprintf(
              "MemberService::synchronizeGroups member %s email %s is on group %s, should be added to group %s",
              $member->getId(),
              $member->getEmail(),
              $code,
          ...
          if (!$member->belongsToGroup(IGroup::Administrators)) {
            Log::debug(
              sprintf(
                "MemberService::synchronizeGroups member %s email %s is not on group %s",
                $member->getId(),
                $member->getEmail(),
                IGroup::Administrators,
              ),
  ...
  public function checkExternalUser(string $email) {
    ...
    Log::debug(
      sprintf("MemberService::checkExternalUser - trying to get member %s from user api", $email),
    );
    $user = $this->external_user_api->getUserByEmail($email);
    // check if primary email is the same if not disregard
    Log::debug(
      sprintf(
        "MemberService::checkExternalUser got entity %s for email %s",
        json_encode($user),
        $email,
      ),
    );
  ...
  public function emitRegistrationRequest(
    ...
    return $this->external_user_api->registerUser($email, $first_name, $last_name, $company);
  }
  ...
  public function signFoundationMembership(Member $member): Member {
    return $this->tx_service->transaction(function () use ($member) {
      if ($member->isFoundationMember()) {
        throw new ValidationException(
          sprintf("Member %s is already a foundation member", $member->getId()),
        );
      }
      ...
      if (is_null($document)) {
        throw new EntityNotFoundException(
          sprintf("Legal Document %s not found.", LegalAgreement::Slug),
        );
      }
  ...
  public function assocSummitOrders(int $member_id): void {
    $member = $this->tx_service->transaction(function () use ($member_id) {
      Log::debug(
        sprintf("MemberService::assocSummitOrders trying to get member id %s.", $member_id),
      );
    ...
    $this->tx_service->transaction(function () use ($member_id) {
      ...
      if (!is_null($orders)) {
        foreach ($orders as $order) {
          ...
          Log::debug(
            sprintf(
              "MemberService::assocSummitOrders got order %s for member %s",
              $order->getNumber(),
              $member_id,
            ),
          );
    ...
    $this->tx_service->transaction(function () use ($member_id) {
      ...
      if (!is_null($attendees)) {
        foreach ($attendees as $attendee) {
          ...
          Log::debug(
            sprintf(
              "MemberService::assocSummitOrders got attendee %s for member %s",
              $attendee->getId(),
              $member_id,
            ),
          );
  ...
  public function updatePendingRegistrationRequest(
    ...
    Log::debug(
      sprintf(
        "MemberService::updatePendingRegistrationRequest - sending new profile info to user api for member %s",
        $email,
      ),
    );
    ...
    if (!is_null($res) && isset($res["total"]) && $res["total"] > 0) {
      Log::debug(
        sprintf("MemberService::updatePendingRegistrationRequest res %s", json_encode($res)),
      );
      $entry = $res["data"][0];
      $this->external_user_api->updateUserRegistrationRequest(
        $entry["id"],
        $first_name,
        $last_name,
        $company_name,
