...
final class SummitAttendeeFactory {
  ...
  public static function populate(
    ...
    if (!is_null($member)) {
      Log::debug(
        sprintf(
          "SummitAttendeeFactory::populate setting member %s to attendee %s",
          $member->getId(),
          $member->getEmail(),
        ),
      );
    ...
    } elseif (isset($payload["company_id"]) && !is_null($payload["company_id"])) {
      ...
      if ($companyId > 0) {
        ...
        if (is_null($company)) {
          throw new ValidationException(
            sprintf(
              "company with id %d not found as a registered company for summit %d",
              $companyId,
              $summit->getId(),
            ),
          );
    ...
    if (count($extra_questions)) {
      ...
      if (!$res && $validate_extra_questions) {
        throw new ValidationException(
          "You neglected to fill in all mandatory questions for the attendee.",
        );
      }
    }
