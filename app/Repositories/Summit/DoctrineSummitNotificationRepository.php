 ...
 */
final class DoctrineSummitNotificationRepository extends SilverStripeDoctrineRepository implements
  ISummitNotificationRepository {
  /**
   * @return array
   */
  protected function getFilterMappings() {
    return [
      ...
      "last_edited" => sprintf(
        "n.last_edited:datetime_epoch|%s",
  ...
  public function getAllByPageByUserBySummit(
    ...
    if (!is_null($current_member)) {
      ...
      $query
        ->orWhere(
          sprintf(
            "r = :current_member and n.channel = '%s'",
            SummitPushNotificationChannel::Members,
          ),
        )
        ->setParameter("current_member", $current_member);

      if (count($groups_ids) > 0) {
        $query
          ->orWhere(
            sprintf(
              "g.id in (:groups_ids) and n.channel = '%s'",
              SummitPushNotificationChannel::Group,
            ),
          )
          ->setParameter("groups_ids", $groups_ids);
      }
      ...
      if (count($events_ids) > 0) {
        $query
          ->orWhere(
            sprintf(
              "e.id in (:events_id) and n.channel = '%s'",
              SummitPushNotificationChannel::Event,
            ),
          )
          ->setParameter("events_id", $events_ids);
        $query->orWhere(sprintf("n.channel = '%s'", SummitPushNotificationChannel::Attendees));
      }

      if (
        $current_member->hasSpeaker() &&
        $current_member->getSpeaker()->isSpeakerOfSummit($summit)
      ) {
   ...
   * @return PagingResponse
   */
  private function applyPaginationLogic(
    QueryBuilder $query,
    PagingInfo $paging_info,
    Filter $filter = null,
    Order $order = null,
  ...
  public function getAllByPageBySummit(
    ...
    return $this->applyPaginationLogic(
      $this->createBaseQuery($summit),
      $paging_info,
      $filter,
      $order,
    );
