...
trait SpeakersPromoCodeTrait {
  ...
  public function addUsage(string $owner_email, int $usage = 1) {
    Log::debug(
      sprintf(
        "SpeakersPromoCodeTrait::addUsage promo_code %s owner_email %s usage %s",
        $this->getId(),
        $owner_email,
        $usage,
      ),
    ...
    $existing_owner = $this->owners
      ->filter(function ($e) use ($owner_email) {
        return strtolower($e->getSpeaker()->getEmail()) == strtolower(trim($owner_email)) &&
          !$e->isRedeemed();
      })
      ->first();
    ...
    if (!$existing_owner instanceof AssignedPromoCodeSpeaker) {
      throw new ValidationException(
        "Can't find an owner with the email {$owner_email} for the promo_code.",
      );
    }
  ...
  public function removeUsage(int $to_restore, string $owner_email = null) {
    Log::debug(
      sprintf(
        "SpeakersPromoCodeTrait::removeUsage promo_code %s owner_email %s to_restore %s",
        $this->getId(),
        $owner_email,
        $to_restore,
      ),
    ...
    if ($owner_email == null) {
      throw new ValidationException(
        "Owner email is mandatory in order to remove usage for promo code {$this->getId()}.",
      );
    }
  ...
  public function checkSubject(string $email, ?string $company): bool {
    ...
    $existing_owner = $this->owners
      ->filter(function ($e) use ($email) {
        return strtolower($e->getSpeaker()->getEmail()) == strtolower(trim($email)) &&
          !$e->isRedeemed();
      })
      ->first();
  ...
  public function getQuantityUsed(): int {
    ...
    try {
      ...
      Log::debug(
        sprintf(
          "SpeakersPromoCodeTrait::getQuantityUsed promo_code %s quantity used %s",
          $this->getCode(),
          $res,
        ),
      );
  ...
  public function setEmailSent(string $recipient = null) {
    ...
    try {
      $existing_owner = $this->owners
        ->filter(function ($e) use ($recipient) {
          return strtolower($e->getSpeaker()->getEmail()) == strtolower(trim($recipient)) &&
            !$e->isSent();
        })
        ->first();
      ...
      if (!$existing_owner instanceof AssignedPromoCodeSpeaker) {
        throw new ValidationException(
          "Can't find an owner with the email {$recipient} for the promo_code.",
        );
      }
  ...
  public function isEmailSent(): bool {
    try {
      ...
      Log::debug(
        sprintf(
          "SpeakersPromoCodeTrait::isEmailSent promo_code %s not sent qty %s",
          $this->getCode(),
          $res,
        ),
      );
  ...
  public function isRedeemed(): bool {
    try {
      ...
      Log::debug(
        sprintf(
          "SpeakersPromoCodeTrait::isRedeemed promo_code %s not redeemed qty %s",
          $this->getCode(),
          $res,
        ),
      );
