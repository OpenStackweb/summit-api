 ...
 */
final class SponsoredProjectService extends AbstractService implements ISponsoredProjectService {
  /**
      ...
      if (isset($payload["parent_project_id"])) {
        ...
        if (is_null($parentProject) || !$parentProject instanceof SponsoredProject) {
          throw new EntityNotFoundException(
            sprintf("sponsored project parent %s not found.", $payload["parent_project_id"]),
          );
        }
      ...
      if (isset($payload["parent_project_id"])) {
        ...
        if ($parent_project_id > 0) {
          ...
          if (!$parentProject instanceof SponsoredProject) {
            throw new EntityNotFoundException(
              sprintf("sponsored project parent %s not found.", $parent_project_id),
            );
          }
  ...
  public function updateProjectSponsorshipType(
    ...
    return $this->tx_service->transaction(function () use ($project_id, $sponsorship_id, $payload) {
      ...
      if (isset($payload["name"])) {
        ...
        if (
          $formerProjectSponsorshipType &&
          $formerProjectSponsorshipType->getId() !== $sponsorship_id
        ) {
          throw new ValidationException(sprintf("sponsorship type %s already exists.", $name));
        }
      }
      ...
      if (
        is_null($projectSponsorshipType) ||
        !$projectSponsorshipType instanceof ProjectSponsorshipType
      ) {
        throw new EntityNotFoundException(sprintf("sponsorship type %s not found.", $project_id));
      }

      $projectSponsorshipType = ProjectSponsorshipTypeFactory::populate(
        $projectSponsorshipType,
        $payload,
      );
      ...
      if (
        isset($payload["order"]) &&
        intval($payload["order"]) != $projectSponsorshipType->getOrder()
      ) {
        // request to update order
        $sponsoredProject->recalculateProjectSponsorshipTypeOrder(
          $projectSponsorshipType,
    ...
    return $this->tx_service->transaction(function () use ($project_id, $sponsorship_id, $payload) {
      ...
      if (!is_null($oldSupportingCompany)) {
        throw new ValidationException(
          sprintf("Company %s already is a supporting company.", $payload["company_id"]),
        );
      }
      ...
      if (isset($payload["order"]) && intval($payload["order"]) != $supportingCompany->getOrder()) {
        // request to update order
        $projectSponsorshipType->recalculateSupportingCompanyOrder(
          $supportingCompany,
          intval($payload["order"]),
        );
      }
  ...
  public function updateCompanyToProjectSponsorshipType(
    ...
    return $this->tx_service->transaction(function () use (
      ...
      if (
        ...
        throw new EntityNotFoundException(
          sprintf("sponsorship type %s not found.", $sponsorship_id),
        );
      }
  ...
  public function addLogo(int $project_id, UploadedFile $file, $max_file_size = 10485760): File {
    return $this->tx_service->transaction(function () use ($project_id, $file, $max_file_size) {
      ...
      if (!in_array($file->extension(), $allowed_extensions)) {
        throw new ValidationException(
          "file does not has a valid extension ('png', 'jpg', 'jpeg', 'svg').",
        );
      }
      ...
      $logo = $this->file_uploader->build(
        $file,
        sprintf("sponsored-projects/%s/logos", $project->getId()),
        true,
      );
