...
class OAuth2SummitBadgeScanApiControllerTest extends ProtectedApiTestCase {
  ...
  public function testUpdateBadgeScan() {
    ...
    $data = [
      "extra_questions" => [["question_id" => 519, "answer" => "None"]],
    ];
  ...
  public function testExportSummitBadgeScansWithReportSettingsRestriction() {
    ...
    if (!$sponsor_question instanceof SummitSponsorExtraQuestionType) {
      self::fail();
    }
    ...
    $this->assertNotEmpty($content);
    $this->assertTrue(
      str_contains($content, AbstractSerializer::getCSVLabel($sponsor_question->getLabel())),
    );
