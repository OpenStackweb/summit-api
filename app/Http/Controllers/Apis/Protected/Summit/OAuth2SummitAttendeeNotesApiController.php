...
final class OAuth2SummitAttendeeNotesApiController extends OAuth2ProtectedController {
  ...
  public function getAttendeeNotes($summit_id, $attendee_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["id", "created", "author_fullname", "author_email"];
      },
  ...
  public function getAttendeeNote($summit_id, $attendee_id, $note_id) {
    return $this->processRequest(function () use ($summit_id, $attendee_id, $note_id) {
      ...
      if (is_null($attendee)) {
        return $this->error404("Attendee id {$attendee_id} not found in summit {$summit_id}.");
      }

      $attendee_note = $attendee->getNoteById(intval($note_id));
      if (is_null($attendee_note)) {
        return $this->error404(
          "Attendee note id {$note_id} not found for attendee {$attendee_id}.",
        );
      }

      return $this->ok(
        SerializerRegistry::getInstance()
          ->getSerializer($attendee_note, SerializerRegistry::SerializerType_Private)
          ->serialize(
  ...
  public function addAttendeeNote($summit_id, $attendee_id) {
    return $this->processRequest(function () use ($summit_id, $attendee_id) {
      ...
      $payload = $this->getJsonPayload(
        SummitAttendeeNoteValidationRulesFactory::buildForAdd(),
        true,
      );

      $note = $this->attendee_service->upsertAttendeeNote(
        $summit,
        $member,
        intval($attendee_id),
        null,
      ...
      return $this->created(
        SerializerRegistry::getInstance()
          ->getSerializer($note, SerializerRegistry::SerializerType_Private)
          ->serialize(
            SerializerUtils::getExpand(),
            SerializerUtils::getFields(),
            SerializerUtils::getRelations(),
  ...
  public function updateAttendeeNote($summit_id, $attendee_id, $note_id) {
    return $this->processRequest(function () use ($summit_id, $attendee_id, $note_id) {
      ...
      $payload = $this->getJsonPayload(
        SummitAttendeeNoteValidationRulesFactory::buildForUpdate(),
        true,
      );
