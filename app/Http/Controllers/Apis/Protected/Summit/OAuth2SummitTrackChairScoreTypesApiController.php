 ...
 */
final class OAuth2SummitTrackChairScoreTypesApiController extends OAuth2ProtectedController {
      ...
      function ($filter) use ($track_chair_rating_type) {
        if ($filter instanceof Filter) {
          $filter->addFilterCondition(
            FilterElement::makeEqual("type_id", $track_chair_rating_type->getId()),
          );
        }
        return $filter;
      },
  ...
  public function addTrackChairScoreType($summit_id, $selection_plan_id, $type_id) {
    return $this->processRequest(function () use ($summit_id, $selection_plan_id, $type_id) {
      ...
      $track_chair_score_type = $this->service->addTrackChairScoreType(
        $selection_plan,
        intval($type_id),
        $payload,
      );
  ...
  public function updateTrackChairScoreType(
    ...
    return $this->processRequest(function () use (
      ...
      $track_chair_score_type = $this->service->updateTrackChairScoreType(
        $selection_plan,
        intval($type_id),
  ...
  public function deleteTrackChairScoreType(
    ...
    return $this->processRequest(function () use (
      ...
      $this->service->deleteTrackChairScoreType(
        $selection_plan,
        intval($type_id),
        intval($score_type_id),
      );
