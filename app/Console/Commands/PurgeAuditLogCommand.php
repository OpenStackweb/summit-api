...
class PurgeAuditLogCommand extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            $this->info(sprintf("PurgeAuditLogCommand::handle purge audit log for summit %s",  $summit_id));
            $start  = time();

            $this->repository->deleteOldLogEntries(intval($summit_id), Carbon::parse($date_backward_from)->toDate());

            $end = time();
            $delta = $end - $start;
            $this->info(sprintf("PurgeAuditLogCommand::handle execution call processed in %s seconds", $delta));
        } catch (Exception $ex) {
            Log::error($ex);
        }
    }
