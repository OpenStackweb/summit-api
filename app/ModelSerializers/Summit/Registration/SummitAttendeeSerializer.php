    ...
    return $this->cache(
      ...
      function () use ($expand, $fields, $relations, $params) {
        ...
        if (in_array("presentation_votes", $relations)) {
          ...
          foreach (
            $attendee->getPresentationVotes($beginVotingDate, $endVotingDate, $track_group_id)
            as $vote
          ) {
        ...
        if (!count($fields) || in_array("votes_count", $fields)) {
          $values["votes_count"] = $attendee->getVotesCount(
            $beginVotingDate,
            $endVotingDate,
            $track_group_id,
          );
            ...
            switch ($relation) {
              ...
              case "presentation_votes":
                ...
                foreach (
                  ...
                  $presentation_votes[] = SerializerRegistry::getInstance()
                    ->getSerializer($vote)
                    ->serialize(
                      AbstractSerializer::filterExpandByPrefix($expand, $relation),
                      AbstractSerializer::filterFieldsByPrefix($fields, $relation),
              ...
              case "company":
                if ($attendee->hasCompany()) {
                  ...
                  $values["company"] = SerializerRegistry::getInstance()
                    ->getSerializer($attendee->getCompany())
                    ->serialize(
                      AbstractSerializer::filterExpandByPrefix($expand, $relation),
                      AbstractSerializer::filterFieldsByPrefix($fields, $relation),
                      AbstractSerializer::filterFieldsByPrefix($relations, $relation),
