...
class SummitAttendee extends SilverstripeBaseModel {
  ...
  public function setSummitHallCheckedIn(bool $summit_hall_checked_in): void {
    ...
    $this->summit_hall_checked_in_date = $summit_hall_checked_in
  ...
  public function getEmittedFeedback() {
    return $this->member
      ->getFeedback()
      ->matching(Criteria::create()->orderBy(["id" => Criteria::ASC]));
  }
  ...
  public function sendRevocationTicketEmail(SummitAttendeeTicket $ticket) {
    ...
    if ($ticket->getOwner()->getId() != $this->getId()) {
      return;
    }
    $email = $this->getEmail();
    $key = md5($email);
  ...
  public function sendInvitationEmail(
    ...
    Log::debug(
      sprintf(
        "SummitAttendee::sendInvitationEmail attendee %s ticket %s delay %s",
        $email,
        $ticket->getId(),
        $delay,
      ),
    ...
    if ($ticket->getOwnerEmail() != $this->getEmail()) {
      return;
    }
    ...
    if (!$ticket->isPaid()) {
      Log::warning(
        sprintf(
          "SummitAttendee::sendInvitationEmail attendee %s ticket %s is not paid",
          $email,
          $ticket->getId(),
        ),
      );
    ...
    if (!$ticket->isActive()) {
      Log::warning(
        sprintf(
          "SummitAttendee::sendInvitationEmail attendee %s ticket %s is not active",
          $email,
          $ticket->getId(),
        ),
      );
    ...
    if ($this->isComplete()) {
      ...
      if (
        Cache::add(sprintf("%s_emit_ticket", $key), true, now()->addMinutes($delay)) ||
        $delay == 0
      ) {
        Log::debug(
          sprintf(
            "SummitAttendee::sendInvitationEmail attendee %s ticket %s sending SummitAttendeeTicketEmail",
            $email,
            $ticket->getId(),
          ),
        );
    ...
    Log::debug(
      sprintf(
        "SummitAttendee::sendInvitationEmail attendee %s ticket %s is not complete",
        $email,
        $ticket->getId(),
      ),
    );
    ...
    if (
      $order->getOwnerEmail() !== $ticket->getOwnerEmail() ||
      $overrideTicketOwnerIsSameAsOrderOwnerRule
    ) {
      // no delay
      // adds a threshold of 5 minutes to avoid duplicates emails
      if (
        Cache::add(sprintf("%s_edit_ticket", $key), true, now()->addMinutes($delay)) ||
        $delay == 0
      ) {
        Log::debug(
          sprintf(
            "SummitAttendee::sendInvitationEmail attendee %s ticket %s sending InviteAttendeeTicketEditionMail",
            $email,
            $ticket->getId(),
          ),
        );
  ...
  public function getFullName(): ?string {
    ...
    if (empty($fullname) && $this->hasMember()) {
      ...
      Log::debug(
        sprintf("SummitAttendee::getFullName id %s Member Full Name %s", $this->id, $fullname),
      );
      if (!empty($fullname)) {
        return $fullname;
      }
  ...
  public function getExtraQuestionAnswerByQuestion(
    ...
    $answer = $this->extra_question_answers
      ->matching($criteria = Criteria::create()->where(Criteria::expr()->eq("question", $question)))
      ->first();
   ...
   * @return string|null
   */
  public function getExtraQuestionAnswerValueByQuestion(
    SummitOrderExtraQuestionType $question,
  ): ?string {
    try {
      $sql = <<<SQL
  ...
  public function clearExtraQuestionAnswers(): void {
    Log::debug(
      sprintf("SummitAttendee::clearExtraQuestionAnswers for attendee %s", $this->getId()),
    );
  ...
  public function setCompanyName(?string $company_name): void {
    Log::debug(
      sprintf(
        "SummitAttendee::setCompanyName attendee %s company_name %s",
        $this->getId(),
        $company_name,
      ),
    );
  ...
  public function setCompany(Company $company): void {
    Log::debug(
      sprintf(
        "SummitAttendee::setCompany attendee %s company %s",
        $this->getId(),
        $company->getId(),
      ),
    );
  ...
  public function updateStatus(): string {
    ...
    if (!empty($this->external_id)) {
      ...
      Log::debug(
        sprintf(
          "SummitAttendee::updateStatus StatusComplete for attendee %s (External).",
          $this->id,
        ),
      );
      return $this->status;
    ...
    if ($is_disclaimer_mandatory && !$this->isDisclaimerAccepted()) {
      ...
      Log::debug(
        sprintf(
          "SummitAttendee::updateStatus StatusIncomplete for attendee %s (disclaimer mandatory)",
          $this->id,
        ),
      );
      return $this->status;
    ...
    if (empty($this->getFirstName())) {
      ...
      Log::debug(
        sprintf(
          "SummitAttendee::updateStatus StatusIncomplete for attendee %s (first name empty)",
          $this->id,
        ),
      );
      return $this->status;
    ...
    if (empty($this->getSurname())) {
      ...
      Log::debug(
        sprintf(
          "SummitAttendee::updateStatus StatusIncomplete for attendee %s (last name empty)",
          $this->id,
        ),
      );
      return $this->status;
    ...
    if (empty($this->getEmail())) {
      ...
      Log::debug(
        sprintf(
          "SummitAttendee::updateStatus StatusIncomplete for attendee %s (email empty)",
          $this->id,
        ),
      );
      return $this->status;
  ...
  public function checkAccessLevels(array $required_access_level_ids = []): bool {
    Log::debug(
      sprintf(
        "SummitAttendee::checkAccessLevels id %s access levels %s",
        $this->id,
        json_encode($required_access_level_ids),
      ),
    );

    if (count($required_access_level_ids) === 0) {
      return true;
    }
    // check access levels
    $isAuth = false;
  ...
  public function hasAccessLevel(string $access_level): bool {
    foreach ($this->tickets as $ticket) {
      ...
      if (!is_null($al)) {
        Log::debug(
          sprintf(
            "SummitAttendee::hasAccessLevel attendee %s has access level %s.",
            $this->id,
            $al->getName(),
          ),
        );
  ...
  public function addPresentationVote(PresentationAttendeeVote $vote) {
    if ($this->presentation_votes->contains($vote)) {
      return;
    }
  ...
  public function getVotesCount(
    ?int $begin_voting_date = null,
    ?int $end_voting_date = null,
    ?int $track_group_id = null,
  ): int {
   ...
   */
  private function getVotesRange(
    ?int $begin_voting_date = null,
    ?int $end_voting_date = null,
    ?int $track_group_id = null,
  ) {
    ...
    $res =
      $criteria != null
        ? $this->presentation_votes->matching($criteria)
        : $this->presentation_votes;
  ...
  public function getExtraQuestions(): array {
    $res = [];
    $eqts = $this->summit->getMainOrderExtraQuestionsByUsage(
      SummitOrderExtraQuestionTypeConstants::TicketQuestionUsage,
    );
    foreach ($eqts as $q) {
      if ($this->isAllowedQuestion($q)) {
        $res[] = $q;
      }
    }
    return $res;
  ...
  public function getQuestionById(int $questionId): ?ExtraQuestionType {
    ...
    if (!$this->isAllowedQuestion($question)) {
      return null;
    }
    return $question;
  }
  ...
  public function canChangeAnswerValue(ExtraQuestionType $q): bool {
    ...
    $currentUserIsAdmin = is_null($currentUser)
      ? false
      : $currentUser->isSummitAllowed($this->summit);
  ...
  public function getAllowedTicketTypes(bool $exclude_deactivated_tickets = true): array {
    ...
    foreach ($this->tickets as $ticket) {
      ...
      if ($exclude_deactivated_tickets && !$ticket->isActive()) {
        continue;
      }
  ...
  public function isAllowedQuestion(ExtraQuestionType $q): bool {
    ...
    if (
      $q->getAllowedTicketTypes()->count() === 0 &&
      $q->getAllowedBadgeFeatureTypes()->count() === 0
    ) {
      return true;
    }

    // attendee does not has any ticket nor badge feature
    if (count($allowed_ticket_type_ids) == 0 && count($allowed_badge_feature_ids) == 0) {
      return false;
    }

    // restricted question
    foreach ($q->getAllowedTicketTypes() as $question_ticket_type) {
      if (in_array($question_ticket_type->getId(), $allowed_ticket_type_ids)) {
        return true;
      }
    }

    foreach ($q->getAllowedBadgeFeatureTypes() as $question_badge_feature_type) {
      if (in_array($question_badge_feature_type->getId(), $allowed_badge_feature_ids)) {
        return true;
      }
    }
  ...
  public function getNoteById(int $note_id): ?SummitAttendeeNote {
    $note = $this->notes
      ->matching(Criteria::create()->where(Criteria::expr()->eq("id", $note_id)))
      ->first();
  ...
  public function getTagByName(string $tag_name): ?Tag {
    $tag = $this->notes
      ->matching(Criteria::create()->where(Criteria::expr()->eq("tag", $tag_name)))
      ->first();
    return $tag ? $tag : null;
  }
