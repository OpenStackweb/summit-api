...
final class SchedScheduleFeed extends AbstractExternalScheduleFeed {
  ...
  public function getEvents(): array {
    ...
    foreach ($response as $event) {
      ...
      $events[] = [
        ...
        "title" => isset($event["name"])
          ? trim(AbstractExternalScheduleFeed::convert2UTF8($event["name"]))
          : "",
        "abstract" => isset($event["description"])
          ? trim(AbstractExternalScheduleFeed::convert2UTF8($event["description"]))
          : "",
        "track" => isset($event["event_type"])
          ? trim(AbstractExternalScheduleFeed::convert2UTF8($event["event_type"]))
          : "",
        "location" => isset($event["venue"])
          ? trim(AbstractExternalScheduleFeed::convert2UTF8($event["venue"]))
          : "",
  ...
  public function getSpeakers(): array {
    ...
    foreach ($response as $speaker) {
      ...
      $speakerFullNameParts = explode(
        " ",
        AbstractExternalScheduleFeed::convert2UTF8($speaker["name"]),
      );
      $speakerLastName = trim(trim(array_pop($speakerFullNameParts)));
      $speakerFirstName = trim(implode(" ", $speakerFullNameParts));
      $email = isset($speaker["email"])
        ? trim(AbstractExternalScheduleFeed::convert2UTF8($speaker["email"]))
        : null;

      if (empty($email)) {
        $email = $this->getDefaultSpeakerEmail(
          trim(AbstractExternalScheduleFeed::convert2UTF8($speaker["name"])),
        );
      }
  ...
  public function getDefaultSpeakerEmail(string $speakerFullName): string {
    return sprintf(
      "%s@sched.com",
      ltrim(str_replace(",", "", str_replace(" ", "", strtolower($speakerFullName)))),
      ".",
    );
  }
}
