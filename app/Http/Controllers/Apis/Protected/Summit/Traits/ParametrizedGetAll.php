...
trait ParametrizedGetAll {
  ...
  protected function defaultQuery(
    ...
    ?Filter $filter,
    ?Order $order,
    ?callable $applyExtraFilters = null,
  ) {
    if (!is_null($applyExtraFilters)) {
      $filter = call_user_func($applyExtraFilters, $filter);
    }

    return $this->getRepository()->getAllByPage(new PagingInfo($page, $per_page), $filter, $order);
  }
  ...
  public function _getAll(
    ...
    return $this->processRequest(function () use (
      ...
      if (!is_null($queryCallable)) {
        ...
        $data = $this->defaultQuery($page, $per_page, $filter, $order, $applyExtraFilters);
      }
      ...
      return $this->ok(
        $data->toArray(
          ...
          $serializerParams,
          $serializerType && is_callable($serializerType)
