 ...
 */
final class ScheduleIngestionService extends AbstractService implements IScheduleIngestionService {
  /**
  ...
  public function ingestAllSummits(): void {
    ...
    foreach ($summits as $summit) {
      ...
      Log::debug(
        sprintf(
          "ScheduleIngestionService::ingestAllSummits trying to get all events to delete for summit %s...",
          $summit->getId(),
        ),
      );

      $summit_events = $this->tx_service->transaction(function () use (
        $summit,
        $processedExternalIds,
      ) {
        return $this->event_repository->getPublishedEventsBySummitNotInExternalIds(
          $summit,
          $processedExternalIds,
        );
      });
      ...
      foreach ($summit_events as $summit_event) {
        $this->tx_service->transaction(function () use ($summit_event) {
          try {
            Log::debug(
              sprintf(
                "ScheduleIngestionService::ingestAllSummits trying to delete event %s...",
                $summit_event->getId(),
              ),
            );
  ...
  public function ingestSummit(Summit $summit): array {
    ...
    try {
      ...
      Log::debug(
        sprintf("ScheduleIngestionService::ingestSummit:: ingesting summit %s", $summit->getId()),
      );
      ...
      $this->tx_service->transaction(function () use ($summit_id) {
        ...
        if ($mainVenues->count() == 0) {
          throw new ValidationException(
            sprintf("summit %s does not has a main venue set!", $summit->getId()),
          );
        }

        if (is_null($summit->getBeginDate()) || is_null($summit->getEndDate())) {
          throw new ValidationException(
            sprintf("summit %s does not has set begin date/end date", $summit->getId()),
          );
        }

        if (is_null($summit->getTimeZone())) {
          throw new ValidationException(
            sprintf("summit %s does not has set a valid time zone", $summit->getId()),
          );
        }
      ...
      foreach ($events as $event) {
        try {
          $external_id = $this->tx_service->transaction(function () use (
            $summit_id,
            $event,
            $speakers,
          ) {
            Log::debug(
              sprintf(
                "processing event %s - %s for summit %s",
                $event["external_id"],
                $event["title"],
                $summit_id,
              ),
            ...
            if (isset($event["speakers"])) {
              foreach ($event["speakers"] as $speakerFullName) {
                ...
                Log::debug(
                  sprintf(
                    "processing event %s - %s for summit %s - speaker %s",
                    $event["external_id"],
                    $event["title"],
                    $summit_id,
                    $speakerFullName,
                ...
                $foundSpeaker = isset($speakers[$speakerFullName])
                ...
                $speakerEmail =
                  $foundSpeaker && isset($foundSpeaker["email"]) ? $foundSpeaker["email"] : null;

                Log::debug(
                  sprintf(
                    "ScheduleIngestionService::ingestSummit event %s - %s for summit %s speakerEmail %s",
                    $event["external_id"],
                    $event["title"],
                    $summit_id,
                    $speakerEmail,
                ...
                $companyName =
                  $foundSpeaker && isset($foundSpeaker["company"])
                    ? $foundSpeaker["company"]
                    : null;
                $companyPosition =
                  $foundSpeaker && isset($foundSpeaker["position"])
                    ? $foundSpeaker["position"]
                    : null;
                $speakerTitle =
                  !empty($companyName) && !empty($companyPosition)
                    ? sprintf("%s, %s", $companyName, $companyPosition)
                    : null;
                // member
                $member = !empty($speakerEmail)
                  ? $this->member_repository->getByEmail($speakerEmail)
                  : $this->member_repository->getByFullName($speakerFullName);
                ...
                if (!is_null($speaker)) {
                  ...
                  if ($speaker->hasMember() && $speaker->getMemberId() !== $member->getId()) {
                    ...
                    Log::debug(
                      sprintf(
                        "ScheduleIngestionService::ingestSummit speaker alredy belongs to another user (%s) creating a new one",
                        $speaker->getMemberId(),
                      ),
                    );
                    $speaker = null;
      ...
      log::debug(
        sprintf(
          "ScheduleIngestionService::ingestSummit execution call %s seconds - summit %s",
          $delta,
          $summit->getId(),
        ),
      );
