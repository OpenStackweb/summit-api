    ...
    foreach ($response as $event) {
      ...
      if (isset($event["speakers"])) {
        foreach ($event["speakers"] as $speaker) {
          $speakerFullName =
            trim(AbstractExternalScheduleFeed::convert2UTF8($speaker["first_name"])) .
            " " .
            trim(AbstractExternalScheduleFeed::convert2UTF8($speaker["last_name"]));
          $speakers[] = $speakerFullName;
          if (!isset($this->speakers[$speakerFullName])) {
            $this->speakers[$speakerFullName] = [
              "full_name" => $speakerFullName,
              "first_name" => trim(
                AbstractExternalScheduleFeed::convert2UTF8($speaker["first_name"]),
              ),
              "last_name" => trim(
                AbstractExternalScheduleFeed::convert2UTF8($speaker["last_name"]),
              ),
              "email" => $this->getDefaultSpeakerEmail($speakerFullName),
              "company" => isset($speaker["company"])
                ? trim(AbstractExternalScheduleFeed::convert2UTF8($speaker["company"]))
                : "",
              "position" => isset($speaker["job_title"])
                ? trim(AbstractExternalScheduleFeed::convert2UTF8($speaker["job_title"]))
                : "",
      ...
      $track = isset($event["track"])
        ? ucwords(str_replace($this::CHARS_TO_REMOVE, " ", trim($event["track"])))
        : "";

      $events[] = [
        "external_id" => trim($event["id"]),
        "title" => isset($event["title"])
          ? trim(AbstractExternalScheduleFeed::convert2UTF8($event["title"]))
          : "",
        "abstract" => isset($event["abstract"])
          ? trim(AbstractExternalScheduleFeed::convert2UTF8($event["abstract"]))
          : "",
        "track" => $track,
        "location" => isset($event["room"])
          ? trim(AbstractExternalScheduleFeed::convert2UTF8($event["room"]))
          : "",
