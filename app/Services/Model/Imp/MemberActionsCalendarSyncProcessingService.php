 ...
 */
final class MemberActionsCalendarSyncProcessingService implements
  IMemberActionsCalendarSyncProcessingService {
              ...
              switch ($request_type) {
                case AbstractCalendarSyncWorkRequest::TypeAdd:
                  if (is_null($summit_event)) {
                    Log::warning(
                      sprintf(
                        "EVENT ADD : event id %s does not exists!",
                        $summit_event_id,
                        $member->getId(),
                      ),
                    );
                  ...
                  if ($calendar_sync_info->isRevoked()) {
                    Log::warning(
                      sprintf(
                        "EVENT ADD : event id %s - member id %s could not be added on external calendar bc credential are revoked!",
                        $summit_event_id,
                        $member->getId(),
                      ),
                    );
                  ...
                  if ($member->isEventSynchronized($calendar_sync_info, $summit_event_id)) {
                    Log::warning(
                      sprintf(
                        "EVENT ADD : event id %s - member id %s already synchronized",
                        $summit_event_id,
                        $member->getId(),
                      ),
                    );
                  ...
                  if (is_null($schedule_sync_info)) {
                    Log::warning(
                      sprintf(
                        "EVENT ADD : event id %s - member id %s could not be added on external calendar",
                        $summit_event_id,
                        $member->getId(),
                      ),
                    );
                ...
                case AbstractCalendarSyncWorkRequest::TypeUpdate:
                  if (is_null($summit_event)) {
                    Log::warning(
                      sprintf(
                        "EVENT UPDATE : event id %s does not exists!",
                        $summit_event_id,
                        $member->getId(),
                      ),
                    );
                  ...
                  $sync_info = $member->getScheduleSyncInfoByEvent(
                    $summit_event_id,
                    $calendar_sync_info,
                  );
                ...
                case AbstractCalendarSyncWorkRequest::TypeRemove:
                  ...
                  $schedule_sync_info = $member->getScheduleSyncInfoByEvent(
                    $summit_event_id,
                    $calendar_sync_info,
                  );
                  if (is_null($schedule_sync_info)) {
                    Log::warning(
                      sprintf(
                        "EVENT REMOVE : event id %s - member id %s could not be removed, schedule synch info is null",
                        $summit_event_id,
                        $member->getId(),
                      ),
                    );
              ...
              switch ($request_type) {
                case AbstractCalendarSyncWorkRequest::TypeAdd:
                  if ($calendar_sync_info->isRevoked()) {
                    Log::warning(
                      sprintf(
                        "CALENDAR ADD : calendar sync info id %s, member id %s is revoked!",
                        $calendar_sync_info->getId(),
                        $member->getId(),
                      ),
                    );
