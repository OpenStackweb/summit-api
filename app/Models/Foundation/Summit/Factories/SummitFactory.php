...
final class SummitFactory {
  ...
  public static function populate(Summit $summit, array $data) {
    ...
    if (isset($data["registration_send_qr_as_image_attachment_on_ticket_email"])) {
      $summit->setRegistrationSendQrAsImageAttachmentOnTicketEmail(
        boolval($data["registration_send_qr_as_image_attachment_on_ticket_email"]),
      );
    }

    if (isset($data["registration_send_ticket_as_pdf_attachment_on_ticket_email"])) {
      $summit->setRegistrationSendTicketAsPdfAttachmentOnTicketEmail(
        boolval($data["registration_send_ticket_as_pdf_attachment_on_ticket_email"]),
      );
    }

    if (isset($data["registration_send_ticket_email_automatically"])) {
      $summit->setRegistrationSendTicketEmailAutomatically(
        boolval($data["registration_send_ticket_email_automatically"]),
      );
    }

    if (isset($data["registration_send_order_email_automatically"])) {
      $summit->setRegistrationSendOrderEmailAutomatically(
        boolval($data["registration_send_order_email_automatically"]),
      );
    }

    if (isset($data["registration_allow_automatic_reminder_emails"])) {
      $summit->setAllowAutomaticReminderEmails(
        boolval($data["registration_allow_automatic_reminder_emails"]),
      );
    }
    ...
    if (isset($data["allow_update_attendee_extra_questions"])) {
      $summit->setAllowUpdateAttendeeExtraQuestions(
        boolval($data["allow_update_attendee_extra_questions"]),
      );
    }
    ...
    // BOOKING PERIOD
    if (
      array_key_exists("begin_allow_booking_date", $data) &&
      array_key_exists("end_allow_booking_date", $data)
    ) {
    ...
    // REGISTRATION PERIOD
    if (
      array_key_exists("registration_begin_date", $data) &&
      array_key_exists("registration_end_date", $data)
    ) {
          ...
          if (!is_null($summit_end_date)) {
            ...
            if ($start_datetime > $summit_end_date) {
              throw new ValidationException(
                "The Registration Begin Date cannot be after the Summit End Date.",
              );
            }
          }
        } else {
    ...
    if (isset($data["registration_reminder_email_days_interval"])) {
      // days
      $summit->setRegistrationReminderEmailDaysInterval(
        intval($data["registration_reminder_email_days_interval"]),
      );
    }
    ...
    if (isset($data["marketing_site_oauth2_client_scopes"])) {
      $summit->setMarketingSiteOauth2ClientScopes(
        trim($data["marketing_site_oauth2_client_scopes"]),
      );
    }
