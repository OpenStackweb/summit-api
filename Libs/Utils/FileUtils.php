...
trait FileUtils{
    ...
    public static function getFileFromRemoteStorageOnTempStorage(string $file_name, string $remotePath): string
    {
        ...
        if ($out === false) {
            fclose($stream);
            throw new \RuntimeException("Unable to open local temp file for writing: {$localPath}");
        }
        ...
        if ($bytes === false) {
            ...
            while (!feof($stream)) {
                ...
                if ($chunk === false) {
                    ...
                    fclose($out);
                    @unlink($localPath);
                    throw new \RuntimeException("Read error from remote stream.");
                }
                if ($chunk !== '') {
                    ...
                    if ($written === false) {
                        ...
                        fclose($out);
                        @unlink($localPath);
                        throw new \RuntimeException("Write error to local file.");
                    }
                    $bytes += $written;
                }
            }
        ...
        try {
            ...
            // If we can't determine size, we still return the path (caller may validate hashes later)
            // You can rethrow here if you prefer strict behavior.
        }
