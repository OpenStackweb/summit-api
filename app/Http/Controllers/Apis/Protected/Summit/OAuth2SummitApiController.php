...
final class OAuth2SummitApiController extends OAuth2ProtectedController {
  ...
  public function getSummits() {
    ...
    if (
      ...
      return $this->error403([
        "message" => sprintf(
          "Member %s has not permission for any Summit",
    ...
    return $this->_getAll(
      ...
      function () {
        return [
          ...
          "registration_end_date" =>
            "sometimes|required_with:start_date|date_format:U|after:registration_begin_date",
          "ticket_types_count" => "sometimes|required|integer",
        ];
      },
      function () {
        return ["id", "name", "start_date", "registration_begin_date"];
      },
      function ($filter) use ($current_member) {
        if ($filter instanceof Filter) {
          ...
          if (!is_null($current_member)) {
            ...
            if (!count($allowed_summits)) {
              $allowed_summits[] = 0;
            }
            $filter->addFilterCondition(
              FilterElement::makeEqual("summit_id", $allowed_summits, "OR"),
            );
          }
        }
        return $filter;
      ...
      function () {
        return new Order([OrderElement::buildAscFor("start_date")]);
      },
      function () {
        return PHP_INT_MAX;
      },
      ...
      [
        "build_default_payment_gateway_profile_strategy" =>
          $this->build_default_payment_gateway_profile_strategy,
      ],
    );
  }
  ...
  public function getSummit($summit_id) {
    return $this->processRequest(function () use ($summit_id) {
      ...
      if (!is_null($current_member) && !$current_member->isSummitAllowed($summit)) {
        return $this->error403([
          "message" => sprintf(
            "Member %s has not permission for this Summit",
            $current_member->getId(),
          ),
        ]);
      }
    ...
    return $this->processRequest(function () use ($id) {
      ...
      if (!in_array($group_by, IStatsConstants::AttendeesCheckinsAllowedGroupBy)) {
        throw new ValidationException(
          "Invalid group by criteria. Valid ones are " .
            join(",", IStatsConstants::AttendeesCheckinsAllowedGroupBy),
        );
      }

      [$page, $per_page] = self::getPaginationParams();
      ...
      [$start_date, $end_date] = FilterUtils::parseDateRangeUTC($filter);

      $response = $summit->getAttendeesCheckinsGroupedBy(
        $group_by,
  ...
  public function getPurchasedTicketsOverTimeStats($id) {
    return $this->processRequest(function () use ($id) {
      ...
      $response = $summit->getPurchasedTicketsGroupedBy(
        $group_by,
        new PagingInfo($page, $per_page),
        $start_date,
        $end_date,
  ...
  public function addSummit() {
    return $this->processRequest(function () {
      $payload = $this->getJsonPayload(SummitValidationRulesFactory::buildForAdd(), true, [
        ...
        "schedule_start_date.before_or_equal" =>
          "Show on schedule page needs to be after the start of the Show And Before of the Show End.",
      ]);
   ...
   * @return mixed
   */
  public function confirmExternalOrderAttendee(
    $summit_id,
    $external_order_id,
  ...
  public function updateFeatureSpeaker($summit_id, $speaker_id) {
    return $this->processRequest(function () use ($summit_id, $speaker_id) {
      ...
      $this->summit_service->updateFeaturedSpeaker(
        intval($summit_id),
        intval($speaker_id),
        $payload,
      );
  ...
  public function getAllFeatureSpeaker($summit_id) {
    ...
    return $this->_getAll(
      ...
      function () {
        return ["first_name", "last_name", "id", "email", "order"];
      },
