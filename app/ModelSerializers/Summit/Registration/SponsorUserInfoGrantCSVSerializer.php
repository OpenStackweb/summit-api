...
class SponsorUserInfoGrantCSVSerializer extends SponsorUserInfoGrantSerializer {
  private static function isAllowedExtraQuestion(
    array $allowed_extra_questions,
    int $extra_question_id,
  ): bool {
    if (count($allowed_extra_questions) > 0 && $allowed_extra_questions[0] == "*") {
      return true;
    }

    foreach ($allowed_extra_questions as $allowed_extra_question) {
      if ($allowed_extra_question["id"] == $extra_question_id) {
        return true;
      }
    }
    return false;
  ...
  public function serialize(
    ...
    $grant = $this->object;
    if (!$grant instanceof SponsorUserInfoGrant) {
      return [];
    }
    ...
    if (
      isset($setting_columns[SummitLeadReportSetting::SponsorExtraQuestionsKey]) &&
      count($setting_columns[SummitLeadReportSetting::SponsorExtraQuestionsKey]) > 0
    ) {
      $allowed_sponsor_extra_questions =
        $setting_columns[SummitLeadReportSetting::SponsorExtraQuestionsKey];

      foreach ($sponsor_questions as $question) {
        if (!$question instanceof SummitSponsorExtraQuestionType) {
          continue;
        }

        if (!self::isAllowedExtraQuestion($allowed_sponsor_extra_questions, $question->getId())) {
          continue;
        }
    ...
    if (
      ...
      $allowed_attendee_extra_questions =
        $setting_columns[SummitLeadReportSetting::AttendeeExtraQuestionsKey];
      ...
      foreach ($params["ticket_questions"] as $question) {
        if (!self::isAllowedExtraQuestion($allowed_attendee_extra_questions, $question->getId())) {
          continue;
        }
